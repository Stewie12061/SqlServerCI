IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AV0302_MK]') AND OBJECTPROPERTY(ID, N'IsView') = 1)
DROP VIEW [DBO].[AV0302_MK]
GO
...
---- Modified by Văn Tài   on 08/10/2020: Tách view cho MEIKO.
---- Modified by Đức Duy on 20/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.

  
CREATE VIEW [dbo].[AV0302_MK] AS   
...
SELECT   '' AS GiveUpID
	...
	, T9.Ana05ID
	, T9.AnaName05
	, T9.DebitBankAccountID
	, T9.CreditBankAccountID
	, T9.InheritTableID
FROM  
(
	SELECT	'' AS GiveUpID
...
INNER JOIN AT1005   WITH (NOLOCK) 
	ON   AT1005.AccountID = T9.CreditAccountID 
...
inner join (
			select Isnull(Max(IsMultiTax), 0) as IsMultiTax
					, DivisionID
					, VoucherID
					, MAX(transactiontypeID) as TransactionTypeID 
			from AT9000  WITH (NOLOCK) 
			Group by DivisionID
					, VoucherID
			) T90  ON T9.DivisionID = T90.DivisionID 
						and T9.VoucherID = T90.VoucherID  
Where (T90.TransactionTypeID IN ('T24', 'T34', 'T99') 
		and IsMultiTax= 0 OR (T90.TransactionTypeID IN ('T34') and IsMultiTax = 1)) -- Bút toán doanh thu hàng bán bị trả lại có nhiều nhóm thế ) 
...
SELECT		'' AS GiveUpID
			...
			, ISNULL(T3.GivedOriginalAmount, 0) AS GivedOriginalAmount
			, ISNULL(T3.GivedConvertedAmount,0) AS GivedConvertedAmount
			, (isnull(T9.OriginalAmount,0)) AS OriginalAmount
			, STATUS
			, (isnull(T9.OriginalAmountCN,0)) AS OriginalAmountCN
			, (isnull(T9.ConvertedAmount,0)) AS ConvertedAmount
			, T9.Ana01ID
...
LEFT JOIN (SELECT	SUM((isnull(T03.ConvertedAmount,0)) )AS GivedConvertedAmount
			...
			Group by	T03.ObjectID
						...
						, T03.DivisionID
						, T03.CreditTableID
			)T3 ON  T3.ObjectID = T9.ObjectID  
						AND T3.CreditVoucherID = T9.VoucherID 
						AND T3.CreditBatchID = T9.BatchID 
						AND T3.CreditTableID = T9.TableID 
						AND T3.AccountID = T9.CreditAccountID 
						AND T3.DivisionID = T9.DivisionID  	   
INNER JOIN AT1005 WITH (NOLOCK) ON AT1005.AccountID = T9.CreditAccountID 
									AND AT1005.GroupID = 'G03'  
LEFT JOIN AT1202 A WITH (NOLOCK) ON  A.DivisionID IN (T3.DivisionID, '@@@') AND A.ObjectID =  T9.ObjectID        
INNER JOIN (
			SELECT ISNULL(MAX(IsMultiTax), 0) AS IsMultiTax
					, DivisionID
					, VoucherID
...
WHERE  (
			IsMultiTax =1 
			AND  T90.TransactionTypeID not IN ('T34')
		) 
		or 
		(
			T90.TransactionTypeID NOT IN ('T24','T34', 'T99') 
			and IsMultiTax = 0
		)  
