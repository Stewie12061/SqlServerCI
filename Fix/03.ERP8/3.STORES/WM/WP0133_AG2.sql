IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[WP0133_AG2]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[WP0133_AG2]
GO
...
---- Modified by Tiểu Mai on 14/08/2017: Bổ sung sắp xếp theo ngày, nhập, xuất
---- Modified on 22/01/2018 by Bảo Anh: Lấy các chứng từ theo kho được chọn xem chi tiết
...
/* 
exec WP0133_AG2 @DivisionID=N'ANG',
@WareHouseID=N'KCC'',''KDD'',''KDH'',''KGT'',''KHN'',''KLZ'',''KM1'',''KM2'',''KNG'',''KNL'',''KPP'',''KTC'',''KTG'',''KVP'',''KVT'',''KXH'',''KXK'',''SL1'',''SL2'',''SL3'',''TCR'',''TDG'',''TEP'',''TGC'',''TIN'',''TKU',@FromInventoryID=N'BNKU03',@ToInventoryID=N'BNKU03',@FromMonth=3,@FromYear=2017,@ToMonth=3,@ToYear=2017,@FromDate='2017-04-04 08:30:20.103',@ToDate='2017-04-04 08:30:20.103',@IsDate=0,@IsInner=1,@IsAll=1
*/

CREATE PROCEDURE [dbo].[WP0133_AG2]
       @DivisionID nvarchar(50) ,
       @WareHouseID AS NVARCHAR(1000) ,
...
EXEC WP0133_AG3 @DivisionID , @WareHouseID , @FromInventoryID , @ToInventoryID , @FromMonth , @FromYear , @ToMonth , @ToYear , @FromDate , @ToDate , @IsDate, @IsAll
...
IF @IsDate = 0
	BEGIN
		...
		SET @sSQlFrom = ' 
			...
			LEFT JOIN AV0133_AG3 AV7015 on AV7015.InventoryID = AT2007.InventoryID AND AV7015.DivisionID = AT2007.DivisionID
	...
	WHERE	AT2007.DivisionID = N''' + @DivisionID + ''' ' +
			...
			(Isnull(AT2007.TranMonth,AT2006.TranMonth) + 100*Isnull(AT2007.TranYear,AT2006.TranYear) between '+@FromMonthYearText+' AND  '+@ToMonthYearText+' )  AND	
			(AT2007.InventoryID between N''' + @FromInventoryID + ''' AND N''' + @ToInventoryID + ''') AND
			( (KindVoucherID in ' + @KindVoucherListEx2 + ' AND AT2006.WareHouseID = ''' + @WareHouseID + ''')
			or  ( KindVoucherID = 3 AND WareHouseID2 = ''' + @WareHouseID + ''')) '
END
ELSE
BEGIN
	SET @sSQlSelect = N'
...
SELECT  AV7015.WareHouseID  AS WareHouseID, AV7015.WareHouseName AS WareHouseName, Null AS VoucherID, NULL AS TranMonth, NULL AS TranYear, Null as TableID, Null as BatchID, Null AS TransactionID, 
		--Null AS Orders,
...
ORDER BY WareHouseID, InventoryID, VoucherDate, ImExOrders
