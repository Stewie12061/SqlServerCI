IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[WP3008_QC]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[WP3008_QC]
GO
...
-- <Summary>
---- IN bao cao nhap xuat ton theo DVT qui doi (theo quy cach)
-- <Param>
---- 
-- <Return>
---- 
...
---- Modified on 11/09/2015 by Hải Long: Sửa lại câu lấy số dư đầu kỳ 
---- Modified by Kim Vu on 18/10/2016: Bổ sung in báo cáo theo kho được chọn
---- Modified by Tiểu Mai on 21/10/2016: Cải tiến tốc độ
---- Modified by Phương Thảo on 26/05/2017: Sửa danh mục dùng chung
-- <Example>
/*
EXEC WP3008_QC
@DivisionID = 'MP', @FromMonth = 2, @ToMonth = 2, @FromYear = 2012, @ToYear = 2012, @FromDate = '2012-02-01', @ToDate = '2012-02-20',
@FromInventoryID = '000000000009', @ToInventoryID = '000000000009', @IsDate = 0, @IsGroupID = 1, @GroupID1 = 'I01', @GroupID2 = '' 
*/



CREATE PROCEDURE [dbo].[WP3008_QC] 
...
DECLARE 
	...
	@dropTBL_WareHouseID NVARCHAR(4000)
...
IF @IsDate = 0 -- theo ky
    BEGIN
        SET @sSQL1 = '
            SELECT V7.DivisionID, InventoryID, InventoryName, UnitID, S1, S2, S3, I01ID, I02ID, I03ID, I04ID, I05ID, UnitName, VATPercent, InventoryTypeID, Specification, 
            ...
            FROM WQ7002 V7
            INNER JOIN TBL_WareHouseID TBL ON TBL.WareHouseID = V7.WareHouseID
            ...
            GROUP BY V7.DivisionID, InventoryID, InventoryName, UnitID, S1, S2, S3, I01ID, I02ID, I03ID, I04ID, I05ID, UnitName, VATPercent, InventoryTypeID, Specification, V7.Notes01, V7.Notes02, V7.Notes03, 
        ...
        IF NOT EXISTS (SELECT 1 FROM sysObjects WHERE Xtype = 'V' AND Name = 'WV7016')
            EXEC(' CREATE VIEW WV7016 AS ' + @sSQL1) -- tao boi store  WP3008_QC
        ELSE
            EXEC(' ALTER VIEW WV7016 AS ' + @sSQL1) ----- tao boi store  WP3008_QC

        SET @sSQL1 = '
            SELECT AV7016.DivisionID, AV7016.InventoryID, AV7016.InventoryName, AV7016.UnitID, 
                AV7016.S1, AV7016.S2, AV7016.S3, AV7016.I01ID, AV7016.I02ID, AV7016.I03ID, AV7016.I04ID, AV7016.I05ID, AV7016.VATPercent, AV7016.InventoryTypeID, AV7016.Specification, 
            ...
            LEFT JOIN (SELECT AV7000.* FROM WQ7001 AV7000 
						  INNER JOIN TBL_WareHouseID TBL ON TBL.WareHouseID = AV7000.WareHouseID
                    WHERE AV7000.DivisionID = ''' + @DivisionID + ''' 
                          ...
                          AND AV7000.TranMonth + 100*AV7000.TranYear BETWEEN ' + @FromMonthYearText + ' AND ' + @ToMonthYearText + ' ) as AV7000 ON 
        ...
        IF NOT EXISTS (SELECT 1 FROM sysObjects WHERE Xtype = 'V' AND Name = 'WV3088')
            EXEC(' CREATE VIEW WV3088 AS ' + @sSQL1 + @sSQL2 + @sSQL3 + @sGroupBy) --- TAO BOI  WP3008_QC
        ELSE
            EXEC(' ALTER VIEW WV3088 AS ' + @sSQL1 + @sSQL2 + @sSQL3 + @sGroupBy) --- TAO BOI  WP3008_QC
        ...
        SET @sSQL1 = '
            ...
            FROM WV3088 AV3088 
            LEFT JOIN AT1309 ON AT1309.InventoryID = AV3088.InventoryID and AV3088.UnitID = AT1309.UnitID
...
--print @sGroupBy
        ...
        IF NOT EXISTS (SELECT 1 FROM sysObjects WHERE Xtype = 'V' AND Name = 'WV3088')
            EXEC(' CREATE VIEW WV3088 AS ' + @sSQL1 + @sSQL2 + @sGroupBy) --- TAO BOI  AP3008
        ELSE
            EXEC(' ALTER VIEW WV3088 AS ' + @sSQL1 + @sSQL2 + @sGroupBy) --- TAO BOI  AP3008
...
IF NOT EXISTS (SELECT 1 FROM sysObjects WHERE Xtype = 'V' AND Name = 'WV3098')
    EXEC(' CREATE VIEW WV3098 AS ' + @sSQL1) -- Tao boi store WP3008_QC
ELSE
    EXEC(' ALTER VIEW WV3098 AS ' + @sSQL1) ---- Tao boi store WP3008_QC
...
SET @dropTBL_WareHouseID = '
								DROP TABLE TBL_WareHouseID'
 -- PRINT @sSQL
 EXEC (@sSQL1 + @dropTBL_WareHouseID)
