IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[WP0119]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[WP0119]
GO
...
GO
-- <Summary>
--- Load danh sách phiếu yêu cầu nhập - xuất - VCNB lên màn hình Duyệt cấp 1
-- <Param>
----
-- <Return>
----
...
-- <Example>
/*
    EXEC WP0119 'HT','2016-08-01 00:00:00.000','2016-08-15 00:00:00.000',8,2016,8,2016,1, 1, 2, N'('''')', N' (0=0) ', N'('''')', N' (0=0) '
*/

 CREATE PROCEDURE WP0119
(
...
Case when Isnull(IsConfirm02,0) = 0 then N''Chưa chấp nhận'' else 
	(case when Isnull(IsConfirm02,0) = 1 then N''Xác nhận'' else N''Từ chối'' end) end as IsConfirmName01,
ConfDescription02
...
'+ CASE WHEN (ISNULL(@Status,0) = 0 OR ISNULL(@Status,0) = 2) THEN 'AND Isnull(W95.IsConfirm01,0) IN (0, 2) ' ELSE ' AND Isnull(W95.IsConfirm01,0) = 1 ' END +'
'+ @sWhere +'
