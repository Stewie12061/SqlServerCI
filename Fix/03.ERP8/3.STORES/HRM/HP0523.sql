IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HP0523]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[HP0523]
GO
...
-- <Summary>
---- Báo cáo lương năng suất (NEWTOYO)
-- <Param>
---- 
-- <Return>
---- 
-- <Reference>
---- ASOFT-HRM \ Danh mục \ Thông tin chấm công \ Tính lương năng suất \ In báo cáo lương năng suất
-- <History>
----Created by Bảo Thy on 29/09/2017
----Modified by Bảo Thy on 24/01/2018: Bổ sung thông tin TK ngân hàng
---- Modified by on 

/*-- <Example>
	HP0523 @DivisionID='CH', @TranMonth=7, @TranYear=2017, @FromDepartmentID='PB1', @ToDepartmentID='Z1', @TeamID = NULL,
	@FromEmployeeID = '0000000001', @ToEmployeeID = '0000000004'
----*/
CREATE PROCEDURE [dbo].[HP0523]
(
...
T1.BaseSalary, T1.ExcessQuantity, T1.OTQuantity, HT1116.RepayTotalAmount,
ISNULL(T1.ExcessAmount,0) + ISNULL(T1.OTAmount,0) + 
(ISNULL(HT3400.Income11,0)+ISNULL(HT3400.Income13,0)+ISNULL(HT3400.Income15,0)+ISNULL(HT3400.Income17,0) +ISNULL(HT3400.Income19,0)+ISNULL(HT3400.Income21,0))AS CapacitySalary, 
T1.CreateUserID +' - '+ (SELECT TOP 1 UserName FROM AT1405 WITH (NOLOCK) WHERE UserID = T1.CreateUserID) CreateUserID, T1.CreateDate,
ISNULL(T1.ExcessAmount,0) AS ExcessAmount, ISNULL(T1.OTAmount,0) AS OTAmount,
ISNULL(HT3400.Income11,0)+ISNULL(HT3400.Income13,0)+ISNULL(HT3400.Income15,0)+ISNULL(HT3400.Income17,0) +ISNULL(HT3400.Income19,0)+ISNULL(HT3400.Income21,0) AS TGAmount,
T6.BankID, T6.BankAccountNo
...
LEFT JOIN HT1402 T6 WITH (NOLOCK) ON T1.DivisionID = T6.DivisionID AND T1.EmployeeID = T6.EmployeeID
WHERE T1.DivisionID = @DivisionID
AND T1.TranMonth = @TranMonth AND T1.TranYear = @TranYear
AND T1.DepartmentID BETWEEN @FromDepartmentID AND @ToDepartmentID
AND T1.EmployeeID BETWEEN @FromEmployeeID AND @ToEmployeeID
