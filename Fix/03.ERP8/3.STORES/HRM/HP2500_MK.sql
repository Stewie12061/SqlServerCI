IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HP2500_MK]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[HP2500_MK]
GO
...
-- exec HP2500_MK @DivisionID=N'MK',@DepartmentID=N'A000000',@ToDepartmentID=N'A000000',@TeamID=N'%',@EmployeeID=N'%',@FromDate='2016-01-01 00:00:00',@Todate='2016-01-31 00:00:00',@TranMonth=1,@TranYear=2016

CREATE PROCEDURE [dbo].[HP2500_MK]  @DivisionID nvarchar(50),
...
BEGIN
	SET @Select = ', HV.Ana04ID AS SubsectionID, HV.Ana05ID AS ProcessID'
	SET @Select1 = ', SubsectionID, ProcessID'
	SET @Group = ', HV.Ana04ID, HV.Ana05ID'
	SET @Where = ' AND ISNULL(HV.Ana04ID,'''') LIKE '''+ISNULL(@SubsectionID,'')+''' AND ISNULL(HV.Ana05ID,'''') LIKE '''+ISNULL(@ProcessID,'')+''' '
END
...
WHERE	Isnull(HT02.AbsentTypeID,'''') <> '''' and 
		...
		AbsentDate between  '''+Convert(Varchar(20),@FromDate,120)+''' And '''+Convert(Varchar(20),@ToDate,120)+''' '+ @Where + '
