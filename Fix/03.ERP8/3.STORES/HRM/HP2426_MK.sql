IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HP2426_MK]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[HP2426_MK]
GO
...
CREATE PROCEDURE [dbo].[HP2426_MK]		
...
BEGIN
	...
	BEGIN
		SET  @TableHT2402 = 'HT2402M'+@sTranMonth+Convert(Varchar(4),@FromTranYear)
	END
	ELSE
	IF(@FromTranMonth <> @ToTranMonth and @FromTranYear = @ToTranYear)
	BEGIN
		SET  @TableHT2402 = 'HT2402Y'+Convert(Varchar(4),@FromTranYear)
	END
	ELSE
		SET  @TableHT2402 = 'HT2402'
END
ELSE
BEGIN
	SET  @TableHT2402 = 'HT2402'
...
IF NOT EXISTS (SELECT  1 FROM SYSOBJECTS WHERE XTYPE = 'V' AND NAME = 'HV2424')
	EXEC('CREATE VIEW HV2424 ----TAO BOI HP2426_MK
			AS '+ @sSQL2)
ELSE
	EXEC('ALTER VIEW HV2424 ----TAO BOI HP2426_MK
			AS ' + @sSQL2)
