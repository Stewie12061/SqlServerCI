IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HP3400]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[HP3400]
GO
...
---- Modified Huỳnh Thử  on 26/02/2021: Tách store HP3400_MEKIO 
-- <Example>
---- EXEC HP3400 'MK', 12, 2016, 'A000000','E000000','P05', 'aaa', 'bbbb'
CREATE PROCEDURE HP3400	
(
...
BEGIN
	EXEC HP3400_NQH @DivisionID = @DivisionID
					, @TranMonth = @TranMonth
					, @TranYear = @TranYear
					...
					, @IsUsedConditionDE = @IsUsedConditionDE 
END
ELSE
IF(@CustomerIndex = 50) -- Khách hàng MEKIO
BEGIN
	EXEC HP3400_MEKIO @DivisionID = @DivisionID
					, @TranMonth = @TranMonth
					, @TranYear = @TranYear
					, @FromDepartment = @FromDepartment 
					, @ToDepartment = @ToDepartment 
