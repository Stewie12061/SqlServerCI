IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HP3400_NQH]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[HP3400_NQH]
GO
...
---- Bang phan ca
-- <History>
---- Modified Văn Minh on 29/11/2019: Tách chuỗi tránh TH quá số ký tự có thể chứa
---- Modified Văn Minh on 11/12/2019: Thay đổi vị trí điều kiện WHERE
---- Modified Văn Tài  on 09/11/2020: Bổ sung điều kiện lấy bảng HT0002, HT0005 theo DivisionID và IsUsed.
---- Modified Văn Tài  on 15/12/2020: Bổ sung ( ) cho điều kiện phân quyền dữ liệu. Dẫn tới lỗi load ( OR TranMonth) double dữ liệu.
---- Modified Văn Tài  on 23/12/2020: Tách store HP3400_NQH cho trường hợp PARTTIME.
-- <Example>
---- EXEC HP3400 'MK', 12, 2016, 'A000000','E000000','P05', 'aaa', 'bbbb'
CREATE PROCEDURE HP3400_NQH
(
...
INTO HTT3400
FROM HT3400 
LEFT JOIN HT2400 on HT3400.DivisionID=HT2400.DivisionID and HT3400.EmployeeID=HT2400.EmployeeID
'
SET @SQL6 = '
...
WHERE 
	  (
		HT3400.EmployeeID LIKE N''%PARTTIME%'' 
		OR ISNULL(HT3400.EmployeeID, '''') <> ''''
	   )
	  AND
		(Isnull(HT3400.DepartmentID,''#'') IN ' + @ConditionDE + ' OR  ' + @IsUsedConditionDE + ')
