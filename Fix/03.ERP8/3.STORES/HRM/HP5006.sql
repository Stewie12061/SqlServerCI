IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WITH(NOLOCK) WHERE ID = OBJECT_ID(N'[DBO].[HP5006]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
...
IF EXISTS ( SELECT TOP 1 1 FROM HT3400
            WHERE DivisionID = @DivisionID AND DepartmentID LIKE @DepartmentID1 AND Isnull(TeamID , '') LIKE isnull(@TeamID1 , '') AND TranMonth = @TranMonth AND TranYear = @TranYear )
BEGIN
    SET @cur = CURSOR SCROLL KEYSET FOR 
					SELECT DISTINCT TaxObjectID
                    FROM	HT2400
                    WHERE	DivisionID = @DivisionID AND
                            TranMonth = @TranMonth AND TranYear = @TranYear AND isnull(TaxObjectID , '''') <> '''' AND DivisionID = @DivisionID 
							AND DepartmentID LIKE @DepartmentID1 AND Isnull(TeamID , '') LIKE isnull(@TeamID1 , '')
    OPEN @cur
    ...
    BEGIN
		...
		IF EXISTS ( SELECT TOP 1 1 FROM sysObjects WHERE Name = 'HV5501' AND Type = 'V' )
		BEGIN						
				DROP VIEW HV5501
		END
		...
		---Tinh thu nhap truoc thue (tong thu nhap truoc thue - tong giam tru truoc thue)
		...
		IF EXISTS ( SELECT TOP 1 1 FROM sysObjects WHERE Name = 'HV5502' AND Type = 'V' )
		BEGIN
				DROP VIEW HV5502
		END
		...
		IF EXISTS ( SELECT TOP 1 1 FROM sysObjects WHERE XType = 'U' AND Name = 'HT5888' )
		BEGIN
			DROP TABLE HT5888
		END
		EXEC ( @sSQLSelect )
		...
		BEGIN
			...
			IF EXISTS ( SELECT TOP 1 1 FROM sysObjects WHERE Name = 'HV5006' AND Type = 'V' )
				BEGIN
					DROP VIEW HV5006
				END
