IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HP2533]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
...
Set @sSQL = 'Select T00.DivisionID, T00.EmployeeID, FullName, T00.DepartmentID, V00.DepartmentName, PayrollMethodID, 
		...
		avg(isnull(Income01,0)) as Income01,  avg(isnull(Income02, 0)) as Income02, avg(isnull(Income03, 0)) as Income03, avg(isnull(Income04, 0)) as Income04, 
		...
		avg(isnull(Income14, 0)) as Income14, avg(isnull(Income15, 0)) as Income15, avg(isnull(Income16, 0)) as Income16, avg(isnull(Income17, 0)) as Income17,
...
Begin
	Set @sSQL = @sSQL + ' Select DivisionID, EmployeeID
	, FullName, DepartmentID, DepartmentName,   InsuranceSalary, 
...
CREATE TABLE [dbo].[HT2537]  (
	...
	[InsuranceSalary] [decimal] (38, 8)  NULL,	
	[BaseSalary] [decimal] (38, 8)  NULL,
	[Salary01] [decimal] (38, 8)  NULL,
	[Salary02]  [decimal] (38, 8) NULL ,
	[Salary03] [decimal] (38, 8) NULL ,
...
Exec ( 'Insert into HT2537  ( DivisionID, EmployeeID
, FullName, DepartmentID, DepartmentName,  InsuranceSalary, 
				BaseSalary, Salary01, Salary02, Salary03,
				Orders, DutyName, TeamID, TeamName, WorkDate, Notes,  Note1, IncomeID, Signs,  Displayed,  Amount,  Caption,  FOrders , a
				)' + @sSQL)
...
begin
	SET @sSQL_Where = ' AND HT02.EmployeeID  in (SELECT EmployeeID FROM #Data) '
end
...
set @sSQL2='SELECT DivisionID, EmployeeID  , FullName ,DepartmentID ,DepartmentName ,  InsuranceSalary , 
	...
	UNION Select DivisionID, EmployeeID  , FullName ,DepartmentID ,DepartmentName ,  InsuranceSalary , 
		 ...
		 + ' as Notes , N''' + @NetPay+ ' ' + @Currency + ''' '		 
		+ ' as Note1, ''I21'' as IncomeID , 2 as Signs , 0 as Displayed, N''' + @NetPay + ' ' +  @Currency + ''' '		
