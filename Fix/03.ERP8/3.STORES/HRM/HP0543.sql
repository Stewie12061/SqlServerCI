IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HP0543]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[HP0543]
GO
...
-- <Summary>
---- In báo cáo lương sản phẩm (VIETFIRST)
-- <Param>
---- 
-- <Return>
---- 
...
/*-- <Example>
	HP0543 @DivisionID = 'VF', @UserID = 'ASOFTADMIN', @TranMonth = 12, @TranYear = 2017

	HP0543 @DivisionID, @UserID, @TranMonth, @TranYear
----*/

CREATE PROCEDURE HP0543
( 
...
IF EXISTS (SELECT TOP 1 1 FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[Temp_HP0543]') AND TYPE IN (N'U'))
DROP TABLE Temp_HP0543

CREATE TABLE Temp_HP0543 (APK VARCHAR(50), [Target] VARCHAR(50), [Min] DECIMAL(28,8), Tar DECIMAL(28,8), [Max] DECIMAL(28,8), [Weight] DECIMAL(28,8))
...
INSERT INTO Temp_HP0543 (APK, [Target], [Min], Tar, [Max], [Weight]) 
...
LEFT JOIN AT1102 WITH (NOLOCK) ON AT1102.DivisionID IN ('''+@DivisionID+''', ''@@@'') AND HT1400.DepartmentID = AT1102.DepartmentID 
