IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WITH(NOLOCK) WHERE ID = OBJECT_ID(N'[DBO].[AP0009]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP0009]
GO
...
-- <Summary>
---- Đồng nhất cấu trúc bảng giữa bảng gốc và bảng nghiệp vụ được tách
-- <Param>
---- 
-- <Return>
---- 
...
CREATE PROCEDURE [dbo].[AP0009] @ModuleID AS Nvarchar(50)           
AS
SET NOCOUNT ON
...
FROM (SELECT DISTINCT TableName FROM AV9991 WHERE Disable = 0 and ModuleID LIKE @ModuleID ) T
