IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP5702_QC]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[MP5702_QC]
GO
...
-- <Summary>
---- Phân bổ chi phí SXC theo bộ hệ số (QL hàng hóa theo quy cách)
-- <Param>
---- 
-- <Return>
---- 
...
CREATE PROCEDURE [dbo].[MP5702_QC] 
...
CREATE TABLE #MP5702_QC (DivisionID Varchar(50), ProductID Varchar(50), CoValue decimal(28, 8), ProductQuantity decimal(28, 8), 
...
BEGIN
	...
	WHERE MT1605.CoefficientID LIKE @CoefficientID AND
...
SET @SUMProductCovalues = (SELECT SUM(ISNULL(ProductCovalues, 0)) FROM #MP5702_QC)

--SELECT * FROM #MP5702_QC  WHERE CoefficientID = @CoefficientID   
--SELECT * FROM #MP5702_QC 
--select * from MT1613
