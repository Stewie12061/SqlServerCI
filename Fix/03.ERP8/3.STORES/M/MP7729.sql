IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP7729]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
...
Set 	@SumProductConverted = (Select  Sum(Isnull(ConvertedUnit,0)*Isnull(MT4444.ProductQuantity,0))
				From MT1603 WITH (NOLOCK) inner join MT4444 WITH (NOLOCK)  on MT4444.ProductID = MT1603.ProductID and MT4444.ResultTypeID = 'R01'
...
Set @ProductlD_cur  = CURSOR  SCROLL  KEYSET  FOR 
		...
		From MT4444 WITH (NOLOCK) inner join MT1603 WITH (NOLOCK) on MT4444.ProductID = MT1603.ProductID and MT4444.ResultTypeID = 'R01'
...
Begin
	...
	Set @ID = (Select top 1 ID
		From MT0444 WITH (NOLOCK) Where ExpenseID ='COST003' and MaterialTypeID = @MaterialTypeID and ResultTypeID = 'R01'
