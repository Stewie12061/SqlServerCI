IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP0621]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[MP0621]
GO
...
'********************************************/
---- Modified by Tiểu Mai on 31/12/2015: Bổ sung trường hợp quản lý hàng theo quy cách (IsSpecificate = 1) cho PP Định mức
---- Modified by Bảo Anh on 24/04/2017: Đưa phần insert dữ liệu MT2222 từ MP5000 vào để tính được Số lượng SPDD = Số lượng SP * ISNULL(% chi phí NVL, % TLHT)
---- Modified by Hải Long on 25/05/2017: Sửa danh mục dùng chung
---- Modified by Phương Thảo on 24/04/2018: Bổ sung quy cách cho pp PB trực tiếp
---- Modified by Bảo Anh on 12/07/2018: Bổ sung quy cách cho pp PB hệ số
...
FROM (
		...
		SELECT 
			...
			D10.UnitID, 
			D08.PeriodID,    
			SUM(D10.Quantity*ISNULL(D10.MaterialRate,D10.PerfectRate)/100) AS ProductQuantity,	
			O99.S01ID, O99.S02ID, O99.S03ID, O99.S04ID, O99.S05ID, O99.S06ID, O99.S07ID, O99.S08ID, O99.S09ID, O99.S10ID,
			O99.S11ID, O99.S12ID, O99.S13ID, O99.S14ID, O99.S15ID, O99.S16ID, O99.S17ID, O99.S18ID, O99.S19ID, O99.S20ID
		FROM MT1001 D10  WITH (NOLOCK)
			INNER JOIN MT0810 D08 WITH (NOLOCK) ON D08.DivisionID = D10.DivisionID AND D08.VoucherID = D10.VoucherID
...
WHILE @@Fetch_Status = 0
    BEGIN
		...
		BEGIN
			IF EXISTS (SELECT TOP 1 1 FROM AT0000 WITH (NOLOCK) WHERE DefDivisionID = @DivisionID AND IsSpecificate = 1)
				EXEC MP5101_QC @DivisionID, @UserID, @PeriodID, @TranMonth, @TranYear, @MaterialTypeID
			ELSE
				EXEC MP5101 @DivisionID, @UserID, @PeriodID, @TranMonth, @TranYear, @MaterialTypeID
		END

        IF @MethodID = 'D02' ---- Phan bo theo he so. Da lam xong Ngay 05/11/2003
		BEGIN
			IF EXISTS (SELECT TOP 1 1 FROM AT0000 WITH (NOLOCK) WHERE DefDivisionID = @DivisionID AND IsSpecificate = 1)
				EXEC MP5102_QC @DivisionID, @UserID, @PeriodID, @TranMonth, @TranYear, @MaterialTypeID, @CoefficientID
			ELSE
				EXEC MP5102 @DivisionID, @UserID, @PeriodID, @TranMonth, @TranYear, @MaterialTypeID, @CoefficientID
		END
        ...
        BEGIN
			...
			IF EXISTS (SELECT TOP 1 1 FROM AT0000 WITH (NOLOCK) WHERE DefDivisionID = @DivisionID AND IsSpecificate = 1)
				EXEC MP6103 @DivisionID, @UserID, @PeriodID, @TranMonth, @TranYear, @MaterialTypeID, @ApportionID
			ELSE
				EXEC MP5103 @DivisionID, @UserID, @PeriodID, @TranMonth, @TranYear, @MaterialTypeID, @ApportionID
        ...
        EXEC MP5106 @DivisionID, @UserID, @PeriodID, @TranMonth, @TranYear, @MaterialTypeID, @CoefficientID

        IF @MethodID = 'D07' --- Phan bo truc tiep ket hop dinh muc. Da lam xong ngay 07/11/2003
        EXEC MP5107 @DivisionID, @UserID, @PeriodID, @TranMonth, @TranYear, @MaterialTypeID, @ApportionID 
