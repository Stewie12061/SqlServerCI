IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP0182]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[MP0182]
GO
...
-- <Summary>
---- Load danh sách đề nghị định mức lên màn hình duyệt MF0180
-- <History>
...
/*
 * exec MP0182 'PC', '2016-08-01', '2016-08-31', 0, '%', 0
 */

CREATE PROCEDURE [dbo].[MP0182] 	
...
WHERE MT0176.DivisionID = '''+@DivisionID+'''
	...
	'+ CASE WHEN ISNULL(@Type,0) = 0 THEN 'AND Isnull(MT0176.IsConfirm02,0) = 0 ' ELSE ' AND Isnull(MT0176.IsConfirm02,0) IN (1,2) ' END +'
	AND MT0135.ObjectID LIKE '''+@ObjectID+'''
	AND ISNULL(MT0176.IsConfirm01,0) = 1 '
+ @sWhere + '
