IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP8011_QC]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[MP8011_QC]
GO
...
---- Purpose:     Tinh chi phi do dang cuoi ky  cho PP cap nhat bang tay (Thiết lập quản lý hàng theo quy cách).
...
CREATE PROCEDURE  [dbo].[MP8011_QC] 
...
IF EXISTS (SELECT TOP 1 1 FROM AT0000 WHERE DefDivisionID = @DivisionID AND IsSpecificate = 1)
	...
	WHERE MT1613.DivisionID= @DivisionID AND PeriodID = @PeriodID AND TranMonth = @TranMonth AND  TranYear =@TranYear  AND Type ='E' 
...
SELECT MT1612.ProductID, 
    ...
    ExpenseID, 
    MaterialTypeID, 
    ISNULL(MT2222.ProductQuantity, 1) AS ProductQuantity, 
...
FROM MT1612 WITH (NOLOCK) 
    ...
    LEFT JOIN MT8899 O99 WITH (NOLOCK) ON O99.DivisionID = MT1612.DivisionID AND O99.VoucherID = MT1612.VoucherID AND O99.TransactionID = MT1612.WipVoucherID AND O99.TableID = 'MT1612'
    LEFT JOIN MT2222 WITH (NOLOCK) ON MT2222.DivisionID = MT1612.DivisionID AND MT2222.ProductID = MT1612.ProductID AND 
						ISNULL(MT1612.PS01ID,'') = ISNULL(MT2222.PS01ID,'') AND
						ISNULL(MT1612.PS02ID,'') = ISNULL(MT2222.PS02ID,'') AND
						ISNULL(MT1612.PS03ID,'') = ISNULL(MT2222.PS03ID,'') AND
						ISNULL(MT1612.PS04ID,'') = ISNULL(MT2222.PS04ID,'') AND
						ISNULL(MT1612.PS05ID,'') = ISNULL(MT2222.PS05ID,'') AND
						ISNULL(MT1612.PS06ID,'') = ISNULL(MT2222.PS06ID,'') AND
						ISNULL(MT1612.PS07ID,'') = ISNULL(MT2222.PS07ID,'') AND
						ISNULL(MT1612.PS08ID,'') = ISNULL(MT2222.PS08ID,'') AND
						ISNULL(MT1612.PS09ID,'') = ISNULL(MT2222.PS09ID,'') AND
						ISNULL(MT1612.PS10ID,'') = ISNULL(MT2222.PS10ID,'') AND
						ISNULL(MT1612.PS11ID,'') = ISNULL(MT2222.PS11ID,'') AND
						ISNULL(MT1612.PS12ID,'') = ISNULL(MT2222.PS12ID,'') AND
						ISNULL(MT1612.PS13ID,'') = ISNULL(MT2222.PS13ID,'') AND
						ISNULL(MT1612.PS14ID,'') = ISNULL(MT2222.PS14ID,'') AND
						ISNULL(MT1612.PS15ID,'') = ISNULL(MT2222.PS15ID,'') AND
						ISNULL(MT1612.PS16ID,'') = ISNULL(MT2222.PS16ID,'') AND
						ISNULL(MT1612.PS17ID,'') = ISNULL(MT2222.PS17ID,'') AND
						ISNULL(MT1612.PS18ID,'') = ISNULL(MT2222.PS18ID,'') AND
						ISNULL(MT1612.PS19ID,'') = ISNULL(MT2222.PS19ID,'') AND
						ISNULL(MT1612.PS20ID,'') = ISNULL(MT2222.PS20ID,'')
WHERE MT1612.DivisionID = @DivisionID  AND MT1612.PeriodID =@PeriodID AND TranMonth = @TranMonth AND TranYear = @TranYear   AND Type ='E'  
GROUP BY MT1612.ProductID, MaterialID, ExpenseID, MaterialTypeID, AT1302_P.UnitID, AT1302_M.UnitID, ISNULL(MT2222.ProductQuantity, 1),
	MT1612.PS01ID, MT1612.PS02ID, MT1612.PS03ID, MT1612.PS04ID, MT1612.PS05ID, MT1612.PS06ID, MT1612.PS07ID, MT1612.PS08ID, MT1612.PS09ID, MT1612.PS10ID,
	MT1612.PS11ID, MT1612.PS12ID, MT1612.PS13ID, MT1612.PS14ID, MT1612.PS15ID, MT1612.PS16ID, MT1612.PS17ID, MT1612.PS18ID, MT1612.PS19ID, MT1612.PS20ID,
	O99.S01ID, O99.S02ID, O99.S03ID, O99.S04ID, O99.S05ID, O99.S06ID, O99.S07ID, O99.S08ID, O99.S09ID, O99.S10ID,
	O99.S11ID, O99.S12ID, O99.S13ID, O99.S14ID, O99.S15ID, O99.S16ID, O99.S17ID, O99.S18ID, O99.S19ID, O99.S20ID
...
BEGIN            
    ...
    INSERT into MT1613 (PeriodID, TranMonth, TranYear, DivisionID, VoucherID, TransactionID, 
        ...
        ProductUnitID, 
        MaterialUnitID, MaterialQuantity, ConvertedAmount, ProductQuantity, ConvertedUnit, QuantityUnit, Type, 
        PS01ID, PS02ID, PS03ID, PS04ID, PS05ID, PS06ID, PS07ID, PS08ID, PS09ID, PS10ID,
		PS11ID, PS12ID, PS13ID, PS14ID, PS15ID, PS16ID, PS17ID, PS18ID, PS19ID, PS20ID,
    ...
    VALUES ( @PeriodID, @TranMonth, @TranYear, @DivisionID, @VoucherID, @TransactionID, 
        @ProductID, @MaterialID, @ExpenseID, @MaterialTypeID, 
        @ProductUnitID, 
        @MaterialUnitID, @WipQuantity, @ConvertedAmount, @ProductQuantity, @ConvertedUnit, @QuantityUnit, 'E',
		@PS01ID, @PS02ID, @PS03ID, @PS04ID, @PS05ID, @PS06ID, @PS07ID, @PS08ID, @PS09ID, @PS10ID,
		@PS11ID, @PS12ID, @PS13ID, @PS14ID, @PS15ID, @PS16ID, @PS17ID, @PS18ID, @PS19ID, @PS20ID,
