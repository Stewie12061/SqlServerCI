IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP0179]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[MP0179]
GO
...
-- <Summary>
---- Load danh sách đề nghị định mức - MF0178
-- <History>
...
/*
 * exec MP0179 'PC', '2016-08-01', '2016-08-30', null, '%', 0
 */

CREATE PROCEDURE [dbo].[MP0179] 	
...
CASE WHEN Isnull(IsConfirm01,0) = 0 THEN N''Chưa xác nhận'' ELSE CASE WHEN Isnull(IsConfirm01,0) = 1 THEN N''Xác nhận'' ELSE N''Từ Chối'' END END AS IsConfirmName01, 
CASE WHEN Isnull(IsConfirm02,0) = 0 THEN N''Chưa xác nhận'' ELSE CASE WHEN Isnull(IsConfirm02,0) = 1 THEN N''Xác nhận'' ELSE N''Từ Chối'' END END AS IsConfirmName02,
ConfDescription02,
ConfDescription01
