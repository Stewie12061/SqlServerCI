IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP8201_MK]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[MP8201_MK]
GO
...
-- <Summary>
---- Tính chi phí dở dang cuối kỳ - chi phí SXNC (Customize Meiko)
-- <Param>
---- 
-- <Return>
---- 
...
CREATE PROCEDURE  [dbo].[MP8201_MK] @DivisionID AS nvarchar(50), 
...
SELECT SUM(ISNULL(MaterialQuantity, 0)) AS MaterialQuantity, 
    ...
    MT0400.DivisionID
FROM MT0400 LEFT JOIN AT1302 AT1302_P ON MT0400.ProductID = AT1302_P .InventoryID AND AT1302_P .DivisionID IN (MT0400.DivisionID,''@@@'')
...
IF NOT EXISTS (SELECT TOP 1 1 FROM SysObjects WHERE name = 'MV8201_MK' AND Xtype ='V')
    EXEC ('CREATE VIEW MV8201_MK  -- tao boi MP8201_MK
    AS '+@sSQL)
ELSE
    EXEC ('ALTER VIEW MV8201_MK -- tao boi MP8201_MK
    AS '+@sSQL)
--print @sSQL
...
--    MT2222.ProductQuantity*ISNULL(MT2222.HumanResourceRate, 0)/100, 
...
SELECT MV8201.MaterialQuantity, ---So luong NVL phat sinh trong ky
       ...
       ISNULL(MT2222.ProductQuantity, 0)*ISNULL(MT2222.HumanResourceRate, 0)/100 AS InProcessQuantity, --AS InPocessQuantity, --- So luong thanh pham do dang quy doi
       TotalInProcessQuantity = (select sum(ISNULL(MT22.ProductQuantity, 0)*ISNULL(MT22.HumanResourceRate, 0)/100) as InProcessQuantity from MT2222 MT22
	   ...
	   (Select sum(MT22.ProductQuantity*MT22.HumanResourceRate/100)
	   ...
	   MV8201.DistributeQty,
	   ( SELECT    SUM(ConvertedAmount)
					  FROM     MT1613
					  WHERE     DivisionID = @DivisionID AND  PeriodID =  @PeriodID
							AND TranMonth = @TranMonth AND TranYear = @TranYear
							AND ProductID=MV8201.ProductID
							AND MaterialTypeID = @MaterialTypeID  
							AND ExpenseID='COST002'          
							AND Type ='B')  AS BSumConverted622        
FROM MV8201_MK MV8201 LEFT JOIN MT2222 ON MV8201.DivisionID = MT2222.DivisionID AND MV8201.ProductID = MT2222.ProductID 
WHERE MV8201.MaterialTypeID = @MaterialTypeID

OPEN @ListProduct_cur 
FETCH NEXT FROM @ListProduct_cur INTO
    @MaterialQuantity, @ConvertedAmount, @ProductID, @ProductQuantity, @InProcessQuantity, @TotalInProcessQuantity, @PerfectRate, @MaterialRate, 
    @HumanResourceRate, @OthersRate, @ProductQuantityEnd, @ProductUnitID, @DistributeQty, @BSumConverted622 
...
WHILE @@Fetch_Status = 0
	...
	IF (ISNULL(@ProductQuantity, 0) +ISNULL(@InProcessQuantity, 0)) <>0
	BEGIN
		SET @InprocessCost =round(((ISNULL(CAST(@BSumConverted622 AS DECIMAL(28,20)), 0) +ISNULL(CAST(@ConvertedAmount AS DECIMAL(28,20)), 0)) *ISNULL(CAST(@InProcessQuantity AS DECIMAL(28,20)), 0))/
...
-------->>>> Lấy thêm các sản phẩm không sản xuất trong kỳ nhưng có số dở dang đầu kỳ
...
WHERE Type = 'B' 
	  ...
	  and  MT1613.ProductID NOT IN (SELECT DISTINCT ProductID FROM MT0400 
                                WHERE PeriodID =@PeriodID AND MaterialTypeID = @MaterialTypeID AND ExpenseID ='COST002'
                                    AND DivisionID = @DivisionID)  --- khong co phat sinh duoc phan bo
...
SET @ListMaterial_cur  = CURSOR SCROLL KEYSET FOR 
	...
	FROM #MP8201_MT1613 T1 
        ...
        BEGIN    
                ...
                VALUES  (@PeriodID, @TranMonth, @TranYear, @DivisionID, @VoucherID, @TransactionID, 
                    @ProductID, @MaterialID, 'COST002', @MaterialTypeID, @ProductUnitID, @MaterialUnitID,
                    @MaterialQuantity, 
                    @ConvertedAmount, 
...
--from mt1613
--Where MT1613.ExpenseID= N'COST002' And MT1613.PeriodID = @PeriodID  And MT1613.DivisionID = @DivisionID
...
--WHERE T1.ExpenseID= N'COST002' And T1.PeriodID = @PeriodID  And T1.DivisionID = @DivisionID
...
--						AND MT1613.ExpenseID='COST002' AND MT1613.Type='E' AND MT1613.MaterialTypeID=@MaterialTypeID
