IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP8202_QC]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[MP8202_QC]
GO
...
--Purpose: Tính CPDD cuối kỳ NC TT theo phương pháp Định mức (theo thiết lập quản lý quy cách hàng hóa).
...
/*
EXEC MP8202_QC 'AS', 'DT.001', 12, 2015, 'M01', 'BDM.TEST', 'IV12201500000001', 12, 2015
*/

CREATE PROCEDURE [dbo].[MP8202_QC]  @DivisionID AS nvarchar(50), 
...
--print 'da vao1'
SET NOCOUNT ON
CREATE TABLE #MV8202(MaterialID NVARCHAR(50), MaterialQuantity DECIMAL(28,8),ConvertedAmount DECIMAL(28,8), ProductID NVARCHAR(50), ProductQuantity DECIMAL(28,8), 
...
SET @PeriodStr = CASE WHEN @TranMonth < 10 THEN '0' + ltrim(@TranMonth) + ltrim(@TranYear) ELSE ltrim(@TranMonth) + ltrim(@TranYear) END
    ...
    SELECT         @MaterialID = MV8202.MaterialID, 
            ...
            @PS01ID = MV8202.PS01ID,
            @PS02ID = MV8202.PS02ID,
            @PS03ID = MV8202.PS03ID,
            @PS04ID = MV8202.PS04ID,
            @PS05ID = MV8202.PS05ID,
            @PS06ID = MV8202.PS06ID,
            @PS07ID = MV8202.PS07ID,
            @PS08ID = MV8202.PS08ID,
            @PS09ID = MV8202.PS09ID,
            @PS10ID = MV8202.PS10ID,
            @PS11ID = MV8202.PS11ID,
            @PS12ID = MV8202.PS12ID,
            @PS13ID = MV8202.PS13ID,
            @PS14ID = MV8202.PS14ID,
            @PS15ID = MV8202.PS15ID,
            @PS16ID = MV8202.PS16ID,
            @PS17ID = MV8202.PS17ID,
            @PS18ID = MV8202.PS18ID,
            @PS19ID = MV8202.PS19ID,
            @PS20ID = MV8202.PS20ID
            
        FROM #MV8202 MV8202 LEFT JOIN MT2222 WITH (NOLOCK) ON MV8202.DivisionID  = MT2222.DivisionID AND MV8202.ProductID  = MT2222.ProductID AND 
										ISNULL(MV8202.PS01ID,'') = ISNULL(MT2222.PS01ID,'') AND
										ISNULL(MV8202.PS02ID,'') = ISNULL(MT2222.PS02ID,'') AND
										ISNULL(MV8202.PS03ID,'') = ISNULL(MT2222.PS03ID,'') AND
										ISNULL(MV8202.PS04ID,'') = ISNULL(MT2222.PS04ID,'') AND
										ISNULL(MV8202.PS05ID,'') = ISNULL(MT2222.PS05ID,'') AND
										ISNULL(MV8202.PS06ID,'') = ISNULL(MT2222.PS06ID,'') AND
										ISNULL(MV8202.PS07ID,'') = ISNULL(MT2222.PS07ID,'') AND
										ISNULL(MV8202.PS08ID,'') = ISNULL(MT2222.PS08ID,'') AND
										ISNULL(MV8202.PS09ID,'') = ISNULL(MT2222.PS09ID,'') AND
										ISNULL(MV8202.PS10ID,'') = ISNULL(MT2222.PS10ID,'') AND
										ISNULL(MV8202.PS11ID,'') = ISNULL(MT2222.PS11ID,'') AND
										ISNULL(MV8202.PS12ID,'') = ISNULL(MT2222.PS12ID,'') AND
										ISNULL(MV8202.PS13ID,'') = ISNULL(MT2222.PS13ID,'') AND
										ISNULL(MV8202.PS14ID,'') = ISNULL(MT2222.PS14ID,'') AND
										ISNULL(MV8202.PS15ID,'') = ISNULL(MT2222.PS15ID,'') AND
										ISNULL(MV8202.PS16ID,'') = ISNULL(MT2222.PS16ID,'') AND
										ISNULL(MV8202.PS17ID,'') = ISNULL(MT2222.PS17ID,'') AND
										ISNULL(MV8202.PS18ID,'') = ISNULL(MT2222.PS18ID,'') AND
										ISNULL(MV8202.PS19ID,'') = ISNULL(MT2222.PS19ID,'') AND
										ISNULL(MV8202.PS20ID,'') = ISNULL(MT2222.PS20ID,'')
        ...
        WHERE MT0135.Disabled = 0 AND MT0135.IsBOM=1 AND MT0136.ProductID= @ProductID AND MT0137.ExpenseID = 'COST002'
