...
---- Modified by Tiểu Mai on 02/03/2016: Bổ sung xử lý phát sinh phiếu từ Nhập kho sản xuất cho ANGEL (CustomizeIndex = 57)
---- Modified by Phương Thảo on 19/07/2016: Bổ sung customize Meiko (CustomizeIndex = 50) : Vẫn chuyển các phiếu là hàng đi đường mặc dù KH chọn loại chứng từ là 'NM'
...
--- Modified by Nhựt Trường on 02/07/2021: Customize Phúc Long: Bỏ kiểm tra @CreditAccountID, RDVoucherNo và VoucherTypeID khi chuyen qua AT9000
--- Modified by Nhựt Trường on 14/10/2021: Customize Phúc Long: Insert thẳng qua AT9000, không cần kiểm tra.
--- Modified by Nhật Thanh on 11/08/2023: Customize Gree: Xử lý luôn cho phiếu láp ráp tháo dở
...
IF(NOT EXISTS ( SELECT top 1 1 FROM AT0000 WHERE DefDivisionID = @DivisionID AND IsSpecificate = 1)) AND (Isnull(@TableID,'') <> 'AT0112' OR @CustomerName = 162)
BEGIN 
...
BEGIN
	IF(@CustomerName = 32)--Customize Phúc Long: Insert thẳng qua AT9000, không cần kiểm tra
	BEGIN
		INSERT AT9000  
	...
	END
	ELSE -- Luồng xử lý chuẩn
	BEGIN
		If( 
		(@RDVoucherNo NOT LIKE '%NM%' AND @VoucherTypeID NOT LIKE '%NM%' AND @CustomerName <> 50)--Customize cho Thuan Loi, LTV neu nhap kho truoc mua hang sau thi không chuyen qua AT9000	
		OR (( (@RDVoucherNo NOT LIKE '%NM%' AND @VoucherTypeID NOT LIKE '%NM%') OR @CreditAccountID NOT LIKE '331%')  AND @CustomerName = 50)
		)
		BEGIN
			INSERT AT9000  
			(VoucherID, BatchID, TransactionID, TableID,   
