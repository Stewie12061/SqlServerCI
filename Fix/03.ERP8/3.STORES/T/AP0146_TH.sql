IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_TH]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_TH]
GO
...
-- AP0146
-- <Summary>
---- Stored load dữ liệu hóa đơn phục vụ phát hành hóa đơn điện tử - Store riêng cho Tiến Hưng
...
---- Modified by Nhựt Trường on 10/05/2022: Khai báo thêm chuỗi để fix lỗi tràn ký tự.
---- Modified by Đức Duy on 20/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.
---- Modified by Nhựt Trường on 24/07/2023: [2023/07/IS/0249] - Bổ sung trường TotalPaymentAmount (lấy tổng tiền quy đổi với điều kiện đơn vị tiền khác 'VND').
---- Modified by Hương Nhung on 23/10/2023: 2023/10/IS/0177 - Fix lỗi tràn dữ liệu @sSQL3.
---- Modified by Đình Định on 29/11/2023: TIẾN HƯNG - Nếu đơn vị tiền tệ là VND thì lấy ConvertedAmount = 0.
---- Modified by Nhựt Trường on 29/11/2023: TIẾN HƯNG - Phát hành hóa đơn lấy nhóm thuế T08.
-- <Param>
---- 
-- <Return>
---- 
...
-- <Example>
---- EXEC AP0146_TH @DivisionID = 'ANG',@UserID='ASOFTADMIN',@VoucherID='AV561ee05d-44de-4d71-8e4c-4927876f60df'

CREATE PROCEDURE [dbo].[AP0146_TH]
...
BEGIN
	...
	- ISNULL((SELECT SUM(ABS(ConvertedAmount)) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T64'')),0) AS TotalPaymentAmount,
	...
	SET @sSQL1=N'
	CASE WHEN AT9000.VATGroupID=''TS0'' THEN -1 ELSE AT1010.VATRate END AS VAT_Rate, AT9000.InventoryID AS ProdID, ISNULL(AT9000.InventoryName1,AT1302.InventoryName) AS ProdName, 
	...
	AT9000.InheritFkey, AT9000.EInvoiceType, AT9000.TypeOfAdjust, ISNULL(AT9000.TDescription,'''') AS TDescription, A02.AnaName as Ana02Name, AT9000.PaymentID AS PaymentMethod,
	...
	+ CASE WHEN (SELECT TOP 1 ExchangeRate FROM AT9000 WHERE VoucherID=@VoucherID) = 1 THEN '1' ELSE 'CAST(AT9000.ExchangeRate AS DECIMAL(28,0))' END + ' AS Extra,
	...
	SET @sSQL5 = '
		SELECT	TOP 100 PERCENT Remark, CusCode, CusName, CusAddress, CusPhone, CusEmail, CusTaxCode, Buyer, CusbankNo, PaymentMethod, 
			...
			CASE WHEN CurrencyID = ''VND'' THEN 0 ELSE ConvertedAmount END ConvertedAmount, ImTaxOriginalAmount, ImTaxConvertedAmount, ExpenseOriginalAmount, ExpenseConvertedAmount, IsStock, VoucherDate,
		...
		SET @sSQL6 = '
			...
			CASE WHEN CurrencyID = ''VND'' THEN NULL ELSE TotalPaymentAmount END AS TotalPaymentAmount
		FROM #TEMP
		ORDER BY Remark
...
EXEC (@sSQL+@sSQL1+@sSQL2+@sSQL3+@sSQL4+@sSQL5+@sSQL6)
