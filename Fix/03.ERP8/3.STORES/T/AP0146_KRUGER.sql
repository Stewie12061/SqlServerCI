...
(
    SELECT TOP 1
           1
    FROM dbo.sysobjects
    WHERE id = OBJECT_ID(N'[DBO].[AP0146_KRUGER]')
          AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
    DROP PROCEDURE [dbo].[AP0146_KRUGER];
GO
...
---- Created on 16/12/2020 Đức Thông
-- <Param>
---- 
-- <Return>
---- 
...
---- 
-- <History>
---- Created  by Đức Thông  on 16/12/2020: Customize Kruger (133): Thêm 3 dòng ghi chú dưới hóa đơn: Tham số 3 (Parameter03), tham số 4 (Parameter04), ngày đáo hạn (DueDate)
---- Modified by Đức Thông  on 22/12/2020: Fix bug KRUGER (133): Chỉ lấy đúng 3 dòng ghi chú (Param 3 + 4 + DueDate)
---- Modified by Đức Thông  on 23/12/2020: 2020/12/IS/0255 Bổ sung chuỗi cứng vào các dòng chú thích và lấy thêm parameter05 thành 1 dòng khác
---- Modified by Đức Thông  on 25/01/2021: [KRUGER] 2021/01/IS/0305: Sửa lại các dòng chú thích
---- Modified by Nhật Thanh  on 15/07/2022: Cập nhật chú thích 10, Người mua hàng lấy parameter02
---- Modified by Nhật Thanh  on 29/08/2022: Bổ sung ISNULL khi lấy invoicesign và serial cha
---- Modified by Đức Duy on 13/02/2023: [2023/02/IS/0052] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục kho hàng - AT1303.
---- Modified by Đức Duy on 20/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.

-- <Example>
---- EXEC AP0146_KRUGER @DivisionID = 'ANG',@UserID='ASOFTADMIN',@VoucherID='AV561ee05d-44de-4d71-8e4c-4927876f60df'
CREATE PROCEDURE [dbo].[AP0146_KRUGER]
    @DivisionID AS NVARCHAR(50),
    @UserID AS NVARCHAR(50),
    @VoucherID AS NVARCHAR(50)
AS
DECLARE @sSQL AS NVARCHAR(MAX) = N'',
        @sSQL1A AS NVARCHAR(MAX) = N'',
        @sSQL1A1 AS NVARCHAR(MAX) = N'',
        @sSQL2A AS NVARCHAR(MAX) = N'',
        @sSQLA AS NVARCHAR(MAX) = N'',
        @sSQLB AS NVARCHAR(MAX) = N'',
        @sSQL3 AS NVARCHAR(MAX) = N'',
        @sSQL4 AS NVARCHAR(MAX) = N'',
        @sSQL5 AS NVARCHAR(MAX) = N'',
        @sSQL6 AS NVARCHAR(MAX) = N'',
		@sSQL7 AS NVARCHAR(MAX) = N'',
        @KindVoucherID AS TINYINT = 0;


SET @KindVoucherID =
(
    SELECT KindVoucherID FROM AT2006 WHERE VoucherID = @VoucherID
);
IF @KindVoucherID = 3
   OR @KindVoucherID = 2
BEGIN
	...
	NULL AS PaymentMethod, A03_2.WareHouseName as CusAddress, A03.WareHouseName AS CusName
	...
	A06.VoucherDate AS ImportDate, (SELECT SUM(ActualQuantity) FROM AT2007 WHERE VoucherID = ''' + @VoucherID
          + N''') AS Total, NULL AS VATRate, 0 AS VATAmount,
	(SELECT SUM(ConvertedAmount) FROM AT2007 WHERE VoucherID = ''' + @VoucherID
          + N''') AS Amount, NULL AS ArisingDate, A04.InventoryName AS ProdName, 
	...
	A07.InventoryID, A04.InventoryName, A07.UnitID, A05.UnitName, A07.ActualQuantity, A07.Unitprice, A07.OriginalAmount, A07.ConvertedAmount, A07.Notes, A07.CurrencyID, A07.ExchangeRate, A07.SaleUnitPrice,
	...
	A07.ReVoucherID AS ReVoucherID_AT2007, A07.Ana01ID, A07.Ana02ID, A07.Ana03ID, A07.Ana04ID, A07.Ana05ID, A07.Ana06ID, A07.Ana07ID,(SELECT REPLACE (A07.Ana08ID,''-'
          + @DivisionID
          + N''','''' )) as Ana08ID, A07.Ana09ID, A07.Ana10ID,
	...
	A06.VoucherDate AS InvoiceDate, 0 AS VAT_Rate
	...
	WHERE A06.DivisionID = ''' + @DivisionID + N''' AND A06.VoucherID = ''' + @VoucherID
          + N'''
	ORDER BY A07.Orders
'   ;
...
BEGIN
	...
	AT9000.Parameter02 AS Buyer, AT1202.TradeName AS CusbankNo, AT1202.TradeName AS CusBankName,
    ...
    SET @sSQL2A
        = N' AT9000.ImVoucherID,AT9000.ImTransactionID,AT9000.SourceNo,AT9000.LimitDate,AT9000.IsPromotionItem,AT9000.ObjectName1,
	...
	(SELECT TOP 1 InvoiceSign  FROM AT9000 A90 WITH (NOLOCK) WHERE A90.VoucherID = AT9000.InheritFkey AND ISNULL(InvoiceSign,'''')!='''') AS ParentInvoiceSign,
	(SELECT TOP 1 Serial  FROM AT9000 A90 WITH (NOLOCK) WHERE A90.VoucherID = AT9000.InheritFkey AND ISNULL(Serial,'''')!='''') AS ParentSerial,
	...
	WHERE AT9000.DivisionID = ''' + @DivisionID + N'''
		...
		SELECT * FROM (
			SELECT	TOP 100 PERCENT Remark, CusCode, CusName, CusAddress, CusPhone, CusEmail,CusEmailCC, CusTaxCode, Buyer, CusbankNo,CusBankName, PaymentMethod, 
				...
				DonDatHangSo, OriginalAmount, KindOfService, Extra1, VATAmount0, VATAmount10, Extra, InvoiceDate, CurrencyID, PaymentID, VoucherNo,
				 ...
				 ';
    SET @sSQLB
        = N'VirtualPrice, VirtualAmount, ReTableID, DParameter01, DParameter02, DParameter03, DParameter04, DParameter05, DParameter06,
				 ...
				 CusBankAccountNo' + @sSQL6 + N'
			FROM #TEMP
			ORDER BY Remark
		) X
	';
    
	--- Customize: Lấy thêm dòng chú thích 1 (Parameter 03) Theo số hợp đồng
	IF EXISTS
    (
        SELECT TOP 1
               1
        ...
        SET @sSQL4
			...
			'''' AS ProdID, Parameter03 AS ProdName,'''' AS ProdUnit, 0 AS ProdQuantity , 0 AS ProdPrice,0 AS Amount , '''' AS InheritFkey , 
		...
		FROM #TEMP
		WHERE ISNULL(Parameter03, '''') <> ''''
		';


		-- Customize: Dòng chú thích 2 (Parameter 04) Theo số phiếu giao hàng
		IF EXISTS
    (
        SELECT TOP 1
               1
        ...
        SET @sSQL5
			...
			999999  AS Remark, CusCode AS CusCode ,   CusName AS CusName,CusAddress AS CusAddress, '''' AS CusPhone,  '''' AS CusEmail, '''' AS CusEmailCC, '''' AS CusTaxCode,
			Buyer AS Buyer, '''' AS CusbankNo,'''' AS CusBankName,  '''' AS PaymentMethod,  0 AS Total,  0 AS DiscountAmount,  0 AS AfterAmount,  0 AS VATAmount ,  0 AS VAT_Rate,
			'''' AS ProdID, Parameter04 AS ProdName,'''' AS ProdUnit, 0 AS ProdQuantity , 0 AS ProdPrice,0 AS Amount , '''' AS InheritFkey , 
			0 AS EInvoiceType,   0 AS TypeOfAdjust, '''' AS TDescription, '''' AS Ana02ID , '''' AS Ana02Name,
			'''' AS DonDatHangSo,  0 AS OriginalAmount, '''' AS KindOfService, CurrencyID AS Extra1 ,  0 AS VATAmount0, 
			0 AS VATAmount10,  1 AS Extra,InvoiceDate , CurrencyID , '''' AS VoucherID , '''' AS PaymentID , '''' AS  VoucherNo ,'''' AS BatchID,  '''' AS TransactionID ,  '''' AS TableID, 0 AS TranMonth , 
			0 AS TranYear,'''' AS TransactionTypeID ,'''' AS ObjectID , '''' AS CreditObjectID , '''' AS VATNo ,'''' AS VATObjectID , 
			...
			0 AS OriginalAfterVATAmount , 0 AS ConvertedAfterVATAmount, '''' AS WSourceNo , '''' AS WLimitDate ,'''' AS I04ID ,'''' AS I04Name,  4 AS ItemTypeID, NULL AS CusBankAccountNo,NULL AS ParentInvoiceSign, NULL AS ParentSerial
		FROM #TEMP
		WHERE ISNULL(Parameter04, '''') <> ''''
		';

		-- Customize: Dòng chú thích 3 (Parameter 10) Khách nhập gì thì nhập
		IF EXISTS
    (
        SELECT TOP 1
               1
        ...
        SET @sSQL6
			...
			0 AS OriginalAfterVATAmount , 0 AS ConvertedAfterVATAmount, '''' AS WSourceNo , '''' AS WLimitDate ,'''' AS I04ID ,'''' AS I04Name,  4 AS ItemTypeID, NULL AS CusBankAccountNo,NULL AS ParentInvoiceSign, NULL AS ParentSerial
		FROM #TEMP
		WHERE ISNULL(Parameter10, '''') <> ''''
		';

		-- Customize: Dòng chú thích 4: Ngày đáo hạn (DueDate) Ngày đến hạn thanh toán
		IF EXISTS
    (
        SELECT TOP 1
               1
        ...
        SET @sSQL7
			...
			999999  AS Remark, CusCode AS CusCode ,   CusName AS CusName,CusAddress AS CusAddress, '''' AS CusPhone,  '''' AS CusEmail, '''' AS CusEmailCC, '''' AS CusTaxCode,
			Buyer AS Buyer, '''' AS CusbankNo,'''' AS CusBankName,  '''' AS PaymentMethod,  0 AS Total,  0 AS DiscountAmount,  0 AS AfterAmount,  0 AS VATAmount ,  0 AS VAT_Rate,
			'''' AS ProdID, N''Ngày đến hạn thanh toán '' + CONVERT(VARCHAR, DueDate, 103) AS ProdName,'''' AS ProdUnit, 0 AS ProdQuantity , 0 AS ProdPrice,0 AS Amount , '''' AS InheritFkey , 
			0 AS EInvoiceType,   0 AS TypeOfAdjust, '''' AS TDescription, '''' AS Ana02ID , '''' AS Ana02Name,
			'''' AS DonDatHangSo,  0 AS OriginalAmount, '''' AS KindOfService, CurrencyID AS Extra1 ,  0 AS VATAmount0, 
			0 AS VATAmount10,  1 AS Extra,InvoiceDate , CurrencyID , '''' AS VoucherID , '''' AS PaymentID , '''' AS  VoucherNo ,'''' AS BatchID,  '''' AS TransactionID ,  '''' AS TableID, 0 AS TranMonth , 
			0 AS TranYear,'''' AS TransactionTypeID ,'''' AS ObjectID , '''' AS CreditObjectID , '''' AS VATNo ,'''' AS VATObjectID , 
    ...
    --print @sSQL
    --print @sSQL3
    EXEC (@sSQL + @sSQL1A + @sSQL1A1 + @sSQL2A + @sSQLA + @sSQLB + @sSQL3 + @sSQL4 + @sSQL5 + @sSQL6 + @sSQL7);
