IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP3206_BBL]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP3206_BBL]
GO
...
---purpose: Customize Bourbon - Load detail cho form ke thua nhieu don hang bán o phieu xuat kho
/***************************************************************
...
---- Modified on 23/03/2023 by Nhựt Trường: Bổ dung trường VoucherTypeID + Bỏ điều kiện DivisionID và VoucherTypeID.

-- <Example> 
--- exec AP3206_BBL @Divisionid=N'KVC',@Lstsorderid=N'',@Voucherid=NULL,@Connid=N'55'

CREATE PROCEDURE [dbo].[AP3206_BBL] 
...
WHERE OT2002.SOrderID in (''' + @lstSOrderID + ''') 
...
IF NOT EXISTS(SELECT TOP 1 1 FROM sysObjects WHERE XType = 'V' AND Name = 'AV3206' + @ConnID)
    EXEC('CREATE VIEW AV3206' + @ConnID + ' -- Tạo bởi AP3206_BBL
            AS ' + @sSQL1 + @sSQL2+ @sSQL3)
ELSE 
    EXEC('ALTER VIEW AV3206' + @ConnID + ' -- Tạo bởi AP3206_BBL
            AS ' + @sSQL1 + @sSQL2 + @sSQL3)

IF NOT EXISTS(SELECT TOP 1 1 FROM sysObjects WHERE XType = 'V' AND Name = 'AV3206')
    EXEC('CREATE VIEW AV3206 -- Tạo bởi AP3206_BBL
            AS ' + @sSQL1 + @sSQL2 + @sSQL3)
ELSE 
    EXEC('ALTER VIEW AV3206 -- Tạo bởi AP3206_BBL
            AS ' + @sSQL1 + @sSQL2 + @sSQL3)
