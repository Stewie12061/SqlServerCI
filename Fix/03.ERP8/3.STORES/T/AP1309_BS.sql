IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP1309_BS]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP1309_BS]
GO
...
/** exec AP1309 
	...
	, @GroupID = N'ADMIN'
**/
CREATE PROCEDURE [dbo].[AP1309_BS] 
...
BEGIN
	...
	BEGIN 
		...
		SET AT2007.UnitPrice = (CASE WHEN ActualQuantity <> 0 
			...
			, AT2007.ConvertedPrice =  (CASE WHEN ActualQuantity <> 0 
										   ...
										   ELSE 0 
									    END)
			, OriginalAmount = ROUND(AT9000.ConvertedAmount,@ConvertedDecimals)
			, ConvertedAmount = ROUND(AT9000.ConvertedAmount,@ConvertedDecimals)
			...
			BEGIN
				...
				BEGIN
					...
					IF @TransactionID IS NOT NULL
						BEGIN
							UPDATE AT2007
							SET ConvertedAmount = ISNULL(ROUND(ConvertedAmount,@ConvertedDecimals), 0) + @Amount, 
								OriginalAmount = ISNULL(ROUND(OriginalAmount,@ConvertedDecimals), 0) + @Amount,
								IsRound = 1
							FROM AT2007 WITH (ROWLOCK) 
	...
	BEGIN
		...
		-- Update Xuất 
        UPDATE AT2007
        SET ConvertedAmount = ISNULL(ROUND(ConvertedAmount,@ConvertedDecimals), 0) + #Temp.EndAmount
			, OriginalAmount = ISNULL(ROUND(OriginalAmount,@ConvertedDecimals), 0) + #Temp.EndAmount
			, IsRound = 1
        FROM AT2007 WITH (ROWLOCK) 
		...
		BEGIN
		        ...
		        UPDATE AT2007
					SET ConvertedAmount = ISNULL(ROUND(ConvertedAmount,@ConvertedDecimals), 0) + @EndAmount
						, OriginalAmount = ISNULL(ROUND(OriginalAmount,@ConvertedDecimals), 0) + @EndAmount
						, IsRound = 1
				FROM AT2007 WITH (ROWLOCK)  
				WHERE AT2007.DivisionID = @DivisionID 
						AND AT2007.TranYear = @TranYear 
