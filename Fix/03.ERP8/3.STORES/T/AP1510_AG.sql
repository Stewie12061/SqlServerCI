IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP1510_AG]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP1510_AG]
GO
...
---- Modified on 25/01/2018 by Bảo Anh: Sửa lại cách tính số kỳ đã khấu hao DepreciatedMonths
---- Modified on 17/03/2018 by Bảo Anh: Sửa lỗi số kỳ đã khấu hao DepreciatedMonths=0 khi chưa khấu hao trong kỳ
...
CREATE PROCEDURE [dbo].[AP1510_AG]    
...
-- Giam trong ky   
 ...
 '
 SET @sSQL2 = N'  AccuDepAmount = Case when exists (select top 1 AssetID   
										 from AT1506 WITH (NOLOCK)    
										 Where AT1506.AssetID = AT1503.AssetID AND AT1506.TranMonth + 100*AT1506.TranYear <='+ str(@ToPeriod )+' ) 
							THEN (select top 1 AT1506.AccruedDepNewAmount
										From AT1506 WITH (NOLOCK)    
										Where AT1506.AssetID = AT1503.AssetID and AT1506.DivisionID = AT1503.DivisionID  
										and AT1506.TranMonth + 100*AT1506.TranYear <=' + str(@ToPeriod ) + '  
										Order by AT1506.TranYear Desc,AT1506.TranMonth Desc) + 
								isnull ((Select Sum(DepAmount)   
										...
										and AT1504.TranMonth+AT1504.TranYear*100 between 
															(select top 1 AT1506.TranMonth + 100*AT1506.TranYear 
															From AT1506 WITH (NOLOCK)    
															Where AT1506.AssetID = AT1503.AssetID and AT1506.DivisionID = AT1503.DivisionID  
															and AT1506.TranMonth + 100*AT1506.TranYear <=' + str(@ToPeriod ) + '  
															Order by AT1506.TranYear Desc,AT1506.TranMonth Desc) 
															and  ' + str(@ToPeriod) + '),0) else
	 ( isnull (AT1503.ConvertedAmount,0)    
   - isnull(AT1503.ResidualValue,0)  
   + isnull((Select Sum(DepAmount)   
      From AT1504  WITH (NOLOCK)   
      Where DivisionID = AT1503.DivisionID and AssetID = At1503.AssetID   
      and AT1504.TranMonth+AT1504.TranYear*100 <= ' + str(@ToPeriod) + '),0)) end ,     
 ...
 --- Sửa lại: Số ky đã khấu hao = Khấu hao lũy kế/Mức khấu hao trong ky  
 --DepreciatedMonths =
	--CASE WHEN	( isnull((Select Sum(DepAmount)   
	--			  From AT1504  WITH (NOLOCK)   
	--			  Where  DivisionID = AT1503.DivisionID   
	--				and AssetID = At1503.AssetID   
	--				and AT1504.TranMonth + AT1504.TranYear * 100 = (Select top 1 TranMonth + TranYear*100 From AT1504 WITH (NOLOCK)
	--																Where AssetID = At1503.AssetID And TranMonth+TranYear*100 <= ' + str(@ToPeriod) + N' Order by TranMonth+TranYear*100 DESC)),0)	
	--			) = 0
	--THEN 0
	--ELSE
	--	( isnull (AT1503.ConvertedAmount,0)    
 ...
 DepreciatedMonths = ISNULL(AT1503.DepMonths,0) - ISNULL((Select Count(AssetID) From AT1504 WITH (NOLOCK) Where DivisionID = AT1503.DivisionID and AssetID = At1503.AssetID),0)
									+ ISNULL((Select Count(AssetID) From AT1504 WITH (NOLOCK) Where DivisionID = AT1503.DivisionID and AssetID = At1503.AssetID And TranMonth+TranYear*100 <= ' + str(@ToPeriod) + '),0)	
						,'
