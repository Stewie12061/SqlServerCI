IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0316_SONGBINH]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0316_SONGBINH]
GO
...
-- <Summary>
---- In báo cáo công nợ theo tuổi nợ.
-- <Param>
---- 
-- <Return>
---- 
...
---- Modified on 14/06/2019 by Kim Thư: Sửa lỗi lấy số tiền công nợ loại trừ khoản chiết khấu T64
---- Modified on 26/08/2019 by Hoàng Trúc: bổ sung bút toán doanh số bán hàng T04 khi xử lý lọc dữ liệu
---- Modified on 18/11/2019 by Van Minh: bổ sung ReCreditLimit Column
---- Modified on 09/01/2020 by Van Minh: bổ sung TradeName Column
---- Modified on 18/02/2020 by Huỳnh Thử: Bổ sung AccountID
---- Modified on 05/03/2020 by Văn Minh: Đẩy trường AccountID vào customize và Fix lỗi @AddColumnReport chưa SET giá trị trả ra câu SQL rỗng
---- Modified on 06/04/2020 by Huỳnh Thử: Lấy Thêm trường Địa chỉ giao hàng: DeAddress, Hạn mức tuổi nợ: ReDays của dổi tượng
---- Modified on 14/05/2020 by Huỳnh Thử: Where thêm GroupID theo G03 phải thu, G04 phải trả theo thiết lập báo cáo
---- Modified on 18/05/2020 by Huỳnh Thử: Bị tách ra nhiều dòng, nhờ em sửa lại cho gom thành 1 dòng nhé. Bỏ trường OrderDate
---- Modified on 14/07/2020 by Nhựt Trường: CustomizeIndex = 110 (Song Bình)-Thêm trường OrderDate (theo điều kiện nếu OrderDate là NULL thì lấy VoucherDate) và bỏ bớt hàm convert để cải thiện tốc độ in.
---- Modified on 26/08/2020 by Nhựt Trường: Thêm Select top 1 vào câu truy vấn con khi lấy trường DiscountOriginalAmount, DiscountConvertedAmount để tạo view AV0327.
---- Modified on 03/11/2020 by Nhựt Trường: Bỏ câu truy vấn con khi lấy trường DiscountOriginalAmount,DiscountConvertedAmount khi tạo view AV0327, để cải tiến tốc độ.
---- Modified on 22/04/2021 by Huỳnh Thử: Tách Store [An Phúc Thịnh]
---- Modified on 16/07/2021 by Huỳnh Thử: Bổ sung 23:59:59.999
---- Modified on 26/05/2022 by Nhựt Trường: [2022/05/IS/0157] - CustomizeIndex = 110 (Song Bình)-Không lấy OrderDate lên báo cáo, chỉ sử dụng để tính thành tiền.
---- Modified on 22/07/2022 by Nhật Thanh: Bổ sung lấy Ana04ID
---- Modified by Đức Duy on 20/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.
---- Modified by Đình Định on 24/04/2023: Bỏ CustomerIndex @AddColumnReport vì sử dụng luồng chuẩn.
---- Modified by Đình Định on 06/10/2023: SONGBINH - Bổ sung in báo cáo theo ngày debit note (OrderDate).
---- Modified by Xuân Nguyên on 07/11/2023: [2023/11/IS/0038] - Sửa lại cách lấy OrderDate(nếu OrderDate null thì vẫn để null)
-- <Example>
---- 
CREATE PROCEDURE [dbo].[AP0316_SONGBINH]
( 
...
DECLARE @sSQL AS nvarchar(MAX),
	...
	BEGIN
		...
		-- 1. Lấy thông tin thiết lập báo cáo từ bảng AT4710.
		SELECT TOP 1	@ReportName1 = replace(ReportName1,'''',''''''), 
						...
						@GroupID = CASE WHEN IsReceivable = 1 THEN 'G03' ELSE 'G04' END
		FROM	AT4710 WITH (NOLOCK)
		WHERE	ReportCode =  @ReportCode AND DivisionID = @DivisionID

		-- 2. Tao view lay so thanh toan cong no
		IF @IsReceivable = 1 
			SET @sSQL = ' 
				SELECT DISTINCT AT0303.DivisionID, AT0303.ObjectID, AT0303.AccountID, AT0303.CurrencyID, 
								AT0303.DebitVoucherID AS VoucherID, AT0303.DebitBatchID AS BatchID,
				...
				LEFT JOIN (	SELECT DISTINCT DivisionID, VoucherID, BatchID, TableID, ObjectID, VoucherDate
							  FROM AV0302 WITH (NOLOCK)
						  ) AS  AV0302 
					ON  AV0302.DivisionID = AT0303.DivisionID
					AND AV0302.VoucherID = AT0303.CreditVoucherID 
					AND AV0302.TableID = AT0303.CreditTableID 
					AND AV0302.ObjectID = AT0303.ObjectID 
					AND AV0302.BatchID = AT0303.DebitBatchID
				LEFT JOIN AT1005 WITH (NOLOCK) ON AT1005.AccountID = AT0303.AccountID
				WHERE AT0303.DivisionID = '''+@DivisionID+''' AND
				  ...
				  AND AT1005.GroupID = '''+@GroupID+'''
				GROUP BY AT0303.DivisionID, AT0303.ObjectID, AT0303.AccountID, AT0303.CurrencyID, AT0303.DebitVoucherID, AT0303.DebitBatchID '

		ELSE   --Cong no phai tra da giai tru
			SET @sSQL = ' 
				SELECT DISTINCT AT0404.DivisionID, AT0404.ObjectID, AT0404.AccountID, AT0404.CurrencyID, 
								AT0404.CreditVoucherID AS VoucherID, AT0404.CreditBatchID AS BatchID,
				 ...
				 LEFT JOIN  ( SELECT DISTINCT DivisionID, VoucherID, BatchID, TableID, ObjectID, VoucherDate 
							    FROM AV0402 WITH (NOLOCK)
						    ) AS  AV0402 
					ON	AV0402.DivisionID = AT0404.DivisionID AND AV0402.VoucherID = AT0404.DebitVoucherID 
					AND AV0402.TableID = AT0404.DebitTableID
					AND AV0402.ObjectID = AT0404.ObjectID
					AND AV0402.BatchID = AT0404.CreditBatchID
				...
				WHERE AT0404.DivisionID = ''' +@DivisionID+ ''' AND
					  ...
					  CONVERT(DATETIME,CONVERT(VARCHAR(10), ISNULL(AT0404.DebitVoucherDate,''01/01/1900''),101), 101)  <='''+CONVERT(nvarchar(10),@ReportDate,101)+''' AND
					  AT1005.GroupID = '''+@GroupID+''' 
				GROUP BY AT0404.DivisionID, AT0404.ObjectID, AT0404.AccountID, AT0404.CurrencyID, AT0404.CreditVoucherID, AT0404.CreditBatchID '

		IF NOT EXISTS (SELECT NAME FROM SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AV0317]') AND OBJECTPROPERTY(ID, N'ISVIEW') = 1)
			EXEC ('  CREATE VIEW AV0317 	---CREATED BY AP0316_SONGBINH
						AS ' + @sSQL)
		ELSE
			EXEC ('  ALTER VIEW AV0317   	---CREATED BY AP0316_SONGBINH
						AS ' + @sSQL)
		-- print @sSQL
		-- 3. Tao view AV0327 de lay du lieu len bao cao
		SET @sSQL = ' 
		SELECT	AV4301.DivisionID, AV4301.ObjectID, AV4301.AccountID, AV4301.CorAccountID, AV4301.VoucherTypeID, 
				...
				A1.OriginalAmount AS DiscountOriginalAmount,
				A1.ConvertedAmount AS DiscountConvertedAmount,
				...
				' + CASE WHEN @CustomerName = 110 THEN ',AV4301.OrderID, AV4301.OrderDate' ELSE '' END +'
		FROM	AV4301 AV4301 WITH (NOLOCK)
		...
		LEFT JOIN AV4301 A1 WITH (NOLOCK) ON A1.VoucherID = AV4301.VoucherID AND A1.TransactionTypeID=''T64'' AND A1.D_C = AV4301.D_C
		...
		IF @IsReceivable = 1   --Cong No phai thu
  			SET @sSQL = @sSQL +'  AND (AV4301.D_C=''D'') ' 
		ELSE
			SET @sSQL = @sSQL +'  AND (AV4301.D_C=''C'') ' 

		IF NOT EXISTS (SELECT NAME FROM SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AV0327]') AND OBJECTPROPERTY(ID, N'ISVIEW') = 1)
			EXEC ('  CREATE VIEW AV0327 	---CREATED BY AP0316_SONGBINH
						AS ' + @sSQL)
		ELSE
			EXEC ('  ALTER VIEW AV0327   	---CREATED BY AP0316_SONGBINH
						AS ' + @sSQL)
		...
		IF @Group1ID != ''
			BEGIN
				...
				SET @sFROM = @sFROM + '
						LEFT JOIN AV6666 V1 WITH (NOLOCK) ON V1.DivisionID = AV0327.DivisionID AND V1.SelectionType = ''' + @Group1ID + ''' AND V1.SelectionID = AV0327.' + @GroupName1
		...
		IF @Group2ID != ''
			BEGIN
				...
				SET @sFROM = @sFROM + '
						LEFT JOIN AV6666 V2 WITH (NOLOCK) ON V2.DivisionID = AV0327.DivisionID AND V2.SelectionType = ''' + @Group2ID + ''' AND V2.SelectionID = AV0327.' + @GroupName2
		...
		IF @Group3ID != ''
			BEGIN
				...
				SET @sFROM = @sFROM + '
						LEFT JOIN AV6666 V3 WITH (NOLOCK) ON V3.DivisionID = AV0327.DivisionID AND V3.SelectionType = ''' + @Group3ID + ''' AND V3.SelectionID = AV0327.' + @GroupName3
		...
		IF @Selection01ID != ''
			BEGIN
				...
				SET @sWHERE = @sWHERE + ' AND 
					(AV0327.' + @SelectionName1 + ' between ''' + @Filter1IDFrom + ''' AND ''' + @Filter1IDTo + '''' + CASE WHEN @CustomerName = 13 THEN ' OR AV0327.TransactionTypeID in ( ''T14'',''T04'')' ELSE '' END + ') '
			END
		...
		IF @Selection02ID != ''
			BEGIN
				...
				SET @sWHERE = @sWHERE + ' AND 
					(AV0327.' + @SelectionName2 + ' between ''' + @Filter2IDFrom + ''' AND ''' + @Filter2IDTo + '''' + CASE WHEN @CustomerName = 13 THEN ' OR AV0327.TransactionTypeID IN ( ''T14'',''T04'')' ELSE '' END + ') '
			END
		...
		IF @Selection03ID != ''
			BEGIN
				...
				SET @sWHERE = @sWHERE + ' AND 
					(AV0327.' + @SelectionName3 + ' between ''' + @Filter3IDFrom + ''' AND ''' + @Filter3IDTo + '''' + CASE WHEN @CustomerName = 13 THEN ' OR AV0327.TransactionTypeID IN ( ''T14'',''T04'')' ELSE '' END + ') '
			END
		...
		IF @CustomerName = 110 and @Datetype = 'DueDate'
		BEGIN 
		---------------------- 1 --------------------------
				SET @SelectTH = 
					'Convert(int, CONVERT(DATETIME,(''' + CONVERT(nvarchar(10),@ReportDate,101) + '''),101) - CONVERT (DATETIME,CONVERT(VARCHAR(10),AV0327.OrderDate,101),101))'
		...
		------------------------ 2 -----------------------
			...
			BEGIN
				SET @sWHERE = @sWHERE + '  AND 
					CONVERT(DATETIME,AV0327.OrderDate,101) >= ''' + CONVERT(nvarchar(10),@ReportDate,101) + 
						(CASE WHEN @MaxDate = 0 THEN '''' ELSE 
					''' AND CONVERT(DATETIME,AV0327.OrderDate,101) <= ''' + CONVERT(nvarchar(10),@ReportDate + @MaxDate,101) + '''' end)
			END

			IF @IsBefore = 1 AND @IsType = 1 -- Quá hạn
				BEGIN				
					SET @sWHERE = @sWHERE + '  AND 
							CONVERT(DATETIME,AV0327.OrderDate,101) <= ''' + CONVERT(nvarchar(10),@ReportDate,101) +  ' 23:59:59.999' +
								(CASE WHEN @MaxDate = 0 THEN '''' ELSE 
								''' AND CONVERT(DATETIME,AV0327.OrderDate,101) >= ''' + CONVERT(nvarchar(10),@ReportDate - @MaxDate,101) + ' 00:00:00.000' + '''' end)
				END
		END
		ELSE
		BEGIN
	    ...
	    BEGIN
		 ...
		 ELSE
 			BEGIN
 				SET @sSELECT1 = @sSELECT1 + '  0 AS Days,MAX(AV0327.Ana04ID) AS Ana04ID, '
				SET @sGROUPBY = @sGROUPBY + ',
					AV0327.VoucherID, AV0327.VoucherDate, AV0327.VoucherNo, AV0327.InvoiceDate, AV0327.Serial, 
					AV0327.DueDate, AV0327.InvoiceNo, AV0327.VDescription'	
 			END
			   ...
			   IF (@IsBefore = 0 AND @IsType = 1) 
					SET @sSELECT2 = @sSELECT2 +
							...
							( CASE WHEN '+ @SelectTH + ' + 1 >= ' + ltrim(str(@FromDay)) + ' 
								THEN SUM(ISNULL(AV0327.OriginalAmountCN,0)) - ISNULL(AV0327.GivedOriginalAmount,0) -ISNULL(AV0327.DiscountOriginalAmount,0) ELSE 0 end)
								as OriginalAmount' + ltrim(str(@Orders)) + ',
							( CASE WHEN '+ @SelectTH + ' + 1 >= ' + ltrim(str(@FromDay)) + '	
								THEN SUM(ISNULL(AV0327.ConvertedAmount,0)) - ISNULL(AV0327.GivedConvertedAmount,0) -ISNULL(AV0327.DiscountConvertedAmount,0) ELSE 0 end) 
					...
					BEGIN
						SET @sSELECT2 = @sSELECT2 +
							...
							( CASE WHEN '+@SelectQH+ ' >= ' + ltrim(str(@FromDay)) + ' 
								THEN SUM(ISNULL(AV0327.OriginalAmountCN,0))  -  ISNULL(AV0327.GivedOriginalAmount,0) -ISNULL(AV0327.DiscountOriginalAmount,0) ELSE 0 end)	as OriginalAmount' + ltrim(str(@Orders)) + ',
							( CASE WHEN '+@SelectQH+ ' >= ' + ltrim(str(@FromDay)) + '
								THEN SUM(ISNULL(AV0327.ConvertedAmount,0)) - ISNULL(AV0327.GivedConvertedAmount,0) -ISNULL(AV0327.DiscountConvertedAmount,0) ELSE 0 end) 	as ConvertedAmount' + ltrim(str(@Orders)) + ',
							( CASE WHEN '+ @SelectQH +' >= ' + ltrim(str(@FromDay)) + '	
								THEN ISNULL(AV0327.GivedOriginalAmount,0) ELSE 0 end) AS GivedOriginalAmount' + ltrim(str(@Orders)) + ',
							( CASE WHEN '+ @SelectQH +' >= ' + ltrim(str(@FromDay)) + '
								THEN ISNULL(AV0327.GivedConvertedAmount,0) ELSE 0 end) AS GivedConvertedAmount' + ltrim(str(@Orders)) + ', '
		...
		Set @sSQL1 = '
			SELECT	AV0327.DivisionID, 
					' + @sSELECT1 + '
					'
		Set @sSQL2='CurrencyIDCN, AV0327.ObjectID, AT1202.ObjectName, 
					AT1202.Address, AT1202.Note, AT1202.Tel, AT1202.Note1,AV0327.CurrencyID, AV0327.BatchID,
					AT1202.BankAccountNo, AT1202.BankName, AT1202.LicenseNo, AT1202.LegalCapital, AT1202.ReDueDays, AT1202.ReCreditLimit, DeAddress, ReDays,
					...
					, AT1202.TradeName, AV0327.AccountID
			FROM	AV0327 WITH (NOLOCK)
			LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.DivisionID IN (''' +@DivisionID+ ''', ''@@@'') AND AT1202.ObjectID = AV0327.ObjectID
			LEFT JOIN AT1005 WITH (NOLOCK) ON AT1005.AccountID = AV0327.AccountID
			' + @sFROM + '
			...
			GROUP BY	AV0327.ObjectID, AT1202.ObjectName, AT1202.Address,  AT1202.Note,  
						...
						AV0327.VoucherID, AV0327.BatchID, AT1202.ReCreditLimit, DeAddress, ReDays,
						AT1202.BankAccountNo, AT1202.BankName, AT1202.LicenseNo, AT1202.LegalCapital, AT1202.ReDueDays, AV0327.' + @DateType + ', AV0327.DiscountOriginalAmount, AV0327.DiscountConvertedAmount
						, AT1202.TradeName, AV0327.AccountID '
		...
		IF NOT EXISTS (SELECT NAME FROM SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AV0326]') AND OBJECTPROPERTY(ID, N'ISVIEW') = 1)
			EXEC ('  CREATE VIEW AV0326 	---CREATED BY AP0316_SONGBINH
					AS ' + @sSQL1 + @sSELECT2 + @sSQL2 )
		ELSE
			EXEC ('  ALTER VIEW AV0326   	---CREATED BY AP0316_SONGBINH
					AS ' + @sSQL1 + @sSELECT2 + @sSQL2 )
		...
		Begin
			...
			SELECT	Days, AV0326.DivisionID
					...
					ObjectID, ObjectName, Address,  Note,  Tel, Note1,AV0326.CurrencyID, ReCreditLimit,DeAddress,ReDays,
			...
			GROUP BY ObjectID, ObjectName, Address, Days, Note, Tel, Note1, AV0326.DivisionID ' + @sGROUPBY1 +',
					...
					BankAccountNo, BankName, LicenseNo, LegalCapital, ReDueDays,ReCreditLimit,DeAddress,ReDays '
	   End
       Else
	   begin
			Set @sSQL = '
			SELECT	Days, AV0326.DivisionID,AV0326.CurrencyID
					' + @sGROUPBY1 + ',MAX(AV0326.Ana04ID) AS Ana04ID,
					AV0326.Title1, SUM(AV0326.ConvertedAmount1) AS ConvertedAmount1, SUM(AV0326.OriginalAmount1) AS OriginalAmount1, 
					AV0326.Title2, SUM(AV0326.ConvertedAmount2) AS ConvertedAmount2, SUM(AV0326.OriginalAmount2) AS OriginalAmount2,
					AV0326.Title3, SUM(AV0326.ConvertedAmount3) AS ConvertedAmount3, SUM(AV0326.OriginalAmount3) AS OriginalAmount3,
					AV0326.Title4, SUM(AV0326.ConvertedAmount4) AS ConvertedAmount4, SUM(AV0326.OriginalAmount4) AS OriginalAmount4,
					...
					AV0326.Title9, SUM(AV0326.ConvertedAmount9) AS ConvertedAmount9, SUM(AV0326.OriginalAmount9) AS OriginalAmount9,
					AV0326.Title10, SUM(AV0326.ConvertedAmount10) AS ConvertedAmount10, SUM(AV0326.OriginalAmount10) AS OriginalAmount10,	
					ObjectID, ObjectName, Address,  Note,  Tel, Note1,ReCreditLimit,DeAddress,ReDays,
					BankAccountNo, BankName, LicenseNo, LegalCapital, ReDueDays, SUM(GivedConvertedAmount) AS GivedConvertedAmount' + CASE WHEN @CustomerName = 13 THEN ', MAX(AV0326.I01ID) AS I01ID, MAX(AV0326.I01Name) AS I01Name' ELSE '' END +  '
					, AV0326.TradeName, AV0326.AccountID
			 FROM	AV0326 WITH (NOLOCK)
			WHERE	ConvertedAmount1+ConvertedAmount2+ConvertedAmount3+ConvertedAmount4+ConvertedAmount5+
					ConvertedAmount6+ConvertedAmount7+ConvertedAmount8+ConvertedAmount9+ConvertedAmount10 <> 0 
			GROUP BY ObjectID, ObjectName, Address, Days, Note, Tel, Note1, AV0326.DivisionID ' + @sGROUPBY1 +',
		...
		IF NOT EXISTS (SELECT NAME FROM SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AV0316]') AND OBJECTPROPERTY(ID, N'ISVIEW') = 1)
			EXEC ('  CREATE VIEW AV0316 	---CREATED BY AP0316_SONGBINH
					AS ' + @sSQL)
		ELSE
			EXEC ('  ALTER VIEW AV0316   	---CREATED BY AP0316_SONGBINH
					AS ' + @sSQL)
	END
