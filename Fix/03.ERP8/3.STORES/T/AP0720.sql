...
---- Modified by Hương Nhung on 25/10/2023: [2023/10/IS/0216] Fix lỗi Incorrect syntax near 'AT1309'.
...
IF @CustomerName = 38 --Customize BBL
BEGIN
	EXEC AP0720_BBL @DivisionID, @FromWareHouseID, @ToWareHouseID,					
			        @FromObjectID,@ToObjectID,@FromInventoryID, @ToInventoryID,
					@FromMonth,@FromYear,
			        @ToMonth, @ToYear, @FromDate, @ToDate,
			        @IsDate, @FromAna01ID, @ToAna01ID,
			        @FromAna02ID, @ToAna02ID,
			        @FromAna03ID, @ToAna03ID,
			        @FromAna04ID, @ToAna04ID,
			        @FromAna05ID, @ToAna05ID,
			        @FromAna06ID, @ToAna06ID,
			        @FromAna07ID, @ToAna07ID,
			        @FromAna08ID, @ToAna08ID,
			        @FromAna09ID, @ToAna09ID,
			        @FromAna10ID, @ToAna10ID
END
ELSE
BEGIN
-- Tao bang tam 
...
INSERT INTO #Tempt EXEC AP9024 @DivisionID, 'ASOFTADMIN', @FromWareHouseID, @ToWareHouseID, @FromInventoryID,
							   @ToInventoryID, @FromObjectID, @ToObjectID, @IsTime, '', '', '', '', '', ''
...
Else 
	Set @strTimeSD =N' and ( D_C=''BD'' or AV7008.TranMonth+ 100*TranYear< '+@FromMonthYearText+N' ) ' ---06/01/2014MTuyen edit

Set @sqlSelect1 = N'
---- Lay so du
SELECT 	 AV7008.WareHouseID, AV7008.WareHouseName, AV7008.ObjectID, AV7008.InventoryID, AV7008.InventoryName, AV7008.RefInventoryID,
        ...
        sum(isnull(SignQuantity,0))  as BeginQuantity, sum(isnull(SignAmount,0)) as BeginAmount, AV7008.DivisionID, AV7008.VoucherDate ' 
...
WHERE 	'+@WareHouseWhere+'
		...
		(AV7008.WareHouseID between N'''+@FromWareHouseID+N''' and N'''+@ToWareHouseID+N''') and		(AV7008.ObjectID between  N'''+@FromObjectID+N''' and  N'''+@ToObjectID+N''')  ' + @AnaWhere + N' ' 
set @sqlWhere1 = @sqlWhere1 + @strTimeSD+' '

Set @sqlGroupBy1 = N' 
	GROUP BY AV7008.DivisionID, AV7008.WareHouseID, AV7008.WareHouseName,
			 ...
			 AV7008.UnitName , AV7008.Specification, AV7008.VoucherDate' + CASE WHEN @CustomerName = 84 THEN', AV7008.Ana04ID' ELSE '' END
...
EXEC (@sqlSelect1 + @sqlFrom1 + @sqlWhere1 + @sqlGroupBy1 + @sqlSelect2 + @sqlFrom2 + @sqlWhere2 + @sqlGroupBy2 + @sqlSelect2a + @sqlFrom2a + @sqlWhere2a + @sqlGroupBy2a + @sqlSelect3 + @sqlSelect3a + @sqlFrom3 + @sqlFrom3a + @sqlGroupBy3 + @sqlGroupBy3a + @sqlSelectunion + @sqlWhereUnion)
