IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WITH (NOLOCK) WHERE ID = OBJECT_ID(N'[DBO].[AP8123]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
...
---- Modified by Nhựt Trường on 19/04/2021: Customer MEKIO: Thêm cột Invoicecheck vào bảng tạm #data để check đối tượng + hóa đơn + serial thì mới đồng bộ thông tin.
---- Modified by Nhựt Trường on 31/08/2021: Tách store AP8123_MK cho khách hàng MEIKO.
...
BEGIN
	...
	return
END
ELSE IF(@CustomerName = 50 OR @CustomerName = 115) -- Mekio và Mte
BEGIN
	EXEC AP8123_MK @DivisionID, @UserID, @ImportTemplateID, @XML
END
ELSE
...
OPEN @cPKey
FETCH NEXT FROM @cPKey INTO @Row, @InvoiceNo, @Serial, @PVoucherDate, @VATGroupID,@ObjectID
WHILE @@FETCH_STATUS = 0
BEGIN
...
(
	...
	InventoryName1,
	WOrderID, WTransactionID, IsWithhodingTax,PaymentTermID, DueDate,OrderID
)
SELECT	D.DivisionID,		D.PVoucherID,		D.PVoucherID,		D.PTransactionID,	'AT9000',
		...
		CASE WHEN ISNULL(D.PaymentTermID,'') = '' THEN A.PaPaymentTermID ELSE PaymentTermID END , D.DueDate,D.PONo
FROM	#Data D
