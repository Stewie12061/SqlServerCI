IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7620_STD]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP7620_STD]
GO
...
---- Modified on 11/01/2019 by Kim Thư: Bổ sung Amount00 - Lũy kế từ đầu năm của kỳ hiện tại đến kỳ hiện tại
---- Modified on 31/01/2019 by Kim Thư: Tách store riêng cho KOYO
---- Modified on 12/05/2022 by Văn Tài: Bổ sung tên bảng cho cột DivisionID.
---- Modified on 27/10/2022 by Đức Tuyên: Tách store riêng cho INNOTEK
-- <Example>
---- 
-- <Summary>
CREATE PROCEDURE [dbo].[AP7620_STD]   
...
IF @CustomerName = 52 --- Customize KOYO
	EXEC AP7620_KY @DivisionID,   @ReportCode,   
  @FromMonth ,     @FromYear  ,     @ToMonth ,     @ToYear  ,  
  @FromValueID ,     @ToValueID ,    @StrDivisionID   
ELSE
IF @CustomerName = 161 --- Customize INNOTEK
	EXEC AP7620_INT @DivisionID,   @ReportCode,   
  @FromMonth ,     @FromYear  ,     @ToMonth ,     @ToYear  ,  
  @FromValueID ,     @ToValueID ,    @StrDivisionID   
ELSE
Begin   
