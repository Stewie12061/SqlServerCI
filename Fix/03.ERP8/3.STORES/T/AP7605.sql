...
DECLARE @CurrentAccumulator AS nvarchar(100),
	...
	UPDATE 	AT7606
		SET		Amount1 = Amount1 + isnull(@Amount1,0),
				...
				Amount10 = Amount10 + isnull(@Amount10,0),
				Amount11 = Amount11 + isnull(@Amount11,0),			
				Amount2A = Amount2A + isnull(@Amount2,0),	
				Amount4A = Amount4A + isnull(@Amount4,0),			
				Amount10A = Amount10A + isnull(@Amount10A,0),
				Amount11A = Amount11A + isnull(@Amount11A,0)								
	...
	Begin
		...
		IF @Amount4A IS NULL
		    SET @Amount4A = 0
		IF @Amount10A IS NULL
		    SET @Amount10A = 0
		IF @Amount11A IS NULL
		    SET @Amount11A = 0		    	            		    
		...
		IF @CurrentAccuSign  = '-'
			...
			SET @Amount10 = @Amount10*-1
			SET @Amount11 = @Amount11*-1	
			SET @Amount2A = @Amount2A*-1	
			SET @Amount4A = @Amount4A*-1	
			SET @Amount10A = @Amount10A*-1
			SET @Amount11A = @Amount11A*-1							
		    END
			...
			SET		Amount1 = Amount1 + isnull(@Amount1,0),
					...
					Amount10 = Amount10 + isnull(@Amount10,0),
					Amount11 = Amount11 + isnull(@Amount11,0),		
					Amount2A = Amount2A + isnull(@Amount2A,0),		
					Amount4A = Amount4A + isnull(@Amount4A,0),		
					Amount10A = Amount10A + isnull(@Amount10A,0),
					Amount11A = Amount11A + isnull(@Amount11A,0)																					
