IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_GODREJ]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_GODREJ]
GO
...
-- AP0146_GODREJ
-- <Summary>
---- Stored load dữ liệu hóa đơn phục vụ phát hành hóa đơn điện tử - Customize GODREJ
---- Created on 10/09/2020 Đức Thông
-- <Param>
-- <History>
---- Modified by Đức Thông on 09/11/2020: Bổ sung thêm mã hàng và trước tên hàng
---- Modified by Đức Thông on 09/22/2020: Bỏ stt ở dòng chú thích
---- Modified by Huỳnh Thử on 09/29/2020: ReName DueDate -> DueDates
---- Modified by Huỳnh Thử on 09/30/2020: Lấy thêm thông tin hoa hồng -> TransactionType = T54@TotalRose
---- Modified by Huỳnh Thử on 10/07/2020: Check IsNull @TotalRose
---- Modify on 02/10/2020 by Nhựt Trường: (Sửa danh mục dùng chung)Bổ sung điều kiện DivisionID IN cho bảng AT1302.
---- Modify on14/10/2020  by Hoài Phong : -Bổ sung điều kiện Cho HDTT GODREJ
---- Modify on 08/12/2020 by Hoài Phong : -Bổ sung  lấy PayMethodID trong Funtion GetPayMethodID
---- Modify on 12/08/2022 by Nhật Thanh : Ngắt @sSQL4 xuống @sSQL5 vì quá dài
...
-- <Reference>
---- 
CREATE PROCEDURE [dbo].[AP0146_GODREJ] @DivisionID AS NVARCHAR(50)
...
SET @sSQL1A1 = 
	...
	CASE AT9000.VATGroupID WHEN ''T00'' THEN 1 WHEN ''T05'' THEN 2 WHEN ''T10'' THEN 3 WHEN ''TS0'' THEN 4 WHEN ''TZ0'' THEN 5 WHEN ''T08'' THEN 9 ELSE NULL END AS TaxRateID,
...
SET @sSQLA = 
	...
	,PaymentName,IsDiscount,DivisionNameE,AddressE,District,DContactPerson,OriginalAfterVATAmount,ConvertedAfterVATAmount,WSourceNo,WLimitDate,I04ID,I04Name,0 AS ItemTypeID,CusBankAccountNo
FROM #TEMP
ORDER BY Remark
		) X
	'
...
IF EXISTS (
		...
		WHERE DefDivisionID = @DivisionID
			AND EInvoicePartner = 'BKAV'
		)
	AND EXISTS (
		SELECT TOP 1 1
		FROM AT9000 WITH (NOLOCK)
		WHERE VoucherID = @VoucherID
			AND ISNULL(Parameter04, '') <> ''
		)
	SET @sSQL3 = 
		'	
UNION ALL
SELECT TOP 1 '''' AS Remark ,CusCode AS CusCode,'+@ASName+',CusAddress AS CusAddress,'''' AS CusPhone,'''' AS CusEmail,'''' AS CusEmailCC,'''' AS CusTaxCode,Buyer AS Buyer,
'''' AS CusbankNo,'''' AS CusBankName, '''' AS PaymentMethod,0 AS Total,0 AS DiscountAmount,0 AS AfterAmount,0 AS VATAmount,0 AS VAT_Rate,'''' AS ProdID,Parameter04 AS ProdName,
'''' AS ProdUnit,0 AS ProdQuantity,0 AS ProdPrice ,0 AS Amount,'''' AS InheritFkey,0 AS EInvoiceType,0 AS TypeOfAdjust,'''' AS TDescription,'''' AS Ana02ID,'''' AS Ana02Name,
'''' AS DonDatHangSo,0 AS OriginalAmount,'''' AS KindOfService ,CurrencyID AS Extra1,0 AS VATAmount0,0 AS VATAmount10,1 AS Extra,InvoiceDate,CurrencyID,'''' AS VoucherID,
'''' AS PaymentID,'''' AS VoucherNo,'''' AS BatchID,'''' AS TransactionID,'''' AS TableID ,0 AS TranMonth,0 AS TranYear,'''' AS TransactionTypeID,'''' AS ObjectID,'''' AS CreditObjectID,
'''' AS VATNo,'''' AS VATObjectID,'''' AS VATObjectName,'''' AS VATObjectAddress,'''' AS DebitAccountID ,'''' AS CreditAccountID,1 AS ExchangeRate,0 AS UnitPrice,0 AS OriginalAmountAT9000,
0 AS ConvertedAmount,0 AS ImTaxOriginalAmount,0 AS ImTaxConvertedAmount,0 AS ExpenseOriginalAmount,0 AS ExpenseConvertedAmount ,0 AS IsStock,'''' AS VoucherDate,'''' AS VoucherTypeID,
'''' AS VATGroupID,'''' AS Serial,'''' AS InvoiceNo,0 AS Orders,'''' AS EmployeeID,'''' AS SenderReceiver,'''' AS SRDivisionName,'''' AS SRAddress ,'''' AS RefNo01,'''' AS RefNo02,
'''' AS VDescription,'''' AS BDescription,0 AS Quantity,'''' AS InventoryID,'''' AS UnitID,'''' AS STATUS,0 AS IsAudit,0 AS IsCost,'''' AS Ana01ID,'''' AS Ana03ID ,'''' AS Ana04ID,
'''' AS Ana05ID,'''' AS Ana06ID,'''' AS Ana07ID,'''' AS Ana08ID,'''' AS Ana09ID,'''' AS Ana10ID,'''' AS PeriodID,'''' AS ExpenseID,'''' AS MaterialTypeID,'''' AS ProductID ,
'''' AS CreateDate,'''' AS CreateUserID,'''' AS LastModifyDate,'''' AS LastModifyUserID,0 AS OriginalAmountCN,1 AS ExchangeRateCN,'''' AS CurrencyIDCN,0 AS DueDays,'''' AS DueDates,
0 AS DiscountRate ,'''' AS OrderID,'''' AS CreditBankAccountID,'''' AS DebitBankAccountID,0 AS CommissionPercent,'''' AS InventoryName1,'''' AS PaymentTermID,0 AS DiscountAmountAT9000,
'''' AS OTransactionID,0 AS IsMultiTax ,0 AS VATOriginalAmount,0 AS VATConvertedAmount,'''' AS ReVoucherID,'''' AS ReBatchID,'''' AS ReTransactionID,'''' AS Parameter01,'''' AS Parameter02,
'''' AS Parameter03,'''' AS Parameter04,'''' AS Parameter05 ,'''' AS Parameter06,'''' AS Parameter07,'''' AS Parameter08,'''' AS Parameter09,'''' AS Parameter10,'''' AS MOrderID,'''' AS SOrderID,
'''' AS MTransactionID,'''' AS STransactionID,'''' AS RefVoucherNo ,0 AS IsLateInvoice,0 AS ConvertedQuantity,0 AS ConvertedPrice,'''' AS ConvertedUnitID,0 AS ConversionFactor,
0 AS UParameter01,0 AS UParameter02,0 AS UParameter03,0 AS UParameter04 ,0 AS UParameter05,'''' AS PriceListID,'''' AS WOrderID,'''' AS WTransactionID,'
	IF EXISTS (
		SELECT TOP 1 1
		FROM AT0000
		WHERE DefDivisionID = @DivisionID
	...
	SET @sSQL3A=N' 0 AS MarkQuantity,'''' AS TVoucherID,
0 AS OldCounter,0 AS NewCounter,0 AS OtherCounter,'''' AS TBatchID,'''' AS ContractDetailID ,'''' AS InvoiceCode,'''' AS InvoiceSign,'''' AS RefInfor,0 AS StandardPrice,0 AS StandardAmount,
0 AS IsCom,0 AS VirtualPrice,0 AS VirtualAmount,'''' AS ReTableID,'''' AS DParameter01,'''' AS DParameter02 ,'''' AS DParameter03,'''' AS DParameter04,'''' AS DParameter05,'''' AS DParameter06,
...
'''' AS ETaxID,0 AS ETaxConvertedUnit,0 AS ETaxConvertedAmount,'''' AS ETaxTransactionID,0 AS AssignedSET,'''' AS SETID,'''' AS SETUnitID,0 AS SETTaxRate ,0 AS SETConvertedUnit,0 AS SETQuantity,
0 AS SETOriginalAmount,0 AS SETConvertedAmount,'''' AS SETConsistID,'''' AS SETTransactionID,0 AS AssignedNRT,0 AS NRTTaxAmount,'''' AS NRTClassifyID,'''' AS NRTUnitID,0 AS NRTTaxRate ,
0 AS NRTConvertedUnit,0 AS NRTQuantity,0 AS NRTOriginalAmount,0 AS NRTConvertedAmount,'''' AS NRTConsistID,'''' AS NRTTransactionID,'''' AS CreditObjectName,'''' AS CreditVATNo,0 AS IsPOCost,
0 AS TaxBaseAmount ,0 AS WTCExchangeRate,0 AS WTCOperator,0 AS IsFACost,0 AS IsInheritFA,'''' AS InheritedFAVoucherID,0 AS AVRExchangeRate,0 AS PaymentExchangeRate,0 AS IsMultiExR,
'''' AS ExchangeRateDate,0 AS DiscountSalesAmount ,0 AS IsProInventoryID,0 AS InheritQuantity ,0 AS DiscountPercentSOrder ,0 AS DiscountAmountSOrder ,0 AS IsWithhodingTax ,0 AS IsSaleInvoice ,
'''' AS WTTransID ,0 AS DiscountSaleAmountDetail ,'''' AS ABParameter01 ,'''' AS ABParameter02 ,'''' AS ABParameter03 ,'''' AS ABParameter04 ,'''' AS ABParameter05 ,'''' AS ABParameter06 ,
...
0 AS IsVATWithhodingTax ,0 AS VATWithhodingRate ,0 AS IsEInvoice ,0 AS EInvoiceStatus ,0 AS IsAdvancePayment ,'''' AS Fkey ,0 AS IsInheritInvoicePOS ,'''' AS InheritInvoicePOS ,0 AS IsInheritPayPOS ,
'''' AS InheritPayPOS ,0 AS IsInvoiceSuggest ,'''' AS RefVoucherDate ,0 AS IsDeposit ,'''' AS ReTransactionTypeID ,'''' AS ImVoucherID ,'''' AS ImTransactionID ,'''' AS SourceNo ,'''' AS LimitDate ,
0 AS IsPromotionItem ,'''' AS ObjectName1 ,'''' AS TaxRateID ,2 AS PayMethodID ,0 AS DiscountedUnitPrice ,0 AS ConvertedDiscountedUnitPrice ,0 AS IsReceived ,'''' AS Ana01Name ,'''' AS Ana03Name ,
'''' AS Ana04Name ,'''' AS Ana05Name ,'''' AS Ana06Name ,'''' AS Ana07Name ,'''' AS Ana08Name ,'''' AS Ana09Name ,'''' AS Ana10Name ,'''' AS BankAccountNo ,'''' AS BankName ,'''' AS ContactPerson ,
0 AS DVATOriginalAmount ,0 AS DVATConvertedAmount ,'''' AS ConvertedUnitName ,'''' AS PaymentName ,0 AS IsDiscount ,'''' AS DivisionNameE ,'''' AS AddressE ,'''' AS District ,'''' AS DContactPerson ,
0 AS OriginalAfterVATAmount ,0 AS ConvertedAfterVATAmount ,'''' AS WSourceNo ,'''' AS WLimitDate ,'''' AS I04ID ,'''' AS I04Name ,4 AS ItemTypeID ,'''' AS CusBankAccountNo


FROM #TEMP'
		IF EXISTS (SELECT TOP 1 1  FROM AT9000 WITH (Nolock) WHERE VoucherID= @VoucherID AND TransactionTypeID='T54')
		BEGIN
		    
			SET @sSQL4= N'
			UNION ALL
			SELECT 999999 AS Remark ,'''' AS CusCode ,'''' AS '+@ASName+' ,'''' AS CusAddress ,'''' AS CusPhone ,'''' AS CusEmail ,'''' AS CusEmailCC ,'''' AS CusTaxCode ,'''' AS Buyer ,
'''' AS CusbankNo ,'''' AS CusBankName, '''' AS PaymentMethod ,Total AS Total ,0 AS DiscountAmount ,0 AS AfterAmount ,0 AS VATAmount ,0 AS VAT_Rate ,'''' AS ProdID ,(N'''+@BDescriptionRose+''') AS ProdName ,
'''' AS ProdUnit ,0 AS ProdQuantity ,0 AS ProdPrice ,('+STR(@TotalRose)+') AS Amount ,'''' AS InheritFkey ,0 AS EInvoiceType ,0 AS TypeOfAdjust ,'''' AS TDescription ,'''' AS Ana02ID ,'''' AS Ana02Name ,
'''' AS DonDatHangSo ,('+STR(@TotalRose)+') AS OriginalAmount ,'''' AS KindOfService ,'''' AS Extra1 ,0 AS VATAmount0 ,0 AS VATAmount10 ,1 AS Extra ,'''' AS InvoiceDate ,'''' ,'''' AS VoucherID ,'''' AS PaymentID ,
'''' AS VoucherNo ,'''' AS BatchID ,'''' AS TransactionID ,'''' AS TableID ,0 AS TranMonth ,0 AS TranYear ,'''' AS TransactionTypeID ,'''' AS ObjectID ,'''' AS CreditObjectID ,'''' AS VATNo ,
'''' AS VATObjectID ,'''' AS VATObjectName ,'''' AS VATObjectAddress ,'''' AS DebitAccountID ,'''' AS CreditAccountID ,1 AS ExchangeRate ,0 AS UnitPrice ,0 AS OriginalAmountAT9000 ,
0 AS ConvertedAmount ,0 AS ImTaxOriginalAmount ,0 AS ImTaxConvertedAmount ,0 AS ExpenseOriginalAmount ,0 AS ExpenseConvertedAmount ,0 AS IsStock ,'''' AS VoucherDate ,'''' AS VoucherTypeID ,
'''' AS VATGroupID ,'''' AS Serial ,'''' AS InvoiceNo ,0 AS Orders ,'''' AS EmployeeID ,'''' AS SenderReceiver ,'''' AS SRDivisionName ,'''' AS SRAddress ,'''' AS RefNo01 ,'''' AS RefNo02 ,
'''' AS VDescription ,'''' AS BDescription ,0 AS Quantity ,'''' AS InventoryID ,'''' AS UnitID ,'''' AS STATUS ,0 AS IsAudit ,0 AS IsCost ,'''' AS Ana01ID ,'''' AS Ana03ID ,'''' AS Ana04ID ,
'''' AS Ana05ID ,'''' AS Ana06ID ,'''' AS Ana07ID ,'''' AS Ana08ID ,'''' AS Ana09ID ,'''' AS Ana10ID ,'''' AS PeriodID ,'''' AS ExpenseID ,'''' AS MaterialTypeID ,'''' AS ProductID ,
...
0 AS DiscountRate ,'''' AS OrderID ,'''' AS CreditBankAccountID ,'''' AS DebitBankAccountID ,0 AS CommissionPercent ,'''' AS InventoryName1 ,'''' AS PaymentTermID ,0 AS DiscountAmountAT9000 ,
'''' AS OTransactionID ,0 AS IsMultiTax ,0 AS VATOriginalAmount ,0 AS VATConvertedAmount ,'''' AS ReVoucherID ,'''' AS ReBatchID ,'''' AS ReTransactionID ,'''' AS Parameter01 ,'''' AS Parameter02 ,
'''' AS Parameter03 ,'''' AS Parameter04 ,'''' AS Parameter05 ,'''' AS Parameter06 ,'''' AS Parameter07 ,'''' AS Parameter08 ,'''' AS Parameter09 ,'''' AS Parameter10 ,'''' AS MOrderID ,
'''' AS SOrderID ,'''' AS MTransactionID ,'''' AS STransactionID ,'''' AS RefVoucherNo ,0 AS IsLateInvoice ,0 AS ConvertedQuantity ,0 AS ConvertedPrice ,'''' AS ConvertedUnitID ,
0 AS ConversionFactor ,0 AS UParameter01 ,0 AS UParameter02 ,0 AS UParameter03 ,0 AS UParameter04 ,0 AS UParameter05 ,'''' AS PriceListID ,'''' AS WOrderID ,'''' AS WTransactionID ,
0 AS MarkQuantity ,'''' AS TVoucherID ,0 AS OldCounter ,0 AS NewCounter ,0 AS OtherCounter ,'''' AS TBatchID ,'''' AS ContractDetailID ,'''' AS InvoiceCode ,'''' AS InvoiceSign ,
'''' AS RefInfor ,0 AS StandardPrice ,0 AS StandardAmount ,0 AS IsCom ,0 AS VirtualPrice ,0 AS VirtualAmount ,'''' AS ReTableID ,'''' AS DParameter01 ,'''' AS DParameter02 ,'''' AS DParameter03 ,
'''' AS DParameter04 ,'''' AS DParameter05 ,'''' AS DParameter06 ,'''' AS DParameter07 ,'''' AS DParameter08 ,'''' AS DParameter09 ,'''' AS DParameter10 ,'''' AS InheritTableID ,
'''' AS InheritVoucherID ,'''' AS InheritTransactionID ,'''' AS ETaxVoucherID ,'''' AS ETaxID ,0 AS ETaxConvertedUnit ,0 AS ETaxConvertedAmount ,'''' AS ETaxTransactionID ,0 AS AssignedSET ,
'''' AS SETID ,'''' AS SETUnitID ,0 AS SETTaxRate ,0 AS SETConvertedUnit ,0 AS SETQuantity ,0 AS SETOriginalAmount ,0 AS SETConvertedAmount ,'''' AS SETConsistID ,'''' AS SETTransactionID ,'
SET @sSQL5 = N'
0 AS AssignedNRT ,0 AS NRTTaxAmount ,'''' AS NRTClassifyID ,'''' AS NRTUnitID ,0 AS NRTTaxRate ,0 AS NRTConvertedUnit ,0 AS NRTQuantity ,0 AS NRTOriginalAmount ,0 AS NRTConvertedAmount ,
'''' AS NRTConsistID ,'''' AS NRTTransactionID
,'''' AS CreditObjectName ,'''' AS CreditVATNo ,0 AS IsPOCost ,0 AS TaxBaseAmount ,0 AS WTCExchangeRate ,0 AS WTCOperator ,0 AS IsFACost ,
0 AS IsInheritFA ,'''' AS InheritedFAVoucherID ,0 AS AVRExchangeRate ,0 AS PaymentExchangeRate ,0 AS IsMultiExR ,'''' AS ExchangeRateDate ,0 AS DiscountSalesAmount ,0 AS IsProInventoryID ,
0 AS InheritQuantity ,0 AS DiscountPercentSOrder ,0 AS DiscountAmountSOrder ,0 AS IsWithhodingTax ,0 AS IsSaleInvoice ,'''' AS WTTransID ,0 AS DiscountSaleAmountDetail ,'''' AS ABParameter01 ,
'''' AS ABParameter02 ,'''' AS ABParameter03 ,'''' AS ABParameter04 ,'''' AS ABParameter05 ,'''' AS ABParameter06 ,'''' AS ABParameter07 ,'''' AS ABParameter08 ,'''' AS ABParameter09 ,
'''' AS ABParameter10 ,'''' AS SOAna01ID ,'''' AS SOAna02ID ,'''' AS SOAna03ID ,'''' AS SOAna04ID ,'''' AS SOAna05ID ,0 AS IsVATWithhodingTax ,0 AS VATWithhodingRate ,0 AS IsEInvoice ,
0 AS EInvoiceStatus ,0 AS IsAdvancePayment ,'''' AS Fkey ,0 AS IsInheritInvoicePOS ,'''' AS InheritInvoicePOS ,0 AS IsInheritPayPOS ,'''' AS InheritPayPOS ,0 AS IsInvoiceSuggest ,
'''' AS RefVoucherDate ,0 AS IsDeposit ,'''' AS ReTransactionTypeID ,'''' AS ImVoucherID ,'''' AS ImTransactionID ,'''' AS SourceNo ,'''' AS LimitDate ,0 AS IsPromotionItem ,'''' AS ObjectName1 ,
'''' AS TaxRateID ,2 AS PayMethodID ,0 AS DiscountedUnitPrice ,0 AS ConvertedDiscountedUnitPrice ,0 AS IsReceived ,'''' AS Ana01Name ,'''' AS Ana03Name ,'''' AS Ana04Name ,'''' AS Ana05Name ,
'''' AS Ana06Name ,'''' AS Ana07Name ,'''' AS Ana08Name ,'''' AS Ana09Name ,'''' AS Ana10Name ,'''' AS BankAccountNo ,'''' AS BankName ,'''' AS ContactPerson ,0 AS DVATOriginalAmount ,
0 AS DVATConvertedAmount ,'''' AS ConvertedUnitName ,'''' AS PaymentName ,1 AS IsDiscount ,'''' AS DivisionNameE ,'''' AS AddressE ,'''' AS District ,'''' AS DContactPerson ,
0 AS OriginalAfterVATAmount ,0 AS ConvertedAfterVATAmount ,'''' AS WSourceNo ,'''' AS WLimitDate ,'''' AS I04ID ,'''' AS I04Name ,4 AS ItemTypeID ,'''' AS CusBankAccountNo
		FROM #TEMP
	GROUP BY BDescription,Total
		'
		END
...
EXEC (@sSQL + @sSQL1A + @sSQL1A1 + @sSQL2A + @sSQLA + @sSQL3+@sSQL3A + @sSQL4 + @sSQL5)
GO
