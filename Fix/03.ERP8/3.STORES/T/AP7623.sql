IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7623]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP7623]
GO
...
-- <Summary>
---- Báo cáo kết quả kinh doanh có group theo mã phân tích
-- <Param>
---- 
-- <Return>
---- 
-- <Reference> T/Báo cáo/ kết quả kinh doanh theo mã phân tích
---- 
-- <History>
----- Created by Phuong Thao, Date 26/09/2016
---- Modified on 26/06/2017 by Phuong Thao: Sửa danh mục dùng chung
---- Modified on 03/11/2022 by Văn Tài: Bổ sung tên bảng.
-- <Example>
---- 
-- <Example>
---- 
-- <Summary>
CREATE PROCEDURE [dbo].[AP7623]   
...
Declare @FieldID AS nvarchar(50),  
		...
		@Group01ID AS NVarchar(50),		
		@Group02ID AS NVarchar(50),
		@Group03ID AS NVarchar(50),
		@Group04ID AS NVarchar(50),
		@Group05ID AS NVarchar(50),		
...
SELECT @Selection01 = '', @Selection02 = '', @Selection03 = '', @Selection04 = '', @Selection05 = '', @Temp = '', @strSQLFilter = ''
...
IF @CustomerName = 16 --- Customize Sieu Thanh
	EXEC AP7620_ST @DivisionID,   @ReportCode,   
  @FromMonth ,     @FromYear  ,     @ToMonth ,     @ToYear  ,  
  @FromValueID ,     @ToValueID ,    @StrDivisionID   
ELSE
Begin   
...
IF @Selection05 IS NOT NULL AND @Selection05 <> '' ---- Tim kiem theo chi tieu 5
	...
	IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id(N'[dbo].[AT7622_LastPeriod]') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
			DROP TABLE AT7622_LastPeriod

		SELECT * 
		INTO AT7622_LastPeriod
		FROM AT7622
		WHERE 1=0

	Declare @DefaultName varchar(200), @SQLconstraint varchar(500)
	If Exists (Select * From sysobjects Where name = 'AT7622_LastPeriod' and xtype ='U') 
	Begin          
		Select @DefaultName = def.name
		from sysobjects def inner join syscomments cmm 
		on def.id = cmm.id inner join syscolumns col on col.cdefault = def.id 
		inner join sysobjects tab on col.id = tab.id  
		where tab.name = 'AT7622_LastPeriod'  and col.name = 'APK'  
			--drop constraint 
		if @DefaultName Is Not Null Execute ('Alter Table AT7622_LastPeriod Drop Constraint ' + @DefaultName)

		If exists (select * from syscolumns col inner join sysobjects tab 
        On col.id = tab.id where tab.name =   'AT3011'  and col.name = 'APK')
			Set @SQLconstraint = 'Alter Table AT7622_LastPeriod  drop Column APK '				  
     
		 Execute(@SQLconstraint)                    
...
IF @CustomerName = 75
BEGIN
	SET @strWhere = ' SignAmount2,'
	SET @strTable = 'AV9090_PCF'
END
ELSE
BEGIN
	SET @strTable = 'AV9090'	
  ...
  Begin   
	...
	SET @Cur = Cursor Scroll KeySet FOR   
	 ...
	 Where	ReportCode = @ReportCode and  LevelID = @LevelID and DivisionID = @DivisionID 
	  -- and LineID between 'BC.0001' and 'BC.0003'
	 ---Order by LineID 
	...
	Begin   	
	 ...
	 Begin  
		 ...
		 SET @sSQL =' 
			SELECT	Ana01ID, Ana02ID, Ana03ID, Ana04ID, Ana05ID, ObjectID, PeriodID, TranMonth, TranYear,   
					' + @strTable + '.DivisionID, AccountID, CorAccountID, D_C, SignAmount,' + @strWhere + '   
	...
	Select SelectionID From AV6666  
		...
		EXEC AP76191 @DivisionID,@FromMonth,@FromYear,@ToMonth,@ToYear,  
		@CaculatorID,@FromAccountID,@ToAccountID,  
		@FromCorAccountID,@ToCorAccountID,  
		@AnaTypeID,@FromAnaID,@ToAnaID,@FieldID,@AnaID,  
		@BudgetID, @StrDivisionID,
		@Selection01, @Selection02, @Selection03,
		@Selection04, @Selection05, @LineID, @i, @ReportCode, 0 
      
		EXEC AP76191 @DivisionID,@FromMonthLastPeriod,@FromYearLastPeriod,@ToMonthLastPeriod,@ToYearLastPeriod,  
		@CaculatorID,@FromAccountID,@ToAccountID,  
		@FromCorAccountID,@ToCorAccountID,  
		@AnaTypeID,@FromAnaID,@ToAnaID,@FieldID,@AnaID,  
		@BudgetID,@StrDivisionID,
		@Selection01, @Selection02, @Selection03,
		@Selection04, @Selection05, @LineID, @i, @ReportCode, 1   
  		
		
		UPDATE	T1 
		SET		T1.[Amount01LastPeriod] = T2.[Amount01],
				T1.[Amount02LastPeriod] = T2.[Amount02],
				T1.[Amount03LastPeriod] = T2.[Amount03],
				T1.[Amount04LastPeriod] = T2.[Amount04],
				T1.[Amount05LastPeriod] = T2.[Amount05],
				T1.[Amount06LastPeriod] = T2.[Amount06],
				T1.[Amount07LastPeriod] = T2.[Amount07],
				T1.[Amount08LastPeriod] = T2.[Amount08],
				T1.[Amount09LastPeriod] = T2.[Amount09],
				T1.[Amount10LastPeriod] = T2.[Amount10],
				T1.[Amount11LastPeriod] = T2.[Amount11],
				T1.[Amount12LastPeriod] = T2.[Amount12],
				T1.[Amount13LastPeriod] = T2.[Amount13],
				T1.[Amount14LastPeriod] = T2.[Amount14],
				T1.[Amount15LastPeriod] = T2.[Amount15],
				T1.[Amount16LastPeriod] = T2.[Amount16],
				T1.[Amount17LastPeriod] = T2.[Amount17],
				T1.[Amount18LastPeriod] = T2.[Amount18],
				T1.[Amount19LastPeriod] = T2.[Amount19],
				T1.[Amount20LastPeriod] = T2.[Amount20],
				T1.[Amount21LastPeriod] = T2.[Amount21],
				T1.[Amount22LastPeriod] = T2.[Amount22],
				T1.[Amount23LastPeriod] = T2.[Amount23],
				T1.[Amount24LastPeriod] = T2.[Amount24],
				T1.[Amount01ALastPeriod] = T2.[Amount01A],
				T1.[Amount02ALastPeriod] = T2.[Amount02A],
				T1.[Amount03ALastPeriod] = T2.[Amount03A],
				T1.[Amount04ALastPeriod] = T2.[Amount04A],
				T1.[Amount05ALastPeriod] = T2.[Amount05A],
				T1.[Amount06ALastPeriod] = T2.[Amount06A],
				T1.[Amount07ALastPeriod] = T2.[Amount07A],
				T1.[Amount08ALastPeriod] = T2.[Amount08A],
				T1.[Amount09ALastPeriod] = T2.[Amount09A],
				T1.[Amount10ALastPeriod] = T2.[Amount10A],
				T1.[Amount11ALastPeriod] = T2.[Amount11A],
				T1.[Amount12ALastPeriod] = T2.[Amount12A],
				T1.[Amount13ALastPeriod] = T2.[Amount13A],
				T1.[Amount14ALastPeriod] = T2.[Amount14A],
				T1.[Amount15ALastPeriod] = T2.[Amount15A],
				T1.[Amount16ALastPeriod] = T2.[Amount16A],
				T1.[Amount17ALastPeriod] = T2.[Amount17A],
				T1.[Amount18ALastPeriod] = T2.[Amount18A],
				T1.[Amount19ALastPeriod] = T2.[Amount19A],
				T1.[Amount20ALastPeriod] = T2.[Amount20A],
				T1.[Amount21ALastPeriod] = T2.[Amount21A],
				T1.[Amount22ALastPeriod] = T2.[Amount22A],
				T1.[Amount23ALastPeriod] = T2.[Amount23A],
				T1.[Amount24ALastPeriod] = T2.[Amount24A]
		FROM	AT7622 T1
		INNER JOIN AT7622_LastPeriod T2 ON T1.LineID = T2.LineID AND T1.ReportCode = T2.ReportCode 
		AND T1.[GroupID1] = T2.[GroupID1] AND T1.[GroupID2] = T2.[GroupID2] AND T1.[GroupID3] = T2.[GroupID3]
		AND T1.[GroupID4] = T2.[GroupID4] AND T1.[GroupID5] = T2.[GroupID5]
		...
		INSERT INTO AT7622 (DivisionID, ReportCode, LineID)
		SELECT	DivisionID, ReportCode, LineID
		FROM	AT7621
		WHERE ReportCode = @ReportCode and LineID = @LineID
		AND NOT EXISTS (SELECT TOP 1 1 FROM AT7622 T1 WHERE AT7621.ReportCode = T1.ReportCode AND AT7621.LineID = T1.LineID)
		...
		SELECT   AT7621.LineID, AT7621.Sign, AT7621.AccuLineID,   
			...
			ISNULL(AT7622.[GroupID1],''), ISNULL(AT7622.[GroupID2],''), ISNULL(AT7622.[GroupID3],''), ISNULL(AT7622.[GroupID4],''),
			ISNULL(AT7622.[GroupID5],'')
		FROM AT7621   
		...
		FETCH NEXT FROM @Cur_ChildLevelID INTO  @ChildLineID, @ChildSign, @ParLineID,  
				...
				@Group01ID, @Group02ID, @Group03ID, @Group04ID, @Group05ID 
		WHILE @@Fetch_Status = 0  
			Begin   
			If @ChildSign =  '+'   
			Update AT7622  Set    
			...
			Where LineID = @ParLineID and ReportCode = @ReportCode and DivisionID = @DivisionID  
				  and Isnull(GroupID1,'') = @Group01ID and Isnull(GroupID2,'') = @Group02ID and Isnull(GroupID3,'') = @Group03ID
				  and Isnull(GroupID4,'') = @Group04ID and Isnull(GroupID5,'') = @Group05ID
