IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP1510]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP1510]
GO
...
---- Modified on 28/03/2018 by Bảo Anh: Bổ sung trường PeriodID cho AV1510
---- Modified by Kim Thư on 19/12/2018: Tính lại khấu hao lũy kế bao gồm trường hợp thay đổi nguyên giá - thêm bảng AT1591
---- Modified by Kim Thư on 15/01/2018: Tính khấu hao lũy kế với tài sản đã hết khấu hao trước khi sử dụng phần mềm = ConvertedAmount
---- Modified by Kim Thư on 17/01/2019: Xử lý drop table AT1591, tính AT1591 cho koyo theo bản cũ, bổ sung DebitAccountID cho KOYO
---- Modified by Kim Thư on 08/03/2019: Tính DepAmount - Khấu hao trong kỳ xét thêm điều kiện tài khoản khấu hao (KOYO)
---- Modified by Kim Thư on 09/04/2019: Bổ sung lọc AT1591 theo Division
---- Modified by Kim Thư on 07/05/2019: Tách store riêng cho Bason, bổ sung truyền biến @Type - loại báo cáo (Bason)
---- Modified by Nhựt Trường on 04/02/2021: Bổ sung điều kiện DivisionID IN khi join bảng AT1103.
---- Modified by Huỳnh Thử   on 16/04/2021: Tách Store Hưng Thịnh
---- Modified by Văn Tài 	 on 21/09/2021: Loại bỏ cách tính sai liên quan phiếu tính lại AT1506.
---- Modified by Nhật Thanh  on 14/04/2022: Tách store angel
---- Modified by Nhật Thanh  on 22/06/2022: Bổ sung cấu trúc đầy đủ cho câu insert into
---- Modified by Văn Tài	 on 18/07/2022: Thay đổi quy tắc đặc tên, sử dụng bảng tạm.
---- Modified by Xuân Nguyên on 12/08/2022: [2022/08/IS/0015] Thay #AP1510Temp bằng AT1591
---- Modified by Nhật Thanh on 17/08/2022: Bổ sung điều kiện DivisionID
---- Modified by Kiều Nga on 05/10/2022: Lấy thêm trường ConvertedNewAmount, fix lỗi trường CountryName không lấy được dữ liệu
---- Modified by Kiều Nga on 18/01/2023: Lấy thêm trường ConvertedOldAmount
---- Modified by Đức Duy on 21/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.

/********************************************  
'* Edited by: [GS] [Ngọc Nhựt] [29/07/2010]  
'********************************************/  
---- exec AP1510 @DivisionID=N'BS',@FromMonth=2,@FromYear=2019,@ToMonth=3,@ToYear=2019,@IsMonth=0,@GroupType=3,@Condition=N'(0,1,2,3,4,9)', @Type=1    
  
CREATE PROCEDURE [dbo].[AP1510]    
	...
	@Type TINYINT  = 0  -- 0: Báo cáo khấu hao trong kỳ / 1: Chi tiết thay đổi trong kỳ
 AS
declare   
	@FromPeriod as int,  
	@ToPeriod as int,
...
IF @CustomerName = 84 -- BASON
	exec AP1510_BS @DivisionID, @Frommonth, @Fromyear, @Tomonth, @Toyear, @Ismonth, @Grouptype, @Condition, @Type
ELSE
IF @CustomerName = 57 -- ANGEL
	exec AP1510_AG @DivisionID, @Frommonth, @Fromyear, @Tomonth, @Toyear, @Ismonth, @Grouptype, @Condition
ELSE
IF @CustomerName = 129 -- Hưng Thịnh
	exec AP1510_HUNGTHINH @DivisionID, @Frommonth, @Fromyear, @Tomonth, @Toyear, @Ismonth, @Grouptype, @Condition
ELSE
BEGIN
...
-- Tính riêng khấu hao lũy kế bao gồm trường hợp thay đổi nguyên giá theo thông tư 200 
-- Nếu TS có giảm nguyên giá thì AT1591 = Nguyên giá mới - Giá trị còn lại ban đầu mới + Tổng số tiền đã khấu hao
-- Nếu TS ko có giảm nguyên giá, còn khấu hao trong tương lai thì AT1591 = Nguyên giá - Giá trị còn lại ban đầu + Tổng số tiền đã khấu hao
-- Nếu TS đã hết khấu hao từ lúc bắt đầu sử dụng phần mềm thì AT1591 = Nguyên giá
If exists (Select Top 1 1 From sysobjects Where Id = Object_ID('AT1591') And xType = 'U')
Begin
    Drop Table AT1591
End
CREATE TABLE AT1591(DivisionID VARCHAR(50), AssetID VARCHAR(250), ChangePeriod INT, AccDepAmount DECIMAL(28,8))
INSERT INTO AT1591(DivisionID, AssetID , ChangePeriod , AccDepAmount)

SELECT A.DivisionID, A.AssetID, A.ChangePeriod, CASE WHEN A.DepPeriods = A.DepMonths THEN A.ConvertedAmount ELSE 
													
															(A.ConvertedAmount-A.ResidualValue+(Select ISNULL(SUM(DepAmount),0) From AT1504 WITH(NOLOCK) 
																		Where AT1504.DivisionID = A.DivisionID and AT1504.AssetID = A.AssetID 
																		and AT1504.TranMonth+AT1504.TranYear*100 <= ltrim(@ToPeriod)))
												END
												AS AccDepAmount
FROM (
	SELECT @DivisionID AS DivisionID, AT1503.AssetID, ISNULL((SELECT MAX(A06.TranMonth+A06.TranYear*100) FROM AT1506 A06 WITH (NOLOCK) WHERE A06.DivisionID = AT1503.DivisionID AND A06.AssetID = AT1503.AssetID
									AND A06.TranMonth+A06.TranYear*100 <=ltrim(@ToPeriod) ),AT1503.BeginMonth+AT1503.BeginYear*100) as ChangePeriod,
			AT1503.ConvertedAmount, AT1503.DepPeriods, AT1503.DepMonths, AT1503.DepAmount, AT1503.ResidualValue
	FROM AT1503 WITH (NOLOCK) WHERE AT1503.DivisionID=@DivisionID
) A
LEFT JOIN AT1506 WITH (NOLOCK) ON A.DivisionID = AT1506.DivisionID AND A.AssetID = AT1506.AssetID AND AT1506.TranMonth+AT1506.TranYear*100 = A.ChangePeriod
...
-- Giam trong ky   
 ...
 '
 SET @sSQL2 =  CASE WHEN @CustomerName = 52 
	THEN 'AT1504.DepAmount AS AccuDepAmount,' ELSE ' AT1591.AccDepAmount as AccuDepAmount, ' END + N'     
...
Set @sSQL3 = N'   
 ...
 DepAmount = isnull((Select Sum(DepAmount)   
      From AT1504 A4 WITH (NOLOCK)   
      Where  A4.DivisionID = AT1503.DivisionID   
        and A4.AssetID = At1503.AssetID
		'+CASE WHEN @CustomerName = 52 THEN 'AND A4.DebitAccountID =  AT1504.DebitAccountID ' ELSE '' END +' 
        and A4.TranMonth + A4.TranYear * 100 between ' + str(@FromPeriod) + ' and ' + str(@ToPeriod) + N'),0),
...
where AT1533.DivisionID = AT1503.DivisionID AND AT1533.AssetID = AT1503.AssetID) as ReObjectName,  
 ...
 AT1503.AssetStatus,  
  ...
  (select TOP 1 AT1506.ConvertedNewAmount   
   ...
   Where AT1506.AssetID = AT1503.AssetID and AT1506.DivisionID = AT1503.DivisionID  
    and AT1506.TranMonth + 100*AT1506.TranYear <=' + str(@ToPeriod ) + '  
   Order by AT1506.TranYear Desc,AT1506.TranMonth Desc) as ConvertedNewAmount,
  (select TOP 1 AT1506.ConvertedOldAmount   
   From AT1506 WITH (NOLOCK)    
   Where AT1506.AssetID = AT1503.AssetID and AT1506.DivisionID = AT1503.DivisionID  
    and AT1506.TranMonth + 100*AT1506.TranYear <=' + str(@ToPeriod ) + '  
   Order by AT1506.TranYear Desc,AT1506.TranMonth Desc) as ConvertedOldAmount,
 '  
...
(Select TOP 1 PeriodID From AT1504 WITH (NOLOCK) Where DivisionID = AT1503.DivisionID And AssetID = AT1503.AssetID
	And TranMonth + TranYear * 100 <= ' + LTRIM(@ToPeriod) + ' ORDER BY (TranMonth + TranYear * 100) DESC) AS PeriodID
 '+CASE WHEN @CustomerName = 52 THEN ', AT1504.DebitAccountID' ELSE '' END +'

FROM 
'+CASE WHEN @CustomerName = 52 THEN '(SELECT DISTINCT AT1504.DivisionID, AT1504.AssetID, A.DebitAccountID, A.DepAmount
										FROM AT1504
											INNER JOIN (
												SELECT AT1504.AssetID, DebitAccountID, SUM(DepAmount) AS DepAmount FROM AT1504 
												Where AT1504.TranMonth+AT1504.TranYear*100 <=  ' + str(@ToPeriod) + '
												GROUP BY AT1504.AssetID, DebitAccountID ) A
											ON A.AssetID = AT1504.AssetID) AT1504
										LEFT JOIN AT1503  WITH (NOLOCK) ON AT1503.DivisionID = AT1504.DivisionID AND AT1503.AssetID = AT1504.AssetID' 
								ELSE ' AT1503 WITH (NOLOCK)' END +'

LEFT JOIN AT1001 WITH (NOLOCK) on AT1503.CountryID = AT1001.CountryID AND AT1001.DivisionID IN (AT1503.DivisionID,''@@@'')   
LEFT JOIN AT1005  WITH (NOLOCK) on AT1005.AccountID = AT1503.AssetAccountID AND AT1005.DivisionID IN (AT1503.DivisionID,''@@@'')   
LEFT JOIN AT1102 WITH (NOLOCK) on AT1102.DepartmentID = AT1503.DepartmentID AND AT1102.DivisionID IN (AT1503.DivisionID,''@@@'')  
...
LEFT JOIN AT1103 WITH (NOLOCK) ON AT1103.DivisionID IN ('''+@DivisionID+''',''@@@'') AND AT1103.EmployeeID = AT1503.EmployeeID  

'+ CASE WHEN @CustomerName = 52 THEN '' 
	ELSE 'LEFT JOIN AT1591 ON AT1591.DivisionID = AT1503.DivisionID AND AT1591.AssetID = AT1503.AssetID' END


SET @sWHERE = N'  
WHERE  AT1503.DivisionID = N''' + @DivisionID + ''' and   
...
SELECT DISTINCT AT1503.DivisionID,AT1503.Notes,  
 ...
 AT1503.AssetAccountID,  
 AT1005.AccountName, 
  AT1591.AccDepAmount as AccuDepAmount, 
 --AccuDepAmount = (   
...
set @sSQL1 = '    
	...
	AT1503.EmployeeID, AT1103.FullName AS EmployeeName   
FROM AT1503   WITH (NOLOCK) 
LEFT JOIN AT1001 WITH (NOLOCK) on AT1503.CountryID = AT1001.CountryID
LEFT JOIN AT1005 WITH (NOLOCK) on AT1005.AccountID = AT1503.AssetAccountID  
LEFT JOIN AT1102 WITH (NOLOCK) on AT1102.DepartmentID = AT1503.DepartmentID 
LEFT JOIN  AT1501 WITH (NOLOCK) on At1501.AssetGroupID = AT1503.AssetGroupID And At1501.DivisionID = AT1503.DivisionID  
LEFT JOIN  AT1523  WITH (NOLOCK) on AT1523.AssetID = AT1503.AssetID And AT1523.DivisionID = AT1503.DivisionID  
...
LEFT JOIN  AT1011 WITH (NOLOCK) ON AT1011.AnaID = K.Ana01ID and AnaTypeID = ''A01''
LEFT JOIN AT1103 WITH (NOLOCK) ON AT1103.DivisionID IN ('''+@DivisionID+''',''@@@'') AND AT1103.EmployeeID = AT1503.EmployeeID
LEFT JOIN AT1591 ON AT1591.DivisionID = AT1503.DivisionID AND AT1591.AssetID = AT1503.AssetID
