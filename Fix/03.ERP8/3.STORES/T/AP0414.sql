IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0414]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP0414]
GO
...
---- Modified by Bảo Thy on 16/05/2017: Bổ sung Ana01ID -> Ana10ID (SGPT)
---- Modified by Hải Long on 18/05/2017: Chỉnh sửa danh mục dùng chung
---- Modified by Phương Thảo on 21/06/2017: Fix lỗi khi in 2DB của KH Siêu Thanh
---- Modified by Huỳnh Thử on 17/08/2020: Merge Code: MEKIO và MTE - Tách Store AP0414_MK
---- Modified by Đức Duy on 20/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.
-- <Example>
---- 

CREATE PROCEDURE [dbo].[AP0414]  
...
BEGIN
	...
	BEGIN
		...
		SET @sSQL = @sSQL + '
				...
				T9.O01ID, T9.O02ID, T9.O03ID,T9.O04ID, T9.O05ID, T9.O01Name , T9.O02Name ,T9.O03Name ,T9.O04Name,T9.O05Name, T9.DivisionID,
				'''+@FromAccountID+'''	AS FromAccountID, '''+@ToAccountID+''' AS ToAccountID,
				T9.Ana01ID, T9.Ana02ID, T9.Ana03ID, T9.Ana04ID, T9.Ana05ID, T9.Ana06ID, 
				T9.Ana07ID, T9.Ana08ID, T9.Ana09ID, T9.Ana10ID	
		FROM		AV0412  T9  
	...
	IF @CustomerName =16 AND @DatabaseName<>'' --Customized In du lieu 2 database cua KH SIEUTHANH
		BEGIN
			...
			SET @sSQLUnionAll = '
				...
				DebitInvoiceNo,DebitVoucherNo,DebitVoucherDate,DebitInvoiceDate,DebitVoucherTypeID,''''AS DebitBatchID,CreditVoucherTypeID,Fromdate,Todate,
				O01ID, O02ID,O03ID,O04ID,O05ID,
				O01Name ,O02Name ,O03Name ,O04Name,O05Name,DivisionID,
				'''+@FromAccountID+'''	AS FromAccountID, '''+@ToAccountID+''' AS ToAccountID,
				'''' AS Ana01ID, '''' AS Ana02ID, '''' AS Ana03ID, '''' AS Ana04ID, '''' AS Ana05ID, '''' AS Ana06ID, 
				'''' AS Ana07ID, '''' AS Ana08ID, '''' AS Ana09ID, '''' AS Ana10ID		
				FROM AV0414_ST
				'
		END		
	ELSE
