...
---- Modified on 17/05/2023 by Xuân Nguyên	 : [2023/05/IS/0042] Bổ sung thêm điều kiện DivisionID dùng chung khi lấy AccountNameE từ AT1005
-- <Example>
---- 
...
BEGIN
    EXEC dbo.AP7409_TIENTIEN @DivisionID = @DivisionID,                 -- nvarchar(50)
					...
					@ReportDate = @ReportDate              -- nvarchar(250)
    
END		
ELSE IF @CustomerName = 147 -- Vĩnh Nam Anh
BEGIN
    EXEC AP7409_VNA @DivisionID = @DivisionID,                 -- nvarchar(50)
                    @FromMonth = @FromMonth,                    -- int
                    @FromYear = @FromYear,                     -- int
                    @ToMonth = @ToMonth,                      -- int
                    @ToYear = @ToYear,                       -- int
...
BEGIN
	...
	IF @CurrencyID = '%'
		SET @sSQL =   '
			...
			VATNo,
			AccountID, 
			AccountName, (SELECT TOP 1 AccountNameE FROM AT1005 WHERE DivisionID IN(''@@@'',AV7419.DivisionID) AND AccountID = AV7419.AccountID) AccountNameE,
			''%'' AS CurrencyID,
			S1, 
			S2,  
			S3, 
		...
		IF @CustomerName = 16  AND @DatabaseName <>'' --- Customize Sieu Thanh in du lieu 2 database
			BEGIN
				...
				SET @sSQLUnion = '
					...
					(SELECT TOP 1 AccountNameE FROM AT1005 WHERE DivisionID IN(''@@@'',AV7409_ST.DivisionID) AND AccountID = AV7409_ST.AccountID) AccountNameE,
					CurrencyID,S1,S2,S3,O01ID,O02ID,O03ID,O04ID,O05ID,Tel,Fax, Email, ObjectTypeID, ObjectTypeName,
					CreditOriginalOpening,DebitOriginalOpening,OriginalOpening,
