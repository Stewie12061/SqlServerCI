IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP3022_GOD]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP3022_GOD]
GO
...
CREATE PROCEDURE [dbo].[AP3022_GOD] 
...
IF @CustomerName = 74 --(Customize GODREJ)	
	...
	SELECT  AT1202.ObjectID,
			...
			CASE WHEN MIN(AT9000.TableID) = N''AT1326''
					  THEN SUM(CONVERT(DECIMAL(28,8), CONVERT(DECIMAL(28,0), (ISNULL(AT726.UnitPrice,0) * AT326.Quantity))))
				 WHEN COUNT(A000.WTransactionID) > 1  and  COUNT(A000.InventoryID) > 1 
				 	 THEN CONVERT(DECIMAL(28,8),CONVERT(DECIMAL(28,0),(isnull(A007.UnitPrice,0) * AT9000.Quantity)))
				 ELSE isnull(A007.ConvertedAmount,0) 
			END AS PrimeCostAmount1,
...
set @sSQL2=N'
			...
			(Select Count(ObjectID) From AT1202 A Where A.DivisionID IN (''' +@DivisionID+ ''', ''@@@'') AND A.O02ID = AT1202.O02ID) As FieldName, 
...
set @sSQL2 = @sSQL2 + @sSQL4 + '
	...
	-- [START] Map details bán hàng theo bộ
	LEFT JOIN AT9000 AT326 WITH (NOLOCK) ON AT9000.TableID = N''AT1326''
												AND AT326.DivisionID = AT9000.DivisionID
												AND AT326.VoucherID = AT9000.VoucherID
												AND AT326.ProductID = AT9000.InventoryID
	LEFT JOIN AT2007 AT726 WITH(NOLOCK) ON AT726.DivisionID = AT326.DivisionID
												AND AT726.VoucherID = AT326.VoucherID 
												AND AT726.InventoryID = AT326.InventoryID 
	-- [END] Map details bán hàng theo bộ
	'
set @sSQL3=N'
