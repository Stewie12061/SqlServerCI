...
------ Modified by Bảo Thy on 10/05/2017: Sửa danh mục dùng chung và sửa cách sinh DepreciationID bằng NEWID()
------ Modified by Bảo Anh on 05/07/2018: Sửa lỗi không tính được DepOriginalAmount
------ Modified by Huỳnh Thử on 19/08/2020: Merge Code: MEKIO và MTE
------ Modified by Đức Thông on 23/03/2021: [MEKIO và MTE] Fix bug merge code
					...
					ELSE
						Begin
							...
							Select case when AT1703.D_C='D' then AT1705.AccountID else AT1703.DebitAccountID end as DebitAccountID,
							 ...
							 From AT1705 WITH (NOLOCK) inner join AT1703 WITH (NOLOCK) on AT1705.JobID = AT1703.JobID and AT1705.DivisionID = AT1703.DivisionID 
							 Where AT1705.JobID = @JobID and AT1705.DivisionID = @DivisionID			
