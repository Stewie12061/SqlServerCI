IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP1510_BS]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP1510_BS]
GO
...
----Created by Kim Thư on 07/05/2019: Tách store riêng cho Bason, lấy theo chi tiết từng kỳ khấu hao bổ sung truyền biến @Type - loại báo cáo (Bason)  
...
---- exec AP1510_BS @DivisionID=N'BS',@FromMonth=2,@FromYear=2019,@ToMonth=3,@ToYear=2019,@IsMonth=0,@GroupType=3,@Condition=N'(0,1,2,3,4,9)', @Type=1  
  
CREATE PROCEDURE [dbo].[AP1510_BS]    
...
Begin
    Drop Table AT1591
End
CREATE TABLE AT1591(DivisionID VARCHAR(50), AssetID VARCHAR(250), ChangePeriod INT, TranMonth INT, TranYear INT,  AccDepAmount DECIMAL(28,8))
INSERT INTO AT1591
SELECT A.DivisionID, A.AssetID, A.ChangePeriod, A04.TranMonth, A04.TranYear, CASE WHEN A.DepPeriods = A.DepMonths THEN A.ConvertedAmount ELSE 
													ISNULL(AT1506.ConvertedNewAmount - AT1506.ResidualNewValue+(Select Sum(DepAmount) From AT1504 WITH(NOLOCK) 
																		Where AT1504.DivisionID = A.DivisionID and AT1504.AssetID = A.AssetID 
																		and AT1504.TranMonth+AT1504.TranYear*100 BETWEEN A.ChangePeriod AND (A04.TranMonth+A04.TranYear*100) ),
															(A.ConvertedAmount-A.ResidualValue+(Select ISNULL(SUM(DepAmount),0) From AT1504 WITH(NOLOCK) 
																		Where AT1504.DivisionID = A.DivisionID and AT1504.AssetID = A.AssetID 
																		and AT1504.TranMonth+AT1504.TranYear*100 <= A04.TranMonth+A04.TranYear*100)))
												END
												AS AccDepAmount
FROM (
...
LEFT JOIN AT1504 A04 WITH (NOLOCK) ON A.DivisionID = A04.DivisionID AND A.AssetID = A04.AssetID
...
-- Giam trong ky   
 ...
 '
 SET @sSQL2 = 'AT1591.AccDepAmount as AccuDepAmount,    
 ...
 DepreciatedMonths = isnull(AT1503.DepMonths,0)  + isnull((Select Count(*)   
        ...
        and A.TranMonth+A.TranYear*100 <= AT1504.TranMonth+AT1504.TranYear*100 ),0),' 
...
AT1504.TranMonth, AT1504.TranYear
...
LEFT JOIN AT1103 WITH (NOLOCK) ON AT1103.EmployeeID = AT1503.EmployeeID 
LEFT JOIN AT1504 WITH (NOLOCK) ON AT1503.DivisionID = AT1504.DivisionID AND AT1503.AssetID = AT1504.AssetID 
LEFT JOIN AT1591 ON AT1591.DivisionID = AT1503.DivisionID AND AT1591.AssetID = AT1503.AssetID AND AT1591.TranMonth = AT1504.TranMonth AND AT1591.TranYear = AT1504.TranYear
LEFT JOIN AT1506 WITH (NOLOCK) ON AT1506.DivisionID = AT1503.DivisionID AND AT1506.AssetID = AT1503.AssetID
...
' + CASE WHEN @Type = 1 THEN 'MONTH(AT1503.EstablishDate) + Year(AT1503.EstablishDate)*100 <= ' + str(@ToPeriod)
	ELSE '(MONTH(AT1503.EstablishDate) + Year(AT1503.EstablishDate)*100 BETWEEN '+ STR(@FromPeriod) +' AND '+ STR(@ToPeriod) +'
