IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_KOYO]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_KOYO]
GO
...
---- Created on 25/09/2017 Huỳnh Thử
-- <Param>
---- 
-- <Return>
---- 
...
---- Modify on 08/06/2022 by Nhật Thanh: Tích hợp phiếu VCNB
---- Modify on 01/07/2022 by Nhật Thanh: Thay đổi cách lấy tiền khi là phiếu ngoại tệ
---- Modify on 29/07/2022 by Nhật Thanh: Koyo không lấy lên người mua hàng và số tài khoản
---- Modify on 15/12/2022 by Nhật Thanh: Cập nhật trường tính chất là 1
...
CREATE PROCEDURE [dbo].[AP0146_KOYO]
...
DECLARE @sSQL AS NVARCHAR(MAX) = '',
		...
		@CustomerName INT,
		@ExchangeRate AS int,
		@sSQL_Sub_Select AS NVARCHAR(MAX),
		@KindVoucherID INT
...
SELECT TOP 1 @ExchangeRate = ExchangeRate	FROM AT9000 WITH (NOLOCK) WHERE DivisionID = @DivisionID AND VoucherID = @VoucherID
IF(@ExchangeRate != 1)
BEGIN
	SET @sSQL_Sub_Select =N'
	...
	ABS(AT9000.OriginalAmount) AS Amount,'
...
SET @KindVoucherID = (SELECT KindVoucherID FROM AT2006 WHERE DivisionID = @DivisionID AND VoucherID = @VoucherID)
	...
	A07.InventoryID, A04.InventoryName, A07.UnitID, A05.UnitName, A07.ActualQuantity, 0 AS Unitprice, 0 AS OriginalAmount, 0 AS ConvertedAmount, A07.Notes, A11.BaseCurrencyID AS CurrencyID, 1 as ExchangeRate, A07.SaleUnitPrice,
	...
	'+CASE WHEN @CustomerName = 67 THEN '''''' ELSE '''1''' END +' AS Extra1,  A06.Description as LDDDong, A06.VoucherDate as NgDDong, A07.Notes07 as NDDong, A07.Notes06 as SLenhDDong, A07.Notes05 as Shipper, A07.Notes04 as Transportation
	'
	SET @sSQL1A1='FROM AT2006 A06 WITH (NOLOCK) INNER JOIN AT2007 A07 WITH (NOLOCK) ON A06.VoucherID = A07.VoucherID
...
BEGIN
	...
	NULL AS Buyer, '''' AS CusbankNo, '''' AS CusBankName,
	...
	AT1202.BankAccountNo as CusBankAccountNo'+@sSQL_Sub_Select+' 
		...
		SELECT * FROM (
			SELECT	TOP 100 PERCENT Remark, CusCode, CusName, CusAddress, CusPhone, CusEmail,CusEmailCC, CusTaxCode, Buyer, CusbankNo,CusBankName, PaymentMethod, 
				...
				DonDatHangSo, OriginalAmount, KindOfService, Extra1, VATAmount0, VATAmount10, Extra, InvoiceDate, CurrencyID, PaymentID, VoucherNo,
				 ...
				 OriginalAfterVATAmount, ConvertedAfterVATAmount, WSourceNo, WLimitDate, I04ID, I04Name, 1 as ItemTypeID,
