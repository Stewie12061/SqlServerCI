IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_APT]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_APT]
GO
...
-- AP0146_APT
-- <Summary>
---- Stored load dữ liệu hóa đơn phục vụ phát hành hóa đơn điện tử - Khách hàng An Phúc Thịnh.
---- Created on 16/08/2017 Hải Long
-- <History>
---- Modified by Đình Định on 09/11/2023: Tách store khách hàng An Phúc Thịnh.
---- Modified by Nhựt Trường on 14/12/2023: 2023/12/IS/0170, Load âm tiền thuế, tiền hàng nếu hóa đơn thuộc loại 'HG','HL'.
---- Modified by Nhựt Trường on 22/12/2023: 2023/12/IS/0258, Thêm dòng diễn giải cho hóa đơn điện tử lấy từ Parameter10

-- <Example>
---- EXEC AP0146_APT @DivisionID = 'APT',@UserID='ASOFTADMIN',@VoucherID='AV561ee05d-44de-4d71-8e4c-4927876f60df'

CREATE PROCEDURE [dbo].[AP0146_APT]
...
BEGIN
	...
	AT1202.TradeName AS CusbankNo, AT1202.TradeName AS CusBankName,
	...
	SET @sSQLA = '
		SELECT * FROM (
			SELECT	TOP 100 PERCENT Remark, CusCode, CusName, CusAddress, CusPhone, CusEmail, CusFax, CusEmailCC, CusTaxCode, Buyer, CusbankNo,CusBankName, PaymentMethod, 
				Total, DiscountAmount, AfterAmount, -VATAmount AS VATAmount, VAT_Rate,
				...
				ProdID, ProdName, ProdUnit, -ProdQuantity AS ProdQuantity, ProdPrice, -Amount AS Amount, InheritFkey, EInvoiceType, TypeOfAdjust, TDescription, Ana02ID, Ana02Name,
				DonDatHangSo, -OriginalAmount AS OriginalAmount, KindOfService, Extra1, -VATAmount0 AS VATAmount0, -VATAmount10 AS VATAmount10, Extra, InvoiceDate, CurrencyID, PaymentID, VoucherNo,
				 ...
				 RefNo01, RefNo02, VDescription, BDescription, -Quantity AS Quantity, InventoryID, UnitID, Status, IsAudit, IsCost,
				 Ana01ID, Ana03ID, Ana04ID, Ana05ID, Ana06ID, Ana07ID, Ana08ID, Ana09ID, Ana10ID, PeriodID,
				 ExpenseID, MaterialTypeID, ProductID, CreateDate, CreateUserID, LastModifyUserID, LastModifyDate, -OriginalAmountCN AS OriginalAmountCN,
				 ExchangeRateCN, CurrencyIDCN, DueDays, DueDate, DiscountRate, OrderID, CreditBankAccountID, DebitBankAccountID,
				 CommissionPercent, InventoryName1, PaymentTermID, DiscountAmountAT9000, OTransactionID, IsMultiTax, -VATOriginalAmount AS VATOriginalAmount, -VATConvertedAmount AS VATConvertedAmount,
				 ...
				 IsLateInvoice, -ConvertedQuantity AS ConvertedQuantity, ConvertedPrice, ConvertedUnitID, ConversionFactor, UParameter01, UParameter02, UParameter03,
				 UParameter04, UParameter05, PriceListID, WOrderID, WTransactionID, -MarkQuantity AS MarkQuantity, TVoucherID, OldCounter, NewCounter,
				 ...
				 BankAccountNo, BankName, ContactPerson, -DVATOriginalAmount AS DVATOriginalAmount, -DVATConvertedAmount AS DVATConvertedAmount, ConvertedUnitName, PaymentName, IsDiscount, DivisionNameE, AddressE, District, DContactPerson,
