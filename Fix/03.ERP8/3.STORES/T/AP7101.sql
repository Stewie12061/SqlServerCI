IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7101]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP7101]
GO
...
---- Modified by Bảo Anh on 02/04/2018: Hiển thị TK đối ứng đúng với cấp đang chọn
---- Modified by Kim Thư on 14/1/2019: Bổ sung CreateDate
---- Modified by Huỳnh Thử on 15/07/2020: Tách Store Meiko -- Mte
---- Modified by Nhựt Trường on 15/02/2022: Bổ sung kiều kiện DivisionID khi JOIN bảng AT1202.
---- Modified by Xuân Nguyên on 15/02/2022: Bổ sung dấu () cho điều kiện lọc của param @StrFinderMaster
---- Modified by Đức Tuyên on 08/05/2023: Fix lỗi chi tiết tài khoản sai số dư đầu kỳ sai
---- Modified by Đình Định on 28/06/2023: Đổi format datetime để lấy phiếu kết chuyển.
---- Modified by Thành Sang on 14/08/2023: Thêm format cho VoucherDate
---- Modified by Kiều Nga on 28/12/2023: [2023/12/IS/0311] Fix lỗi lấy sai dữ liệu in theo từ ngày đến ngày
...
IF (@CustomerIndex = 50 OR @CustomerIndex = 115) -- MEIKO - MTE
	EXEC AP7101_MK @DivisionID,@TranMonthFrom, @TranYearFrom, @TranMonthTo, @TranYearTo, 
	@FromDate, @ToDate, @IsDate,@AccountIDFrom, @AccountIDTo, @Level1, @UserID, @StrDivisionID,@StrFinderMaster
ELSE
BEGIN
	IF @Level1 = 0
		SET @strSQL = '
			SELECT  '''+@DivisionID +''' AS DivisionID,			BatchID,				TransactionTypeID,
					...
					AV5000.IsWithhodingTax, AV5000.CreateDate'
	ELSE
		SET @strSQL = '
			SELECT 	'''+@DivisionID +''' AS DivisionID,			BatchID,				TransactionTypeID,
					AccountID,			LEFT(CorAccountID, ' + STR(@Level1 + 2) + ') AS CorAccountID,			D_C,			
					DebitAccountID, 
					CreditAccountID,	
					Case when TransactionTypeID=''T98'' then  DateAdd(hh,1, VoucherDate) else  VoucherDate end AS  VoucherDate,		
					VoucherTypeID,		VoucherNo,
	...
	Set @sSQLUnion = '
			...
			SELECT  '''+@DivisionID +''' AS DivisionID,	NULL as	BatchID, TransactionTypeID,
					...
					0 as IsWithhodingTax, CreateDate
			FROM AV9004
			WHERE	DivisionID like ''' + @StrDivisionID +'''
					AND (AccountID like ''' + @AccountIDFrom +'%''  OR AccountID like ''' + @AccountIDTo +'%''
					OR (AccountID >=  ''' + @AccountIDFrom +''' AND AccountID <=  ''' + @AccountIDTo +'''))
...
BEGIN 
	...
	SET @strSQL = @strSQL + ' LEFT JOIN AT1005 AS T01 ON T01.AccountID = V10.LinkAccountID '
...
Begin
	...
	SET @strSQL = '
		SELECT		ISNULL(V16.DivisionID, V15.DivisionID) AS DivisionID,  V16.BatchID, V16.TransactionTypeID, V16.Period, 
					...
					Isnull(V16.IsWithhodingTax, 0) as IsWithhodingTax, V16.CreateDate'
	...
	SET @strSQL = @strSQL + ' LEFT JOIN AV4316  AS V16 ON V16.LinkAccountID = V15.LinkAccountID AND V15.DivisionID = V16.DivisionID
							  ...
							  LEFT JOIN AT1005 A05 ON V16.CorAccountID = A05.AccountID
