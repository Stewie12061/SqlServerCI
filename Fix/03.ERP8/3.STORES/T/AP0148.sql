IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0148]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0148]
GO
...
---- Modified on 05/12/2018 by Kim Thư: lấy thêm cột Ngày phát hành hóa đơn - InvoicePublishDate
---- Modified on 09/01/2018 by Kim Thư: Đưa hóa đơn Chưa phát hành (InvoiceNo=null) và Đã phát hành chờ ký (InvoiceNo = 0000000) về cùng 1 tab Chưa phát hành
---- Modified on 11/02/2019 by Kim Thư: Thêm cột InvoiceDate
---- Modified on 20/02/2019 by Kim Thư: Thêm cột InvoiceGuid, cho hiện hóa đơn chiết khấu
---- Modified on 27/05/2019 by Kim Thư: Không cho hiện dòng hóa đơn chiết khấu đối với AN Phúc Thịnh (CustomizeIndex = 27)
...
----									Nếu Hóa đơn chỉ có dòng chiết khấu (ko có T04, T14, T24, T34, T25, T35) thì cho hiện để phát hành
---- Modified on 22/11/2019 by Văn Tài: Bổ sung load dữ liệu BranchID
---- Modified on 23/03/2020 by Huỳnh Thử: Update Status = 1 để cập nhật số hóa đơn
---- Modified on 01/04/2020 by Huỳnh Thử: Check Isnull(AT1035.InvoiceNo)
---- Modified on 01/04/2020 by Huỳnh Thử: Customer FUYUEH: Sort Hóa đơn chưa phát hàng theo Số serial, Ngày đơn hàng
---- Modified on 24/04/2020 by Huỳnh Thử: Lọc theo Serial
---- Modified on 21/10/2020 by Hoài Phong: bổ sung cột để phân biệt HDBH theo bộ [Types]
---- Modified on 18/11/2020 by Đức Thông: FEIYUEH: Bổ sung order by theo thứ tự tăng dần: Ngày hóa đơn, số hóa đơn, số chứng từ
---- Modified on 08/03/2021 by Huỳnh Thử: Tách Store KOYO
---- Modified on 21/05/2021 by Xuân Nguyên: 2021/05/IS/0066 - Fix lỗi Hóa đơn thành tiền bị sai ( chưa trừ chiết khấu)
---- Modified on 15/07/2021 by Xuân Nguyên: bổ sung cột Ana02ID
---- Modified on 05/01/2022 by Nhựt Trường: Bổ sung ISNULL() các điều kiện có sử dụng like.
---- Modified on 15/02/2022 by Xuân Nguyên: Bổ sung điều kiện số hóa đơn 8 số theo thông tư 78
---- Modified on 03/03/2022 by Xuân Nguyên: Bổ sung điều kiện số hóa đơn 7 số để hiển thị các hóa đơn 7 số sũ
---- Modified on 23/03/2022 by Nhật Thanh: Bổ sung điều kiện sort giảm dần số chứng từ 
---- Modified on 23/03/2022 by Nhật Thanh: Cập nhật điều kiện sort tăng dần
---- Modified on 07/06/2022 by Thành Sang: Bổ sung điều kiện DivisionID cho khách hàng Song Bình - 110
---- Modified on 03/08/2022 by Nhật Thanh: Update Status = 1 để cập nhật số hóa đơn
---- Modified on 29/12/2022 by Nhật Thanh: Bỏ sung điều kiện divisionID khi AT1035 join AT9000
...
@sSQL1 AS NVARCHAR(MAX) = '', @CustomerName as int

SET @CustomerName = (SELECT Customername FROM CustomerIndex)

IF @CustomerName = 52 -- KoYo
BEGIN
    EXEC dbo.AP0148_KOYO @DivisionID ,                 -- nvarchar(50)
                    @UserID ,                     -- nvarchar(50)
                    @FromDate , -- datetime
                    @ToDate ,   -- datetime
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM (
			...
			--(SELECT SUM(ConvertedAmount) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'',''T14'',''T34'',''T35'')) AS AfterConvertedAmount,
			CASE WHEN EXISTS ( select Top 1 1 from AT9000 AT90 where TransactionTypeID = ''T64'' and AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID  )Then SUM(ConvertedAmount)-(SELECT SUM(ConvertedAmount) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T64'')) 
				ELSE SUM(ConvertedAmount) End AS ConvertedAmount ,
			CASE WHEN EXISTS ( select Top 1 1 from AT9000 AT90 where TransactionTypeID = ''T64'' and AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID  ) THEN (SELECT SUM(ConvertedAmount)  - (SELECT SUM(ConvertedAmount) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T64''))FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'',''T14'',''T34'',''T35'') )
				ELSE  (SELECT SUM(ConvertedAmount) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'',''T14'',''T34'',''T35'') ) END AS AfterConvertedAmount, 
			...
			AT9000.BranchID,AT9000.Ana02ID,
			...
			AT9000.BranchID,AT9000.Ana02ID, AT9000.TableID
...
SET @sSQL_0 = N'
		...
		SET @sSQL1 = N' 	
			...
			A35.InvoiceCode, A35.InvoiceSign, A35.ObjectID, A02.ObjectName, A35.CurrencyID, A35.VATConvertedAmount, 
			CASE WHEN EXISTS ( select Top 1 1 from AT9000 AT90 where TransactionTypeID = ''T64'' and AT90.DivisionID = A35.DivisionID AND AT90.VoucherID = A35.VoucherID  )Then SUM(ConvertedAmount)-(SELECT SUM(ConvertedAmount) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = A35.DivisionID AND AT90.VoucherID = A35.VoucherID AND TransactionTypeID IN (''T64'')) 
				ELSE SUM(A90.ConvertedAmount) End AS ConvertedAmount ,
			CASE WHEN EXISTS ( select Top 1 1 from AT9000 AT90 where TransactionTypeID = ''T64'' and AT90.DivisionID = A35.DivisionID AND AT90.VoucherID = A35.VoucherID  ) THEN (SELECT SUM(ConvertedAmount)  - (SELECT SUM(ConvertedAmount) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = A35.DivisionID AND AT90.VoucherID = A35.VoucherID AND TransactionTypeID IN (''T64''))FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = A35.DivisionID AND AT90.VoucherID = A35.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'',''T14'',''T34'',''T35'') )
				ELSE  (SELECT SUM(ConvertedAmount) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = A35.DivisionID AND AT90.VoucherID = A35.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'',''T14'',''T34'',''T35'',''T64'') ) END AS AfterConvertedAmount, 
			...
			A35.BranchID,A90.Ana02ID,
			CASE WHEN A90.TableID = ''AT9000'' THEN N''Hóa đơn bán hàng'' ELSE N''Hóa đơn theo bộ'' END AS [Types]
			FROM AT1035 A35 WITH (NOLOCK) INNER JOIN AT9000 A90 WITH (NOLOCK) ON A35.DivisionID = A90.DivisionID AND A35.VoucherID = A90.VoucherID AND A90.TransactionTypeID IN (''T04'',''T25'',''T24'') 
			...
			WHERE A35.DivisionID = ''' + @DivisionID + '''
				AND (A35.InvoiceNo=''00000000'' or A35.InvoiceNo=''0000000'')
			...
			A35.InvoiceDate, A35.InvoiceGuid, A35.BranchID, A90.Ana02ID,
			A90.TableID
			--ORDER BY A35.VoucherDate DESC
		) X
		'+ CASE WHEN @CustomerName = 110 then '	WHERE X.DivisionID = ''' + @DivisionID + '''' ELSE '' END + '
	...
	BEGIN
		...
		AT9000.BDescription AS Description, (CASE WHEN AT35A.IsCancel = 1 THEN 3 ELSE ISNULL(AT35B.EInvoiceType, 0) END) AS EInvoiceStatus, AV0027.EInvoiceStatusName,
			...
			CASE WHEN AT9000.TableID = ''AT9000'' THEN N''Hóa đơn bán hàng'' ELSE N''Hóa đơn theo bộ'' END AS [Types]
			FROM AT9000 WITH (NOLOCK)
			INNER JOIN AT1035 AT35A WITH (NOLOCK) ON AT35A.DivisionID = AT9000.DivisionID AND AT35A.AT9000VoucherID = AT9000.VoucherID AND AT35A.EInvoiceType = 0 AND ISNULL(AT35A.InvoiceNo,0) <> ''00000000'' and ISNULL(AT35A.InvoiceNo,0) <> ''0000000''
			LEFT JOIN 
			(
			...
			AT35A.InvoicePublishDate, AT9000.InvoiceGuid, AT9000.BranchID,AT9000.Ana02ID,AT9000.TableID
		'
		...
		SET @sSQL1 = N'
			...
			CASE WHEN AT9000.TableID = ''AT9000'' THEN N''Hóa đơn bán hàng'' ELSE N''Hóa đơn theo bộ'' END AS [Types]	 
			FROM AT9000 WITH (NOLOCK)
			INNER JOIN AT1035 AT35A WITH (NOLOCK) ON AT35A.DivisionID = AT9000.DivisionID AND AT35A.AT9000VoucherID = AT9000.VoucherID AND AT35A.EInvoiceType = 0 AND ISNULL(AT35A.InvoiceNo,0) <> ''0000000'' AND ISNULL(AT35A.InvoiceNo,0) <> ''00000000''
			LEFT JOIN 
			(
				SELECT DivisionID, InheritVoucherID, EInvoiceType
				FROM AT1035 WITH (NOLOCK)
			...
			GROUP BY AT9000.Fkey, AT35A.VoucherID, AT9000.DivisionID, AT9000.VoucherTypeID, AT9000.VoucherID, AT9000.VoucherNo, AT9000.VoucherDate, AT9000.InvoiceNo, AT9000.InvoiceDate, AT9000.Serial, AT9000.InvoiceCode, AT9000.InvoiceSign, 
			AT9000.ObjectID, AT1202.ObjectName, AT9000.CurrencyID, AT9000.BDescription, AT35B.EInvoiceType, AT35A.IsCancel, AV0027.EInvoiceStatusName, AT9000.TransactionTypeID, 
			AT35A.InvoicePublishDate, AT9000.InvoiceGuid, AT9000.BranchID,AT9000.Ana02ID, 
			AT9000.TableID
		) X 
		ORDER BY X.InvoiceDate , X.InvoiceNo , X.VoucherNo ' 
