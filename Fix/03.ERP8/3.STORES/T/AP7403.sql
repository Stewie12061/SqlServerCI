...
---- Modified on 26/04/2017 by Hải Long: Bổ sung mã loại Đối tượng ObjectTypeID và tên loại đối tượng ObjectTypeName (HHP)
...
---- Modified on 06/06/2019 by Kim Thư : Bổ sung thêm trường mã phân tích nghiệp vụ Ana01->Ana10, AnaName1 -> AnaName10 (CustomizeIndex = 73)
---- Modified on 14/10/2019 by My Tuyen: bo sung them truong Ghi chu trong thong tin doi tuong
---- Modified on 29/10/2019 by Văn Tài: Điều chỉnh cách map cột Ghi chú: Note
---- Modified on 06/04/2020 by Huỳnh Thử: Lây thêm trường: Object.RePaymentTermID,Object.DeAddress,Object.Note1,Object.ReDays,Object.ReCreditLimit của đối tượng
---- Modified on 10/05/2021 by Huỳnh Thử : [TienTien] -- Tách Store
---- Modified on 19/04/2021 by Nhật Thanh	: Bổ sung divisionID @@@
---- Modified on 29/03/2023	by Văn Tài		: [2023/03/IS/0234] [VNA] Lấy danh sách MPT.
-- <Example>
---- 
...
BEGIN
	EXEC dbo.AP7403_TIENTIEN @DivisionID = @DivisionID,                 -- nvarchar(50)
	                ...
	                @FromObjectID = @FromObjectID,               -- nvarchar(50)
	                @ToObjectID = @ToObjectID,                 -- nvarchar(50)
	                @Groupby = @Groupby,                      -- tinyint
	                @StrDivisionID = @StrDivisionID,              -- nvarchar(4000)
	                @DatabaseName = @DatabaseName,  -- nvarchar(250)
...
BEGIN	
	...
	SET @sSQL1 = '					   
			...
			'+ CASE WHEN @CustomerName = 73 OR @CustomerName = 147 -- VNA
			THEN 
			'
			, (Select Top 1 AV7402.Ana01ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana01ID, 
			(Select Top 1 AV7402.Ana02ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana02ID, 
			(Select Top 1 AV7402.Ana03ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana03ID, 
			(Select Top 1 AV7402.Ana04ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana04ID, 
			(Select Top 1 AV7402.Ana05ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana05ID, 
			(Select Top 1 AV7402.Ana06ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana06ID, 
			(Select Top 1 AV7402.Ana07ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana07ID, 
			(Select Top 1 AV7402.Ana08ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana08ID, 
			(Select Top 1 AV7402.Ana09ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana09ID, 
			(Select Top 1 AV7402.Ana10ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana10ID' ELSE '' END
	SET @sSQL2 = '
		FROM AV7402 D3 
			INNER JOIN AT1202 Object WITH (NOLOCK)  on  Object.ObjectID = D3.ObjectID and Object.DivisionID in (''@@@'',D3.DivisionID)
			...
			INNER JOIN AT1005 AS Account WITH (NOLOCK) on Account.AccountID = D3.AccountID AND Account.DivisionID in (''@@@'',D3.DivisionID)
	...
	SET @sSQL1 = '
			...
			AT1201.ObjectTypeID, AT1201.ObjectTypeName
			'+ CASE WHEN @CustomerName = 73 OR @CustomerName = 147 -- Vĩnh Nam Anh
			THEN 
			'
			, (Select Top 1 AV7402.Ana01ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana01ID, 
			(Select Top 1 AV7402.Ana02ID FROM AV7402 WHERE AV7402.ObjectID = D3.ObjectID) AS Ana02ID, 
	...
	SET @sSQL2 = '
		FROM AV7402 D3 	
			...
			LEFT JOIN AT1207 OS2 WITH (NOLOCK) on OS2.S = Object.S2 AND OS2.STypeID = ''O02''
			LEFT JOIN AT1207 OS3 WITH (NOLOCK) on OS3.S = Object.S3 AND OS3.STypeID = ''O03''
			INNER JOIN AT1005 Account WITH (NOLOCK) on Account.AccountID = D3.AccountID and Account.DivisionID in (''@@@'',D3.DivisionID)
			LEFT JOIN AT1201 WITH (NOLOCK) ON AT1201.ObjectTypeID = Object.ObjectTypeID 		
	...
	IF @CurrencyID <> '%'
		Set @sSQL =  ' 
			...
			A6.AnaName AS Ana06Name, A7.AnaName AS Ana07Name, A8.AnaName AS Ana08Name, A9.AnaName AS Ana09Name,A10.AnaName AS Ana10Name' ELSE '' END +' 
		FROM AV7413 
			'+ CASE WHEN @CustomerName = 73 OR @CustomerName = 147 -- Vĩnh Nam Anh
			THEN '
			LEFT JOIN AT1011 A1 on	( A1.AnaID = AV7413.Ana01ID AND  A1.AnaTypeID = ''A01'')
			LEFT JOIN AT1011 A2 on	( A2.AnaID = AV7413.Ana02ID AND  A2.AnaTypeID = ''A02'')
			LEFT JOIN AT1011 A3 on	( A3.AnaID = AV7413.Ana03ID AND  A3.AnaTypeID = ''A03'')
			LEFT JOIN AT1011 A4 on	( A4.AnaID = AV7413.Ana04ID AND  A4.AnaTypeID = ''A04'')
			LEFT JOIN AT1011 A5 on	( A5.AnaID = AV7413.Ana05ID AND  A5.AnaTypeID = ''A05'')
			LEFT JOIN AT1011 A6 on	( A6.AnaID = AV7413.Ana06ID AND  A6.AnaTypeID = ''A06'')
			LEFT JOIN AT1011 A7 on	( A7.AnaID = AV7413.Ana07ID AND  A7.AnaTypeID = ''A07'')
			LEFT JOIN AT1011 A8 on	( A8.AnaID = AV7413.Ana08ID AND  A8.AnaTypeID = ''A08'')
			LEFT JOIN AT1011 A9 on	( A9.AnaID = AV7413.Ana09ID AND  A9.AnaTypeID = ''A09'')
			LEFT JOIN AT1011 A10 on	( A10.AnaID = AV7413.Ana10ID AND  A10.AnaTypeID = ''A10'')
			' ELSE '' END +' 
	...
	Else
		...
		'+ CASE WHEN @CustomerName = 73 OR @CustomerName = 147 -- Vĩnh Nam Anh
			...
			LEFT JOIN AT1011 A8 on	( A8.AnaID = AV7413.Ana08ID AND  A8.AnaTypeID = ''A08'')
			LEFT JOIN AT1011 A9 on	( A9.AnaID = AV7413.Ana09ID AND  A9.AnaTypeID = ''A09'')
			LEFT JOIN AT1011 A10 on	( A10.AnaID = AV7413.Ana10ID AND  A10.AnaTypeID = ''A10'')' ELSE '' END +'
		WHERE DebitOriginalOpening - CreditOriginalOpening <> 0  
			OR DebitConvertedOpening - CreditConvertedOpening <> 0 OR OriginalDebit <> 0 
			OR ConvertedDebit <> 0 OR OriginalCredit <> 0 OR ConvertedCredit <> 0 
			OR DebitOriginalClosing - CreditOriginalClosing <> 0 
		...
		GROUP BY AV7413.DivisionID, GroupID, GroupName, GroupID1, GroupName1, GroupID2, GroupName2,
			...
			'
			, Ana01ID, Ana02ID, Ana03ID, Ana04ID, Ana05ID,Ana06ID,Ana07ID, Ana08ID, Ana09ID, Ana10ID,
			A1.AnaName, A2.AnaName, A3.AnaName, A4.AnaName, A5.AnaName, A6.AnaName, A7.AnaName , A8.AnaName, A9.AnaName ,A10.AnaName' ELSE '' END +' '
		...
		IF @CustomerName = 16 AND @DatabaseName <>''  --- Customize Sieu Thanh in du lieu 2 database
			BEGIN
				...
				SET @sSQLUnion  = '
					...
					DebitConvertedClosing,ConvertedClosing,OriginalDebitYTD,ConvertedDebitYTD,OriginalCreditYTD,ConvertedCreditYTD, ObjectTypeID, ObjectTypeName 
					FROM AV7403_ST '
