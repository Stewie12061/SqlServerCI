IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7302_TIENTIEN]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP7302_TIENTIEN]
GO
...
---- Modified by Huỳnh Thử on 01/04/2021: Bố sung print nhiều Division

CREATE PROCEDURE [dbo].[AP7302_TIENTIEN] 
...
SET @sSQL = '
    ...
    LEFT JOIN AT1006 AS T06 WITH (NOLOCK) ON T05.GroupID = T06.GroupID AND T06.DivisionID '+@StrDivisionID_New+'
    WHERE T73.DivisionID '+@StrDivisionID_New+''
...
BEGIN
	...
	GROUP BY T.DivisionID,ObjectID, AccountID
    ...
    BEGIN
        ...
        SET @tmp = @tmp + 
            ...
            WHERE len(T73.AccountID) > ' + LTRIM(RTRIM(str(2 + @count))) + ' 
					AND T73.DivisionID '+@StrDivisionID_New+'
...
IF NOT EXISTS(SELECT 1 FROM SYSOBJECTS WITH (NOLOCK) WHERE SYSOBJECTS.NAME = 'AV7302' AND SYSOBJECTS.XTYPE = 'V')
    ...
    AS SELECT * FROM (' + @sSQL+ @SQL1 + @SQL2 + @SQL3 + @SQL4+ ') A WHERE DivisionID '+@StrDivisionID_New+'' )
ELSE
...
IF NOT EXISTS(SELECT 1 FROM SYSOBJECTS WITH (NOLOCK) WHERE SYSOBJECTS.NAME = 'AT7302' AND SYSOBJECTS.XTYPE = 'U')
    EXEC('SELECT * INTO AT7302 FROM AV7302 WHERE DivisionID '+@StrDivisionID_New+'')
ELSE
    BEGIN
