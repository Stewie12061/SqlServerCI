IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0316]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0316]
GO
...
---- Modified by Đình Định on 06/10/2023: SONGBINH - Tách store.
-- <Example>
---- 
CREATE PROCEDURE [dbo].[AP0316]
( 
...
Declare @sSQL AS nvarchar(MAX),
	...
	--IF @CustomerName = 110
	--BEGIN
	--	SET @AddColumnReport = ', AT1202.TradeName,AV0327.AccountID'
	--END
	...
	ELSE
	IF @CustomerName = 27 --- Customize AN PHÚC THỊNH
		EXEC AP0316_ANPHUCTHINH @DivisionID, @ReportCode,@FromObjectID,  @ToObjectID ,@FromAccountID ,  
					@ToAccountID, @CurrencyID,@Filter1IDFrom ,@Filter1IDTo ,@Filter2IDFrom ,
					@Filter2IDTo ,@Filter3IDFrom ,@Filter3IDTo,@ReportDate ,@IsBefore,@IsType
	ELSE
	IF @CustomerName = 110 --- Customize SONG BÌNH
		EXEC AP0316_SONGBINH @DivisionID, @ReportCode, @FromObjectID, @ToObjectID, @FromAccountID,  
							 @ToAccountID, @CurrencyID, @Filter1IDFrom, @Filter1IDTo, @Filter2IDFrom,
							 @Filter2IDTo, @Filter3IDFrom, @Filter3IDTo, @ReportDate, @IsBefore, @IsType
	ELSE
	Begin
		...
		SELECT	AV4301.DivisionID, AV4301.ObjectID, AV4301.AccountID, AV4301.CorAccountID, AV4301.VoucherTypeID, 
				...
				' + CASE WHEN @CustomerName = 110 THEN ',AV4301.OrderID,ISNULL(AV4301.OrderDate, AV4301.VoucherDate) AS OrderDate' ELSE '' END +'
