IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0599_AG]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0599_AG]
GO
...
---- Modified on 29/07/2013 by Lê Th? Thu Hi?n : B? sung RefNo01, RefNo02
--- Modified by Ti?u Mai on 01/03/2016: B? sung x? lý phát sinh phi?u t? Nh?p kho s?n xu?t cho ANGEL (CustomizeIndex = 57)
--- Modified by Kim Thư on 15/11/2018: Bổ sung check @IsLedgerAT2006: ko định khoản vào sổ cái (ko insert AT9000) (Angel)


CREATE PROCEDURE [dbo].[AP0599_AG]
