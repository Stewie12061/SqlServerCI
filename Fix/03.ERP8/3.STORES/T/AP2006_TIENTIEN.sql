IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP2006_TIENTIEN]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP2006_TIENTIEN]
GO
...
---- Modified on 18/11/2021 by Nhật Thanh: Đồng bộ cột AcountID thành inventoryAcountID cho in theo ngày giống theo kỳ

CREATE PROCEDURE [dbo].[AP2006_TIENTIEN]
	...
	BEGIN
		...
		BEGIN
			...
			ELSE
				...
				SET @sSQL0 = N'		
								SELECT	   SUM(ISNULL(BeginQuantity,0)) AS BeginQuantity, 
		                        		...
		                        		WHERE 	
											T03.IsTemp = 0 AND
											T08.DivisionID '+@StrDivisionID_New+' AND
			...
			SET  @sSQL2_2 = 'AT1302.I01ID,AT1302.I02ID,AT1302.I03ID,AT1302.I04ID,AT1302.I05ID,
					...
					INNER JOIN AT1302 WITH (NOLOCK) ON AT1302.DivisionID '+@StrDivisionID_New+' AND AT1302.InventoryID = AT2008.InventoryID
					INNER JOIN AT1304 WITH (NOLOCK) ON AT1302.DivisionID '+@StrDivisionID_New+' AND AT1304.UnitID = AT1302.UnitID
				...
				LEFT JOIN #BEGIN BEGIN1 WITH (NOLOCK) ON BEGIN1.InventoryID = AT2008.InventoryID AND BEGIN1.InventoryAccountID = AT2008.InventoryAccountID
					...
					WHERE 	--AT1302.Disabled = 0 AND
							...
							AT2008.DivisionID '+@StrDivisionID_New+' AND
		...
		BEGIN
			...
			WHERE A1.DivisionID '+@StrDivisionID_New+' AND A1.DebitAccountID BETWEEN '''+@FromAccountID+''' AND '''+@ToAccountID+''' AND A6.IsTemp = 0
			'
			SET @SQL1='
			UNION ALL
			...
			LEFT JOIN AT1015 I05 WITH (NOLOCK) ON I05.AnaTypeID = ''I05'' AND I05.AnaID = A3.I05ID
			LEFT JOIN AT1303 A6 WITH (NOLOCK) ON A6.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A2.WarehouseID = A6.WarehouseID
			WHERE A1.DivisionID '+@StrDivisionID_New+' AND A1.DebitAccountID BETWEEN '''+@FromAccountID+''' AND '''+@ToAccountID+'''
			...
			WHERE A1.DivisionID '+@StrDivisionID_New+' AND A1.CreditAccountID BETWEEN '''+@FromAccountID+''' AND '''+@ToAccountID+'''
			...
			LEFT JOIN AT1015 I04 WITH (NOLOCK) ON I04.AnaTypeID = ''I04'' AND I04.AnaID = A3.I04ID
			LEFT JOIN AT1015 I05 WITH (NOLOCK) ON I05.AnaTypeID = ''I05'' AND I05.AnaID = A3.I05ID
			WHERE A1.DivisionID '+@StrDivisionID_New+' AND A1.DebitAccountID BETWEEN '''+@FromAccountID+''' AND '''+@ToAccountID+''' AND A2.KindVoucherID = 3
					AND A6.IsTemp = 0
			GROUP BY A1.DivisionID, A1.InventoryID, A3.InventoryName, A3.UnitID, A4.UnitName, A3.InventoryTypeID, A3.Specification, A3.Notes01, A3.Notes02, A3.Notes03,
			...
			LEFT JOIN AT1015 I04 WITH (NOLOCK) ON I04.AnaTypeID = ''I04'' AND I04.AnaID = A3.I04ID
			LEFT JOIN AT1015 I05 WITH (NOLOCK) ON I05.AnaTypeID = ''I05'' AND I05.AnaID = A3.I05ID
			WHERE A1.DivisionID '+@StrDivisionID_New+' AND A1.CreditAccountID BETWEEN '''+@FromAccountID+''' AND '''+@ToAccountID+''' AND A2.KindVoucherID = 3
					AND A6.IsTemp = 0
			GROUP BY A1.DivisionID, A1.InventoryID, A3.InventoryName, A3.UnitID, A4.UnitName, A3.InventoryTypeID, A3.Specification, A3.Notes01, A3.Notes02, A3.Notes03,
						A1.CreditAccountID, A5.AccountName,A3.I01ID,A3.I02ID,A3.I03ID,A3.I04ID,A3.I05ID,  I01.AnaName,I02.AnaName,
						I03.AnaName,I04.AnaName,I05.AnaName,A3.Barcode
			...
			SELECT #TEMP.DivisionID, #TEMP.InventoryID, #TEMP.InventoryName, #TEMP.UnitID, #TEMP.UnitName, #TEMP.InventoryTypeID,
					#TEMP.Specification, #TEMP.Notes01, #TEMP.Notes02, #TEMP.Notes03, #TEMP.AccountID as InventoryAccountID, #TEMP.AccountName,
