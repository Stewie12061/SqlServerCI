IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_PL]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_PL]
GO
...
-- AP0146_PL
-- <Summary>
---- Stored load dữ liệu hóa đơn phục vụ phát hành hóa đơn điện tử (Customer PHÚC LONG).
...
---- Modified by Nhựt Trường on 01/12/2021: Bổ sung và thay đổi cách lấy dữ liệu một số trường cho phù hợp với hóa đơn điện tử FPT.
...
CREATE PROCEDURE [dbo].[AP0146_PL]
...
---- Tự động sinh mã bí mật
SET @Salt=NEWID()
SET @Salt = SUBSTRING(@Salt,0,7)
...
BEGIN
	SET @sSQL6 = ',AT9000.InvoiceDate AS ArisingDate'
END
...
BEGIN
	...
	A06.VoucherDate AS InvoiceDate, 0 AS VAT_Rate, 1 AS Extra, A02.TradeName AS CusBankName, '''' AS KindOfService, '''' AS CusTaxCode, 1 AS Extra1,
...
'
	...
	CASE WHEN ISNULL(AT9000.VATObjectName,'''') <> '''' THEN AT9000.VATObjectName ELSE AT1202.ObjectName END AS CusCom,
	A01.AnaName AS CusName, 
	...
	ABS(AT9000.ConvertedAmount) AS Total,
	...
	ISNULL(ABS(AT9000.VATConvertedAmount),0) AS VATAmount,
	...
	- ISNULL((SELECT SUM(ABS(ConvertedAmount)) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T64'')),0)
	+ ISNULL((SELECT SUM(ABS(VATConvertedAmount)) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID),0) 
	AS AfterAmount,
	
	ISNULL((SELECT SUM(ABS(OriginalAmount)) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'') AND VATGroupID = ''KT''), 0) AS GrossValue,
	ISNULL((SELECT SUM(ABS(OriginalAmount)) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'') AND VATGroupID = ''T00''), 0) AS GrossValue0,
	ISNULL((SELECT SUM(ABS(OriginalAmount)) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'') AND VATGroupID = ''T05''), 0) AS GrossValue5,
	ISNULL((SELECT SUM(ABS(OriginalAmount)) FROM AT9000 AT90 WITH (NOLOCK) WHERE AT90.DivisionID = AT9000.DivisionID AND AT90.VoucherID = AT9000.VoucherID AND TransactionTypeID IN (''T04'',''T24'',''T25'') AND VATGroupID = ''T10''), 0) AS GrossValue10,
	'
	SET @sSQL1A=N'
	...
	CASE WHEN @CustomerName = 2 THEN 'CASE WHEN AT9000.Serial = ''EX/21E'' THEN ABS(AT9000.OriginalAmount) ELSE ABS(AT9000.ConvertedAmount) END AS Amount,'
								ELSE 'ISNULL(ABS(AT9000.ConvertedAmount),0) + ISNULL(ABS(AT9000.VATConvertedAmount),0) AS Amount,' END + '
	...
	REPLACE(
			REPLACE(CONCAT(''ASOFT'',A11.VATNo,AT9000.Serial,AT9000.InvoiceSign),''/'','''')
																							,''-'','''') AS FPTEInvoiceCreate,
	--''BLUEPOS031687171901GTKT0001AS21E'' AS FPTEInvoiceCreate,
	REPLACE(
			REPLACE(
					CONCAT(''ASOFT'',A11.VATNo,AT9000.Serial,AT9000.InvoiceSign,(SELECT TOP 1 InvoiceNo FROM AT9000 T90 WHERE T90.VoucherID = AT9000.InheritFkey))
																																								  ,''/'','''')
																																											  ,''-'','''') AS FPTEInvoiceAdjust,
	'''+@Salt+''' AS Salt, '''' AS VinTich, '''' AS VinTieu, '''' AS VinTraLai,
	...
	SET @sSQLA = '
		SELECT * FROM (
			SELECT	TOP 100 PERCENT Remark, CusCode, CusCom, CusName, CusAddress, CusPhone, CusEmail,CusEmailCC, CusTaxCode, Buyer, CusbankNo,CusBankName, PaymentMethod, 
				Total,(SELECT ISNULL(SUM(Total),0) FROM #TEMP) AS InvoiceTotal, DiscountAmount, AfterAmount, (SELECT ISNULL(SUM(VATAmount),0) FROM #TEMP) AS VATAfterAmount,	VATAmount, VAT_Rate,
				...
				DonDatHangSo, OriginalAmount, KindOfService, Extra1, GrossValue, GrossValue0, GrossValue5, GrossValue10, 
				(SELECT ISNULL(SUM(VATAmount),0) FROM #TEMP WHERE VATGroupID = ''T00'') AS VATAmount0,
				(SELECT ISNULL(SUM(VATAmount),0) FROM #TEMP WHERE VATGroupID = ''T05'') AS VATAmount5,
				(SELECT ISNULL(SUM(VATAmount),0) FROM #TEMP WHERE VATGroupID = ''T10'') AS VATAmount10,
				Extra, InvoiceDate, CurrencyID, PaymentID, VoucherNo,
				 ...
				 FPTEInvoiceCreate, FPTEInvoiceAdjust,
				 Salt, '''' AS VinTich, '''' AS VinTieu, '''' AS VinTraLai,
	...
	IF EXISTS (SELECT TOP 1 1  FROM AT0000 WHERE DefDivisionID=@DivisionID AND EInvoicePartner='BKAV') AND EXISTS (SELECT TOP 1 1 FROM AT9000 WITH (NOLOCK) WHERE VoucherID = @VoucherID AND ISNULL(Parameter10,'') <> '')
		begin
			...
			SELECT DISTINCT
			999999  AS Remark, CusCode AS CusCode , CusCom AS CusCom,  CusName AS CusName,CusAddress AS CusAddress, '''' AS CusPhone,  '''' AS CusEmail, '''' AS CusEmailCC, '''' AS CusTaxCode,
			Buyer AS Buyer, '''' AS CusbankNo,'''' AS CusBankName,  '''' AS PaymentMethod,  0 AS Total, 0 AS InvoiceTotal,  0 AS DiscountAmount,  0 AS AfterAmount,  0 AS VATAfterAmount, 0 AS VATAmount ,  0 AS VAT_Rate,
			...
			'''' AS DonDatHangSo,  0 AS OriginalAmount, '''' AS KindOfService, CurrencyID AS Extra1, 0 AS GrossValue, 0 AS GrossValue0, 0 AS GrossValue5, 0 AS GrossValue10,
			0 AS VATAmount0, 0 AS VATAmount5, 0 AS VATAmount10,
			1 AS Extra,InvoiceDate , CurrencyID , '''' AS VoucherID , '''' AS PaymentID , '''' AS  VoucherNo ,'''' AS BatchID,  '''' AS TransactionID ,  '''' AS TableID, 0 AS TranMonth , 
			...
			0 AS OriginalAfterVATAmount , 0 AS ConvertedAfterVATAmount, '''' AS WSourceNo , '''' AS WLimitDate ,'''' AS I04ID ,'''' AS I04Name,  4 AS ItemTypeID, NULL AS CusBankAccountNo,NULL AS ParentInvoiceSign, NULL AS ParentSerial, NULL  AS FPTEInvoiceCreate, NULL  AS FPTEInvoiceAdjust,
			Salt, '''' AS VinTich, '''' AS VinTieu, '''' AS VinTraLai,
		FROM #TEMP
