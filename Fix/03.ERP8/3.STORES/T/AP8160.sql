IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WITH (NOLOCK) WHERE ID = OBJECT_ID(N'[DBO].[AP8160]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP8160]
GO
...
-- <Summary>
---- Xử lý Import dữ liệu Số dư tài khoản Nợ đầu kỳ
-- <Param>
---- 
-- <Return>
---- 
...
SELECT	X.Data.query('Row').value('.', 'INT') AS Row,
		...
		(CASE WHEN X.Data.query('DebitBankAccountID').value('.', 'VARCHAR(50)') = '' THEN CONVERT(VARCHAR(50), NULL)
			  ELSE X.Data.query('DebitBankAccountID').value('.', 'VARCHAR(50)') END) AS DebitBankAccountID,--DebitBankAccountID
...
INSERT INTO #Data (
		...
		DebitAccountID,DebitBankAccountID,ObjectID,OriginalAmount,ConvertedAmount,TDescription,
...
IF EXISTS (SELECT TOP 1 1 FROM #Data WHERE ISNULL(DebitBankAccountID,'') <> '')
BEGIN
EXEC AP8100 @UserID = @UserID, @ImportTemplateID = @ImportTemplateID, @CheckCode = 'CheckIdenticalValues', @ColID = 'VoucherNo', @Param1 = 'VoucherNo,VoucherTypeID,VoucherDate,CurrencyID,ExchangeRate,EmployeeID,DebitAccountID,DebitBankAccountID,VDescription'
END
ELSE
BEGIN
...
-- Kiem tra nếu DebitBankAccountID <> '' thì DebitAccountID phải là AccountID của BankAccountID ở AT1016
...
CurrencyID,ObjectID,VoucherTypeID,VoucherNo,VoucherDate,EmployeeID,DebitAccountID,DebitBankAccountID,
