IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP8125_THTP]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP8125_THTP]
GO
...
-- <Summary>
---- ** Khách hàng Tổng hợp THỊNH PHÁT** 
...
---- Modified by Nhựt Trường on 23/02/2021: Bổ sung lấy InventoryName1.
---- Modified by Đức Duy on 21/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.
-- <Example>																			  
---- 
---- 

CREATE PROCEDURE AP8125_THTP
( 
...
SELECT	X.Data.query('Row').value('.', 'INT') AS Row,
		...
		X.Data.query('InventoryName1').value('.', 'NVARCHAR(250)') AS InventoryName1,
...
(
	...
	Quantity,		InventoryID,	InventoryName1,		UnitID,	
...
SELECT	D.DivisionID,		D.VoucherID,		D.BatchID,		D.TransactionID,	'AT9000',
		...
		D.Quantity,		D.InventoryID,	D.InventoryName1,	D.UnitID,		
		0,		0,		0,	
...
SELECT	D.DivisionID,		D.VoucherID,		D.BatchID,		NewID(),	'AT9000',
		...
		CASE ISNULL(D.IsMultiTax, 0) WHEN 1 THEN -- Nhiều nhóm thuế nguyên tệ
				SUM(ROUND(A1.VATRate / 100 * (CASE WHEN D.InventoryID != 'Z001' THEN D.OriginalAmount - ISNULL(D.DiscountAmount, 0) ELSE 0 END), A2.ConvertedDecimals)) - ROUND(A1.VATRate / 100 * SUM(CASE D.InventoryID WHEN 'Z001' THEN OriginalAmount ELSE 0 END), A2.ConvertedDecimals)
			ELSE -- Một nhóm thuế nguyên tệ				
				ROUND(A1.VATRate / 100 * (SUM(CASE WHEN D.InventoryID != 'Z001' THEN D.OriginalAmount - ISNULL(D.DiscountAmount, 0) ELSE 0 END) - SUM(CASE D.InventoryID WHEN 'Z001' THEN D.OriginalAmount ELSE 0 END)), A2.ConvertedDecimals)
				 END, -- OriginalAmount
		CASE ISNULL(D.IsMultiTax, 0) WHEN 1 THEN -- Nhiều nhóm thuế qui đổi
				SUM(ROUND(A1.VATRate / 100 * (CASE WHEN D.InventoryID != 'Z001' THEN D.ConvertedAmount - ISNULL(D.DiscountAmount, 0) ELSE 0 END), A2.ConvertedDecimals)) - ROUND(A1.VATRate / 100 * SUM(CASE D.InventoryID WHEN 'Z001' THEN ConvertedAmount ELSE 0 END), A2.ConvertedDecimals)
			ELSE -- Một nhóm thuế qui đổi
				ROUND(A1.VATRate / 100 * (SUM(CASE WHEN D.InventoryID != 'Z001' THEN D.ConvertedAmount - ISNULL(D.DiscountAmount, 0) ELSE 0 END) - SUM(CASE D.InventoryID WHEN 'Z001' THEN D.ConvertedAmount ELSE 0 END)), A2.ConvertedDecimals)
				 END, -- ConvertedAmount		
...
(
	...
	InventoryID,	InventoryName1,	UnitID,			SourceNo,
...
SELECT	D.Orders,
		...
		D.InventoryID,		D.InventoryName1,		D.UnitID,			D.SourceNo,
