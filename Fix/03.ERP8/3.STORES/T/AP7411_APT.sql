IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7411_APT]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP7411_APT]
GO
...
---- Modified by Nhựt Trường on 18/12/2023: 2023/12/IS/0170, Bổ sung điều kiện theo loại chứng từ 'HL', xử lý tương tự loại 'HG' (lấy âm tiền thuế và tiền hàng).

CREATE PROCEDURE [dbo].[AP7411_APT]
...
Else
	...
	CASE  WHEN ( T7419.TransactionTypeID IN (''T04'', ''T14'') OR TypeOfAdjust = 0) AND (T7419.VoucherTypeID NOT LIKE ''HG%'' AND T7419.VoucherTypeID NOT LIKE ''HL%'') THEN 1
		  WHEN ( T7419.TransactionTypeID IN (''T24'', ''T34'') OR T7419.VoucherTypeID LIKE ''HG%'' OR T7419.VoucherTypeID LIKE ''HL%'' OR TypeOfAdjust = 3) THEN - 1
...
SET @strSQL = @strSQL + ') AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and (T7419.TransactionTypeID =''T34'' or  T7419.VoucherTypeID IN (''HG'',''HL'') or TypeOfAdjust = 3) )) THEN T7419.ConvertedAmount 
ELSE 
CASE WHEN ((T7419.AccountID >=''' + ISNULL( @TaxAccountID1From,'') + ''' AND T7419.AccountID <='''+ ISNULL(@TaxAccountID1To,'') +''')
'
IF (@TaxAccountID2From is Not NULL OR @TaxAccountID2From <> '')
...
SET @strSQL = @strSQL + ') AND T7419.D_C = ''D'' and Isnull(T7419.TransactionTypeID,'''') <>''T34'' 
							AND (T7419.AccountID <> T7419.CorAccountID) AND ISNULL(T7419.VoucherTypeID,'''') NOT IN (''HG'',''HL'') THEN T7419.ConvertedAmount * (-1) ELSE 0 END END ) AS ConvertedTaxAmount,'
END
...
SET @strSQL = @strSQL + ') AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and (T7419.TransactionTypeID =''T34'' or  T7419.VoucherTypeID IN (''HG'',''HL'') or TypeOfAdjust = 3) )) THEN T7419.OriginalAmount 
ELSE 
CASE WHEN ((T7419.AccountID >=''' + ISNULL( @TaxAccountID1From,'') + ''' AND T7419.AccountID <='''+ ISNULL(@TaxAccountID1To,'') +''')
'
IF (@TaxAccountID2From is Not NULL OR @TaxAccountID2From <> '')
...
SET @strSQL = @strSQL + ') AND T7419.D_C = ''D''  and Isnull(T7419.TransactionTypeID,'''') <>''T34'' 
							AND (T7419.AccountID <> T7419.CorAccountID)  AND  Isnull(T7419.VoucherTypeID,'''') NOT IN (''HG'',''HL'') THEN T7419.OriginalAmount * (-1) ELSE 0 END END) AS OriginalTaxAmount,'
END
...
BEGIN
	...
	Else		---- Xac dinh doanh so ban ra
		SET @strSQL = @strSQL +  ')  AND ( T7419.D_C like ''C'' and T7419.CorAccountID NOT BETWEEN ''3331102'' AND ''3331102'' or (T7419.D_C like ''D'' and  (T7419.TransactionTypeID in (''T24'',''T65'') or  T7419.VoucherTypeID IN (''HG'',''HL'') or (TypeOfAdjust = 3 AND T7419.TransactionTypeID <> ''T04'')) )) '
...
BEGIN
	If @IsVATIn = 1  --- Xac dinh doanh so mua vao
		SET @strSQL = @strSQL +  ') AND ((T7419.D_C like ''D'' and T7419.TransactionTypeID not in (''T25'',''T35'')) or (T7419.D_C like ''C'' and T7419.TransactionTypeID =''T25'')) '
	Else		---- Xac dinh doanh so ban ra
		SET @strSQL = @strSQL +  ')  AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and  (T7419.TransactionTypeID in (''T24'',''T65'') or  T7419.VoucherTypeID IN (''HG'',''HL'') or TypeOfAdjust = 3) )) '
...
Else		---- Xac dinh doanh so ban ra
	...
	SET @strSQL = @strSQL +  ')  AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and  (T7419.TransactionTypeID in (''T24'',''T65'') or  T7419.VoucherTypeID IN (''HG'',''HL'')) )) '
...
Else		----- Xac dinh doanh so dau ra
	SET @strSQL1 = @strSQL1 +  ')  AND ( T7419.D_C like ''C''  and T7419.CorAccountID NOT BETWEEN ''' + @TaxAccountID1From + ''' AND '''+ @TaxAccountID1To +''' or (T7419.D_C like ''D'' and (T7419.TransactionTypeID in (''T24'',''T65'') or  T7419.VoucherTypeID IN (''HG'',''HL'') or TypeOfAdjust = 3) )) '
