IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP2008_BBL]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP2008_BBL]
GO
...
---- Modified by Đình Định on 10/08/2023: BBL - Lấy cột BeginConvertedQuantity, DebitConvertedQuantity, CreditConvertedQuantity, EndConvertedQuantity .
---- Modified by Đình Định on 24/08/2023: BBL - Select lại các cột ConvertedQuantity, cải tiến tốc độ in.
/*
    EXEC AP2008 'KHV', 1, 2016, 1,2016, '01/01/2016', '01/10/2016', 'CRA00710', 'G80A4550', 'HGK', 1, 0, '', ''
*/

CREATE PROCEDURE AP2008_BBL
(
...
BEGIN
   DECLARE	@sSQLDrop AS NVARCHAR(MAX)='',
			...
			@sSQLUnion_1 AS NVARCHAR(MAX) ='',
			@sSQLUpdate AS NVARCHAR(MAX) ='',
			...
			@TableTemp1 NVARCHAR(MAX),
			@TableTemp1_1 NVARCHAR(MAX)
		...
		'
			SET @TableTemp1 = N'
		SELECT AV7000.DivisionID, AV7000.WareHouseID, AV7000.InventoryID, AV7000.D_C, AV7000.TranMonth, AV7000.TranYear
		  INTO #TEMP
		  FROM (
		SELECT D17.DivisionID, D16.WareHouseID, D02.InventoryTypeID, D17.InventoryID, D02.UnitID,
			   D02.InventoryName, D04.UnitName, D03.WarehouseName,
			   D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID, 
			   D17.TranMonth, D17.TranYear, D02.S1, D02.S2, D02.S3, 
			   D02.Notes01 , D02.Notes02, D02.Notes03,
			   D02.SalePrice01,D02.Specification, D02.Barcode, 
			   D17.ActualQuantity AS SignQuantity, D17.ConvertedAmount AS SignAmount,
			   D02.Varchar01, D02.ETaxConvertedUnit, D16.
			   , ''BD'' AS D_C,
	     ...
	     WHERE ISNULL(D17.DebitAccountID,'''') <>'''' 
				UNION ALL -- Số dư có
		SELECT D17.DivisionID, D16.WareHouseID, D02.InventoryTypeID, D02.UnitID, D17.InventoryID,
			   D02.InventoryName, D04.UnitName, D03.WarehouseName,
			   D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID,
			   D17.TranMonth, D17.TranYear, D02.S1, D02.S2, D02.S3,
			   D02.Notes01 , D02.Notes02, D02.Notes03,
		 ...
		 INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN (D17.DivisionID,''@@@'') AND D02.InventoryID = D17.InventoryID
	      LEFT JOIN AT1304 D04 WITH (NOLOCK) ON D04.DivisionID IN (D17.DivisionID,''@@@'') AND D04.UnitID = D02.UnitID
		 INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN (D17.DivisionID, ''@@@'') AND D03.WareHouseID = D16.WareHouseID
	     WHERE ISNULL(D17.CreditAccountID,'''') <>''''
		'
		SET @TableTemp1_1 = N'
			UNION ALL -- Nhập kho
		SELECT D07.DivisionID, D06.WareHouseID, D02.InventoryTypeID, D02.UnitID, D07.InventoryID,
			   D02.InventoryName, D04.UnitName, D03.WarehouseName,
			   D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID, 
			   D07.TranMonth, D07.TranYear, D02.S1, D02.S2, D02.S3, 
			   D02.Notes01, D02.Notes02, D02.Notes03,
			   D02.SalePrice01,D02.Specification, D02.Barcode,    
		...
		SELECT D07.DivisionID, CASE WHEN D06.KindVoucherID = 3 THEN D06.WareHouseID2 ELSE D06.WareHouseID END AS WareHouseID,
			   D02.InventoryName, D04.UnitName, D03.WareHouseName,
			   D02.InventoryTypeID, D02.UnitID, D07.InventoryID,
			   D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID,
			   D07.TranMonth, D07.TranYear, D02.S1, D02.S2, D02.S3,  
			   D02.Notes01 , D02.Notes02, D02.Notes03,
			   D02.SalePrice01,  D02.Specification, D02.Barcode,   
		 ...
		 INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN (D07.DivisionID,''@@@'') AND D02.InventoryID = D07.InventoryID
		  LEFT JOIN AT1304 D04 WITH (NOLOCK) ON D04.DivisionID IN (D07.DivisionID,''@@@'') AND D04.UnitID = D02.UnitID
		 INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''', ''@@@'') 
											AND D03.WareHouseID = CASE WHEN D06.KindVoucherID = 3 THEN D06.WareHouseID2 ELSE  D06.WareHouseID END
		 WHERE D06.KindVoucherID IN (2,3,4,6,8,10,14,20)
		 ) AV7000	 '

		EXEC(@TableTemp1 + @TableTemp1_1)
		--PRINT @TableTemp1
		--PRINT @TableTemp1_1
	...
	IF @IsDate = 0 --Xac dinh so du theo ky
		BEGIN
			...
			SET @sSQLSelect = '
				...
				SUM(CASE WHEN TranMonth + TranYear * 100 = ' + @FromMonthYearText + ' THEN ISNULL(BeginQuantity, 0) *ISNULL(ConversionFactor,0) ELSE 0 END) AS BeginConvertedQuantity, 
				SUM(ISNULL(DebitQuantity, 0)) AS DebitQuantity,
				SUM(ISNULL(DebitQuantity, 0)*ISNULL(ConversionFactor,0)) AS DebitConvertedQuantity,
				SUM(ISNULL(CreditQuantity, 0)) AS CreditQuantity, 
				SUM(ISNULL(CreditQuantity, 0)*ISNULL(ConversionFactor,0)) AS CreditConvertedQuantity, 
				SUM(CASE WHEN TranMonth + TranYear * 100 = ' + @ToMonthYearText + ' THEN ISNULL(EndQuantity, 0) ELSE 0 END) AS EndQuantity, 
				SUM(CASE WHEN TranMonth + TranYear * 100 = ' + @ToMonthYearText + ' THEN ISNULL(EndQuantity, 0)*ISNULL(ConversionFactor,0) ELSE 0 END) AS EndConvertedQuantity, 
		...
		BEGIN
			...
			SET @sSQLSelect = ' 
				SELECT AV7000.S1, AV7000.S2, AV7000.S3, AV7000.I01ID, AV7000.I02ID, AV7000.I03ID, AV7000.I04ID, AV7000.I05ID, 
					   ...
					   SUM(SignQuantity) AS BeginConvertedQuantity, 
					   --ISNULL(AV7000.ConvertedQuantity,0) AS BeginConvertedQuantity,
					   0 AS CreditQuantity,
				       0 AS CreditConvertedQuantity, 
					   0 AS DebitQuantity,
				       0 AS DebitConvertedQuantity,
					   0 AS DebitAmount, SUM(SignAmount) AS BeginAmount, 0 AS CreditAmount, 
				       0 AS EndQuantity,
				       0 AS EndConvertedQuantity,
					   0 AS EndAmount, 0 AS InDebitQuantity, 0 AS InCreditQuantity, 0 AS InDebitAmount, 
			...
			SET @sSQLWhere = N'
			WHERE AV7000.DivisionID = ''' + @DivisionID + ''' AND AV7000.WareHouseID = ''' + @WareHouseID + ''' 
			...
			GROUP BY DivisionID, WareHouseID, InventoryID, InventoryName, UnitID, UnitName, S1, S2, S3, 
			         I01ID, I02ID, I03ID, I04ID, I05ID, InventoryTypeID, Specification, Notes02, Notes03,
			         SalePrice01, Barcode, WarehouseName, AV7000.Varchar01, AV7000.ETaxConvertedUnit,
					 --AV7000.ConvertedQuantity,
				     AV7000.CreditConvertedQuantity, AV7000.DebitConvertedQuantity'
		END
		...
		EXEC(@sSQLDrop + @sSQLSelect + @sSQLFrom +@sSQLFrom2+ @sSQLFrom1 + @sSQLWhere)
		...
		SET @sSQLSelect = N'
			SELECT AV7018.S1, AV7018.S2, AV7018.S3, AV7018.I01ID, AV7018.I02ID, AV7018.I03ID, AV7018.I04ID, AV7018.I05ID, 
			       ...
			       SUM(ISNULL(AV7018.BeginQuantity, 0)) AS BeginQuantity,
			       SUM(ISNULL(AV7018.BeginConvertedQuantity, 0)) AS BeginConvertedQuantity,
			       SUM(ISNULL(AV7018.BeginAmount, 0)) AS BeginAmount,
			       AV7018.DebitQuantity,
			       AV7018.DebitConvertedQuantity,
			       AV7018.CreditQuantity, 
			       AV7018.CreditConvertedQuantity, 
			       AV7018.EndQuantity, 
			       AV7018.EndConvertedQuantity, 
			       AV7018.DebitAmount, AV7018.CreditAmount, 
		...
		SET @sSQLFrom = N' INTO ##AV2088'+ltrim(@@SPID)+'
			               FROM ##AV7018'+ltrim(@@SPID)+' AV7018	'

		IF @IsDate <> 0
			SET @sSQLWhere = N'GROUP BY AV7018.S1, AV7018.S2, AV7018.S3, AV7018.I01ID, AV7018.I02ID, AV7018.I03ID, AV7018.I04ID, AV7018.I05ID, 
										 AV7018.Notes01, AV7018.Notes02, AV7018.Notes03,AV7018.SalePrice01, AV7018.InventoryID, AV7018.InventoryName,
										 AV7018.WareHouseID, AV7018.DivisionID, AV7018.Barcode, AV7018.WarehouseName,
										 AV7018.InventoryTypeID, AV7018.Specification, AV7018.UnitID, AV7018.UnitName,
										 AV7018.DebitQuantity,
										 AV7018.DebitConvertedQuantity,
										 AV7018.CreditQuantity, 
										 ...
										 AV7018.EndConvertedQuantity, 
										 AV7018.DebitAmount, AV7018.CreditAmount, 
										 AV7018.EndAmount, AV7018.InDebitQuantity, AV7018.InCreditQuantity, 
										 AV7018.Varchar01, AV7018.ETaxConvertedUnit	'
		ELSE
			SET @sSQLWhere = N'  WHERE AV7018.WareHouseID = '''+@WareHouseID+''' AND AV7018.DivisionID = '''+@DivisionID+''' 
		                           AND AV7018.InventoryID NOT IN ( SELECT InventoryID FROM  #TEMP AS AV7000
																	 WHERE AV7000.DivisionID ='''+@DivisionID+'''  AND AV7000.WareHouseID = '''+@WareHouseID+''' 
																	   AND AV7000.InventoryID BETWEEN '''+@FromInventoryID+''' AND '''+@ToInventoryID+'''
																	   AND AV7000.D_C IN (''D'', ''C'') 
																	   AND '+@WhereTime+'	 )
								GROUP BY AV7018.S1, AV7018.S2, AV7018.S3, AV7018.I01ID, AV7018.I02ID, AV7018.I03ID, AV7018.I04ID, AV7018.I05ID, 
										 AV7018.Notes01, AV7018.Notes02, AV7018.Notes03,AV7018.SalePrice01, AV7018.InventoryID, AV7018.InventoryName,
										 AV7018.WareHouseID, AV7018.DivisionID, AV7018.Barcode, AV7018.WarehouseName,
										 AV7018.InventoryTypeID, AV7018.Specification, AV7018.UnitID, AV7018.UnitName,
										 AV7018.DebitQuantity,
		...
		SELECT AV7000.S1, AV7000.S2, AV7000.S3, AV7000.I01ID, AV7000.I02ID, AV7000.I03ID, AV7000.I04ID, AV7000.I05ID, 
			   ...
			   AV7000.InventoryTypeID, AV7000.Specification, AV7000.UnitID, AV7000.UnitName,'
		IF @IsDate = 0
			SET @sSQLUnion = @sSQLUnion + ' ISNULL(AV7018.BeginQuantity,0) AS BeginQuantity, ISNULL(AV7018.BeginConvertedQuantity,0) AS BeginConvertedQuantity, ISNULL(AV7018.BeginAmount,0) AS BeginAmount,'
		ELSE
			SET @sSQLUnion = @sSQLUnion + ' 0 AS BeginQuantity, 0 AS BeginAmount, 0 AS BeginConvertedQuantity, '
			
		SET @sSQLUnion = @sSQLUnion + '
			SUM(CASE WHEN D_C = ''D'' THEN ISNULL(AV7000.ActualQuantity, 0) ELSE 0 END) AS DebitQuantity, 
			--SUM(CASE WHEN D_C = ''D'' THEN ISNULL(AV7000.DebitConvertedQuantity, 0) ELSE 0 END) AS DebitConvertedQuantity, 
			ISNULL(AV7018.DebitConvertedQuantity, 0) AS DebitConvertedQuantity, 
			SUM(CASE WHEN D_C = ''C'' THEN ISNULL(AV7000.ActualQuantity, 0) ELSE 0 END) AS CreditQuantity,
			--SUM(CASE WHEN D_C = ''C'' THEN ISNULL(AV7000.CreditConvertedQuantity, 0) ELSE 0 END) AS CreditConvertedQuantity,
			ISNULL(AV7018.CreditConvertedQuantity, 0) AS CreditConvertedQuantity,
			--0 AS EndQuantity,
			--0 AS EndConvertedQuantity,
			ISNULL(AV7018.EndQuantity, 0) AS EndQuantity,
			ISNULL(AV7018.EndConvertedQuantity, 0) AS EndConvertedQuantity,
		...
		SET @sSQLUnion1='
					...
					SELECT D07.TranMonth, D07.TranYear, D02.S1, D02.S2, D02.S3, D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID, 
					 ...
					 INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN (D07.DivisionID,''@@@'') AND D02.InventoryID = D07.InventoryID
					  LEFT JOIN AT1304 D04 WITH (NOLOCK) ON D04.DivisionID IN (D07.DivisionID,''@@@'') AND D04.UnitID = D02.UnitID
					 INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''', ''@@@'') AND D03.WareHouseID = D06.WareHouseID
			          LEFT JOIN AT1309 WITH (NOLOCK) ON D02.DivisionID IN (AT1309.DivisionID,''@@@'') AND AT1309.InventoryID = D02.InventoryID
					 WHERE D06.KindVoucherID IN (1,3,5,7,9,15,17) AND Isnull(D06.TableID,'''') <> ''AT0114'' ------- Phiếu nhập bù của ANGEL
					UNION ALL -- Xuất kho
					SELECT D07.TranMonth, D07.TranYear, D02.S1, D02.S2, D02.S3, D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID, 
					       D02.Notes01 , D02.Notes02, D02.Notes03,
		...
		SET @sSQLUnion1 = @sSQLUnion1 + '
			WHERE AV7000.DivisionID ='''+@DivisionID+''' AND AV7000.WareHouseID = '''+@WareHouseID+''' 
			  ...
			  AND AV7000.D_C IN (''D'', ''C'') 
			  AND '+@WhereTime+'
			GROUP BY  AV7000.S1, AV7000.S2, AV7000.S3, AV7000.I01ID, AV7000.I02ID, AV7000.I03ID, AV7000.I04ID, AV7000.I05ID, 
			          AV7000.SalePrice01, AV7000.InventoryID, AV7000.InventoryName, AV7000.InventoryTypeID, AV7000.Specification, AV7000.UnitID, AV7000.UnitName,
					  AV7018.DebitConvertedQuantity,AV7018.CreditConvertedQuantity,AV7018.EndConvertedQuantity,AV7018.EndQuantity,'

		IF @IsDate = 0
		SET @sSQLUnion1 = @sSQLUnion1 + ' AV7018.BeginQuantity, AV7018.BeginConvertedQuantity, AV7018.BeginAmount,	'
		...
		EXEC(@TableTemp1 + @TableTemp1_1+@sSQLSelect + @sSQLFrom + @sSQLWhere + @sSQLUnion+@sSQLUnion_1 + @sSQLUnion1)
		...
		SET @sSQLSelect = '
			SELECT AV2088.S1, AV2088.S2, AV2088.S3, AV2088.I01ID, AV2088.I02ID, AV2088.I03ID,
				   ...
				   AV2088.BeginQuantity,
				   SUM(AV2088.BeginConvertedQuantity) AS BeginConvertedQuantity,
				  -- SUM(AV2088.DebitQuantity) AS DebitQuantity, 
				   AV2088.DebitQuantity AS DebitQuantity, 
				   SUM(AV2088.DebitConvertedQuantity) AS DebitConvertedQuantity, 
				   --SUM(AV2088.DebitConvertedQuantity) AS DebitConvertedQuantity, 
				   AV2088.CreditQuantity AS CreditQuantity, 
				   SUM(AV2088.CreditConvertedQuantity) AS CreditConvertedQuantity, 
				   --AV2088.CreditConvertedQuantity AS CreditConvertedQuantity, 
				   (AV2088.BeginQuantity + DebitQuantity - CreditQuantity) AS EndQuantity, 
				   SUM(AV2088.BeginConvertedQuantity + DebitConvertedQuantity - CreditConvertedQuantity) AS EndConvertedQuantity,
				   --SUM(AV2088.BeginAmount) AS BeginAmount,
				   AV2088.BeginAmount AS BeginAmount,
				   --SUM(AV2088.DebitAmount) AS DebitAmount,  
				   AV2088.DebitAmount AS DebitAmount, 
				   --SUM(AV2088.CreditAmount) AS CreditAmount, 
				   AV2088.CreditAmount AS CreditAmount, 
				   --SUM(AV2088.BeginAmount + DebitAmount - CreditAmount) AS EndAmount,
				   (AV2088.BeginAmount + DebitAmount - CreditAmount) AS EndAmount,
				   0 AS InDebitAmount, 0 AS InCreditAmount, SUM(AV2088.InDebitQuantity) AS InDebitQuantity, 
				   SUM(AV2088.InCreditQuantity) AS InCreditQuantity, AV2088.WareHouseID, AV2088.DivisionID, AV2088.Barcode, AV2088.WarehouseName, AV2088.Varchar01,
				   AV2088.ETaxConvertedUnit		'
		...
		SET @sSQLWhere = ' 
			...
			GROUP BY AV2088.S1, AV2088.S2, AV2088.S3, AV2088.I01ID, AV2088.I02ID, AV2088.I03ID, AV2088.I04ID, AV2088.I05ID, 
			         ...
			         AV2088.SalePrice01, AV2088.InventoryID, AV2088.InventoryName, AV2088.BeginQuantity,AV2088.DebitQuantity,AV2088.CreditQuantity,AV2088.EndQuantity,
					 AV2088.BeginAmount, AV2088.DebitAmount, AV2088.CreditAmount,  AV2088.EndAmount,
		...
		SET @sSQLUpdate = '
			UPDATE T1 SET T1.ConversionUnitID = AT1309.ConvertUnitID, T1.ConversionFactor = AT1309.ConversionFactor, T1.Operator = AT1309.Operator
			FROM ##AV2098'+ltrim(@@SPID)+' T1
			LEFT JOIN AV1399 AT1309 WITH (NOLOCK) ON AT1309.InventoryID = T1.InventoryID AND AT1309.UnitID = T1.UnitID	'
	...
	----Theo ngay----------------------------------------------------
		...
		IF ((@IsGroupID >= 2) AND (@GroupField1 <> '') AND (@GroupField2 <> ''))
			BEGIN
				...
				LEFT JOIN	AV1310 V1 WITH (NOLOCK) ON V1.DivisionID = AV2098.DivisionID AND V1.ID = AV2098.' + @GroupField1 + ' AND V1.TypeID =''' + @GroupID1 + ''' 
				LEFT JOIN	AV1310 V2 WITH (NOLOCK) ON V2.DivisionID = AV2098.DivisionID AND V2.ID = AV2098.' + @GroupField2 + ' AND V2.TypeID =''' + @GroupID2 + '''
				'
		...
		SET @sSQLSelect = @sSQLSelect + '
			...
			AV2098.BeginConvertedQuantity,
			AV2098.DebitQuantity, 
			AV2098.DebitConvertedQuantity, 
			AV2098.CreditQuantity,
			AV2098.CreditConvertedQuantity,
			AV2098.EndQuantity,
			AV2098.EndConvertedQuantity,
			AV2098.BeginAmount, 
			...
			CASE WHEN (SELECT SUM(ActualQuantity) FROM AT2007 WITH (NOLOCK) WHERE InventoryID = AV2098.InventoryID 
				...
				ELSE AV2098.EndQuantity/ ((SELECT SUM(ActualQuantity) FROM AT2007 WITH (NOLOCK) WHERE InventoryID = AV2098.InventoryID 
				AND TranMonth + TranYear * 100 >= '+STR(@3MonthPrevious + @YearPrevious * 100)+'
		...
		SET @sSQLFrom = @sSQLFrom + @sSQLFrom1 + ' 
			...
			WHERE (BeginQuantity <> 0 OR BeginAmount <> 0 
			   ...
			   AND AV2098.InventoryID BETWEEN '''+@FromInventoryID+''' AND '''+@ToInventoryID+'''
