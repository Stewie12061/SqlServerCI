...
BEGIN
	...
	GROUP BY	V01.DivisionID, V01.AccountID, V01.ObjectID, V01.Ana04ID, V01.Ana06ID
	'	
END
ELSE
BEGIN
...
BEGIN
	...
	INTO #TEMP2		   
	FROM #TEMP1	
	LEFT JOIN AT1011 WITH (NOLOCK) ON AT1011.AnaID = #TEMP1.Ana04ID AND AT1011.AnaTypeID = ''A04''
	WHERE #TEMP1.Ana04ID BETWEEN ''' + @FromAna04ID + ''' AND ''' + @ToAna04ID + '''
...
BEGIN
	...
	INTO #TEMP2			   
	FROM #TEMP1	
	LEFT JOIN AT1011 WITH (NOLOCK) ON AT1011.AnaID = #TEMP1.Ana06ID AND AT1011.AnaTypeID = ''A06''
...
SET @sSQL4 = '
	SELECT #TEMP2.DivisionID, GroupID, GroupName, Opening,
		   ...
		   #TEMP3.FNOpening, #TEMP3.FNClosing		   	
	FROM #TEMP3 	
	LEFT JOIN #TEMP2 ON #TEMP3.DivisionID = #TEMP2.DivisionID AND (#TEMP2.PeriodDebit <> 0 OR #TEMP2.PeriodCredit <> 0) 		   		   
'
