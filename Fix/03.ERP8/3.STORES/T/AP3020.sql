...
---- Modified by Tiểu Mai on 01/03/2017: Bổ sung tên MPT mặt hàng số 2 cho Nhựa Chí Thành
---- Modified by Bảo Thy on 25/04/2017: Fix lỗi Union khi in Hóa đơn GTGT (tự in) có quy cách (AF0093)
---- Modify on 25/04/2017 by Bảo Anh: Sửa danh mục dùng chung (bỏ kết theo DivisionID)
---- Modified by Tiểu Mai on 12/05/2017: Bổ sung lấy công nợ đến thời điểm in phiếu
---- Modified by Tiểu Mai on 24/08/2017: Fix bug vì join thiếu DivisionID
---- Modified by Phương Thảo on 16/10/2017: Bổ sung đơn giá ĐH bán: SOUnitPrice 
---- Modified by Phương Thảo on 20/10/2017: Bổ sung người tạo phiếu
---- Modified by Hải Long on 25/10/2017: Sửa lỗi double do lấy công nợ
---- Modified by Bảo Anh on 03/07/2018: Bổ sung store customize cho GODREJ
---- Modified by Kim Thư on 12/09/2018: Bổ sung WITH(NOLOCK) tránh bị deadlock, bổ sung CustomizeStore cho Phúc Long
---- Modified by Kim Thư on 06/11/2018: Sửa lỗi in hóa đơn SELECT * FROM AT3020 WITH (NOLOCK) ' + @ConnID -> SELECT * FROM AT3020 ' + @ConnID WITH (NOLOCK)
---- Modified by Kim Thư on 11/12/2018: Lấy thêm thông tin phiếu xuất kho 
---- Modified by Kim Thư on 17/12/2018: Sửa lỗi tạo bảng tạm chứa ConnID
---- Modified by Kim Thư on 11/01/2019: Bổ sung cột AT9000.PaymentID và Name tương ứng
---- Modified by Kim Thư on 13/03/2019: thêm trường BankName , BankAccountNo lên view AV3014 cho Siêu Thanh ( CustomizeIndex = 16 )
---- Modified by Kim Thư on 27/03/2019: thêm trường kho xuất WareHouseID lên view AV3014
---- Modified by Văn Minh on 29/11/2019: Chỉnh sửa cách lấy dữ liệu trường Standard
---- Modified on 11/05/2020 by Huỳnh Thử: Lây thêm trường: VATConvertedAmount, VATOriginalAmount
---- Modified on 12/05/2020 by Huỳnh Thử: Lây thêm trường: I01Name -> I10Name
---- Modified by Nhựt Trường on 02/10/2020: (Sửa danh mục dùng chung)Bổ sung điều kiện DivisionID IN cho bảng AT1302.
---- Modified by Nhựt Trường on 22/02/2021: Bổ sung điều kiện DivisionID IN khi join bảng AT1304 và AT1103.
---- Modified by Nhật Thanh on 12/01/2022: Xử lí ambiguous column InvoiceNo
---- Modified by Nhật Thanh on 16/03/2022: [ANGEL] Bổ sung các cột AT9000.DiscountSaleAmountDetail, AT9000.DiscountSalesAmount, AT9000.IsProInventoryID
---- Modified by Nhật Thanh on 15/04/2022: Bổ sung cột AT9000.ExInvoiceNo
---- Modified by Xuân Nguyên on 15/08/2022: [2022/08/IS/0090] Bổ sung điều kiện DivisionID khi join bảng AT1202.
---- Modified by Nhật Quang on 06/09/2022: Bổ sung cột SignAmount From AQ4301 (số dư công nợ)
---- Modified by Nhựt Trường on 07/11/2022: [2022/11/IS/0067] Bổ sung điều kiện DivisionID khi join bảng AT1011.
...
IF @CustomizeIndex = 74 --- GODREJ
BEGIN
	EXEC AP3020_GOD @DivisionID, @VoucherID, @ConnID, @IsParticularBill, @InvoiceRow
	return
END
ELSE
IF @CustomizeIndex = 32 --- PHÚC LONG
BEGIN
	EXEC AP3020_PL @DivisionID, @VoucherID
	--return
END
ELSE
BEGIN
	...
	SET @sSQL5 = ''
	SELECT TOP 1 @TransactionTypeID =  TransactionTypeID,@ObjectID = ObjectID, @VoucherDate = VoucherDate, @Month = MONTH(VoucherDate), @Year = YEAR(VoucherDate)
	FROM AT9000
	...
	EXEC AP7403 @DivisionID,@Month,@Year,@Month,@Year,2,@VoucherDate,@VoucherDate,@CurrencyID,N'1311',N'1312',@ObjectID,@ObjectID,0,N'',N''
	...
	SELECT 0 AS TaxOrders, AT9000.Orders, AT9000.Ana01ID, AT9000.Ana02ID, AT9000.Ana03ID, AT9000.Ana04ID, AT9000.Ana05ID,
		...
		AT9000.BDescription, AT9000.TDescription, T2.CommissionDescription, AT9000.Refinfor AS AT9000_Refinfor,
	...
	SET @sSql1 ='	
		...
		'+STR(@PrintedTimes)+' PrintedTimes, AT2006A.VoucherNo as VoucherNo_AT2006A, I1.AnaName AS I01Name,I2.AnaName AS I02Name ,I3.AnaName AS I03Name, I4.AnaName AS I04Name, I5.AnaName AS I05Name,
		AV7403.OriginalOpening, AV7403.ConvertedOpening, OT2002.SalePrice AS SOUnitPrice,
		AT9000.CreateUserID, AT1405.UserName AS CreateUserName,  AT2007.UnitPrice as UnitPriceAT2007, AT2007.OriginalAmount as OriginalAmountAT2007, AT2007.ConvertedAmount as ConvertedAmountAT2007,
		AT9000.PaymentID AS PaymentID_AT9000, A05.PaymentName AS PaymentName_AT9000, AT2006.WareHouseID,AT9000.VATConvertedAmount, AT9000.VATOriginalAmount,
		AT9000.DiscountSaleAmountDetail, AT9000.DiscountSalesAmount, AT9000.IsProInventoryID, AT9000.ExInvoiceNo
		'+CASE WHEN @CustomizeIndex = 16 then ', AT1016.BankName AS BankNameAT1016, AT1016.BankAccountNo AS BankAccountNoAT1016' else '' END+',
		(select ISNULL (Sum(SignAmount), 0)  From AQ4301 where AccountID = ''131'' AND ObjectID = '''+@ObjectID+''' AND VoucherDate <= '''+CONVERT(VARCHAR(10),@VoucherDate,101)+''') as DebitClosing

		'

	SET @sSql2 = '	
	FROM AT9000  WITH (NOLOCK) 
	LEFT JOIN (SELECT DivisionID, VoucherID, BatchID, MAX(BDescription) AS CommissionDescription FROM AT9000 T1 
	...
	LEFT JOIN AT1302  WITH (NOLOCK) ON (AT1302.DivisionID IN ('''+@DivisionID+''',''@@@'') AND AT1302.InventoryID = AT9000.InventoryID)
	LEFT JOIN AT1304  WITH (NOLOCK) ON (AT1304.DivisionID IN ('''+@DivisionID+''',''@@@'') AND AT1304.UnitID = AT9000.UnitID)
	LEFT JOIN AT1202 A  WITH (NOLOCK) ON (A.ObjectID = AT9000.ObjectID) and A.DivisionID IN ('''+@DivisionID+''', ''@@@'') 
	LEFT JOIN AT1202 B  WITH (NOLOCK) ON (B.ObjectID = AT9000.VATObjectID) and B.DivisionID IN ('''+@DivisionID+''', ''@@@'')
	LEFT JOIN AT1015 O1  WITH (NOLOCK) on	 (O1.AnaID = A.O01ID AND  O1.AnaTypeID = ''O01'')
	LEFT JOIN AT1015 O2  WITH (NOLOCK) on	 (O2.AnaID = A.O02ID AND  O2.AnaTypeID = ''O02'')
	LEFT JOIN AT1015 O3  WITH (NOLOCK) on	 (O3.AnaID = A.O03ID AND  O3.AnaTypeID = ''O03'')
	LEFT JOIN AT1015 O4  WITH (NOLOCK) on	 (O4.AnaID = A.O04ID AND  O4.AnaTypeID = ''O04'')
	LEFT JOIN AT1015 O5  WITH (NOLOCK) on	 (O5.AnaID = A.O05ID AND  O5.AnaTypeID = ''O05'')
	...
	LEFT JOIN AT1011 A1  WITH (NOLOCK) on	A1.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A1.AnaID = AT9000.Ana01ID AND  A1.AnaTypeID = ''A01'')
	LEFT JOIN AT1011 A2  WITH (NOLOCK) on	A2.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A2.AnaID = AT9000.Ana02ID AND  A2.AnaTypeID = ''A02'')
	LEFT JOIN AT1011 A3  WITH (NOLOCK) on	A3.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A3.AnaID = AT9000.Ana03ID AND  A3.AnaTypeID = ''A03'')
	LEFT JOIN AT1011 A4  WITH (NOLOCK) on	A4.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A4.AnaID = AT9000.Ana04ID AND  A4.AnaTypeID = ''A04'')
	LEFT JOIN AT1011 A5  WITH (NOLOCK) on	A5.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A5.AnaID = AT9000.Ana05ID AND  A5.AnaTypeID = ''A05'')

	LEFT JOIN AT1011 A6  WITH (NOLOCK) on	A6.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A6.AnaID = AT9000.Ana06ID AND  A6.AnaTypeID = ''A06'')
	LEFT JOIN AT1011 A7  WITH (NOLOCK) on	A7.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A7.AnaID = AT9000.Ana07ID AND  A7.AnaTypeID = ''A07'')
	LEFT JOIN AT1011 A8  WITH (NOLOCK) on	A8.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A8.AnaID = AT9000.Ana08ID AND  A8.AnaTypeID = ''A08'')
	LEFT JOIN AT1011 A9  WITH (NOLOCK) on	A9.DivisionID IN ('''+@DivisionID+''',''@@@'') AND ( A9.AnaID = AT9000.Ana09ID AND  A9.AnaTypeID = ''A09'')
	...
	LEFT JOIN AT1009  WITH (NOLOCK) ON (AT9000.VATTypeID = AT1009.VATTypeID)
	LEFT JOIN AT1205  WITH (NOLOCK) ON (AT1205.PaymentID = OT2001.PaymentID)
	LEFT JOIN AT1208 AT1208_P  WITH (NOLOCK) ON ( AT1208_P.PaymentTermID = AT9000.PaymentTermID)
	LEFT JOIN AT1208 AT1208_R  WITH (NOLOCK) ON ( AT1208_R.PaymentTermID = Isnull(OT2001.PaymentTermID, A.RepaymentTermID))

	LEFT JOIN AT1103  WITH (NOLOCK) ON (AT1103.DivisionID IN ('''+@DivisionID+''',''@@@'') AND AT1103.EmployeeID = isnull(AT9000.EmployeeID,OT2001.EmployeeID))
	LEFT JOIN AT1103 T3  WITH (NOLOCK) ON (T3.DivisionID IN ('''+@DivisionID+''',''@@@'') AND T3.EmployeeID = OT2001.SalesManID)
	LEFT JOIN AT0000  WITH (NOLOCK) ON AT0000.DefDivisionID = AT9000.DivisionID
	LEFT JOIN AT1001  WITH (NOLOCK) ON A.CountryID = AT1001.CountryID
	LEFT JOIN AT1002  WITH (NOLOCK) ON A.CityID = AT1002.CityID
	LEFT JOIN AT1015  WITH (NOLOCK) ON AT1015.AnaID = A.O02ID and AT1015.AnaTypeID = ''O02'''
	SET @sSQL3 = '
	...
	LEFT JOIN AT1405  WITH (NOLOCK) ON AT9000.DivisionID = AT1405.DivisionID AND AT9000.CreateUserID = AT1405.UserID
	LEFT JOIN AT1205 A05 WITH (NOLOCK) ON A05.PaymentID = AT9000.PaymentID
	'+CASE WHEN @CustomizeIndex = 16 then 'LEFT JOIN AT1016  WITH (NOLOCK) ON AT1016.DivisionID = AT1101.DivisionID AND AT1016.BankAccountID = AT1101.BankAccountID 'else '' end 
	...
	BEGIN
		...
		SET @sSQL5 = ',
			...
			A01.StandardName AS S01Name, A02.StandardName AS S02Name, A03.StandardName AS S03Name, A04.StandardName AS S04Name, A052.StandardName AS S05Name,
		...
		SET @sSQL3 = @sSql3 + '
			...
			LEFT JOIN AT0128 A052  WITH (NOLOCK) ON A052.StandardID = O99.S05ID AND A052.StandardTypeID = ''S05''
			...
			LEFT JOIN AT0128 A010  WITH (NOLOCK) ON A010.StandardID = O99.S10ID AND A010.StandardTypeID = ''S10''
			...
			LEFT JOIN
			(
				SELECT ObjectID, DivisionID, SUM(OriginalOpening) AS OriginalOpening, SUM(ConvertedOpening) AS ConvertedOpening
				FROM AV7403
				GROUP BY ObjectID, DivisionID
			) AV7403 ON AT9000.DivisionID = AV7403.DivisionID AND AT9000.ObjectID = AV7403.ObjectID	 	 
		' + '
	...
	IF EXISTS (SELECT TOP 1 1 FROM SYSOBJECTS WHERE NAME = 'AV3014'  AND  XTYPE ='V')
		...
		EXEC ('CREATE VIEW AV3014  --Create by AP3020 
			AS '+@sSql + @sSql1 + @sSQL5 + @sSQL2 + @sSql2a + @sSQL3) --- Tao boi store AP3020
		--select * from AV3014	
	...
	BEGIN
		...
		Xtype ='V')
			Exec ('Create view AV3014' + @ConnID + ' --Create by AP3020 
				as '+@sSql+@sSql1+@sSQL5+@sSQL2+@sSQL2a+@sSQL3 + '
				Union All
				SELECT * FROM AT3020' + @ConnID +' WITH (NOLOCK) ') --- Tao boi store AP3020
		Else
