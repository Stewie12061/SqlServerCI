IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7503_TIENTIEN]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP7503_TIENTIEN]
GO
...
---- Modified on 07/03/2022 by Xuân Nguyên : [TienTien] -[2022/03/IS/0063]- Bổ sung begin end khi kiểm tra tồn tại Temp_AP7503
---- Modified by Đức Duy on 21/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.
--exec AP7503 'VG','11103','VND','2014-09-24 00:00:00','2014-09-24 00:00:00','',N'  (ISNULL(Ana01ID, '''') LIKE N''%1%'')'

CREATE PROCEDURE [dbo].[AP7503_TIENTIEN]
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Temp_AP7503]') AND type in (N'U'))
BEGIN 
DELETE  Temp_AP7503 
	SELECT * FROM Temp_AP7503
END
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Temp_AP7503]') AND type in (N'U'))
BEGIN 
CREATE table Temp_AP7503  (DivisionID NVARCHAR(50),StartOriginalAmount decimal(28,8),StartConvertedAmount decimal(28,8))
END	
...
IF(@DivisionID <> 'AA')
	...
	SET @SQlCalculator='SELECT DivisionID, SUM(CASE WHEN DebitAccountID = '''+@AccountID+''' then  OriginalAmount else - OriginalAmount End ) StartOriginalAmount,
		...
		INSERT INTO Temp_AP7503 (DivisionID,StartOriginalAmount, StartConvertedAmount) EXEC(@SQlCalculator)
	...
	--- Xac dinh so phat sinh truoc giai doan (FromDate )
		...
		SET @SQlCalculator = 'Select DivisionID, - Sum(CASE WHEN  TransactionTypeID IN (''T11'',''T16'') and CurrencyIDCN ='''+@BaseCurrencyID+''' then  ConvertedAmount
													...
													CONVERT(DATETIME,CONVERT(VARCHAR(20),VoucherDate,101), 101) < CONVERT(VARCHAR(20),'''+ LTRIM(@FromDate)+''',101) AND '+@SqlFind+'
													Group by DivisionID'
		...
		SET @SQlCalculator = 'Select DivisionID, 0,- Sum(ConvertedAmount)  
										From AT9000 WITH (NOLOCK) Where 	DivisionID IN (SELECT DivisionID FROM A00007 WITH (NOLOCK) WHERE SPID = @@SPID) and
														...
														AND '+@SqlFind+'
														Group by DivisionID'
			INSERT INTO dbo.Temp_AP7503 (DivisionID,StartOriginalAmount, StartConvertedAmount) EXEC(@SQlCalculator)
			PRINT @SQlCalculator
	...
	----- Buoc 2,  Xac dinh so phat sinh ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		SELECT 	(CASE WHEN DebitAccountID = '''+@AccountID+''' then 0  else 1 end) AS TransactionTypeID, TransactionID, BatchID, VoucherID,		
			...
			(SELECT SUM(StartOriginalAmount)  FROM Temp_AP7503 WHERE Temp_AP7503.DivisionID = AT9000.DivisionID)  AS BegOriginalAmount,
			(SELECT SUM(StartConvertedAmount)  FROM Temp_AP7503 WHERE Temp_AP7503.DivisionID = AT9000.DivisionID ) AS BegConvertedAmount,
			SenderReceiver	,
			AT9000.CreateDate,
			AT9000.Orders,
			...
			AT9000.Parameter07,AT9000.Parameter08,AT9000.Parameter09,AT9000.Parameter10,ISNULL(AT9000.IsWithhodingTax,0) IsWithhodingTax,AT9000.CreateUserID, AT1405.UserName
	'
	SET @sSQL1 = '
	FROM		AT9000  WITH (NOLOCK)
	...
	Else
		...
		SELECT 	null AS TransactionTypeID, 
				...
				'''' AS BDescription, 
				'''' AS TDescription,
				0 AS BegOriginalAmount,
				0 AS BegConvertedAmount,
				'''' AS SenderReceiver,
				null AS CreateDate,
				0 AS Orders,
