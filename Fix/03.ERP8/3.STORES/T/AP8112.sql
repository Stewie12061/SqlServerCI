IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WITH (NOLOCK) WHERE ID = OBJECT_ID(N'[DBO].[AP8112]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
...
---- Modified by Tieu Mai on 27/05/2016: Fix bug import số tiền hàng tỷ không được
...
---- Modified on 19/08/2022 by Xuân Nguyên: Điều chỉnh trường SRAddress thành NVARCHAR(250)
...
SELECT	Row,	OrderNo,		DivisionID,			Period,				VoucherTypeID,		VoucherNo,			VoucherDate,		EmployeeID,	
		...
		DebitBankAccountID, CreditAccountID,	
		--VATTypeID, 
...
INNER JOIN	AT1016 BA
		ON	BA.BankAccountID = DT.DebitBankAccountID
...
(
	...
	TableID,			TransactionTypeID,RefNo01, AVRExchangeRate
)
SELECT	APK,				Orders,
		VoucherID,			BatchID,			TransactionID,
		...
		'AT9000',			'T21',RefNo01, ExchangeRate
FROM	#Data
