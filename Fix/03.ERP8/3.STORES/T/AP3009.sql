IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WITH(NOLOCK) WHERE ID = OBJECT_ID(N'[DBO].[AP3009]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
...
---- Modified by Tiểu Mai on 30/05/2016: Bổ sung trường quy cách.
---- Modified by Tiểu Mai on 14/06/2016: Bổ sung xem bc theo nhiều kho
...
---- Modified by Bảo Thy on 13/03/2018: fix lỗi in báo cáo (http://192.168.0.204:8069/web?db=ASERP#id=3346&view_type=form&model=project.issue&action=390)
---- Modified by Bảo Thy on 03/04/2018: bổ sung InventoryID_QC
...
-- <Example>
---- EXEC AP3009 'MK',1,1,2016, 2016, '', '', '', '', 1, 1, '', ''
...
(
    ...
    @LstWareHouseID as nvarchar(1000),
...
BEGIN
	CREATE TABLE #StandardList_AP3009 (InventoryID VARCHAR(50), StandardTypeID VARCHAR(50), StandardID VARCHAR(50))
	
	INSERT INTO #StandardList_AP3009 (InventoryID, StandardTypeID, StandardID)
	...
	--PRINT @sSQL2    
		...
		IF ((@IsGroupID >= 2) AND (@GroupField1 <> '') AND (@GroupField2 <> ''))
			BEGIN
				...
				SET @sSQL2 = N'
					'+CASE WHEN ISNULL(@IsSearchStandard,0) = 1 THEN N'INTO #AP3009_Report' ELSE '' END+'
					FROM ##AV3098 AV3098
				...
				SET @sSQL3 = '
					...
					a.S01, a.S02, a.S03, a.S04, a.S05, a.S06, a.S07, a.S08, a.S09, a.S10,
					a.S11, a.S12, a.S13, a.S14, a.S15, a.S16, a.S17, a.S18, a.S19, a.S20, AT1401.GroupName
					'
			END 
		...
		BEGIN
			...
			(
				SELECT T1.*
				FROM #AP3009_Report AS T1
				INNER JOIN #StandardList_AP3009 T2 ON T1.InventoryID = T2.InventoryID
				WHERE 
				...
				SELECT  T1.*
				FROM #AP3009_Report AS T1
				WHERE NOT EXISTS (SELECT TOP 1 1 FROM #StandardList_AP3009 T2 WHERE T1.InventoryID = T2.InventoryID)
