IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_EXEDY]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_EXEDY]
GO
...
---- Modified by Văn Tài	 on 25/04/2022: [2022/04/IS/0268] - Bổ sung CusFax và ẩn các MPT tại dòng mô tả.
---- Modified by Đức Tuyên	on 18/01/2023: [2022/12/IS/0146] - Điều chỉnh dòng tổng hợp mặt hàng đặc thù (Parameter06 thay cho Ana04Name).
...
CREATE PROCEDURE [dbo].[AP0146_EXEDY]
...
'
	...
	SET @sSQLA = '
		...
		SELECT * FROM (
			SELECT	TOP 100 PERCENT Remark, CusCode, CusName, CusAddress, CusPhone, CusFax, CusEmail,CusEmailCC, CusTaxCode, Buyer, CusbankNo,CusBankName, PaymentMethod, 
	...
	SELECT TOP 1 @Parameter = Parameter06
	FROM AT9000 
	WHERE AT9000.DivisionID = @DivisionID 
		  AND AT9000.VoucherID = @VoucherID 
		  AND AT9000.TransactionTypeID IN ('T04','T25','T24','T64')
		  
	-- [EXEDY] Lấy thêm trường parameter06 vào dòng ghi chú
	IF (@CustomerName = 151 AND ISNULL(@Parameter,'') <> '')
	BEGIN
		SET @sSQL4 = '
			--SELECT * FROM #TEMP

			SELECT top 1
			0  AS Remark, CusCode AS CusCode , CusName AS CusName,CusAddress AS CusAddress, CusPhone, CusFax, CusEmail, CusEmailCC, CusTaxCode,
			Buyer AS Buyer, CusbankNo, CusBankName, PaymentMethod, Total, DiscountAmount, AfterAmount, VATAmount ,  VAT_Rate,
			'''' AS ProdID, Parameter06 AS ProdName,'''' AS ProdUnit, 0 AS ProdQuantity , 0 AS ProdPrice, NULL Amount , InheritFkey, 
			EInvoiceType, TypeOfAdjust, TDescription, Ana02ID, Ana02Name,
				DonDatHangSo, OriginalAmount, KindOfService, Extra1, VATAmount0, VATAmount10, Extra, InvoiceDate, CurrencyID, PaymentID, VoucherNo,
				 ...
				 IsLateInvoice, ConvertedQuantity, ConvertedPrice, ConvertedUnitID, ConversionFactor, UParameter01, UParameter02, UParameter03,
				 UParameter04, UParameter05, PriceListID, WOrderID, WTransactionID, MarkQuantity, TVoucherID, OldCounter, NewCounter,
				 OtherCounter, TBatchID, ContractDetailID, InvoiceCode, InvoiceSign, RefInfor, StandardPrice, StandardAmount, IsCom, 0VirtualPrice, VirtualAmount, ReTableID, '''' DParameter01, '''' DParameter02, '''' DParameter03, '''' DParameter04, '''' DParameter05, DParameter06,
				 DParameter07, DParameter08, DParameter09, DParameter10, InheritTableID, InheritVoucherID, InheritTransactionID, ETaxVoucherID,
				 ETaxID, ETaxConvertedUnit, ETaxConvertedAmount, ETaxTransactionID, AssignedSET, SETID, SETUnitID, SETTaxRate,
				 SETConvertedUnit, SETQuantity, SETOriginalAmount, SETConvertedAmount, SETConsistID, SETTransactionID, AssignedNRT, NRTTaxAmount,
				 NRTClassifyID, NRTUnitID, NRTTaxRate, NRTConvertedUnit, NRTQuantity, NRTOriginalAmount, NRTConvertedAmount, NRTConsistID, '
	SET @sSQL5 = '
			NRTTransactionID, CreditObjectName, CreditVATNo, IsPOCost, TaxBaseAmount, WTCExchangeRate, WTCOperator, IsFACost,
				 ...
				 InheritInvoicePOS, IsInheritPayPOS, InheritPayPOS, IsInvoiceSuggest, RefVoucherDate, IsDeposit, ReTransactionTypeID,
				 ImVoucherID, ImTransactionID, SourceNo, LimitDate, IsPromotionItem, ObjectName1, TaxRateID, PayMethodID, DiscountedUnitPrice, 
				 ConvertedDiscountedUnitPrice, IsReceived, '''' Ana01Name, '''' Ana03Name, Parameter06 AS Ana04Name, '''' Ana05Name, '''' Ana06Name, '''' Ana07Name, '''' Ana08Name, '''' Ana09Name, '''' Ana10Name,
				 BankAccountNo, BankName, ContactPerson, DVATOriginalAmount, DVATConvertedAmount, ConvertedUnitName, PaymentName, IsDiscount, DivisionNameE, AddressE, District, DContactPerson,
				 OriginalAfterVATAmount, ConvertedAfterVATAmount, WSourceNo, WLimitDate, I04ID, I04Name, 4 as ItemTypeID,
				 ParentInvoiceSign, ParentSerial,
				 CusBankAccountNo
				 ,TaxAgentContractNo as ConTractNo,CONVERT(NVARCHAR(10),TaxAgentContractDate,103) as ConTractDate
				 , PaymentTermName
	...
	print @sSQLA
	print @sSQLB
	EXEC (@sSQL+@sSQL1A+@sSQL1A1+@sSQL2A+@sSQL4+@sSQL5+@sSQLA+@sSQLB)
	END
