IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP9024_BBL]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP9024_BBL]
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO

-- <Summary>
---- Lấy dữ liệu Hàng tồn kho
---- Thay cho View AV7000 để dữ liệu khi lấy không bị chậm
-- <Param>
---- 
-- <Return>
---- 
-- <Reference>
---- 
-- <History>
---- Create on 21/03/2023 by Đức Duy
---- Modified on 26/04/2023 by Đình Định: THIENNAM - Lấy lên các cột AnaID khi in báo cáo nhập xuất tồn theo đối tượng AR0720.
---- Modified on 01/06/2023 by Đình Định: THIENNAM - Bổ sung VoucherDate vào bảng tạm khi in theo ngày.
---- Modified on 11/08/2023 by Đình Định: BBL - Lấy thêm cột ConvertedQuantity.
---- Modified on 24/08/2023 by Xuân Nguyên: BBL - Bổ sung cột số lô(SourceNo)
---- Modified on 29/08/2023 by Xuân Nguyên : Bổ sung cột SParameter01,SParameter02,SParameter03
---- Modified on 23/10/2023 by Thanh Sang : Lấy thêm cột MarkQuantity
-- <Example>
---- EXEC AP9024_BBL N'TN', 'ASOFTADMIN', N'CT01', N'CT01',N'AB00000', N'YM00215', N'0', N'Z0000Z001',
----			1, '', '', '', '', '', ''

CREATE PROCEDURE AP9024_BBL
( 
		@DivisionID AS NVARCHAR(50),
		@UserID AS NVARCHAR(50),
		@FromWareHouseID AS NVARCHAR(50),
		@ToWareHouseID AS NVARCHAR(50),
		@FromInventoryID AS nvarchar(50),
		@ToInventoryID AS nvarchar(50),
		@FromObjectID  AS nvarchar(50),
		@ToObjectID AS nvarchar(50),
		@IsTime AS TINYINT, -- 1 : Ky
							-- 2 : Ngay
		@FromMonth AS int,
		@FromYear AS int,
		@ToMonth AS int,
		@ToYear AS int,
		@FromDate AS Datetime,
		@ToDate AS Datetime		
) 
AS 

DECLARE @sSQL AS NVARCHAR(MAX),
		@sSQL1 AS NVARCHAR(MAX),
		@sSQL2 AS NVARCHAR(MAX),
		@sSQL3 AS NVARCHAR(MAX),
		@sSQL4 AS NVARCHAR(MAX),
		@sSQL5 AS NVARCHAR(MAX),
		@sSQL6 AS NVARCHAR(MAX),
		@sSQL7 AS NVARCHAR(MAX),
		@sWhere AS NVARCHAR(MAX),
		@sWhere1 AS NVARCHAR(MAX),
		@sWhere2 AS NVARCHAR(MAX),
		@sWhere3 AS NVARCHAR(MAX),
		@sWhere4 AS NVARCHAR(MAX),
		@sWhere5 AS NVARCHAR(MAX)

SET @sWhere = ''
SET @sWhere1 = ''
SET @sWhere2 = ''
SET @sWhere3 = ''
SET @sWhere4 = ''
SET @sWhere5 = ''

IF @FromWareHouseID <> '%' AND @FromWareHouseID <> '' AND @ToWareHouseID <> '%' AND @ToWareHouseID <> ''
BEGIN
	SET @sWhere1 = @sWhere1 + N'
		AND D16.WareHouseID >= '''+@FromWareHouseID+'''
		AND D16.WareHouseID <= '''+@ToWareHouseID+''' '
	SET @sWhere2 = @sWhere2 + N'
		AND D16.WareHouseID >= '''+@FromWareHouseID+'''
		AND D16.WareHouseID <= '''+@ToWareHouseID+''' '	
	SET @sWhere3 = @sWhere3 + N'
		AND D06.WareHouseID >= '''+@FromWareHouseID+'''
		AND D06.WareHouseID <= '''+@ToWareHouseID+''' '	
	SET @sWhere4 = @sWhere4 + N'
		AND CASE WHEN D06.KindVoucherID = 3 THEN D06.WareHouseID2 ELSE  D06.WareHouseID END >= '''+@FromWareHouseID+'''
		AND CASE WHEN D06.KindVoucherID = 3 THEN D06.WareHouseID2 ELSE  D06.WareHouseID END <= '''+@ToWareHouseID+''' '	
END
IF @FromInventoryID <> '%' AND @FromInventoryID <> '' AND @ToInventoryID <> '%' AND @ToInventoryID <> ''
BEGIN
	SET @sWhere1 = @sWhere1 + N'
		AND D17.InventoryID >= '''+@FromInventoryID+'''
		AND D17.InventoryID <= '''+@ToInventoryID+''' '
	SET @sWhere2 = @sWhere2 + N'
		AND D17.InventoryID >= '''+@FromInventoryID+'''
		AND D17.InventoryID <= '''+@ToInventoryID+''' '
	SET @sWhere3 = @sWhere3 + N'
		AND D07.InventoryID >= '''+@FromInventoryID+'''
		AND D07.InventoryID <= '''+@ToInventoryID+''' '
	SET @sWhere4 = @sWhere4 + N'
		AND D07.InventoryID >= '''+@FromInventoryID+'''
		AND D07.InventoryID <= '''+@ToInventoryID+''' '
END
IF @FromObjectID <>'%' AND @FromObjectID <> '' AND @ToObjectID <> '%' AND @ToObjectID <> ''
BEGIN
	SET @sWhere1 = @sWhere1 + N'
		AND D16.ObjectID >= '''+@FromObjectID+'''
		AND D16.ObjectID <= '''+@ToObjectID+''' '
	SET @sWhere2 = @sWhere2 + N'
		AND D16.ObjectID >= '''+@FromObjectID+'''
		AND D16.ObjectID <= '''+@ToObjectID+''' '
	SET @sWhere3 = @sWhere3 + N'
		AND D06.ObjectID >= '''+@FromObjectID+'''
		AND D06.ObjectID <= '''+@ToObjectID+''' '
	SET @sWhere4 = @sWhere4 + N'
		AND D06.ObjectID >= '''+@FromObjectID+'''
		AND D06.ObjectID <= '''+@ToObjectID+''' '
END
IF @IsTime = 1 --- Kỳ
	AND (ISNULL(@FromMonth,'') <> '' OR ISNULL(@FromYear,'') <> '' OR ISNULL(@ToMonth,'') <> '' OR ISNULL(@ToYear,'') <> '')	
BEGIN
	SET @sWhere1 = @sWhere1 + N'
		AND D17.TranYear*100+D17.TranMonth <= '+STR(@FromYear*100+@FromMonth)+' '
	SET @sWhere2 = @sWhere2 + N'
		AND D17.TranYear*100+D17.TranMonth <= '+STR(@FromYear*100+@FromMonth)+'  '
	SET @sWhere3 = @sWhere3 + N'
		AND D07.TranYear*100+D07.TranMonth >= '+STR(@FromYear*100+@FromMonth)+' 
		AND D07.TranYear*100+D07.TranMonth <= '+STR(@ToYear*100+@ToMonth)+' '
	SET @sWhere4 = @sWhere4 + N'
		AND D07.TranYear*100+D07.TranMonth >= '+STR(@FromYear*100+@FromMonth)+' 
		AND D07.TranYear*100+D07.TranMonth <= '+STR(@ToYear*100+@ToMonth)+' '
END
IF @IsTime = 2 --- Ngày
	AND (ISNULL(@FromDate,'') <> '' OR ISNULL(@ToDate,'') <> '')
BEGIN
	SET @sWhere1 = @sWhere1 + N'
		AND D16.VoucherDate <= '''+CONVERT(NVARCHAR(20), @FromDate, 101)+''' '
	SET @sWhere2 = @sWhere2 + N'
		AND D16.VoucherDate <= '''+CONVERT(NVARCHAR(20), @FromDate, 101)+'''  '
	SET @sWhere3 = @sWhere3 + N'
		AND D06.VoucherDate >= '''+CONVERT(NVARCHAR(20), @FromDate, 101)+''' 
		AND D06.VoucherDate <= '''+CONVERT(NVARCHAR(20), @ToDate, 101)+ ' 23:59:59'+''' '
	SET @sWhere4 = @sWhere4 + N'
		AND D06.VoucherDate >= '''+CONVERT(NVARCHAR(20), @FromDate, 101)+''' 
		AND D06.VoucherDate <= '''+CONVERT(NVARCHAR(20), @ToDate, 101)+ ' 23:59:59'+''' '
END

--- So du No cua tai khoan ton kho
SET @sSQL = N'
SELECT  D17.DivisionID, D17.TranMonth, D17.TranYear,
		D16.WareHouseID, D17.InventoryID,
		''BD'' AS D_C,  --- So du No
		D16.ObjectID,
		D02.InventoryName, D02.RefInventoryID ,D02.UnitID, D04.UnitName, 
		D03.WareHouseName,	
		ActualQuantity AS SignQuantity, 
		ConvertedAmount AS SignAmount,	
		D02.S1,	D02.S2, D02.S3 ,
		D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID, D02.Specification,
		D17.Notes01, D17.Notes02, D17.Notes03,
		ISNULL(D03.IsTemp,0) AS IsTemp,
		D17.Ana01ID, D17.Ana02ID, D17.Ana03ID, D17.Ana04ID, D17.Ana05ID,
		D17.Ana06ID, D17.Ana07ID, D17.Ana08ID, D17.Ana09ID, D17.Ana10ID,
		D16.VoucherDate,
		D17.ConvertedQuantity,
		D17.MarkQuantity,
		0 AS DebitConvertedQuantity,
		0 AS CreditConvertedQuantity,
		SourceNo,NULL AS SParameter01,NULL AS SParameter02,NULL AS SParameter03	'

SET @sSQL1 = N'
	FROM AT2017 D17 WITH (NOLOCK)
	INNER JOIN AT2016 D16 WITH (NOLOCK) ON D16.DivisionID = D17.DivisionID AND D16.VoucherID = D17.VoucherID
	LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.ObjectID = D16.ObjectID
	INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D02.InventoryID = D17.InventoryID
	LEFT JOIN AT1304 D04 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D04.UnitID = D02.UnitID
	INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D03.WareHouseID = D16.WareHouseID
	WHERE ISNULL(DebitAccountID,'''') <>''''
	  AND D17.DivisionID LIKE '''+@DivisionID+'''
	'

--- So du co hang ton kho
SET @sSQL2 = N'
	UNION ALL  --- Số dư có
	SELECT  D17.DivisionID , D17.TranMonth, D17.TranYear,
			D16.WareHouseID, D17.InventoryID,
			''BC'' AS D_C, 
			D16.ObjectID,
			D02.InventoryName, D02.RefInventoryID, D02.UnitID, D04.UnitName,
			D03.WareHouseName,	
			-ActualQuantity AS SignQuantity, 
			-ConvertedAmount AS SignAmount,	
			D02.S1,	D02.S2, D02.S3 ,
			D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID, D02.Specification, 
			D17.Notes01, D17.Notes02, D17.Notes03,
			ISNULL(D03.IsTemp,0) AS IsTemp,
			D17.Ana01ID, D17.Ana02ID, D17.Ana03ID, D17.Ana04ID, D17.Ana05ID,
			D17.Ana06ID, D17.Ana07ID, D17.Ana08ID, D17.Ana09ID, D17.Ana10ID,
			D16.VoucherDate,
			D17.ConvertedQuantity,
			D17.MarkQuantity,
			0 AS DebitConvertedQuantity,
			0 AS CreditConvertedQuantity,
		SourceNo,NULL AS SParameter01,NULL AS SParameter02,NULL AS SParameter03	'

SET @sSQL3 = N'	
	FROM AT2017 D17 WITH (NOLOCK) 
	INNER JOIN AT2016 D16 WITH (NOLOCK) ON D16.DivisionID = D17.DivisionID AND D16.VoucherID = D17.VoucherID
	LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.ObjectID = D16.ObjectID
	INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D02.InventoryID = D17.InventoryID
	LEFT JOIN AT1304 D04 WITH (NOLOCK) ON D04.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D04.UnitID = D02.UnitID
	INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D03.WareHouseID = D16.WareHouseID
	WHERE ISNULL(CreditAccountID,'''') <>''''
	  AND D17.DivisionID LIKE '''+@DivisionID+'''
	'

--- Nhap kho
SET @sSQL4 = N'
UNION ALL  --- Nhập kho
SELECT  D07.DivisionID, D07.TranMonth, D07.TranYear,
	    D06.WareHouseID, 
	    D07.InventoryID,
	    ''D'' AS D_C,  --- Phat sinh No
	    D06.ObjectID,
	    D02.InventoryName, D02.RefInventoryID, D02.UnitID, D04.UnitName, 
	    D03.WareHouseName,	
	    ActualQuantity AS SignQuantity, 
	    ConvertedAmount AS SignAmount,	
	    D02.S1,	D02.S2, D02.S3, 
	    D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID, D02.Specification,
	    D07.Notes01, D07.Notes02, D07.Notes03,
	    ISNULL(D03.IsTemp,0) AS IsTemp,
	    D07.Ana01ID, D07.Ana02ID, D07.Ana03ID, D07.Ana04ID, D07.Ana05ID,
	    D07.Ana06ID, D07.Ana07ID, D07.Ana08ID, D07.Ana09ID, D07.Ana10ID,
		D06.VoucherDate,
		D07.ConvertedQuantity,
		D07.MarkQuantity,
		D07.ConvertedQuantity AS DebitConvertedQuantity,
		0 AS CreditConvertedQuantity,
		SourceNo,D06.SParameter01,D06.SParameter02,D06.SParameter03	'

SET  @sSQL5 = N'    
	FROM AT2007 D07 WITH (NOLOCK)
	INNER JOIN AT2006 D06 WITH (NOLOCK) ON D06.DivisionID = D07.DivisionID AND D06.VoucherID = D07.VoucherID
	LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.ObjectID = D06.ObjectID
	INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D02.InventoryID = D07.InventoryID
	LEFT JOIN AT1304 D04 WITH (NOLOCK) ON D04.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D04.UnitID = D02.UnitID
	INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D03.WareHouseID = D06.WareHouseID
	WHERE D06.KindVoucherID IN (1,3,5,7,9,15,17) 
		AND D07.DivisionID LIKE '''+@DivisionID+'''
		AND ISNULL(D06.TableID,'''') <> ''AT0114'' ' ------- Phiếu nhập bù của ANGEL 

--- Xuat kho
SET @sSQL6 = N'
UNION ALL  -- Xuất kho
SELECT  D07.DivisionID, D07.TranMonth, D07.TranYear,
		CASE WHEN D06.KindVoucherID = 3 then D06.WareHouseID2 Else  D06.WareHouseID End AS WareHouseID, 
		D07.InventoryID, 
		''C'' AS D_C,  --- So du Co
		D06.ObjectID,
		D02.InventoryName, D02.RefInventoryID,
		D02.UnitID, D04.UnitName, 
		CASE WHEN D06.KindVoucherID = 3 then D031.WareHouseName Else  D03.WareHouseName End  AS WareHouseName,	
		-ActualQuantity AS SignQuantity, 
		-ConvertedAmount AS SignAmount,	
		D02.S1,	D02.S2, D02.S3, 
		D02.I01ID, D02.I02ID, D02.I03ID, D02.I04ID, D02.I05ID, D02.Specification,
		D07.Notes01, D07.Notes02, D07.Notes03,
		ISNULL(D03.IsTemp,0) AS IsTemp,
		D07.Ana01ID, D07.Ana02ID, D07.Ana03ID, D07.Ana04ID, D07.Ana05ID,
		D07.Ana06ID, D07.Ana07ID, D07.Ana08ID, D07.Ana09ID, D07.Ana10ID,
		D06.VoucherDate,
		D07.ConvertedQuantity,
		D07.MarkQuantity,
		0 AS DebitConvertedQuantity,
		D07.ConvertedQuantity AS CreditConvertedQuantity,
		SourceNo,D06.SParameter01,D06.SParameter02,D06.SParameter03	'

	SET @sSQL7 = N'	
	FROM AT2007  D07 WITH (NOLOCK)
	INNER JOIN AT2006 D06 WITH (NOLOCK) ON D06.DivisionID = D07.DivisionID AND D06.VoucherID = D07.VoucherID
	LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.ObjectID = D06.ObjectID 
	INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D02.InventoryID = D07.InventoryID
	LEFT JOIN AT1304 D04 WITH (NOLOCK) ON D04.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D04.UnitID = D02.UnitID 
	INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D03.WareHouseID = D06.WareHouseID
	LEFT JOIN AT1303 D031 WITH (NOLOCK) ON D031.WareHouseID = D06.WareHouseID2
	Where D06.KindVoucherID in (2,3,4,6,8,10,14,20)
		AND D07.DivisionID LIKE '''+@DivisionID+'''
	'

IF EXISTS (SELECT TOP 1 1 FROM AT0000 WITH (NOLOCK) WHERE DefDivisionID = @DivisionID AND IsSpecificate = 1)
BEGIN
	SET @sSQL1 = N', WT8899.S01ID, WT8899.S02ID, WT8899.S03ID, WT8899.S04ID, WT8899.S05ID, WT8899.S06ID, WT8899.S07ID, WT8899.S08ID, WT8899.S09ID, WT8899.S10ID,
					WT8899.S11ID, WT8899.S12ID, WT8899.S13ID, WT8899.S14ID, WT8899.S15ID, WT8899.S16ID, WT8899.S17ID, WT8899.S18ID, WT8899.S19ID, WT8899.S20ID
		FROM AT2017 D17 WITH (NOLOCK) 
	    INNER JOIN AT2016 D16 WITH (NOLOCK) ON D16.DivisionID = D17.DivisionID AND D16.VoucherID = D17.VoucherID
	    LEFT JOIN WT8899 WITH (NOLOCK) ON WT8899.DivisionID = D17.DivisionID AND WT8899.VoucherID = D17.VoucherID AND WT8899.TransactionID = D17.TransactionID
	    LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.ObjectID = D16.ObjectID
	    INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN (D17.DivisionID,''@@@'') AND D02.InventoryID = D17.InventoryID
	    INNER JOIN AT1304 D04 WITH (NOLOCK) ON D02.DivisionID IN (D04.DivisionID,''@@@'') AND D04.UnitID = D02.UnitID
	    INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN (''' + @DivisionID + ''',''@@@'') AND D03.WareHouseID = D16.WareHouseID
	    LEFT JOIN AT1310 S1 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S1.STypeID = ''I01'' AND S1.S = D02.S1
	    LEFT JOIN AT1310 S2 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S2.STypeID = ''I02'' AND S2.S = D02.S2
	    LEFT JOIN AT1310 S3 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S3.STypeID = ''I03'' AND S3.S = D02.S3
	    LEFT JOIN AT1015 I1 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I1.AnaTypeID = ''I01'' AND I1.AnaID = D02.I01ID
	    LEFT JOIN AT1015 I2 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I2.AnaTypeID = ''I02'' AND I2.AnaID = D02.I02ID
	    LEFT JOIN AT1015 I3 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I3.AnaTypeID = ''I03'' AND I3.AnaID = D02.I03ID
	    LEFT JOIN AT1015 I4 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I4.AnaTypeID = ''I04'' AND I4.AnaID = D02.I04ID
	    LEFT JOIN AT1015 I5 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I5.AnaTypeID = ''I05'' AND I5.AnaID = D02.I05ID
	    LEFT JOIN AT1304 AS D05 WITH (NOLOCK) ON D05.UnitID = D17.ConvertedUnitID
	    LEFT JOIN AT1011 A01 WITH (NOLOCK) ON A01.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A01.AnaTypeID = ''A01'' AND A01.AnaID = D17.Ana01ID
	    LEFT JOIN AT1011 A02 WITH (NOLOCK) ON A02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A02.AnaTypeID = ''A02'' AND A02.AnaID = D17.Ana02ID
	    LEFT JOIN AT1011 A03 WITH (NOLOCK) ON A03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A03.AnaTypeID = ''A03'' AND A03.AnaID = D17.Ana03ID
	    LEFT JOIN AT1011 A04 WITH (NOLOCK) ON A04.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A04.AnaTypeID = ''A04'' AND A04.AnaID = D17.Ana04ID
	    LEFT JOIN AT1011 A05 WITH (NOLOCK) ON A05.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A05.AnaTypeID = ''A05'' AND A05.AnaID = D17.Ana05ID
	    LEFT JOIN AT1011 A06 WITH (NOLOCK) ON A06.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A06.AnaTypeID = ''A06'' AND A06.AnaID = D17.Ana06ID
	    LEFT JOIN AT1011 A07 WITH (NOLOCK) ON A07.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A07.AnaTypeID = ''A07'' AND A07.AnaID = D17.Ana07ID
	    LEFT JOIN AT1011 A08 WITH (NOLOCK) ON A08.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A08.AnaTypeID = ''A08'' AND A08.AnaID = D17.Ana08ID
	    LEFT JOIN AT1011 A09 WITH (NOLOCK) ON A09.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A09.AnaTypeID = ''A09'' AND A09.AnaID = D17.Ana09ID
	    LEFT JOIN AT1011 A10 WITH (NOLOCK) ON A10.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A10.AnaTypeID = ''A10'' AND A10.AnaID = D17.Ana10ID
	    LEFT JOIN AT0114 WITH (NOLOCK) ON AT0114.Revoucherid=D17.VoucherID AND AT0114.ReTransactionID = D17.TransactionID AND D17.InventoryID = AT0114.InventoryID -- 03/01/2014 My Tuyen add new
	   WHERE ISNULL(DebitAccountID,'''') <>''''
		 AND D17.DivisionID LIKE '''+@DivisionID+''''

	SET @sSQL3 = N', WT8899.S01ID, WT8899.S02ID, WT8899.S03ID, WT8899.S04ID, WT8899.S05ID, WT8899.S06ID, WT8899.S07ID, WT8899.S08ID, WT8899.S09ID, WT8899.S10ID,
					 WT8899.S11ID, WT8899.S12ID, WT8899.S13ID, WT8899.S14ID, WT8899.S15ID, WT8899.S16ID, WT8899.S17ID, WT8899.S18ID, WT8899.S19ID, WT8899.S20ID
	FROM AT2017 D17 WITH (NOLOCK)
	INNER JOIN AT2016 D16 WITH (NOLOCK) ON D16.VoucherID = D17.VoucherID AND D16.DivisionID = D17.DivisionID
	LEFT JOIN WT8899 WITH (NOLOCK) ON WT8899.DivisionID = D17.DivisionID AND WT8899.VoucherID = D17.VoucherID AND WT8899.TransactionID = D17.TransactionID 
	LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.ObjectID = D16.ObjectID
	INNER JOIN AT1302 AS D02 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D02.InventoryID = D17.InventoryID
	INNER JOIN AT1304 AS D04 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D04.UnitID = D02.UnitID
	INNER JOIN AT1303 AS D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D03.WareHouseID = D16.WareHouseID
	LEFT JOIN AT1310 S1 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S1.STypeID = ''I01'' AND S1.S = D02.S1
	LEFT JOIN AT1310 S2 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S2.STypeID = ''I02'' AND S2.S = D02.S2
	LEFT JOIN AT1310 S3 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S3.STypeID = ''I03'' AND S3.S = D02.S3
	LEFT JOIN AT1015 I1 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I1.AnaTypeID = ''I01'' AND I1.AnaID = D02.I01ID
	LEFT JOIN AT1015 I2 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I2.AnaTypeID = ''I02'' AND I2.AnaID = D02.I02ID
	LEFT JOIN AT1015 I3 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I3.AnaTypeID = ''I03'' AND I3.AnaID = D02.I03ID
	LEFT JOIN AT1015 I4 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I4.AnaTypeID = ''I04'' AND I4.AnaID = D02.I04ID
	LEFT JOIN AT1015 I5 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I5.AnaTypeID = ''I05'' AND I5.AnaID = D02.I05ID
	LEFT JOIN AT1304 AS D05 WITH (NOLOCK) ON D05.UnitID = D17.ConvertedUnitID
	LEFT JOIN AT1011 A01 WITH (NOLOCK) ON A01.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A01.AnaTypeID = ''A01'' AND A01.AnaID = D17.Ana01ID
	LEFT JOIN AT1011 A02 WITH (NOLOCK) ON A02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A02.AnaTypeID = ''A02'' AND A02.AnaID = D17.Ana02ID
	LEFT JOIN AT1011 A03 WITH (NOLOCK) ON A03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A03.AnaTypeID = ''A03'' AND A03.AnaID = D17.Ana03ID
	LEFT JOIN AT1011 A04 WITH (NOLOCK) ON A04.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A04.AnaTypeID = ''A04'' AND A04.AnaID = D17.Ana04ID
	LEFT JOIN AT1011 A05 WITH (NOLOCK) ON A05.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A05.AnaTypeID = ''A05'' AND A05.AnaID = D17.Ana05ID
	LEFT JOIN AT1011 A06 WITH (NOLOCK) ON A06.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A06.AnaTypeID = ''A06'' AND A06.AnaID = D17.Ana06ID
	LEFT JOIN AT1011 A07 WITH (NOLOCK) ON A07.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A07.AnaTypeID = ''A07'' AND A07.AnaID = D17.Ana07ID
	LEFT JOIN AT1011 A08 WITH (NOLOCK) ON A08.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A08.AnaTypeID = ''A08'' AND A08.AnaID = D17.Ana08ID
	LEFT JOIN AT1011 A09 WITH (NOLOCK) ON A09.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A09.AnaTypeID = ''A09'' AND A09.AnaID = D17.Ana09ID
	LEFT JOIN AT1011 A10 WITH (NOLOCK) ON A10.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A10.AnaTypeID = ''A10'' AND A10.AnaID = D17.Ana10ID
	LEFT JOIN AT0114 WITH (NOLOCK) ON AT0114.Revoucherid=D17.VoucherID AND AT0114.ReTransactionID = D17.TransactionID AND D17.InventoryID = AT0114.InventoryID  -- 03/01/2014 My Tuyen add new
	WHERE ISNULL(CreditAccountID,'''') <>''''
			AND D17.DivisionID LIKE '''+@DivisionID+'''	'

	SET @sSQL5 = N', WT8899.S01ID, WT8899.S02ID, WT8899.S03ID, WT8899.S04ID, WT8899.S05ID, WT8899.S06ID, WT8899.S07ID, WT8899.S08ID, WT8899.S09ID, WT8899.S10ID,
					 WT8899.S11ID, WT8899.S12ID, WT8899.S13ID, WT8899.S14ID, WT8899.S15ID, WT8899.S16ID, WT8899.S17ID, WT8899.S18ID, WT8899.S19ID, WT8899.S20ID
	FROM AT2007 D07  WITH (NOLOCK)
	INNER JOIN AT2006 D06 WITH (NOLOCK) ON D06.VoucherID = D07.VoucherID AND D06.DivisionID = D07.DivisionID
	LEFT JOIN WT8899 WITH (NOLOCK) ON WT8899.DivisionID = D07.DivisionID AND WT8899.VoucherID = D07.VoucherID AND WT8899.TransactionID = D07.TransactionID
	LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.ObjectID = D06.ObjectID
	INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D02.InventoryID = D07.InventoryID
	INNER JOIN AT1304 D04 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D04.UnitID = D02.UnitID
	INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D03.WareHouseID = D06.WareHouseID
	LEFT JOIN AT1302 P02 WITH (NOLOCK) ON P02.InventoryID = D07.ProductID
	LEFT JOIN AT1310 S1 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S1.STypeID = ''I01'' AND S1.S = D02.S1
	LEFT JOIN AT1310 S2 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S2.STypeID = ''I02'' AND S2.S = D02.S2
	LEFT JOIN AT1310 S3 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S3.STypeID = ''I03'' AND S3.S = D02.S3
	LEFT JOIN AT1015 I1 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I1.AnaTypeID = ''I01'' AND I1.AnaID = D02.I01ID
	LEFT JOIN AT1015 I2 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I2.AnaTypeID = ''I02'' AND I2.AnaID = D02.I02ID
	LEFT JOIN AT1015 I3 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I3.AnaTypeID = ''I03'' AND I3.AnaID = D02.I03ID
	LEFT JOIN AT1015 I4 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I4.AnaTypeID = ''I04'' AND I4.AnaID = D02.I04ID
	LEFT JOIN AT1015 I5 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I5.AnaTypeID = ''I05'' AND I5.AnaID = D02.I05ID
	LEFT JOIN AT1304 AS D05 WITH (NOLOCK) ON D05.UnitID = D07.ConvertedUnitID
    LEFT JOIN AT1011 A01 WITH (NOLOCK) ON A01.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A01.AnaTypeID = ''A01'' AND A01.AnaID = D07.Ana01ID
    LEFT JOIN AT1011 A02 WITH (NOLOCK) ON A02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A02.AnaTypeID = ''A02'' AND A02.AnaID = D07.Ana02ID
    LEFT JOIN AT1011 A03 WITH (NOLOCK) ON A03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A03.AnaTypeID = ''A03'' AND A03.AnaID = D07.Ana03ID
    LEFT JOIN AT1011 A04 WITH (NOLOCK) ON A04.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A04.AnaTypeID = ''A04'' AND A04.AnaID = D07.Ana04ID
    LEFT JOIN AT1011 A05 WITH (NOLOCK) ON A05.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A05.AnaTypeID = ''A05'' AND A05.AnaID = D07.Ana05ID
    LEFT JOIN AT1011 A06 WITH (NOLOCK) ON A06.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A06.AnaTypeID = ''A06'' AND A06.AnaID = D07.Ana06ID
    LEFT JOIN AT1011 A07 WITH (NOLOCK) ON A07.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A07.AnaTypeID = ''A07'' AND A07.AnaID = D07.Ana07ID
    LEFT JOIN AT1011 A08 WITH (NOLOCK) ON A08.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A08.AnaTypeID = ''A08'' AND A08.AnaID = D07.Ana08ID
    LEFT JOIN AT1011 A09 WITH (NOLOCK) ON A09.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A09.AnaTypeID = ''A09'' AND A09.AnaID = D07.Ana09ID
    LEFT JOIN AT1011 A10 WITH (NOLOCK) ON A10.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A10.AnaTypeID = ''A10'' AND A10.AnaID = D07.Ana10ID
	LEFT JOIN AT0114 WITH (NOLOCK) ON D07.RevoucherID=AT0114.RevoucherID AND AT0114.ReTransactionID = D07.ReTransactionID AND D07.InventoryID = AT0114.InventoryID    -- 03/01/2014 My Tuyen add new
	WHERE D06.KindVoucherID in (1,3,5,7,9,15,17)
			AND D07.DivisionID LIKE '''+@DivisionID+'''		'

	SET @sSQL7 = N', WT8899.S01ID, WT8899.S02ID, WT8899.S03ID, WT8899.S04ID, WT8899.S05ID, WT8899.S06ID, WT8899.S07ID, WT8899.S08ID, WT8899.S09ID, WT8899.S10ID,
	WT8899.S11ID, WT8899.S12ID, WT8899.S13ID, WT8899.S14ID, WT8899.S15ID, WT8899.S16ID, WT8899.S17ID, WT8899.S18ID, WT8899.S19ID, WT8899.S20ID
	FROM AT2007 AS D07 WITH (NOLOCK) 
	INNER JOIN AT2006 D06 WITH (NOLOCK) ON D06.VoucherID = D07.VoucherID AND D06.DivisionID = D07.DivisionID
	LEFT JOIN WT8899 WITH (NOLOCK) ON WT8899.DivisionID = D07.DivisionID AND WT8899.VoucherID = D07.VoucherID AND WT8899.TransactionID = D07.TransactionID
	LEFT JOIN AT1202 WITH (NOLOCK) ON AT1202.ObjectID = D06.ObjectID
	INNER JOIN AT1302 D02 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D02.InventoryID = D07.InventoryID
	INNER JOIN AT1304 D04 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D04.UnitID = D02.UnitID
	INNER JOIN AT1303 D03 WITH (NOLOCK) ON D03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D03.WareHouseID = D06.WareHouseID
	LEFT JOIN AT1302 P02 WITH (NOLOCK) ON P02.InventoryID = D07.ProductID
	LEFT JOIN AT1303 D031 WITH (NOLOCK) ON D031.DivisionID IN ('''+@DivisionID+''',''@@@'') AND D031.WareHouseID = D06.WareHouseID2	
	LEFT JOIN AT1310 S1 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S1.STypeID = ''I01'' AND S1.S = D02.S1
	LEFT JOIN AT1310 S2 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S2.STypeID = ''I02'' AND S2.S = D02.S2
	LEFT JOIN AT1310 S3 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND S3.STypeID = ''I03'' AND S3.S = D02.S3
	LEFT JOIN AT1015 I1 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I1.AnaTypeID = ''I01'' AND I1.AnaID = D02.I01ID
	LEFT JOIN AT1015 I2 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I2.AnaTypeID = ''I02'' AND I2.AnaID = D02.I02ID
	LEFT JOIN AT1015 I3 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I3.AnaTypeID = ''I03'' AND I3.AnaID = D02.I03ID
	LEFT JOIN AT1015 I4 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I4.AnaTypeID = ''I04'' AND I4.AnaID = D02.I04ID
	LEFT JOIN AT1015 I5 WITH (NOLOCK) ON D02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND I5.AnaTypeID = ''I05'' AND I5.AnaID = D02.I05ID
	LEFT JOIN AT1304 AS D05 WITH (NOLOCK) ON D05.UnitID = D07.ConvertedUnitID
    LEFT JOIN AT1011 A01 WITH (NOLOCK) ON A01.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A01.AnaTypeID = ''A01'' AND A01.AnaID = D07.Ana01ID
    LEFT JOIN AT1011 A02 WITH (NOLOCK) ON A02.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A02.AnaTypeID = ''A02'' AND A02.AnaID = D07.Ana02ID
    LEFT JOIN AT1011 A03 WITH (NOLOCK) ON A03.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A03.AnaTypeID = ''A03'' AND A03.AnaID = D07.Ana03ID
    LEFT JOIN AT1011 A04 WITH (NOLOCK) ON A04.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A04.AnaTypeID = ''A04'' AND A04.AnaID = D07.Ana04ID
    LEFT JOIN AT1011 A05 WITH (NOLOCK) ON A05.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A05.AnaTypeID = ''A05'' AND A05.AnaID = D07.Ana05ID
    LEFT JOIN AT1011 A06 WITH (NOLOCK) ON A06.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A06.AnaTypeID = ''A06'' AND A06.AnaID = D07.Ana06ID
    LEFT JOIN AT1011 A07 WITH (NOLOCK) ON A07.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A07.AnaTypeID = ''A07'' AND A07.AnaID = D07.Ana07ID
    LEFT JOIN AT1011 A08 WITH (NOLOCK) ON A08.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A08.AnaTypeID = ''A08'' AND A08.AnaID = D07.Ana08ID
    LEFT JOIN AT1011 A09 WITH (NOLOCK) ON A09.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A09.AnaTypeID = ''A09'' AND A09.AnaID = D07.Ana09ID
    LEFT JOIN AT1011 A10 WITH (NOLOCK) ON A10.DivisionID IN ('''+@DivisionID+''',''@@@'') AND A10.AnaTypeID = ''A10'' AND A10.AnaID = D07.Ana10ID
	LEFT JOIN AT0114 WITH (NOLOCK) ON D07.RevoucherID=AT0114.RevoucherID AND AT0114.ReTransactionID = D07.ReTransactionID AND D07.InventoryID = AT0114.InventoryID  -- 03/01/2014 My Tuyen add new

	WHERE D06.KindVoucherID in (2,3,4,6,8,10,14,20)
			AND D07.DivisionID LIKE '''+@DivisionID+''''
END

PRiNT @sSQL
print @sSQL1
print @sWhere1
print @sSQL2
print @sSQL3
print @sWhere2
print @sSQL4
print @sSQL5
print @sWhere3
print @sSQL6
print @sSQL7
print @sWhere4

EXEC (@sSQL+@sSQL1+@sWhere1+@sSQL2+@sSQL3+@sWhere2+@sSQL4+@sSQL5+@sWhere3+@sSQL6+@sSQL7+@sWhere4)

GO
SET QUOTED_IDENTIFIER OFF
GO
SET ANSI_NULLS ON
GO