IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP1309_AG1_ALL]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP1309_AG1_ALL]
GO
...
CREATE PROCEDURE [dbo].[AP1309_AG1_ALL] 
...
LEFT JOIN (SELECT  InventoryID, SUM(ActualQuantity) AS Quantity, SUM(ConvertedAmount) AS Amount
			...
			GROUP BY  InventoryID) B
	ON B.InventoryID = AT2008.InventoryID 
...
-- Test
if @FromInventoryID ='NLPPPBL4005' 	
	begin
		print @sSQL
		select @FromInventoryID,* From AV1309
	end
--PRINT @sSQL1
--PRINT @sSQL2    
------------ Xu ly chi tiet gia 
...
--) 
--BEGIN
--    EXEC AP1308_AG_All @DivisionID, @TranMonth, @TranYear, @QuantityDecimals, @UnitCostDecimals, @ConvertedDecimals
--END
...
if @FromInventoryID ='NLPPPBL4005' 	
	begin
		print @sSQL
		select N'Chuyển kho',* From AV1309
	end
...
-------------- Update về phiếu tháo dở phần nhập kho	
	...
	INNER JOIN AV1309 AV1309 WITH (NOLOCK) ON AT0113.DivisionID = AV1309.DivisionID AND AT0113.InventoryID = AV1309.InventoryID 
				--and AT0113.DebitAccountID = AV1309.InventoryAccountID 
...
IF @FromInventoryID ='NLPPPBL4005'
	begin
		SET @sSQL = N'
		select Isnull(AT0113.ConvertedAmount,0),
	 Isnull(AT0113.OriginalAmount,0),
	 CASE WHEN ISNULL(AT2007.ActualQuantity,0) = 0 THEN 0 ELSE ROUND(Isnull(AT0113.OriginalAmount,0)/ISNULL(AT2007.ActualQuantity,0), '+str(@UnitCostDecimals)+') END,
	 CASE WHEN ISNULL(AT2007.ActualQuantity,0) = 0 THEN 0 ELSE ROUND(Isnull(AT0113.ConvertedAmount,0)/ISNULL(AT2007.ActualQuantity,0), '+str(@UnitCostDecimals)+') END
	FROM AT20072 AT2007 WITH (ROWLOCK)
	LEFT JOIN AT0113 WITH (NOLOCK) ON AT2007.DivisionID = AT0113.DivisionID AND AT2007.InheritVoucherID = AT0113.VoucherID AND AT2007.InheritTransactionID = AT0113.TransactionID
	LEFT JOIN AT0112 WITH (NOLOCK) ON AT0112.DivisionID = AT0113.DivisionID AND AT0112.VoucherID = AT0113.VoucherID
	WHERE AT0113.KindVoucherID = 1
