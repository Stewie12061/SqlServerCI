IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7411_PL]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP7411_PL]
GO
...
---------  In bang ke, duoc goi tu AP7419 (Customer PHÚC LONG).
...
---- Modified by Nhựt Trường on 03/11/2021: [2021/10/IS/0110] - Hóa đơn có VATTypeID = VGTGT9 thì lên âm trên bảng kê đầu vào, và cho các hóa đơn này nằm chung nhóm với VGTGT1.
---- Modified by Đức Duy on 21/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.

CREATE PROCEDURE [dbo].[AP7411_PL]
...
IF @CustomerName =17 --(Customized IPL : 1 HD co nhieu phieu)
	SET @strSQL = @strSQL + ' 
	SELECT	(CASE WHEN T7419.VATTypeID in (''VHDSD'',''VGTGT9'') Then ''VGTGT1'' Else T7419.VATTypeID END) AS VATTypeID , 
...
If @IsVATIn = 1  
	Set @strSQL = @strSQL +' Sum(CASE WHEN T7419.TransactionTypeID in (''T25'', ''T35'') or VATTypeID = ''VGTGT9'' then - 1 else  1 end ) AS  SignValues, '
Else
 	Set @strSQL = @strSQL + ' Sum(CASE WHEN ( T7419.TransactionTypeID in (''T24'', ''T34'') or T7419.VoucherTypeID = ''HG'' or TypeOfAdjust = 3 or VATTypeID = ''RGTGT9'')   then - 1 else  1 end ) AS  SignValues, '
...
SET @strSQL = @strSQL + ') AND (T7419.D_C like ''D'' or (T7419.D_C like ''C'' and T7419.TransactionTypeID =''T35'') or T7419.VATTypeID = ''VGTGT9'') THEN T7419.ConvertedAmount 
ELSE 
CASE WHEN ((T7419.AccountID >=''' + ISNULL( @TaxAccountID1From,'') + ''' AND T7419.AccountID <='''+ ISNULL(@TaxAccountID1To,'') +''')
'
IF (@TaxAccountID2From is Not NULL OR @TaxAccountID2From <> '')
...
SET @strSQL = @strSQL + ') AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and (T7419.TransactionTypeID =''T34'' or  T7419.VoucherTypeID = ''HG'' or TypeOfAdjust = 3  or VATTypeID = ''RGTGT9'') )) THEN T7419.ConvertedAmount 
ELSE 
CASE WHEN ((T7419.AccountID >=''' + ISNULL( @TaxAccountID1From,'') + ''' AND T7419.AccountID <='''+ ISNULL(@TaxAccountID1To,'') +''')
'
IF (@TaxAccountID2From is Not NULL OR @TaxAccountID2From <> '')
...
SET @strSQL = @strSQL + ') AND (T7419.D_C like ''D''  or (T7419.D_C like ''C'' and T7419.TransactionTypeID =''T35'') or T7419.VATTypeID = ''VGTGT9'') THEN T7419.OriginalAmount 
ELSE 
CASE WHEN ((T7419.AccountID >=''' + ISNULL( @TaxAccountID1From,'') + ''' AND T7419.AccountID <='''+ ISNULL(@TaxAccountID1To,'') +''')
'
IF (@TaxAccountID2From is Not NULL OR @TaxAccountID2From <> '')
...
SET @strSQL = @strSQL + ') AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and (T7419.TransactionTypeID =''T34'' or  T7419.VoucherTypeID = ''HG'' or TypeOfAdjust = 3 or VATTypeID = ''RGTGT9'') )) THEN T7419.OriginalAmount 
ELSE 
CASE WHEN ((T7419.AccountID >=''' + ISNULL( @TaxAccountID1From,'') + ''' AND T7419.AccountID <='''+ ISNULL(@TaxAccountID1To,'') +''')
'
IF (@TaxAccountID2From is Not NULL OR @TaxAccountID2From <> '')
...
If @IsVATIn = 1  --- Xac dinh doanh so mua vao
	SET @strSQL = @strSQL +  ') AND ((T7419.D_C like ''D'' and T7419.TransactionTypeID not in (''T25'',''T35'')) or (T7419.D_C like ''C'' and (T7419.TransactionTypeID =''T25'' or VATTypeID = ''VGTGT9''))) '
Else		---- Xac dinh doanh so ban ra
	SET @strSQL = @strSQL +  ')  AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and  (T7419.TransactionTypeID in (''T24'',''T65'') or  T7419.VoucherTypeID = ''HG'' or TypeOfAdjust = 3 or VATTypeID = ''RGTGT9'') )) '
...
If @IsVATIn = 1 ---- Xac dinh doanh so dau vao
	SET @strSQL1 = @strSQL1 +  ') AND ((T7419.D_C like ''D'' and T7419.TransactionTypeID not in (''T25'',''T35'') and T7419.CorAccountID NOT BETWEEN ''' + @TaxAccountID1From + ''' AND '''+ @TaxAccountID1To +''') 
									or (T7419.D_C like ''C'' and (T7419.TransactionTypeID =''T25'' or T7419.VoucherTypeID = ''THDC'' or T7419.VATTypeID = ''VGTGT9''))) '
Else		----- Xac dinh doanh so dau ra
	SET @strSQL1 = @strSQL1 +  ')  AND ( T7419.D_C like ''C''  and T7419.CorAccountID NOT BETWEEN ''' + @TaxAccountID1From + ''' AND '''+ @TaxAccountID1To +''' or (T7419.D_C like ''D'' and (T7419.TransactionTypeID in (''T24'',''T65'') or  T7419.VoucherTypeID = ''HG'' or TypeOfAdjust = 3 or VATTypeID = ''RGTGT9'') )) '

SET @strSQL1 = @strSQL1 + ' THEN CASE WHEN T7419.VoucherTypeID = ''THDC'' THEN - T7419.SignAmount ELSE T7419.SignAmount END WHEN T7419.VoucherTypeID = ''VG'' THEN - T7419.SignAmount  ELSE 0 END) AS SignNetAmount'
...
--PRINT @strSQL5
--PRINT @strSQL6
EXEC (@strSQL+@strSQL1+@strDeclare+@strSQL2+ @strSQL3+ @strSQL4+ @strSQL5+@strSQL6)
