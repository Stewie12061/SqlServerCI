IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_SB]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_SB]
GO
...
---- Modified by Mỹ Tuyền on 27/08/2019 :Sửa lỗi Phát hành hóa đơn
---- Modified by Mỹ Tuyền on 22/11/2019 : Điều chỉnh cách lấy DVATOriginalAmount, DVATConvertedAmount
...
---- Modified by Nhật Thanh on 15/08/2022: Chỉ lấy 1 lần dòng ghi chú. Lấy thêm thông tin master lên dòng ghi chú
---- Modified by Nhật Thanh on 15/08/2022: Lấy Paymethod theo hóa đơn, không lấy cứng = 2 cho dòng ghi chú
---- Modified by Xuân Nguyên on 27/09/2022:Bổ sung select PaymentMethod,BankAccountNo,BankName ở Remark 999 
---- Modified by Xuân Nguyên on 27/09/2022: Lấy Extra và ExchangeRate theo hóa đơn, không lấy cứng = 1 cho dòng ghi chú
---- Modified by Đức Duy on 20/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.
---- Modified by Xuân Nguyên on 27/09/2022: bổ sung nhóm thuế KKKNT
---- Modified by Xuân Nguyên on 08/09/2023: bổ sung tách chuỗi @sSQL1A1 do bị tràn chuỗi
-- <Example>
---- EXEC AP0146_SB @DivisionID = 'ANG',@UserID='ASOFTADMIN',@VoucherID='AV561ee05d-44de-4d71-8e4c-4927876f60df'

CREATE PROCEDURE [dbo].[AP0146_SB]
...
BEGIN
	...
	SET @sSQL1A=N'
	CASE WHEN AT9000.VATGroupID=''TS0'' THEN -1 WHEN AT9000.VATGroupID=''KKKNT'' THEN -2 ELSE AT1010.VATRate END AS VAT_Rate, AT9000.InventoryID AS ProdID, 
	...
	'
	SET @sSQL1A2 ='AT9000.DiscountSaleAmountDetail, A01.AnaName as Ana01Name, A03.AnaName as Ana03Name, A04.AnaName as Ana04Name, A05.AnaName as Ana05Name, A06.AnaName as Ana06Name,
	...
	CASE AT9000.VATGroupID WHEN ''T00'' THEN 1 WHEN ''T05'' THEN 2 WHEN ''T10'' THEN 3 WHEN ''TS0'' THEN 4 WHEN ''TZ0'' THEN 5 WHEN ''T08'' THEN 9 WHEN ''KKKNT'' THEN 5 ELSE NULL END AS TaxRateID,
	CASE AT9000.VATGroupID WHEN ''T00'' THEN 0 WHEN ''T05'' THEN 5 WHEN ''T10'' THEN 10 WHEN ''TS0'' THEN -1 WHEN ''TZ0'' THEN -2  WHEN ''KKKNT'' THEN -2 ELSE -4 END AS TaxRate,
	...
	IF EXISTS (SELECT TOP 1 1 FROM AT9000 WITH (NOLOCK) WHERE VoucherID = @VoucherID AND ISNULL(TDescription,'') <> '')
		SET @sSQLUnion='
			SELECT TOP 1 (SELECT TOP 1 Remark FROM #TEMP ORDER BY Remark) AS ORDERS,
			-999  AS Remark, CusCode AS CusCode ,   CusName AS CusName,CusAddress AS CusAddress, '''' AS CusPhone,  REPLACE(CusEmail,'','','';'') as CusEmail, CusTaxCode,
			Buyer AS Buyer, '''' AS CusbankNo,   PaymentMethod,  0 AS Total,  0 AS DiscountAmount,  0 AS AfterAmount,  0 AS VATAmount ,  0 AS VAT_Rate,
			...
			0 AS VATAmount10,  Extra,  VoucherID ,   BatchID,  TransactionID ,  '''' AS TableID, 0 AS TranMonth , 
			0 AS TranYear,'''' AS TransactionTypeID ,'''' AS ObjectID , '''' ASCreditObjectID , '''' ASVATNo ,'''' AS VATObjectID , 
			'''' AS VATObjectName,'''' AS VATObjectAddress, '''' AS DebitAccountID , '''' AS CreditAccountID ,  ExchangeRate, 
			...
			0 AS ExpenseOriginalAmount , 0 AS ExpenseConvertedAmount , 0 AS IsStock ,'''' AS VoucherDate , InvoiceDate AS InvoiceDate ,CurrencyID AS  CurrencyID ,'''' AS VoucherTypeID , 
			 VATGroupID,VoucherNo ,  Serial , InvoiceNo , 0 AS OrdersAT9000,'''' AS EmployeeID , 
			 SenderReceiver,SRDivisionName,SRAddress, '''' AS RefNo01 ,'''' AS RefNo02 , '''' AS VDescription, 
			...
			'''' AS CreateUserID , '''' AS LastModifyDate ,'''' AS LastModifyUserID ,0 AS OriginalAmountCN , ExchangeRateCN ,'''' AS CurrencyIDCN , 
			...
			0 AS IsEInvoice , 0 AS EInvoiceStatus , 0 AS IsAdvancePayment ,Fkey ,  0 AS IsInheritInvoicePOS , '''' AS InheritInvoicePOS , 
			0 AS IsInheritPayPOS ,  '''' AS InheritPayPOS, 0 AS IsInvoiceSuggest  , '''' AS RefVoucherDate ,0 AS IsDeposit  ,'''' AS ReTransactionTypeID , 
			'''' AS ImVoucherID ,'''' AS ImTransactionID ,'''' AS SourceNo ,'''' AS LimitDate , 0 AS IsPromotionItem  , '''' AS ObjectName1, '''' AS TaxRateID, PayMethodID ,
			...
			'''' AS Ana05Name, '''' AS Ana06Name, '''' AS Ana07Name,'''' AS Ana08Name, '''' AS Ana09Name, '''' AS Ana10Name,
			 BankAccountNo ,  BankName ,'''' AS  ContactPerson ,  0 AS DVATOriginalAmount,0 AS DVATConvertedAmount , 
		...
		'
	SET @sSQLA = '
			SELECT (SELECT TOP 1 Remark FROM #TEMP ORDER BY Remark) AS ORDERS,
			Remark , CusCode , CusName , CusAddress , CusPhone , REPLACE(CusEmail,'','','';'') as CusEmail, CusTaxCode ,
			Buyer , CusbankNo , PaymentMethod , Total , DiscountAmount , AfterAmount , VATAmount , VAT_Rate ,
			...
			DiscountedUnitPrice , ConvertedDiscountedUnitPrice , IsReceived , Ana01Name , Ana03Name , Ana04Name ,
			Ana05Name , Ana06Name , Ana07Name , Ana08Name , Ana09Name , Ana10Name ,
			BankAccountNo , BankName , ContactPerson , DVATOriginalAmount , DVATConvertedAmount , 
			ConvertedUnitName , PaymentName , IsDiscount , DivisionNameE , AddressE , District , DContactPerson ,
...
EXEC (@sSQL+@sSQL1A+@sSQL1A1+@sSQL1A2+@sSQL2A+@sSQLUnion+@sSQLA )
