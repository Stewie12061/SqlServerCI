...
---- Modified by Nhựt Trường on 23/02/2021: Tách store AP8125_PL (Khách hàng PHÚC LONG).
---- Modified by Nhựt Trường on 01/07/2021: Bổ sung điều kiện DivisionID IN khi Join bảng AT1004.
---- Modified by Nhựt Trường on 23/07/2021: Bổ sung insert đơn vị tính quy đổi và chứng từ nhập vào phiếu xuất kho + Lấy đơn giá của phiếu xuất theo chứng từ nhập.
---- Modified by Nhựt Trường on 26/07/2021: Bổ sung insert thêm LimitDate cho phiếu xuất theo chứng từ nhập.
---- Modified by Nhưt Trường on 28/07/2021: Bổ sung điều kiện lấy Max(EndQuantity) từ AT0114 khi truy vẫn dữ liệu insert vào AT2007.
---- Modified by Xuân Nguyên on 21/09/2021: 2021/09/IS/0095: Thay đổi công thức tính OriginalAmount,ConvertedAmount, OriginalAmountCN,VATOriginalAmount,VATConvertedAmount
---- Modified by Nhựt Trường on 14/10/2021: [2021/10/IS/0044] - Điều chỉnh cách insert vào AT2007 để khắc phục lỗi (1 phiếu xuất kho có 2 dòng, số chứng từ nhập trong 2 dòng đó giống nhau trong at0114).
---- Modified by Văn Tài	 on 13/01/2022: [2022/01/IS/0111] - Điều chỉnh trả về #Data.
---- Modified by Xuân Nguyên on 05/05/2022: [2022/05/IS/0017] - Điều chỉnh cách insert vào AT2007 theo cách cũ nếu MethodID khác 3.
---- Modified by Nhựt Trường on 17/08/2022: [2022/08/IS/0136] - Bổ sung thêm DivisionID khi join các bảng dùng chung.
-- <Example>
---- 
---- 
...
IF (@CustomerName = 120) /*Khách hàng Tổng hợp THỊNH PHÁT*/
	BEGIN
		EXEC AP8125_THTP @DivisionID = @DivisionID, @UserID = @UserID, @ImportTemplateID = @ImportTemplateID, @XML = @XML
	END
ELSE IF (@CustomerName = 32) /*Khách hàng PHÚC LONG*/
	BEGIN
		EXEC AP8125_PL @DivisionID = @DivisionID, @UserID = @UserID, @ImportTemplateID = @ImportTemplateID, @XML = @XML
	END
ELSE
BEGIN
	CREATE TABLE #Data
	...
	SELECT	D.DivisionID,		D.VoucherID,		D.BatchID,		D.TransactionID,	'AT9000',
			...
			ROUND(D.ConvertedAmount*A1.VATRate/100,A4.ExchangeRateDecimal), -- VATOriginalAmount
			ROUND(D.ConvertedAmount*A1.VATRate/100,ISNULL(A2.ConvertedDecimals,0)) , --	VATConvertedAmount
	...
	LEFT JOIN AT1004 A4 WITH (NOLOCK) ON  A4.DivisionID IN (D.DivisionID,'@@@') AND D.CurrencyID = A4.CurrencyID   
	...
	SELECT	D.DivisionID,		D.VoucherID,		D.BatchID,		NewID(),	'AT9000',
			...
			CASE ISNULL(D.IsMultiTax, 0) WHEN 1 THEN -- Nhiều nhóm thuế nguyên tệ
					SUM(ROUND((CASE WHEN D.InventoryID != 'Z001' THEN D.OriginalAmount  ELSE	0 END)*A1.VATRate / 100, A2.ConvertedDecimals)) - ROUND(SUM(CASE D.InventoryID WHEN 'Z001' THEN OriginalAmount ELSE 0	 END)*A1.VATRate / 100, A2.ConvertedDecimals)
				ELSE -- Một nhóm thuế nguyên tệ				
					ROUND( (SUM(CASE WHEN D.InventoryID != 'Z001' THEN D.OriginalAmount  ELSE	0 END) - SUM(CASE D.InventoryID WHEN 'Z001' THEN D.OriginalAmount ELSE 0 END))*A1.VATRate / 100, A2.ConvertedDecimals)
					 END, -- OriginalAmount
			CASE ISNULL(D.IsMultiTax, 0) WHEN 1 THEN -- Nhiều nhóm thuế qui đổi
					SUM(ROUND((CASE WHEN D.InventoryID != 'Z001' THEN D.ConvertedAmount  ELSE    0 END)*A1.VATRate / 100, A2.ConvertedDecimals)) - ROUND( SUM(CASE D.InventoryID WHEN 'Z001' THEN ConvertedAmount ELSE	 0 END)*A1.VATRate / 100, A2.ConvertedDecimals)
				ELSE -- Một nhóm thuế qui đổi
					ROUND(  (SUM(CASE WHEN D.InventoryID != 'Z001' THEN D.ConvertedAmount ELSE    0 END) - SUM(CASE D.InventoryID WHEN 'Z001' THEN D.ConvertedAmount ELSE 0 END))*A1.VATRate / 100, A2.ConvertedDecimals)
					 END, -- ConvertedAmount				
			...
			GETDATE(),	@UserID,	GETDATE(),	@UserID,
			SUM(ROUND(A1.VATRate*D.OriginalAmount/100,A2.ConvertedDecimals)),
	...
	DECLARE	@cReTransactionID AS CURSOR,
			@WHTransactionID AS NVARCHAR(50),
			@ReTransactionID AS NVARCHAR(50),
			@EndQuantity AS INT,
			@InventoryID AS NVARCHAR(50)

	SET @cReTransactionID = CURSOR FOR

		SELECT	D.WHTransactionID, B4.ReVoucherID, B4.ReTransactionID, B4.EndQuantity, D.InventoryID
	
		FROM	#Data D
		LEFT JOIN AT1101 A2 WITH (NOLOCK) ON A2.DivisionID = D.DivisionID --AND A2.BaseCurrencyID = D.CurrencyID
		LEFT JOIN AT1004 A4 WITH (NOLOCK) ON D.CurrencyID = A4.CurrencyID
		LEFT JOIN AT0114 B4 WITH (NOLOCK) ON D.SourceNo = B4.ReSourceNO and D.InventoryId = B4.InventoryId and
											 D.WarehouseEx = B4.WareHouseID AND D.DivisionID = B4.DivisionID AND 
											 D.WareHouseImVoucherNo = B4.ReVoucherNo
		LEFT JOIN AT1309 A6 WITH (NOLOCK) ON A6.InventoryID = D.InventoryID  AND A6.UnitID = D.UnitID
	
	OPEN @cReTransactionID
	FETCH NEXT FROM @cReTransactionID INTO  @WHTransactionID, @ReVoucherID, @ReTransactionID, @EndQuantity,@InventoryID
	WHILE @@FETCH_STATUS = 0
	BEGIN
	IF (SELECT Top 1 MethodID from AT1302 where DivisionID IN(DivisionID,'@@@') and InventoryID = @InventoryID)=3
	BEGIN
	IF NOT EXISTS (SELECT TOP 1 1 FROM AT2007 WHERE TransactionID = @WHTransactionID AND ReVoucherID = @ReVoucherID)
	BEGIN
		INSERT INTO AT2007
		(
			Orders,
			...
			Ana01ID,		Ana02ID,		Ana03ID,		Ana04ID,		Ana05ID,
			Ana06ID,		Ana07ID,		Ana08ID,		Ana09ID,		Ana10ID,
			ReVoucherID,	ReTransactionID,				OrderID,		ConversionFactor,
			Notes,			ConvertedUnitID,				LimitDate
		...
		SELECT	D.Orders,
				...
				D.Quantity,			B4.UnitPrice,
				D.Quantity,			B4.UnitPrice,			
				Round(B4.UnitPrice * D.Quantity,A4.ExchangeRateDecimal),		Round(B4.UnitPrice * D.Quantity,A2.ConvertedDecimals),
				D.WHDebitAccountID,	D.WHCreditAccountID,		
				D.Ana01ID,			D.Ana02ID,				D.Ana03ID,			D.Ana04ID,		D.Ana05ID,
				D.Ana06ID,			D.Ana07ID,				D.Ana08ID,			D.Ana09ID,		D.Ana10ID,
				B4.ReVoucherID,		B4.ReTransactionID,		D.VoucherNo,		ISNULL(A6.ConversionFactor,1),
				D.TDescription,		D.UnitID,				B4.LimitDate
		...
		LEFT JOIN AT0114 B4 WITH (NOLOCK) ON D.SourceNo = B4.ReSourceNO and D.InventoryId = B4.InventoryId and
											 D.WarehouseEx = B4.WareHouseID AND D.DivisionID = B4.DivisionID AND 
											 D.WareHouseImVoucherNo = B4.ReVoucherNo AND B4.EndQuantity > 0
		LEFT JOIN AT1309 A6 WITH (NOLOCK) ON A6.InventoryID = D.InventoryID  AND A6.UnitID = D.UnitID
		WHERE D.WHTransactionID = @WHTransactionID AND B4.ReVoucherID = @ReVoucherID AND ReTransactionID = @ReTransactionID
	END
	END
	ELSE 
	BEGIN
	...
	LEFT JOIN AT1004 A4 WITH (NOLOCK) ON D.CurrencyID = A4.CurrencyID 
	LEFT JOIN AT1309 A6 WITH (NOLOCK) ON A6.InventoryID = D.InventoryID  AND A6.UnitID = D.UnitID
	WHERE D.WHTransactionID = @WHTransactionID 
	END
	...
	BEGIN
		...
		--- Xử lý cho việc code AS0067 cần lấy bảng dữ liệu
		SELECT * FROM #Data
	END
END
