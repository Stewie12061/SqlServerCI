IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146]
GO
...
---- Modified by Nhựt Trường on 29/11/2021: Customize KHV - Điều chỉnh rỗng khi lấy giá trị trường Extra1 ở phiếu VCNB và phiếu Xuất Kho.
---- Modified by Nhựt Trường on 08/12/2021: Customize cho PL, Bỏ điều kiện nếu mã hàng là BK001 thì lấy TDescription là ProdName, trở về như chuẩn.
---- Modified by Nhựt Trường on 21/12/2021: Tách lại store cho khách hàng Kim Hoàn Vũ (store AP0146_KHV).
---- Modified by Nhựt Trường on 05/01/2021: Customer Tiên Tiến, Nếu Parameter01 không có dữ liệu thì ItemTypeID mặc định là NULL.
---- Modified by Nhựt Trường on 05/01/2021: [2022/01/IS/0074] - Customer Tiên Tiến, Nếu Parameter01 không có dữ liệu thì không load lên.
---- Modified by Xuân nguyên on 22/03/2022: [2022/02/IS/0155] - Sửa lại cách DVATOriginalAmount và VATConvertedAmount
---- Modified by Xuân nguyên on 07/04/2022: [2022/03/IS/0311] - Bổ sung ContractNo,ContractDate
---- Modified by Văn Tài	 on 18/04/2022: [2022/04/IS/0168] - Tách store EXEDY
---- Modified by Nhật Thanh	 on 04/05/2022: Bổ sung trường bị thiếu + lấy ghi chú lên TDescription cho trường hợp BKAV có Parameter10
---- Modified by Nhựt Trường on 04/05/2022: [2022/05/IS/0012] - Convert lại code điều chỉnh cách lấy DVATOriginalAmount và VATConvertedAmount.
---- Modified by Văn Tài 	 on 05/05/2022: Bổ sung lấy CusFax, diễn giải phương thức thanh toán.
---- Modified by Xuân Nguyên on 11/05/2022: Bổ sung loại thuế suất T08 theo TT78.
---- Modified by Nhựt Trường on 16/05/2022: [2022/05/IS/0086] - Customize Tiên Tiến, thiết lập lại định dạng ShippingDate (mm/dd/yyyy) theo yêu cầu từ BKAV.
---- Modified by Nhật Thanh on 20/05/2022: Update trường cho Fei TT78
---- Modified by Nhật Thanh on 22/06/2022: Update cột @sSQL5
---- Modified by Nhật Thanh on 04/07/2022: Update cột @sSQL3_A
---- Modified by Nhật Thanh on 19/09/2022: Update Tiên Tiến lấy Notes10 làm PTVChuyen
---- Modified by Nhật Thanh on 19/09/2022: Bổ sung order by Remark ở cuối
---- Modified by Nhật Thanh on 11/01/2023: Tách store SeaHourse
---- Modified by Đức Duy on 13/02/2023: [2023/02/IS/0052] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục kho hàng - AT1303.
---- Modified by Đức Duy on 20/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.
---- Modified by Đình Định on 10/11/2023: Tách store khách hàng An Phúc Thịnh.
---- Modified by Xuân Nguyên on 01/12/2023: [2023/11/IS/0210][NGOCDANG] C23TXK thì trả OriginalAmount
-- <Example>
---- EXEC AP0146 @DivisionID = 'ANG',@UserID='ASOFTADMIN',@VoucherID='AV561ee05d-44de-4d71-8e4c-4927876f60df'

CREATE PROCEDURE [dbo].[AP0146]
...
BEGIN
	SET @sSQL6 = ',CONVERT(varchar,AT9000.InvoiceDate,103) AS ArisingDate'
END

IF @CustomerName = 16 -- Siêu Thanh
	EXEC AP0146_ST @DivisionID,@UserID,@VoucherID
ELSE
IF @CustomerName = 57 -- Angel
	EXEC AP0146_AG @DivisionID,@UserID,@VoucherID
ELSE
IF @CustomerName = 31 -- vimec
BEGIN
	EXEC AP0146_VM @DivisionID,@UserID,@VoucherID
END
ELSE	
IF @CustomerName = 90 -- Tiến Hưng
BEGIN
	EXEC AP0146_TH @DivisionID,@UserID,@VoucherID
END
ELSE
IF @CustomerName = 83 -- Panalpina
BEGIN
	EXEC AP0146_PANAL @DivisionID,@UserID,@VoucherID
END
ELSE
IF @CustomerName = 109 -- Seabornes, SONGBINH
BEGIN
	EXEC AP0146_SB @DivisionID,@UserID,@VoucherID
END
ELSE
IF @CustomerName = 44 -- SAVI
BEGIN
	EXEC AP0146_SV @DivisionID,@UserID,@VoucherID
END
ELSE
IF @CustomerName = 46 -- huynDae
BEGIN
	EXEC AP0146_HD @DivisionID,@UserID,@VoucherID
END
ELSE
IF @CustomerName = 110 -- SONGBINH
BEGIN
	EXEC AP0146_SONGBINH @DivisionID,@UserID,@VoucherID
END ELSE
IF @CustomerName = 120 -- THTP (Tổng hợp THỊNH PHÁT)
BEGIN
	EXEC AP0146_THTP @DivisionID, @UserID, @VoucherID
END
ELSE
IF @CustomerName = 124 -- Liên BANG
	EXEC AP0146_LB @DivisionID,@UserID,@VoucherID
ELSE
IF @CustomerName = 41 -- QTC
	EXEC AP0146_QTC @DivisionID,@UserID,@VoucherID
ELSE
IF @CustomerName = 74 -- GODREJ
	EXEC AP0146_GODREJ @DivisionID,@UserID,@VoucherID
ELSE
IF @CustomerName = 29 -- TBIV-2019
	EXEC AP0146_TOHO @DivisionID,@UserID,@VoucherID
ELSE
IF @CustomerName = 52 -- KOYO
	EXEC AP0146_KOYO @DivisionID,@UserID,@VoucherID
ELSE
IF @CustomerName = 129 -- Hưng Thịnh
	EXEC AP0146_HungThinh @DivisionID,@UserID,@VoucherID
ELSE 
IF @CustomerName = 127 -- TECHNO
	EXEC AP0146_TECHNO @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 60 -- HOA SANG
	EXEC AP0146_HS @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 54 -- Hưng Phát
	EXEC AP0146_HP @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 133 -- KRUGER
	EXEC AP0146_KRUGER @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 135 -- TELLBE
	EXEC AP0146_TELLBE @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 32 -- PHÚC LONG
	EXEC AP0146_PL @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 67 -- Kim Hoàn Vũ
	EXEC AP0146_KHV @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 151 -- EXEDY
	EXEC AP0146_EXEDY @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 100 -- SeaHourse
	EXEC AP0146_SH @DivisionID, @UserID, @VoucherID
ELSE
IF @CustomerName = 27 -- Khách hàng An Phúc Thịnh.
BEGIN
	EXEC AP0146_APT @DivisionID, @UserID, @VoucherID
END
ELSE
BEGIN
	...
	BEGIN
	IF @CustomerName = 12 or @CustomerName = 13 -- Tiên Tiến
		Set @sSQL_PXK = ', A07.Notes10 as PTVChuyen'
	ELSE 
	...
	'+CASE WHEN @CustomerName = 12 /*TIÊN TIẾN*/ THEN N'CONVERT(CHAR(10), A06.VoucherDate, 101)' ELSE 'A06.VoucherDate' END +' AS ShippingDate,
	...
	'+CASE WHEN @CustomerName = 67 THEN '''''' ELSE '''1''' END +' AS Extra1,
	'+CASE WHEN @CustomerName = 67 THEN 'A03.WareHouseName +''|''+ A06.VoucherID' ELSE '''''' END +' AS ExtraKHV'+ @sSQL_PXK+' 
...
'
	...
	AT1202.ObjectName AS Buyer, ' + 
	CASE WHEN @CustomerName in (61,2) THEN 'AT1202.BankAccountNo AS CusbankNo, AT1202.BankName AS CusBankName,'
	...
	ABS(AT9000.UnitPrice) AS ProdPrice, ' + 
	CASE WHEN @CustomerName = 2 THEN 'CASE WHEN AT9000.Serial = ''EX/21E'' OR AT9000.Serial = ''C23TXK'' THEN ABS(AT9000.OriginalAmount) ELSE ABS(AT9000.ConvertedAmount) END AS Amount,'
								ELSE 'ABS(AT9000.ConvertedAmount) AS Amount,' END + '
	...
	CASE WHEN @CustomerName = 2 THEN 'CASE WHEN AT9000.Serial = ''EX/21E'' OR AT9000.Serial = ''C23TXK'' THEN ABS(AT9000.VATOriginalAmount)
	...
	CASE WHEN @CustomerName = 2 THEN 'CASE WHEN AT9000.Serial = ''EX/21E'' OR AT9000.Serial = ''C23TXK'' THEN ISNULL(AT1010.VATRate*AT9000.OriginalAmount/100,0)
	...
	IF EXISTS (SELECT TOP 1 1  FROM AT0000 WHERE DefDivisionID=@DivisionID AND EInvoicePartner='BKAV') AND EXISTS (SELECT TOP 1 1 FROM AT9000 WITH (NOLOCK) WHERE VoucherID = @VoucherID AND ISNULL(Parameter10,'') <> '')
		begin
			...
			SELECT DISTINCT
			999999  AS Remark, CusCode AS CusCode ,   CusName AS CusName,CusAddress AS CusAddress, '''' AS CusPhone, '''' AS CusFax, '''' AS CusEmail, '''' AS CusEmailCC, '''' AS CusTaxCode,
			...
			0 AS EInvoiceType,   0 AS TypeOfAdjust, CONCAT(Parameter02,'' '',Parameter10) AS TDescription, '''' AS Ana02ID , '''' AS Ana02Name,
			...
			,NULL as ConTractNo,NULL as ConTractDate
			, '''' AS PaymentTermName,
			NULL AS ImWarehouseID, NULL AS ImWarehouseName, NULL AS ExWarehouseID, NULL AS ExWarehouseName
		FROM #TEMP
	...
	SELECT TOP 1 @Parameter01 = Parameter01
	FROM AT9000 
	WHERE AT9000.DivisionID = @DivisionID 
		  AND AT9000.VoucherID = @VoucherID 
		  AND AT9000.TransactionTypeID IN ('T04','T25','T24','T64')

	IF (@CustomerName = 13 AND ISNULL(@Parameter01,'') <> '')
	BEGIN
		SET @sSQL4 = '
			UNION ALL
	...
	SET @sSQL5 = '
			...
			'''' AS BankAccountNo , '''' AS BankName ,'''' AS  ContactPerson ,  0 AS DVATOriginalAmount,0 AS DVATConvertedAmount , 
			'''' AS ConvertedUnitName, '''' AS PaymentName , 0 AS IsDiscount , '''' AS DivisionNameE , '''' AS AddressE ,'''' AS District ,'''' AS DContactPerson ,
			0 AS OriginalAfterVATAmount , 0 AS ConvertedAfterVATAmount, '''' AS WSourceNo , '''' AS WLimitDate ,'''' AS I04ID ,'''' AS I04Name,  
			CASE WHEN ISNULL(Parameter01,'''') = '''' THEN NULL ELSE 4 END AS ItemTypeID, NULL AS CusBankAccountNo,NULL AS ParentInvoiceSign, NULL AS ParentSerial,
			NULL as ConTractNo,NULL as ConTractDate
			, '''' AS PaymentTermName,NULL AS ImWarehouseID, NULL AS ImWarehouseName, NULL AS ExWarehouseID, NULL AS ExWarehouseName
		FROM #TEMP
	...
	--PRINT @sSQL4
	--PRINT @sSQL5
	EXEC (@sSQL+@sSQL1A+@sSQL1A1+@sSQL2A+@sSQLA+@sSQLB+@sSQL3+@sSQL3_A+@sSQL4+@sSQL5+'ORDER BY Remark')
	END
