...
---- Modified by Xuân Nguyên on 18/10/2023: [2023/10/IS/0172] - Bổ sung thêm điều kiện Case When để lấy đúng đối tượng thuế của trường hợp không phải transactionType = T99
...
ELSE
	...
	INSERT AT7419 (	DivisionID, VoucherID, BatchID, TransactionID, TransactionTypeID, AccountID, CorAccountID,
					...
					ObjectID, VATObjectID, VATNo , VATObjectName, IsMultiTax, VATOriginalAmount, VATConvertedAmount,
	...
	SELECT 	A90.DivisionID, A90.VoucherID, A90.BatchID, A90.TransactionID, A90.TransactionTypeID, 
			...
			THEN ObjectID
			WHEN EXISTS (SELECT TOP 1 1 FROM AT9000 A900 WITH (NOLOCK) where A900.DivisionID = A90.DivisionID AND A900.TransactionTypeID = ''T99''
								AND A900.VoucherID = A90.VoucherID AND (A900.CreditAccountID LIKE ''133%'' or A900.CreditAccountID LIKE ''333%''))
			THEN CreditObjectID
			ELSE
				VatObjectID
			END AS VATObjectID
			, A90.VATNo, A90.VATObjectName, ISNULL(A90.IsMultiTax,0) AS IsMultiTax, A90.VATOriginalAmount, A90.VATConvertedAmount,
	...
	SELECT AT35A.DivisionID, AT35A.VoucherID, '''' AS BatchID, AT36A.TransactionID, '''' AS TransactionTypeID, '''' AS AccountID, '''' AS CorAccountID, '''' AS D_C,
		...
		AT35A.ObjectID, AT35A.VATObjectID, AT02A.VATNo, AT02A.ObjectName AS VATObjectName, ISNULL(A90.IsMultiTax,0) AS IsMultiTax, AT35A.VATOriginalAmount, AT35A.VATConvertedAmount, AT02B.Address AS ObjectAddress, 
	...
	SELECT AT35A.DivisionID, AT35A.VoucherID, '''' AS BatchID, AT36A.TransactionID, '''' AS TransactionTypeID, '''' AS AccountID, '''' AS CorAccountID, '''' AS D_C,
		...
		AND AT35A.AT9000VoucherID NOT IN (SELECT DISTINCT VoucherID FROM AT9000 WITH (NOLOCK) WHERE DivisionID = '''+@DivisionID+''' 
