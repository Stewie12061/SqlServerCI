IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7419_SAVI]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP7419_SAVI]
GO
...
---- Modified by Xuân Nguyên on 10/08/2021: Bỏ lấy Ana02ID và Ana02Name
...
CREATE PROCEDURE [dbo].[AP7419_SAVI]
...
SET @SQL1='
 INSERT AT7419 (	DivisionID, VoucherID, BatchID, TransactionID, TransactionTypeID, AccountID, CorAccountID,
					...
					ObjectAddress, VATTypeID, VATGroupID, ImTaxOriginalAmount, ImTaxConvertedAmount, DueDate, InvoiceCode, InvoiceSign, InventoryID,
					--Ana02ID, Ana02Name
...
SELECT 	A90.DivisionID, A90.VoucherID, A90.BatchID, A90.TransactionID, A90.TransactionTypeID, 
		...
		CASE WHEN A90.TransactionTypeID =''T99'' and ( (A90.VATTypeID like ''V%'' AND A90.VATTypeID <> ''VGTGT9'') OR A90.VATTypeID = ''RGTGT9'' ) then A90.CreditObjectID else A90.ObjectID end AS ObjectID ,
		A90.VATObjectID, A90.VATNo ,A90.VATObjectName, 
		'''' AS ObjectAddress, A90.VATTypeID, A90.VATGroupID , 0,0, A90.DueDate, A90.InvoiceCode, A90.InvoiceSign, A90.InventoryID,
		--ISNULL(A90.Ana02ID,'''') AS Ana02ID, ISNULL(A2.AnaName,'''') AS Ana02Name
...
--LEFT JOIN AT1011 A2 WITH (NOLOCK) ON A90.Ana02ID = A2.AnaID AND A2.AnaTypeID = ''A02''
...
SELECT AT35A.DivisionID, AT35A.VoucherID, '''' AS BatchID, AT36A.TransactionID, '''' AS TransactionTypeID, '''' AS AccountID, '''' AS CorAccountID, '''' AS D_C,
	...
	AT35A.VATTypeID, AT35A.VATGroupID , 0 as ImTaxOriginalAmount, 0 as ImTaxConvertedAmount, '''' AS DueDate, AT35A.InvoiceCode, AT35A.InvoiceSign, AT36A.InventoryID,
	--ISNULL(A90.Ana02ID,'''') AS Ana02ID, ISNULL(A2.AnaName,'''') AS Ana02Name
	'+CASE WHEN @CustomerName=84 THEN ',ISNULL(A90.Ana06ID,'''') AS Ana06ID, ISNULL(A6.AnaName,'''') AS Ana06Name ,' ELSE '' END +'
	A90.TypeOfAdjust
...
EXEC AP7411 @DivisionID,			
			...
			@PeriodFrom, @PeriodTo ,
			@VATTypeID, @VATGroupIDFrom, @VATGroupIDTo ,
		    @VATObjectIDFrom, @VATObjectIDTo,
		    @VATTypeID1, @VATTypeID2,
		    @VATTypeID3, @VATTypeID4,
		    @VATTypeID5,
		    @VATTypeID6, @VATTypeID7,
		    @VoucherTypeID, @VoucherTypeIDFrom,
