IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_LQ]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_LQ]
GO
...
-- AP0146
-- <Summary>
---- Stored load dữ liệu hóa đơn phục vụ phát hành hóa đơn điện tử - Liên Quân
---- Created on 14/02/2020 Huỳnh Thử 
...
-- <Example>
---- EXEC AP0146 @DivisionID = 'BE',@UserID='ASOFTADMIN',@VoucherID='AVf0984226-d6e8-4e04-8be1-ab6cd097a9e2'

CREATE PROCEDURE [dbo].[AP0146_LQ]
...
SELECT TOP 1 @VoucherTypeID = VoucherTypeID FROM AT9000 WITH(NOLOCK) WHERE VoucherID = @VoucherID
...
BEGIN
	...
	ABS(AT9000.ConvertedPrice) AS ProdPrice, (AT9000.Quantity * ABS(AT9000.ConvertedPrice)) AS TotalAmount, ABS(AT9000.ConvertedAmount) AS Amount,
...
SET @sSQL2A='
	...
	IF ISNULL(@VoucherTypeID,'') = 'BH'
	BEGIN
	SET @sSQLA = '

			SELECT TOP 100 PERCENT 1 as Remark, CusCode, CusName, CusAddress, CusPhone, CusEmail, CusTaxCode, Buyer, CusbankNo, PaymentMethod, 
				Total, DiscountAmount, AfterAmount, VATAmount, VAT_Rate,
				VDescription as ProdName, sum(TotalAmount) as TotalAmount, Sum (Amount) as Amount, InheritFkey, EInvoiceType, TypeOfAdjust, TDescription, Ana02ID, Ana02Name,
				DonDatHangSo, sum(OriginalAmount) as OriginalAmount, KindOfService, Extra1, VATAmount0, VATAmount10, Extra, InvoiceDate, CurrencyID, PaymentID, VoucherNo,
				 VoucherID, BatchID, TableID, TranMonth, TranYear, ObjectID, CreditObjectID, VATNo,
				 VATObjectID, VATObjectName,VATCusPhone,VATCusTaxCode,VATCusbankNo, VATObjectAddress, ExchangeRate, sum(UnitPrice) as UnitPrice, sum(OriginalAmountAT9000) as OriginalAmountAT9000,
				 ...
				 RefNo01, RefNo02,VDescription, BDescription, sum(Quantity) as Quantity, Status, IsAudit, IsCost,
				 Ana01ID, Ana03ID, Ana04ID, Ana05ID, Ana06ID, Ana07ID, Ana08ID, Ana09ID, Ana10ID, PeriodID,
				 ExpenseID, MaterialTypeID, ProductID, sum(OriginalAmountCN) as OriginalAmountCN,
				 ExchangeRateCN, CurrencyIDCN, DueDays, DueDate, DiscountRate, OrderID, CreditBankAccountID, DebitBankAccountID,
				 CommissionPercent, PaymentTermID, DiscountAmountAT9000, OTransactionID, IsMultiTax, VATOriginalAmount, VATConvertedAmount,
				 ...
				 IsLateInvoice, sum(ConvertedQuantity) as ConvertedQuantity, sum(ConvertedPrice) as ConvertedPrice, ConversionFactor, UParameter01, UParameter02, UParameter03,
				 UParameter04, UParameter05, PriceListID, WOrderID, WTransactionID, sum(MarkQuantity) as MarkQuantity, TVoucherID, OldCounter, NewCounter,
				 ...
				 BankAccountNo, BankName, ContactPerson, SUM(DVATOriginalAmount) as DVATOriginalAmount, SUM(DVATConvertedAmount) as DVATConvertedAmount, PaymentName, IsDiscount, DivisionNameE, AddressE, District, DContactPerson,
				 sum(OriginalAfterVATAmount) as OriginalAfterVATAmount, sum(ConvertedAfterVATAmount) as ConvertedAfterVATAmount, WSourceNo, WLimitDate, I04ID, I04Name, 0 as ItemTypeID, ParentInvoiceSign, ParentSerial
			FROM #TEMP
		        Group by  CusCode, CusName, CusAddress, CusPhone, CusEmail, CusTaxCode, Buyer, CusbankNo, PaymentMethod, 
				Total, DiscountAmount, AfterAmount, VATAmount, VAT_Rate,
				VDescription , InheritFkey, EInvoiceType, TypeOfAdjust, TDescription, Ana02ID, Ana02Name,
				DonDatHangSo, KindOfService, Extra1, VATAmount0, VATAmount10, Extra, InvoiceDate, CurrencyID, PaymentID, VoucherNo,
				 VoucherID, BatchID, TableID, TranMonth, TranYear, ObjectID, CreditObjectID, VATNo,
				 VATObjectID, VATObjectName,VATCusPhone,VATCusTaxCode,VATCusbankNo, VATObjectAddress, ExchangeRate, 
				 ImTaxOriginalAmount, ImTaxConvertedAmount, ExpenseOriginalAmount, ExpenseConvertedAmount, IsStock, VoucherDate,
				 VoucherTypeID, VATGroupID, Serial, InvoiceNo, EmployeeID, SenderReceiver, SRDivisionName, SRAddress,
				 RefNo01, RefNo02,VDescription, BDescription, Status, IsAudit, IsCost,
				 Ana01ID, Ana03ID, Ana04ID, Ana05ID, Ana06ID, Ana07ID, Ana08ID, Ana09ID, Ana10ID, PeriodID,
				 ...
				 ReVoucherID, ReBatchID, ReTransactionID, Parameter01, Parameter02, Parameter03, Parameter04, Parameter05, Parameter06,
				 Parameter07, Parameter08, Parameter09, Parameter10, MOrderID, SOrderID, MTransactionID, STransactionID, RefVoucherNo,
				 IsLateInvoice,  ConversionFactor, UParameter01, UParameter02, UParameter03,
				 UParameter04, UParameter05, PriceListID, WOrderID, WTransactionID, TVoucherID, OldCounter, NewCounter,
				 OtherCounter, TBatchID, ContractDetailID, InvoiceCode, InvoiceSign, RefInfor, StandardPrice, StandardAmount, IsCom,
				 VirtualPrice, VirtualAmount, ReTableID, DParameter01, DParameter02, DParameter03, DParameter04, DParameter05, DParameter06,
				 DParameter07, DParameter08, DParameter09, DParameter10, InheritTableID, InheritVoucherID, InheritTransactionID, ETaxVoucherID,
				 ETaxID, ETaxConvertedUnit, ETaxConvertedAmount, ETaxTransactionID, AssignedSET, SETID, SETUnitID, SETTaxRate,
				 ...
				 ImVoucherID, ImTransactionID, SourceNo, LimitDate, IsPromotionItem, ObjectName1, TaxRateID, PayMethodID, DiscountedUnitPrice, 
				 ConvertedDiscountedUnitPrice, IsReceived, Ana01Name, Ana03Name, Ana04Name, Ana05Name, Ana06Name, Ana07Name, Ana08Name, Ana09Name, Ana10Name,
				 BankAccountNo, BankName, ContactPerson, PaymentName, IsDiscount, DivisionNameE, AddressE, District, DContactPerson,
				  WSourceNo, WLimitDate, I04ID, I04Name, ParentInvoiceSign, ParentSerial
	...
	IF EXISTS (SELECT TOP 1 1  FROM AT0000 WHERE DefDivisionID=@DivisionID AND EInvoicePartner='BKAV') AND EXISTS (SELECT TOP 1 1 FROM AT9000 WITH (NOLOCK) WHERE VoucherID = @VoucherID AND ISNULL(Parameter10,'') <> '')
		SET @sSQL3='
			...
			NULL AS DonDatHangSo, NULL AS OriginalAmount, NULL AS KindOfService, NULL AS Extra1, NULL AS VATAmount0, NULL AS VATAmount10, NULL AS Extra, NULL AS InvoiceDate, NULL AS CurrencyID, NULL AS PaymentID, NULL AS VoucherNo,
			 ...
			 NULL AS OriginalAfterVATAmount, NULL AS ConvertedAfterVATAmount, NULL AS WSourceNo, NULL AS WLimitDate, NULL AS I04ID, NULL AS I04Name, 4 as ItemTypeID,NULL AS ParentInvoiceSign,NULL AS  ParentSerial
		FROM #TEMP
