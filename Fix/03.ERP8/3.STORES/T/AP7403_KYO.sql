IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7403_KYO]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP7403_KYO]
GO
...
CREATE PROCEDURE [dbo].[AP7403_KYO]	 	
...
SELECT	D3.DivisionID, ' + @SqlGroupBy + '	
		...
		Object.Tel, Object.Fax, Object.Email, D3.Ana05ID,
        ...
        SUM (CASE WHEN (TranMonth + TranYear * 100 > ' + str(@FromYear) + '*100) AND CONVERT(DATETIME,CONVERT(varchar(10),' + LTRIM(RTRIM(@TypeDate)) + ',101),101) <= ''' + convert(nvarchar(10), @ToDate, 101) + ''' AND (IsNull(TransactiontypeID, '''') <> ''T00'') AND RPTransactionType = ''00'' THEN SignOriginalAmount ELSE 0 END) AS OriginalDebit,
        SUM (CASE WHEN (TranMonth + TranYear * 100 > ' + str(@FromYear) + '*100) AND CONVERT(DATETIME,CONVERT(varchar(10),' + LTRIM(RTRIM(@TypeDate)) + ',101),101) <= ''' + convert(nvarchar(10), @ToDate, 101) + ''' AND (IsNull(TransactiontypeID, '''') <> ''T00'') AND RPTransactionType = ''00'' THEN SignConvertedAmount ELSE 0 END) AS ConvertedDebit,
        SUM (CASE WHEN (TranMonth + TranYear * 100 > ' + str(@FromYear) + '*100) AND CONVERT(DATETIME,CONVERT(varchar(10),' + LTRIM(RTRIM(@TypeDate)) + ',101),101) <= ''' + convert(nvarchar(10), @ToDate, 101) + ''' AND (IsNull(TransactiontypeID, '''') <> ''T00'') AND RPTransactionType = ''01'' THEN -SignOriginalAmount ELSE 0 END) AS OriginalCredit,
		SUM (CASE WHEN (TranMonth + TranYear * 100 > ' + str(@FromYear) + '*100) AND CONVERT(DATETIME,CONVERT(varchar(10),' + LTRIM(RTRIM(@TypeDate)) + ',101),101) <= ''' + convert(nvarchar(10), @ToDate, 101) + ''' AND (IsNull(TransactiontypeID, '''') <> ''T00'') AND  RPTransactionType = ''01'' THEN -SignConvertedAmount ELSE 0 END) AS ConvertedCredit,'
SET @sSQL1 = '					   
...
SET @sSQL2 = ' 		
	...
	GROUP BY	D3.DivisionID, D3.ObjectID,  D3.AccountID, ObjectName,  Object.Address, Object.VATNo, Object.Tel, Object.Fax, Object.Email,
				...
				Object.S1, Object.S2,  Object.S3,  D3.Ana05ID,
...
GROUP BY  D3.DivisionID, D3.ObjectID, D3.AccountID, ObjectName,  Object.Address, Object.VATNo, Object.Tel, Object.Fax, Object.Email, D3.Ana05ID,
		AccountName, AccountNameE, D3.CurrencyID, 
		Object.S1, Object.S2,  Object.S3, 
		OS1.SName, OS2.SName, OS3.SName, 
		Object.O01ID,  Object.O02ID, Object.O03ID, Object.O04ID, Object.O05ID,  
...
Else
	...
	Set @sSQL =  ' 
		...
		0 AS ConvertedCreditYTD, Ana05ID 
	FROM AV7413
	WHERE DebitOriginalOpening - CreditOriginalOpening <> 0  
		OR DebitConvertedOpening - CreditConvertedOpening <> 0 OR OriginalDebit <> 0 
		OR ConvertedDebit <> 0 OR OriginalCredit <> 0 OR ConvertedCredit <> 0 
