...
---- Modified by Đình Định on 11/08/2023: BBL - Tách store.,
---- Modified by Thanh Sang on 04/12/2023: [2023/11/IS/0217] - Customer RUDHOLM : Thêm trường WareHouseID, WareHouseName
...
BEGIN
	...
	BEGIN
		EXEC AP3888 @DivisionID, @FromMonth, @ToMonth, @FromYear, @ToYear, @FromDate, @ToDate, @FromInventoryID, @ToInventoryID, @IsDate, @IsGroupID, @GroupID1, @GroupID2,@UserID 
	END
	ELSE IF @CustomerName = 16 -- Siêu Thanh
	BEGIN
		EXEC AP3008_ST @DivisionID, @FromMonth, @ToMonth, @FromYear, @ToYear, @FromDate, @ToDate, @FromInventoryID, @ToInventoryID, @IsDate, @IsGroupID, @GroupID1, @GroupID2, @LstWareHouseID
	END
	ELSE IF @CustomerName = 38 -- Bourbon
	BEGIN
		EXEC AP3008_BBL @DivisionID, @FromMonth, @ToMonth, @FromYear, @ToYear, @FromDate, @ToDate, @FromInventoryID, @ToInventoryID, @IsDate, @IsGroupID, @GroupID1, @GroupID2, @LstWareHouseID, @IsSearchStandard, @StandardList, @UserID
	END
	ELSE
	BEGIN
		DECLARE 
		...
		BEGIN
			...
			SET @sSQL1 = 
				...
				AT2008.DivisionID
				'
			IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
			BEGIN SET @sSQL1 =  @sSQL1 + N', AT1303.WarehouseID, AT1303.WarehouseName' END
			...
			SET @sSQL2b=N'GROUP BY AT2008.InventoryID, InventoryName, AT1302.RefInventoryID, AT1302.UnitID, AT1304.UnitName, AT1302.VATPercent, AT1302.Barcode,
			...
			IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
			BEGIN SET @sSQL2b =  @sSQL2b + N', AT1303.WarehouseID, AT1303.WarehouseName'  END
		...
		ELSE -- theo ngay
			BEGIN
				...
				SET @sSQL1 = N'
					...
					'
				IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
					BEGIN SET @sSQL1 =  @sSQL1 + N', V7.WarehouseID, V7.WarehouseName' END
				...
				SET @sSQL2e = N'GROUP BY V7.DivisionID, V7.InventoryID, V7.InventoryName, V7.RefInventoryID, V7.UnitID, V7.Barcode,
					...
					'
				IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
					BEGIN SET @sSQL2e =  @sSQL2e + N', V7.WarehouseID, V7.WarehouseName' END
		...
		*/
		...
		'
		IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
			BEGIN SET @sSQL1 =  @sSQL1 + N', WarehouseID, WarehouseName' END
		...
		IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
			BEGIN SET @sSQL2a =  @sSQL2a + N', AV7001.WarehouseID, AV7001.WarehouseName' END
		...
		IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
			BEGIN SET @sSQL2e =  @sSQL2e + N', AV7001.WarehouseID, AV7001.WarehouseName' END
		...
		*/
		...
		SET @sSQL1 = N'
			...
			'
		IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
			BEGIN SET @sSQL1 =  @sSQL1 + N', AV3088.WarehouseID, AV3088.WarehouseName' END	
		...
		SET @sSQL2 = N' INTO ##AV3098'+LTRIM(@@SPID)+'
			...
			AV3088.DivisionID '
		IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
			BEGIN SET @sSQL2 =  @sSQL2 + N', AV3088.WarehouseID, AV3088.WarehouseName' END
...
*/		    
	...
	BEGIN
		...
		IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
		BEGIN SET @sSQL1 = @sSQL1 + N', AV3098.WarehouseID, AV3098.WarehouseName' END
		...
		SET @sSQL2 = N'
			...
			'
		IF (@CustomerName = 84 OR @CustomerName = 159)  -- BASON, RUDHOLM
		BEGIN SET @sSQL2 = @sSQL2 + N', AV3098.WarehouseID, AV3098.WarehouseName' END
