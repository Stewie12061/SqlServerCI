IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7411_SAVI]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP7411_SAVI]
GO
...
---------  Customize SAVI - In bang ke, duoc goi tu AP7419
...
---- Modified by Nhựt Trường on 12/12/2023: [2023/12/IS/0088] - Loại trừ phát sinh đối ứng với tài khoản thuế 333%.
----														  - Trả ra doanh thu âm, tiền thuế âm nếu loại chứng từ thuộc 'CK%'.

CREATE PROCEDURE [dbo].[AP7411_SAVI]
...
SET @strSQL = @strSQL + ') AND T7419.D_C = ''D'' and Isnull(T7419.TransactionTypeID,'''') <>''T34'' 
							AND (T7419.AccountID <> T7419.CorAccountID) AND ISNULL(T7419.VoucherTypeID,'''') <> ''HG'' AND ISNULL(T7419.VoucherTypeID,'''') LIKE ''CK%'' THEN T7419.ConvertedAmount * (-1) ELSE 0 END END ) AS ConvertedTaxAmount,'
END
...
SET @strSQL = @strSQL + ') AND T7419.D_C = ''D''  and Isnull(T7419.TransactionTypeID,'''') <>''T34'' 
							AND (T7419.AccountID <> T7419.CorAccountID)  AND  Isnull(T7419.VoucherTypeID,'''') <> ''HG'' AND ISNULL(T7419.VoucherTypeID,'''') LIKE ''CK%'' THEN T7419.OriginalAmount * (-1) ELSE 0 END END) AS OriginalTaxAmount,'
END
...
BEGIN
	If @IsVATIn = 1  --- Xac dinh doanh so mua vao
		SET @strSQL = @strSQL +  ') AND T7419.DebitAccountID NOT LIKE N''333%'' AND ((T7419.D_C like ''D'' and T7419.TransactionTypeID not in (''T25'',''T35'')) or (T7419.D_C like ''C'' and T7419.TransactionTypeID =''T25'')) '
	Else		---- Xac dinh doanh so ban ra
		SET @strSQL = @strSQL +  ')  AND T7419.DebitAccountID NOT LIKE N''333%'' AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and  (T7419.TransactionTypeID in (''T24'',''T65'') or  T7419.VoucherTypeID = ''HG'' or TypeOfAdjust = 3) )) '
	
	SET @strSQL = @strSQL + ' THEN CASE WHEN ISNULL(T7419.VoucherTypeID,'''') LIKE N''CK%'' THEN (-1) * (T7419.ConvertedAmount + T7419.ImTaxConvertedAmount) ELSE T7419.ConvertedAmount + T7419.ImTaxConvertedAmount END ELSE 0 END) AS ConvertedNetAmount,'
END
...
Else		---- Xac dinh doanh so ban ra
	...
	SET @strSQL = @strSQL +  ')  AND T7419.DebitAccountID NOT LIKE N''333%'' AND ( T7419.D_C like ''C'' or (T7419.D_C like ''D'' and  (T7419.TransactionTypeID in (''T24'',''T65'') or  T7419.VoucherTypeID = ''HG'') )) '


SET @strSQL = @strSQL + ' THEN CASE WHEN ISNULL(T7419.VoucherTypeID,'''') LIKE N''CK%'' THEN (-1) * (T7419.OriginalAmount + T7419.ImTaxOriginalAmount) ELSE T7419.OriginalAmount + T7419.ImTaxOriginalAmount END ELSE 0 END) AS OriginalNetAmount,'
