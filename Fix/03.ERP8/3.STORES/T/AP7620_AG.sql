IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7620_AG]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP7620_AG]
GO
...
---- Modified on 06/07/2016 by Tiểu Mai: Bổ sung in báo cáo đặc thù cho An PHÁT (mẫu AR7620_AP)
-- <Example>
---- 
-- <Summary>
CREATE PROCEDURE [dbo].[AP7620_AG]   
...
IF EXISTS (SELECT TOP 1 1 FROM AT0000 WITH (NOLOCK) WHERE DefDivisionID = @DivisionID AND IsSpecificate = 1) AND @ReportID = 'AR7620_AP'
	EXEC AP7620_AP @DivisionID, @ReportCode, @FromMonth, @FromYear, @ToMonth, @ToYear, @FromValueID, @ToValueID, @StrDivisionID, @UserID
ELSE
BEGIN
...
select @FromID =orderby from AV6666 where SelectionType = @FieldID and DivisionID = @DivisionID and SelectionID = @FromValueID 
select @ToID =orderby from AV6666 where SelectionType = @FieldID and DivisionID = @DivisionID and SelectionID = @ToValueID   
