IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP7620_KY]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP7620_KY]
GO
...
----------------- Purpose: In bao cao bang ket qua kinh doanh. theo ma phan tich  - KOYO (52)
---- Modified by Kim Thư on 17/04/2019: Sửa cách tạo view AV9091

-- <Example>
---- 
-- <Summary>
CREATE PROCEDURE [dbo].[AP7620_KY]   
			...
			PRINT 'Begin '+@LineID
			 ...
			 Begin  
				...
				SET @sSQL ='  SELECT	Ana01ID, Ana02ID, Ana03ID, ObjectID, PeriodID, TranMonth, TranYear,   
									DivisionID, AccountID, CorAccountID, D_C, SignAmount,' + @strSelect + '   
