IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WITH (NOLOCK) WHERE ID = OBJECT_ID(N'[DBO].[AP8113]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP8113]
GO
...
-- <Summary>
---- Xử lý Import dữ liệu phiếu chi qua ngân hàng
-- <Param>
---- 
-- <Return>
---- 
...
---- Modified on 26/05/2016 by Tieu Mai: Fix bug khong import duoc so tien quy doi
...
---- Modified by Kim Thư on 27/05/2019: Bổ sung cột Mã tham chiếu 1(RefNo01)
---- Modified by Kim Thư on 05/06/2019: Cho update TK Nợ và Loại tiền trước khi update làm tròn
---- Modified on 14/06/2019 by Kim Thư :Điều chỉnh các cột người dùng để trống sẽ insert là null thay vì giá trị rỗng
---- Modified on 11/06/2020 by Huỳnh Thử :Insert thêm trường RefNo01, InvoiceCode, InvoiceSign
---- Modified on 22/08/2022 by Thành Sang: Tăng ký tự import cho diễn giải (từ 250 ký tự lên 500 ký tự)
---- Modified on 28/11/2023 by Kiều Nga: Bổ sung trường PeriodID,ProductID.

-- <Example>
---- 
CREATE PROCEDURE [DBO].[AP8113]
( 
...
SELECT	X.Data.query('Row').value('.', 'INT') AS Row,
		...
		X.Data.query('RefNo01').value('.', 'NVARCHAR(250)') AS RefNo01,
		...
		X.Data.query('VDescription').value('.', 'NVARCHAR(500)') AS VDescription,
		X.Data.query('BDescription').value('.', 'NVARCHAR(500)') AS BDescription,
		...
		X.Data.query('CreditBankAccountID').value('.', 'NVARCHAR(50)') AS CreditBankAccountID,
		...
		X.Data.query('TDescription').value('.', 'NVARCHAR(500)') AS TDescription,
		...
		X.Data.query('InvoiceSign').value('.', 'NVARCHAR(250)') AS InvoiceSign,
...
INSERT INTO #Data (
		...
		CreditBankAccountID, DebitAccountID,	VATTypeID, 			VATGroupID, 		ObjectID,			
		TDescription,		Ana01ID, 			Ana02ID, 			Ana03ID, 			Ana04ID,			Ana05ID,Ana06ID,Ana07ID,Ana08ID,Ana09ID,Ana10ID,
		RefNo01, InvoiceCode, InvoiceSign,PeriodID,ProductID)		
SELECT Row,				DivisionID,			Period,				VoucherTypeID,		VoucherNo,			VoucherDate,		EmployeeID,			
		...
		--ConvertedAmount, 	
		CreditBankAccountID, DebitAccountID,	
		--VATTypeID, 
...
EXEC AP8100 @UserID = @UserID, @ImportTemplateID = @ImportTemplateID, @CheckCode = 'CheckIdenticalValues', @ColID = 'VoucherNo', @Param1 = 'VoucherNo,VoucherTypeID,VoucherDate,EmployeeID,RefNo01,SenderReceiver,SRDivisionName,SRAddress,CreditBankAccountID,ExchangeRate,VDescription'

-- Cập nhật tài khoản nợ và loại tiền theo tài khoản ngân hàng
UPDATE		DT
SET			CurrencyID = BA.CurrencyID,
			CreditAccountID = BA.AccountID  
FROM		#Data DT
INNER JOIN	AT1016 BA WITH (NOLOCK)
		ON	BA.BankAccountID = DT.CreditBankAccountID
...
FROM #Data INNER JOIN  AT1004 ON #Data.CurrencyID = AT1004.CurrencyID
...
(
	...
	DebitAccountID,		CreditAccountID,	CreditBankAccountID,
	VATTypeID,			VATGroupID,
	ObjectID,			TDescription,
	Ana01ID,			Ana02ID,			Ana03ID,			Ana04ID,			Ana05ID,Ana06ID,Ana07ID,Ana08ID,Ana09ID,Ana10ID,
	CreateDate,			CreateUserID,		LastModifyDate,		LastModifyUserID,
	TableID,			TransactionTypeID,	RefNo01, InvoiceCode,InvoiceSign,PeriodID,ProductID
)
SELECT	APK,				Orders,
		VoucherID,			BatchID,			TransactionID,
		...
		'AT9000',			'T22',				RefNo01, InvoiceCode,InvoiceSign,PeriodID,ProductID
FROM	#Data
...
BEGIN	
	...
	BEGIN
		...
		BEGIN
			...
			SET @PaymentExR = COALESCE(@PaymentExR, (SELECT ExchangeRate FROM AT1004 WITH (NOLOCK) WHERE CurrencyID = @CurrencyID AND DivisionID IN (@DivisionID,'@@@')), 0)
		...
		SET @AVRExR = COALESCE(@AVRExR, (SELECT ExchangeRate FROM AT1004 WITH (NOLOCK) WHERE CurrencyID = @CurrencyID AND DivisionID IN (@DivisionID,'@@@')), 0)
		...
		BEGIN
			...
			SELECT	#Data.APK,			#Data.Orders,
					...
					ROUND(
						...
						,CUR.ConvertedDecimals),
					#Data.DebitAccountID,		@ProfitAccountID,	CreditBankAccountID,					
					#Data.ObjectID,			#Data.TDescription,
					#Data.Ana01ID,			#Data.Ana02ID,			#Data.Ana03ID,			#Data.Ana04ID,	#Data.Ana05ID,
					#Data.Ana06ID,#Data.Ana07ID,#Data.Ana08ID,#Data.Ana09ID,#Data.Ana10ID,
					GETDATE(),			@UserID,			GETDATE(),			@UserID,
					'AT9000',			'T10'	,
			...
			SELECT	#Data.APK,			#Data.Orders,
					...
					ROUND(
							CASE WHEN @Operator = 0 THEN #Data.OriginalAmount * ABS(@AVRExR-@PaymentExR) 
								ELSE  ABS(#Data.OriginalAmount/@AVRExR-#Data.OriginalAmount/@PaymentExR) END
						,CUR.ConvertedDecimals),				
					@LostAccountID,		#Data.CreditAccountID,	CreditBankAccountID,					
					#Data.ObjectID,			#Data.TDescription,
					#Data.Ana01ID,			#Data.Ana02ID,			#Data.Ana03ID,			#Data.Ana04ID,	#Data.Ana05ID,
					#Data.Ana06ID,#Data.Ana07ID,#Data.Ana08ID,#Data.Ana09ID,#Data.Ana10ID,
					GETDATE(),			@UserID,			GETDATE(),			@UserID,
