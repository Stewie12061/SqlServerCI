IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP1309_AG]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP1309_AG]
GO
...
--------- Update by Huỳnh Thử on 21/02/2020: Update giá tiền Lắp ráp khi bị trên lệch
...
CREATE PROCEDURE [dbo].[AP1309_AG] 
...
INSERT INTO AT20082 (DivisionID,WareHouseID,TranMonth,TranYear,InventoryID,InventoryAccountID,BeginQuantity,EndQuantity,DebitQuantity,CreditQuantity,
					InDebitQuantity,InCreditQuantity,BeginAmount,EndAmount,DebitAmount,CreditAmount,InDebitAmount,InCreditAmount,UnitPrice)
...
BEGIN	
	...
	WHERE AT2007.DebitAccountID BETWEEN @FromAccountID AND @ToAccountID
		...
		AND AT2007.WareHouseID BETWEEN @FromWareHouseID AND @ToWareHouseID
	...
	WHILE @@Fetch_Status = 0 
		BEGIN
			EXEC AP1309_AG1 @DivisionID,@TranMonth, @TranYear, @InventoryID, @InventoryID, @FromWareHouseID, @ToWareHouseID, @FromAccountID, @ToAccountID, @UserID, @GroupID
			FETCH NEXT FROM @Cur_Ware INTO  @InventoryID
		END
	CLOSE @Cur_Ware
END 

EXEC AP1309_AG2 @DivisionID,@TranMonth, @TranYear, @FromInventoryID, @ToInventoryID, @FromWareHouseID, @ToWareHouseID, @FromAccountID, @ToAccountID, @UserID, @GroupID		
...
WHERE AT9000.TableID = 'AT0112' AND TranMonth = @TranMonth AND TranYear = @TranYear
	...
	----- XU LY LAM TRON
		...
		OPEN @Cur_Ware2
		FETCH NEXT FROM @Cur_Ware2 INTO  @WareHouseID, @InventoryID, @AccountID, @Amount
