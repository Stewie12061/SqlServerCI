IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP2008_AG]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP2008_AG]
GO
...
---- Modified on 07/01/2019 by Kim Thư: Bổ sung biến @IsExceptReturn - option load hàng bị trả lại
...
---- Modified on 28/03/2022 by Nhật Thanh: Bổ sung DivisionID dùng chung khi join bảng
---- Modified on 05/04/2022 by Nhật Thanh: Bổ sung ISNULL cho BeginQuantity khi tính EndQuantity
...
IF EXISTS (SELECT TOP 1 1 FROM AT0000 WITH (NOLOCK) WHERE DefDivisionID = @DivisionID AND IsSpecificate = 1)
	EXEC AP2008_QC @DivisionID, @FromMonth, @ToMonth, @FromYear, @ToYear, @FromDate, @ToDate, @FromInventoryID, @ToInventoryID, @WareHouseID, @IsDate, @IsGroupID, @GroupID1, @GroupID2
ELSE
BEGIN 
...
BEGIN
	...
	IF EXISTS (SELECT TOP 1 1 FROM tempdb.dbo.sysobjects WITH (NOLOCK) WHERE ID = OBJECT_ID(N'tempdb.dbo.##AV7018')) 
		DROP TABLE ##AV7018

	IF EXISTS (SELECT TOP 1 1 FROM tempdb.dbo.sysobjects WITH (NOLOCK) WHERE ID = OBJECT_ID(N'tempdb.dbo.##AV2088')) 
		DROP TABLE ##AV2088

	IF EXISTS (SELECT TOP 1 1 FROM tempdb.dbo.sysobjects WITH (NOLOCK) WHERE ID = OBJECT_ID(N'tempdb.dbo.##AV2098')) 
		DROP TABLE ##AV2098

	IF EXISTS (SELECT TOP 1 1 FROM tempdb.dbo.sysobjects WITH (NOLOCK) WHERE ID = OBJECT_ID(N'tempdb.dbo.##AV2008')) 
		DROP TABLE ##AV2008
	...
	BEGIN
		...
		SET @sSQLFrom = ' INTO ##AV7018
			...
			LEFT JOIN AT1309 WITH (NOLOCK) ON AT1309.DivisionID = AT2008.DivisionID AND AT1309.InventoryID = AT1302.InventoryID AND AT1309.UnitID = AT1302.UnitID
...
*/
	...
	IF @IsDate = 0
		SET @sSQLUnion = @sSQLUnion + 'ISNULL(AV7018.BeginQuantity,0) AS BeginQuantity, AV7018.BeginAmount AS BeginAmount,'
	ELSE
	...
	SET @sSQLUnion = @sSQLUnion + 'AV7000.WareHouseID, AV7000.DivisionID, AV7000.Barcode, AV7000.WarehouseName, AV7000.Varchar01, AV7000.ETaxConvertedUnit'
...
*/
	...
	SET @sSQLUpdate = '
		UPDATE T1
		SET T1.ConversionUnitID = AT1309.unitID, T1.ConversionFactor = AT1309.ConversionFactor, T1.Operator = AT1309.Operator
		FROM ##AV2098 T1
		LEFT JOIN AV1399 as AT1309 ON AT1309.DivisionID = T1.DivisionID AND AT1309.InventoryID = T1.InventoryID AND AT1309.UnitID = T1.UnitID
	'
...
*/
	...
	LEFT JOIN (	SELECT	A.InventoryID, SUM(A.MinQuantity) AS MinQuantity, SUM(A.MaxQuantity) AS MaxQuantity, A.DivisionID, A.WareHouseID
		...
		ON		AT1314.DivisionID = AV2098.DivisionID AND AT1314.InventoryID = AV2098.InventoryID AND AV2098.WareHouseID = AT1314.WareHouseID
	...
	BEGIN
		...
		ELSE
			...
			'
		SET @sSQL2 = N'
		SELECT A1.GroupID1, A1.GroupName1, A1.GroupID2, A1.GroupName2, A1.S1, A1.S2, A1.S3, A1.I01ID, A1.I02ID, A1.I03ID, A1.I04ID, A1.I05ID, A1.Notes01, A1.Notes02, A1.Notes03, 
		A1.SalePrice01, A1.InventoryID, A1.InventoryName, A1.InventoryTypeID, A1.InventoryTypeName, A1.Specification, A1.UnitID, A1.UnitName, A1.ConversionUnitID, 
		A1.ConversionFactor, A1.Operator, A1.ConversionQuantity,
		...
		A1.DivisionID, A1.MinQuantity, A1.MaxQuantity, A1.WarehouseID, A1.WarehouseName, A1.Varchar01, A1.ETaxConvertedUnit, A1.TimeOfUse, A1.BarCode
		'
		SET @sSQL3 = N'FROM ##AV2008 A1 --LEFT JOIN #BEGINRETURN A2 ON A1.WarehouseID = ISNULL(A2.WareHouseID,'''') AND A1.InventoryID = ISNULL(A2.InventoryID,'''')
		...
		LEFT JOIN #ENDRETURN A4 ON A1.WarehouseID = ISNULL(A4.WareHouseID,'''') AND A1.InventoryID = ISNULL(A4.InventoryID,'''') AND A4.KindVoucherID = 3
		...
		GROUP BY A1.GroupID1, A1.GroupName1, A1.GroupID2, A1.GroupName2, A1.S1, A1.S2, A1.S3, A1.I01ID, A1.I02ID, A1.I03ID, A1.I04ID, A1.I05ID, A1.Notes01, A1.Notes02, A1.Notes03, A1.SalePrice01,
		A1.InventoryID, A1.InventoryName, A1.InventoryTypeID, A1.InventoryTypeName, A1.Specification, A1.UnitID, A1.UnitName, A1.ConversionUnitID, A1.ConversionFactor, A1.Operator, A1.ConversionQuantity,
		A1.BeginQuantity, A1.DebitQuantity, A1.CreditQuantity, A1.EndQuantity, A1.BeginAmount, A1.DebitAmount, A1.CreditAmount, A1.EndAmount, A1.InDebitAMount, A1.InCreditAmount, A1.InDebitQuantity,
		A1.InCreditQuantity, A1.DivisionID, A1.MinQuantity, A1.MaxQuantity, A1.WarehouseID, A1.WarehouseName, A1.Varchar01, A1.ETaxConvertedUnit, A1.TimeOfUse, A1.BarCode
		ORDER BY  A1.InventoryID
		'
		--SELECT @sSQL1
		--SELECT @sSQL2
		--SELECT @sSQL3
		...
		END
	ELSE
		SELECT * FROM ##AV2008 ORDER BY InventoryID
