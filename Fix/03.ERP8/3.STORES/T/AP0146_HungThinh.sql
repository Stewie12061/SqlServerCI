IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP0146_HungThinh]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[AP0146_HungThinh]
GO
...
-- AP0146
-- <Summary>
---- Stored load dữ liệu hóa đơn phục vụ phát hành hóa đơn điện tử -- Tách store Hưng Thịnh
---- Created on 06/10/2020 Huỳnh Thử
-- <Param>
---- 
-- <Return>
---- 
...
---- 
-- <History>
---- Modified on 09/12/2020 by Nhựt Trường: Gộp các phiếu có chung Lô nhập(SourceNo) hoặc Hạn sử dụng(LimitDate).
---- Modified by Đức Duy on 20/02/2023: [2023/02/IS/0091] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục đối tượng - AT1202.
-- <Example>
---- EXEC AP0146 @DivisionID = 'ANG',@UserID='ASOFTADMIN',@VoucherID='AV561ee05d-44de-4d71-8e4c-4927876f60df'

CREATE PROCEDURE [dbo].[AP0146_HungThinh]
...
BEGIN
	...
	(ISNULL(AT9000.InventoryName1,AT1302.InventoryName) + '' '' + TDescription)  AS ProdName, 
	AT1304.UnitName AS ProdUnit, SUM(AT9000.Quantity) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS ProdQuantity, 
	ABS(AT9000.ConvertedPrice) AS ProdPrice, SUM(ABS(AT9000.ConvertedAmount)) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS Amount,
	...
	AT9000.BDescription as DonDatHangSo, SUM(ABS(AT9000.OriginalAmount)) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS OriginalAmount, ISNULL(A02.AnaName+''/''+AT9000.VDescription,AT9000.VDescription) AS KindOfService, 
	...
	AT9000.TableID, AT9000.TranMonth, AT9000.TranYear, AT9000.TransactionTypeID, AT9000.ObjectID, AT9000.CreditObjectID, AT9000.VATNo,
	AT9000.VATObjectID, AT9000.VATObjectName, AT9000.VATObjectAddress, AT9000.DebitAccountID, AT9000.CreditAccountID, AT9000.ExchangeRate, AT9000.UnitPrice, SUM(AT9000.OriginalAmount) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) as OriginalAmountAT9000,
	SUM(AT9000.ConvertedAmount) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS ConvertedAmount, AT9000.ImTaxOriginalAmount, AT9000.ImTaxConvertedAmount, AT9000.ExpenseOriginalAmount, AT9000.ExpenseConvertedAmount, AT9000.IsStock, AT9000.VoucherDate,
	AT9000.VoucherTypeID, AT9000.VATGroupID, AT9000.Serial, AT9000.InvoiceNo,
	--AT9000.Orders, 
	AT9000.EmployeeID, AT9000.SenderReceiver, AT9000.SRDivisionName, AT9000.SRAddress,
	AT9000.RefNo01, AT9000.RefNo02, AT9000.VDescription, AT9000.BDescription, SUM(AT9000.Quantity) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS Quantity, AT9000.InventoryID, AT9000.UnitID, AT9000.Status, AT9000.IsAudit, AT9000.IsCost, 
	...
	AT9000.ExpenseID, AT9000.MaterialTypeID, AT9000.ProductID,
	--AT9000.CreateDate,
	AT9000.CreateUserID, AT9000.LastModifyUserID, 
	--AT9000.LastModifyDate,
	SUM(AT9000.OriginalAmountCN) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS OriginalAmountCN,
	...
	AT9000.IsLateInvoice,SUM(AT9000.ConvertedQuantity) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS ConvertedQuantity,AT9000.ConvertedPrice,AT9000.ConvertedUnitID,AT9000.ConversionFactor,AT9000.UParameter01,AT9000.UParameter02,AT9000.UParameter03,
	AT9000.UParameter04,AT9000.UParameter05,AT9000.PriceListID,AT9000.WOrderID,AT9000.WTransactionID, SUM(AT9000.MarkQuantity) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS MarkQuantity,AT9000.TVoucherID,AT9000.OldCounter,AT9000.NewCounter,
	...
	SET @sSQL2A='--AT9000.ImVoucherID,	AT9000.ImTransactionID,
		AT9000.SourceNo,AT9000.LimitDate,AT9000.IsPromotionItem,AT9000.ObjectName1,
	...
	SUM(ISNULL(AT1010.VATRate*AT9000.OriginalAmount/100,0)) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) as DVATOriginalAmount, SUM(ISNULL(AT1010.VATRate*AT9000.ConvertedAmount/100,0)) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) as DVATConvertedAmount,
	...
	SUM(ISNULL(AT9000.OriginalAmount,0) + ISNULL(AT1010.VATRate*AT9000.OriginalAmount/100,0)) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) as OriginalAfterVATAmount, 
	SUM(ISNULL(AT9000.ConvertedAmount,0) + ISNULL(AT1010.VATRate*AT9000.ConvertedAmount/100,0)) OVER(PARTITION BY ISNULL(AT9000.SourceNo, AT9000.LimitDate)) AS ConvertedAfterVATAmount,
	...
	SET @sSQLA = '
		SELECT * FROM (
			SELECT	TOP 100 PERCENT Remark, CusCode, CusName, CusAddress, CusPhone, CusEmail,CusEmailCC, CusTaxCode, Buyer, CusbankNo,CusBankName, PaymentMethod, 
				...
				VoucherID, BatchID,
				--TransactionID, 
				TableID, TranMonth, TranYear, TransactionTypeID, ObjectID, CreditObjectID, VATNo,
				...
				VoucherTypeID, VATGroupID, Serial, InvoiceNo,
				--Orders,
				...
				ExpenseID, MaterialTypeID, ProductID, 
				--CreateDate, 
				...
				--ImVoucherID, ImTransactionID,
				SourceNo, LimitDate, IsPromotionItem, ObjectName1, TaxRateID, PayMethodID, DiscountedUnitPrice, 
			...
			WHERE X.Remark IN (
						   SELECT MIN(Remark)
						   FROM #TEMP
						   GROUP BY InventoryID, ISNULL(SourceNo,LimitDate)
						   )
	'	
	IF EXISTS (SELECT TOP 1 1  FROM AT0000 WHERE DefDivisionID=@DivisionID AND EInvoicePartner='BKAV') AND EXISTS (SELECT TOP 1 1 FROM AT9000 WITH (NOLOCK) WHERE VoucherID = @VoucherID AND ISNULL(Parameter10,'') <> '')
		SET @sSQL3='
			...
			0 AS VATAmount10,  1 AS Extra,InvoiceDate , CurrencyID , '''' AS VoucherID , '''' AS PaymentID , '''' AS  VoucherNo ,'''' AS BatchID, 
			--'''' AS TransactionID ,
			'''' AS TableID, 0 AS TranMonth , 
			...
			'''' AS VATGroupID, '''' AS Serial ,'''' AS InvoiceNo , 
			--0 AS Orders,
			'''' AS EmployeeID , 
			...
			'''' AS Ana10ID ,'''' AS PeriodID , '''' AS ExpenseID ,'''' AS MaterialTypeID , '''' AS ProductID ,
			--'''' AS CreateDate , 
			'''' AS CreateUserID , 
			--'''' AS LastModifyDate ,
			'''' AS LastModifyUserID ,0 AS OriginalAmountCN ,1 AS ExchangeRateCN ,'''' AS CurrencyIDCN , 
			...
			--'''' AS ImVoucherID ,'''' AS ImTransactionID ,
			'''' AS SourceNo ,'''' AS LimitDate , 0 AS IsPromotionItem  , '''' AS ObjectName1, '''' AS TaxRateID,  2 AS PayMethodID ,
...
--print @sSQL
--print @sSQL3
	EXEC (@sSQL+@sSQL1A+@sSQL1A1+@sSQL2A+@sSQLA+@sSQLB+@sSQL3)
END	
