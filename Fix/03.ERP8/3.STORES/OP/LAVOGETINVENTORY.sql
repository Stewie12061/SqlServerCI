IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[LAVOGETINVENTORY]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[LAVOGETINVENTORY]
GO
...
--- Modified by Văn Tài	  on 28/09/2020: Bổ sung customize cho Savi. Duyệt tồn kho mặt hàng là thành phẩm (F)
--- Modified by Huỳnh Thử on 28/09/2020 : Bổ sung danh mục dùng chung
--- Modified by Đức Duy on 13/02/2023: [2023/02/IS/0052] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục kho hàng - AT1303.

--	LAVOGETINVENTORY 'LG', 'ASOFTADMIN', 'NLDAU', '%', '%'

CREATE PROCEDURE LAVOGETINVENTORY
(
...
IF(@CustomerIndex = 44)
	BEGIN 
		EXEC LAVOGETINVENTORY_SAVI @DivisionID, @user_id, @product_id, @product_name, @warehouse_id
	END 
ELSE
	BEGIN
		...
		EXEC OP2504LAVO @DivisionID, @TranMonth, @TranYear, @product_id
		...
		FROM OV2504 V54
		LEFT JOIN AT1302 A02 ON A02.DivisionID IN (''@@@'', V54.DivisionID) AND A02.InventoryID = V54.InventoryID
