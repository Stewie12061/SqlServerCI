...
---- Modified by Tieu Mai on 27/01/2016: Bo sung dieu kien join OT8899
---- Modified by Tiểu Mai on 06/06/2016: Bổ sung WITH (NOLOCK)
---- Modified by Hải Long on 21/08/2016: Bổ sung trường cvarchar cho ABA
---- Modify on 26/04/2017 by Bảo Anh: Sửa danh mục dùng chung (bỏ kết theo DivisionID)
---- Modified by Phương Thảo on 10/05/2017: Bổ sung các trường kế thừa
---- Modified by Bảo Thy on 03/07/2017: Bổ sung IsPriceID (GODREJ), bỏ vào chuỗi để fix lỗi khi chạy fix
---- Modified by Kim Thư on 28/02/2019: Tạo bảng tạm thay OV1302 tránh xung đột process khác
---- Modified by Như Hàn on 19/03/2019: Bổ sung @APKMaster, @Type khi gọi từ Form duyệt OOF2050
---- Modified by Như Hàn on 05/04/2019: Thêm Thông tin người duyệt
---- Modified by Kim Thư on 02/05/2019: Không chạy store OP1302 trong trường hợp khách hàng ko dùng bảng giá (cải thiện tốc độ)
---- Modified by Kim Thư on 19/06/2019: Bổ sung load thêm cột RequestQuantity và RequestConvertedQuantity
---- Modified by Lê Hoàng on 29/05/2020: Thêm Thông tin Mã phụ - RefInventoryID (Tân Hòa Lợi - 122)
---- Modified by Huỳnh Thử on 22/09/2020: Bổ sung trường IsProInventoryID
---- Modified by Huỳnh Thử on 28/09/2020 : Bổ sung danh mục dùng chung
---- Modified by Trọng Kiên on 05/11/2020 : Fix lỗi độ dài cột Specification của table @OV1302
---- Modified by Minh Hiếu on 22/02/2022 : Thêm custom cho VNA
---- Modified by Nhật Thanh on 04/05/2022 : Customize cho angel
---- Modified by Hoài Bảo on 20/09/2022 : Bổ sung kiểm tra ISNULL cho các trường có tính tổng cột trên lưới
---- Modified by Đức Duy on 13/02/2023: [2023/02/IS/0052] - Bổ sung thêm điều kiện DivisionID dùng chung cho bảng danh mục kho hàng - AT1303.
---- Modified by Văn Tài on 01/04/2023: [2023/03/IS/0279] - Bổ sung kiểm tra key OOT9000 thì kiếm POrderID bên OT3001.
---- Modified by Đình Định on 14/07/2023: Bổ sung WITH (NOLOCK).
...
IF EXISTS(SELECT TOP 1 1 FROM OT3001 WITH (NOLOCK) WHERE OT3001.POrderID = @POrderID)
BEGIN
	SELECT @OrderDate = OrderDate, @ObjectID = ObjectID, @PriceListID = ISNULL(PriceListID,'') FROM OT3001 WITH (NOLOCK) WHERE OT3001.DivisionID in (@DivisionID, '@@@') AND OT3001.POrderID = @POrderID
END
ELSE
BEGIN
	SET @POrderID = (SELECT TOP 1 POrderID 
						FROM OT3001 WITH (NOLOCK) 
						WHERE DivisionID = @DivisionID 
							AND VoucherTypeID = 'DHM' 
							AND VoucherNo = (
												SELECT TOP 1 ID FROM OOT9000 WITH (NOLOCK) WHERE DivisionID = @DivisionID AND [Type] = N'DHM' AND APK = @POrderID
											)
					)
...
IF EXISTS (SELECT TOP 1 * FROM OT1301 WITH (NOLOCK))
BEGIN
	INSERT INTO @OV1302
...
SET @TotalRow = ''
IF((SELECT CustomerName From CustomerIndex WITH (NOLOCK) ) = 151)
	SET @sSQL4 ='O32.SourceNo,O32.ReceivedDate,'
ELSE 
	SET @sSQL4 =''
...
BEGIN
	...
	SELECT @Level = MAX(ApproveLevel) FROM OT3002 WITH (NOLOCK) WHERE OT3002.DivisionID IN (@DivisionID,'@@@') AND APKMaster_9000 = @APKMaster  
END
ELSE 
BEGIN
	SET @Swhere = @Swhere + 'AND O32.POrderID = '''+@POrderID+''''
	SELECT @Level = MAX(ApproveLevel) FROM OT3002 WITH (NOLOCK) WHERE OT3002.DivisionID IN (@DivisionID,'@@@') AND POrderID = @POrderID 
END
...
SET @sSQL = 'SELECT 
		...
		O31.InventoryTypeID, A01.InventoryTypeName, A02.IsStocked, A02.Barcode, O32.InventoryID,  O32.UnitID, O32.PONumber,
		...
		O32.ImTaxPercent, O32.ImTaxOriginalAmount, O32.ImTaxConvertedAmount,'+@sSQL4+'
...
SET @sSQL1 = '
		...
		LEFT JOIN OV2902 V92 WITH (NOLOCK) ON V92.POrderID = O32.POrderID AND V92.TransactionID = O32.TransactionID
		LEFT JOIN AT1302 A32 WITH (NOLOCK) ON A32.DivisionID IN (''@@@'', O32.DivisionID)
										   AND A32.InventoryID = O32.InventoryID
		LEFT JOIN AT1303 A03 WITH (NOLOCK) ON A03.DivisionID IN (''@@@'', O32.DivisionID) 
										   AND A03.WareHouseID = O32.WareHouseID
		LEFT JOIN OT1003 O03 WITH (NOLOCK) ON O03.DivisionID = O32.DivisionID
										   AND O03.MethodID = O32.MethodID
		LEFT JOIN AT1304 A04 WITH (NOLOCK) ON A04.DivisionID IN (''@@@'', O32.DivisionID) 
										   AND A04.UnitID = O32.UnitID
		LEFT JOIN AT1302 A02 WITH (NOLOCK) ON A02.DivisionID IN (''@@@'', O32.DivisionID) 
										   AND A02.InventoryID = O32.InventoryID
		LEFT JOIN OT3001 O31 WITH (NOLOCK) ON O31.DivisionID = O32.DivisionID 
										   AND O31.POrderID = O32.POrderID
		LEFT JOIN OT3003 O33 WITH (NOLOCK) ON O33.POrderID = O31.POrderID
										   AND O33.DivisionID = O31.DivisionID 
		LEFT JOIN AT1301 A01 WITH (NOLOCK) ON A01.InventoryTypeID = O31.InventoryTypeID
										   AND A01.DivisionID = O32.DivisionID 
		'+ CASE WHEN @IsPriceID = 0 THEN '' ELSE 'LEFT JOIN #OV1302 OV1302 ON OV1302.DivisionID = O32.DivisionID 
																		   AND OV1302.InventoryID = O32.InventoryID 
																		   AND OV1302.UnitID = O32.UnitID' END +'
		LEFT JOIN OT8899 O99 WITH (NOLOCK) ON O99.DivisionID = O32.DivisionID 
										   AND O99.VoucherID = O32.POrderID
										   AND O99.TransactionID = O32.TransactionID'
SET @sSQL2 = '
		LEFT JOIN AT0128 AT01 WITH (NOLOCK) ON AT01.StandardID = O99.S01ID
											AND AT01.StandardTypeID = ''S01''
											AND AT01.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT02 WITH (NOLOCK) ON AT02.StandardID = O99.S02ID
											AND AT02.StandardTypeID = ''S02''
											AND AT02.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT03 WITH (NOLOCK) ON AT03.StandardID = O99.S03ID 
											AND AT03.StandardTypeID = ''S03''
											AND AT03.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT04 WITH (NOLOCK) ON AT04.StandardID = O99.S04ID 
											AND AT04.StandardTypeID = ''S04''
											AND AT04.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT05 WITH (NOLOCK) ON AT05.StandardID = O99.S05ID 
											AND AT05.StandardTypeID = ''S05''
		LEFT JOIN AT0128 AT06 WITH (NOLOCK) ON AT06.StandardID = O99.S06ID
											AND AT06.StandardTypeID = ''S06''
											AND AT06.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT07 WITH (NOLOCK) ON AT07.StandardID = O99.S07ID
											AND AT07.StandardTypeID = ''S07''
											AND AT07.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT08 WITH (NOLOCK) ON AT08.StandardID = O99.S08ID 
											AND AT08.StandardTypeID = ''S08''
											AND AT08.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT09 WITH (NOLOCK) ON AT09.StandardID = O99.S09ID 
											AND AT09.StandardTypeID = ''S09''
											AND AT09.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT10 WITH (NOLOCK) ON AT10.StandardID = O99.S10ID 
											AND AT10.StandardTypeID = ''S10''
											AND AT10.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT11 WITH (NOLOCK) ON AT11.StandardID = O99.S11ID 
											AND AT11.StandardTypeID = ''S11''
											AND AT11.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT12 WITH (NOLOCK) ON AT12.StandardID = O99.S12ID 
											AND AT12.StandardTypeID = ''S12''
											AND AT12.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT13 WITH (NOLOCK) ON AT13.StandardID = O99.S13ID AND AT13.StandardTypeID = ''S13'' AND AT13.DivisionID = O99.DivisionID			
		LEFT JOIN AT0128 AT14 WITH (NOLOCK) ON AT14.StandardID = O99.S15ID AND AT14.StandardTypeID = ''S14'' AND AT14.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT15 WITH (NOLOCK) ON AT15.StandardID = O99.S15ID AND AT15.StandardTypeID = ''S15'' AND AT15.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT16 WITH (NOLOCK) ON AT16.StandardID = O99.S16ID AND AT16.StandardTypeID = ''S16'' AND AT16.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT17 WITH (NOLOCK) ON AT17.StandardID = O99.S17ID AND AT17.StandardTypeID = ''S17'' AND AT17.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT18 WITH (NOLOCK) ON AT18.StandardID = O99.S18ID AND AT18.StandardTypeID = ''S18'' AND AT18.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT19 WITH (NOLOCK) ON AT19.StandardID = O99.S19ID AND AT19.StandardTypeID = ''S19'' AND AT19.DivisionID = O99.DivisionID
		LEFT JOIN AT0128 AT20 WITH (NOLOCK) ON AT20.StandardID = O99.S20ID AND AT20.StandardTypeID = ''S20'' AND AT20.DivisionID = O99.DivisionID
		'
SET @sSQL3 = N'
		LEFT JOIN AT1011 T01 WITH (NOLOCK) ON T01.AnaID = O32.Ana01ID AND T01.AnaTypeID = ''A01'' AND T01.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T02 WITH (NOLOCK) ON T02.AnaID = O32.Ana02ID AND T02.AnaTypeID = ''A02'' AND T02.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T03 WITH (NOLOCK) ON T03.AnaID = O32.Ana03ID AND T03.AnaTypeID = ''A03'' AND T03.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T04 WITH (NOLOCK) ON T04.AnaID = O32.Ana04ID AND T04.AnaTypeID = ''A04'' AND T04.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T05 WITH (NOLOCK) ON T05.AnaID = O32.Ana05ID AND T05.AnaTypeID = ''A05'' AND T05.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T06 WITH (NOLOCK) ON T06.AnaID = O32.Ana06ID AND T06.AnaTypeID = ''A06'' AND T06.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T07 WITH (NOLOCK) ON T07.AnaID = O32.Ana07ID AND T07.AnaTypeID = ''A07'' AND T07.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T08 WITH (NOLOCK) ON T08.AnaID = O32.Ana08ID AND T08.AnaTypeID = ''A08'' AND T08.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T09 WITH (NOLOCK) ON T09.AnaID = O32.Ana09ID AND T09.AnaTypeID = ''A09'' AND T09.DivisionID IN (''@@@'', O32.DivisionID) 
		LEFT JOIN AT1011 T10 WITH (NOLOCK) ON T10.AnaID = O32.Ana10ID AND T10.AnaTypeID = ''A10'' AND T10.DivisionID IN (''@@@'', O32.DivisionID) 
...
'
END
ELSE IF((select CustomerName From CustomerIndex WITH (NOLOCK)) = 147)
BEGIN
	SET @sSQL='
	SELECT
		'+CASE WHEN @IsViewDetail = 1 THEN ' ROW_NUMBER() OVER (ORDER BY O32.Orders) AS RowNum, '+@TotalRow+' AS TotalRow, ' ELSE '' END +'
		...
		LEFT JOIN AT1303 A03 WITH (NOLOCK) ON A03.DivisionID IN (''@@@'', ''' + @DivisionID + ''') AND A03.WareHouseID = O32.WareHouseID
		...
		'+@sSQLJon+'
	WHERE O32.DivisionID IN ('''+@DivisionID+''', ''@@@'')  '+@Swhere+'
	Order by O32.Orders'
END
ELSE
IF ((select CustomerName From CustomerIndex WITH (NOLOCK)) = 57)
BEGIN
	SET @sSQL='
	...
	FROM OT3002 O32 WITH (NOLOCK)
		...
		LEFT JOIN OT1003 O03 WITH (NOLOCK) ON O03.DivisionID = O32.DivisionID
										   AND O03.MethodID = O32.MethodID
		LEFT JOIN AT1304 A04 WITH (NOLOCK) ON A04.UnitID = O32.UnitID
										   AND A04.DivisionID in (''@@@'',O32.DivisionID)
		LEFT JOIN AT1302 A02 WITH (NOLOCK) ON A02.DivisionID in (''@@@'',O32.DivisionID) 
										   AND A02.InventoryID = O32.InventoryID
		LEFT JOIN OT3001 O31 WITH (NOLOCK) ON O31.DivisionID = O32.DivisionID 
										   AND O31.POrderID = O32.POrderID
		LEFT JOIN OT3003 O33 WITH (NOLOCK) ON O33.POrderID = O31.POrderID 
										   AND O33.DivisionID = O31.DivisionID 
		LEFT JOIN AT1301 A01 WITH (NOLOCK) ON A01.InventoryTypeID = O31.InventoryTypeID
										   AND A01.DivisionID in (''@@@'',O32.DivisionID) 
	WHERE O32.DivisionID IN ('''+@DivisionID+''', ''@@@'')  '+@Swhere+'
	Order by O32.Orders'
...
BEGIN
	...
	SELECT
		...
		SET @sSQL1 = '
		LEFT JOIN OOT9000 OOT90 WITH (NOLOCK) ON O32.APKMaster_9000 = OOT90.APK
		LEFT JOIN OV2902 V92 WITH (NOLOCK) ON V92.DivisionID IN (''@@@'', V92.DivisionID) AND V92.POrderID = O32.POrderID AND V92.TransactionID = O32.TransactionID
		LEFT JOIN AT1302 A32 WITH (NOLOCK) ON A32.DivisionID IN (''@@@'', O32.DivisionID) AND A32.InventoryID = O32.InventoryID
		LEFT JOIN AT1303 A03 WITH (NOLOCK) ON A03.DivisionID IN (''@@@'', O32.DivisionID) AND  A03.WareHouseID = O32.WareHouseID
		LEFT JOIN OT1003 O03 WITH (NOLOCK) ON O03.DivisionID = O32.DivisionID AND O03.MethodID = O32.MethodID
		LEFT JOIN AT1304 A04 WITH (NOLOCK) ON A04.DivisionID IN (''@@@'', O32.DivisionID) AND A04.UnitID = O32.UnitID
		LEFT JOIN AT1302 A02 WITH (NOLOCK) ON A02.DivisionID IN (''@@@'', O32.DivisionID) AND A02.InventoryID = O32.InventoryID
		LEFT JOIN OT3001 O31 WITH (NOLOCK) ON O31.DivisionID = O32.DivisionID AND O31.POrderID = O32.POrderID
		LEFT JOIN OT3003 O33 WITH (NOLOCK) ON O33.POrderID = O31.POrderID AND O33.DivisionID = O31.DivisionID 
		LEFT JOIN AT1301 A01 WITH (NOLOCK) ON A01.DivisionID IN (''@@@'', O32.DivisionID) AND A01.InventoryTypeID = O31.InventoryTypeID   
		'+ CASE WHEN @IsPriceID = 0 THEN '' ELSE 'LEFT JOIN #OV1302 OV1302 ON OV1302.DivisionID = O32.DivisionID 
																		   AND OV1302.InventoryID = O32.InventoryID 
																		   AND OV1302.UnitID = O32.UnitID' END + '
		LEFT JOIN AT1011 T01 WITH (NOLOCK) ON T01.DivisionID IN (''@@@'', O32.DivisionID) AND T01.AnaID = O32.Ana01ID AND T01.AnaTypeID = ''A01''  
		LEFT JOIN AT1011 T02 WITH (NOLOCK) ON T02.DivisionID IN (''@@@'', O32.DivisionID) AND T02.AnaID = O32.Ana02ID AND T02.AnaTypeID = ''A02''  
		LEFT JOIN AT1011 T03 WITH (NOLOCK) ON T03.DivisionID IN (''@@@'', O32.DivisionID) AND T03.AnaID = O32.Ana03ID AND T03.AnaTypeID = ''A03''  
		LEFT JOIN AT1011 T04 WITH (NOLOCK) ON T04.DivisionID IN (''@@@'', O32.DivisionID) AND T04.AnaID = O32.Ana04ID AND T04.AnaTypeID = ''A04''  
		LEFT JOIN AT1011 T05 WITH (NOLOCK) ON T05.DivisionID IN (''@@@'', O32.DivisionID) AND T05.AnaID = O32.Ana05ID AND T05.AnaTypeID = ''A05''  
		LEFT JOIN AT1011 T06 WITH (NOLOCK) ON T06.DivisionID IN (''@@@'', O32.DivisionID) AND T06.AnaID = O32.Ana06ID AND T06.AnaTypeID = ''A06''  
		LEFT JOIN AT1011 T07 WITH (NOLOCK) ON T07.DivisionID IN (''@@@'', O32.DivisionID) AND T07.AnaID = O32.Ana07ID AND T07.AnaTypeID = ''A07''  
		LEFT JOIN AT1011 T08 WITH (NOLOCK) ON T08.DivisionID IN (''@@@'', O32.DivisionID) AND T08.AnaID = O32.Ana08ID AND T08.AnaTypeID = ''A08''  
		LEFT JOIN AT1011 T09 WITH (NOLOCK) ON T09.DivisionID IN (''@@@'', O32.DivisionID) AND T09.AnaID = O32.Ana09ID AND T09.AnaTypeID = ''A09''  
		LEFT JOIN AT1011 T10 WITH (NOLOCK) ON T10.DivisionID IN (''@@@'', O32.DivisionID) AND T10.AnaID = O32.Ana10ID AND T10.AnaTypeID = ''A10''  
