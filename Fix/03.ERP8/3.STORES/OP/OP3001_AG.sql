IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OP3001_AG]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OP3001_AG]
GO
...
CREATE PROCEDURE [dbo].[OP3001_AG] 
				@DivisionID AS nvarchar(50),
				@TranMonth AS int,
				@TranYear AS int,
				@OrderID AS nvarchar(4000),
				@OrderBy AS nvarchar(500)
...
) x LEFT JOIN A00003 WITH(NOLOCK) on x.InventoryID = A00003.InventoryID	and x.DivisionID = A00003.DivisionID '
set @sSQL6 = @sSQL6 + @OrderBy
