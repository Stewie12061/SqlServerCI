IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP2010]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP2010]
GO
...
-- <Summary>
---- Load Grid Form OOF2010: Đơn xin nghỉ phép
-- <Param>
---- 
-- <Return>
---- 
...
---- Modified by Bảo Anh on 02/01/2019: không lấy các cột người duyệt, trạng thái duyệt, SectionID, SubsectionID, ProcessID
...
---- Modified on 12/07/2021 by Đoàn Duy: Chỉnh sửa điều kiện để có thể load được đơn xin phép của user khác theo @ConditionPermissionFormID
/*-- <Example>
	OOP2010 @DivisionID='MK',@UserID='001429',@PageNumber=1,@PageSize=25,@IsSearch=1, @TranMonth=8, @TranYear=2016,
	@ID=NULL,@DepartmentID=NULL,@SectionID=NULL, @SubsectionID=NULL,@ProcessID=NULL,@CreateUserID=NULL,
	@LeaveFromDate=NULL,@LeaveToDate=NULL,@Status=NULL, @CreateFromDate = '2016-08-01', @CreateToDate = '2016-08-01'
----*/

CREATE PROCEDURE OOP2010
( 
	...
	@ConditionPermissionFormID VARCHAR(MAX)
)
AS 
...
IF Isnull(@ConditionPermissionFormID, '') = ''
		SET @sWhere = @sWhere + ' AND (OOT90.CreateUserID = AT0010.UserID OR  OOT90.CreateUserID = '''+@UserID+''')'

--SET @Level=ISNULL((SELECT TOP 1 LEVEL FROM OOT0010 WITH (NOLOCK) WHERE DivisionID=@DivisionID AND AbsentType ='DXP'AND TranMonth=@TranMonth AND TranYear=@TranYear),0)              
...
BEGIN
	...
	IF (@LeaveFromDate IS NOT NULL AND @LeaveToDate IS NULL) SET @sWhere = @sWhere + '
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2010.LeaveFromDate,120), 126) >= '''+CONVERT(VARCHAR(10),@LeaveFromDate,126)+''' '
	IF (@LeaveFromDate IS NULL AND @LeaveToDate IS NOT NULL) SET @sWhere = @sWhere + '
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2010.LeaveToDate,120), 126) <= '''+CONVERT(VARCHAR(10),@LeaveToDate,126)+''' '
	IF (@LeaveFromDate IS NOT NULL AND @LeaveToDate IS NOT NULL) SET @sWhere = @sWhere + '
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2010.LeaveFromDate,120), 126) BETWEEN '''+CONVERT(VARCHAR(10),@LeaveFromDate,126)+''' AND '''+CONVERT(VARCHAR(10),@LeaveToDate,126)+''' 
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2010.LeaveToDate,120), 126) BETWEEN '''+CONVERT(VARCHAR(10),@LeaveFromDate,126)+''' AND '''+CONVERT(VARCHAR(10),@LeaveToDate,126)+''' '
...
IF Isnull(@ConditionPermissionFormID, '') != ''
		SET @sWhere = @sWhere + ' AND ISNULL(OOT90.CreateUserID,'''') in (N'''+@ConditionPermissionFormID+''' )'
...
SET @sSQL =' 
	...
	(
		SELECT OOT90.APK, OOT90.DivisionID, OOT90.TranMonth, OOT90.TranYear, OOT90.ID,
		       ISNULL(OOT90.[Description],'''') [Description], OOT90.DepartmentID, NULL AS SectionID, NULL AS SubsectionID, NULL AS ProcessID, MIN(OOT2010.Status) AS Status,
			   OOT90.CreateUserID , AT1405.UserName AS CreateUserName, OOT90.CreateDate, OOT90.LastModifyUserID, OOT90.LastModifyDate,
		...
		LEFT JOIN OOT2010 WITH (NOLOCK) ON OOT2010.DivisionID = OOT90.DivisionID AND OOT90.APK = OOT2010.APKMaster
...
SET @sSQL1='
		...
		GROUP BY OOT90.APK, OOT90.DivisionID, OOT90.TranMonth, OOT90.TranYear, OOT90.ID,
			OOT90.[Description], OOT90.DepartmentID, OOT90.CreateUserID , AT1405.UserName, OOT90.CreateDate,
...
IF @IsSearch = 1 AND @Status IS NOT NULL
	SET @sSQL1=@sSQL1+'
		HAVING MIN(OOT2010.Status) LIKE ''%'+@Status+'%'' '
