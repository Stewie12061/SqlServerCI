IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP2022]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP2022]
GO
...
-- <Summary>
---- Load thông tin master Đơn xin phép ra ngoài
-- <Param>
---- 
-- <Return>
---- 
...
BEGIN
	...
	SET @sSQLLanguage2 ='O991.Description'
END
ELSE 
BEGIN
	SET @sSQLLanguage1='O99.DescriptionE'
	SET @sSQLLanguage2 ='O991.DescriptionE'		
END
...
SET @sSQL ='SELECT OOT90.APK, OOT90.DivisionID,  A051.UserName CreateUserName, OOT90.TranMonth, OOT90.TranYear, OOT90.ID, OOT90.[Description],
		OOT90.DepartmentID, OOT90.SectionID, OOT90.SubsectionID, OOT90.ProcessID,
		A11.DepartmentName, A12.TeamName SectionName, A13.AnaName SubsectionName, A14.AnaName ProcessName, '+STR(@Level)+' Level,
		OOT90.AppoveLevel, OOT90.ApprovingLevel, O99.[Description] Status,OOT90.AskForVehicle,OOT90.UseVehicle,
		'+@sSQLLanguage2+' UseVehicleName, OOT90.HaveLunch,'+@sSQLLanguage1+' StatusName,
		OOT90.CreateUserID +'' - ''+ A051.UserName CreateUserID, OOT90.CreateDate, 
		OOT90.LastModifyUserID +'' - ''+ A052.UserName LastModifyUserID, OOT90.LastModifyDate
		'+@sSQLSL+'
        FROM OOT9000 OOT90
		LEFT JOIN AT1405 ON AT1405.DivisionID = OOT90.DivisionID AND AT1405.UserID=OOT90.CreateUserID
		...
		LEFT JOIN OOT0099 O991 ON O991.ID1=OOT90.UseVehicle AND O991.CodeMaster=''UseVehicle''
		LEFT JOIN AT1405 A051 ON A051.UserID = OOT90.CreateUserID
		LEFT JOIN AT1405 A052 ON A052.UserID = OOT90.LastModifyUserID '
