IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP2230]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP2230]
GO
...
-- <History>
----Create on 31/08/2020 by Tấn Lộc

CREATE PROCEDURE [dbo].[OOP2230]
( 
	 ...
	 @TypeOfEmail NVARCHAR(250),
	 @CreateUserID NVARCHAR (250),
	 @IsCommon NVARCHAR(100), 
	 @Disabled NVARCHAR(100),
	 @ConditionSendEmailManagement VARCHAR(MAX),
	 @UserID VARCHAR(50), 
	 @PageNumber INT,
	 @PageSize INT
)
...
ELSE IF @IsPeriod = 1 AND ISNULL(@PeriodList, '') != ''
	...
	SET @sSQLPermission = N'IF OBJECT_ID(''tempdb..#PermissionOOT2230'') IS NOT NULL DROP TABLE #PermissionOOT2230
								
							SELECT Value
							INTO #PermissionOOT2230
							FROM STRINGSPLIT(''' + ISNULL(@ConditionSendEmailManagement, '') + ''', '','')

							IF OBJECT_ID(''tempdb..#FilterSendEmailAPK'') IS NOT NULL DROP TABLE #FilterSendEmailAPK

							SELECT DISTINCT M.APK
							INTO #FilterSendEmailAPK
							FROM CMNT90051 M WITH (NOLOCK)
									...
									INNER JOIN #PermissionOOT2230 T1 ON T1.Value IN (M.CreateUserID)
							WHERE ' + @sWhere + ' '

	
	SET @sSQL = @sSQL + N'
		SELECT DISTINCT M.APK, M.DivisionID, M.SubjectName, M.[From]
			   ...
			   , M.Description, M.ReplyTo, M.References_ReplyTo, M.APKEmail_ChooseReply, M.Forward, M.References_Forward, M.APKEmail_ChooseForward, M.GroupReceiverID
		INTO #TempCMNT90051
		FROM CMNT90051 M WITH (NOLOCK)
			INNER JOIN #FilterSendEmailAPK T1 ON T1.APK = M.APK
			LEFT JOIN AT1103 A1 WITH (NOLOCK) ON A1.EmployeeID = M.CreateUserID
			LEFT JOIN OOT1040 O2 WITH (NOLOCK) ON M.StatusID = O2.StatusID
