IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP2032]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP2032]
GO
...
-- <Summary>
---- Load thông tin master Đơn xin làm thêm giờ
-- <Param>
---- 
-- <Return>
---- 
...
---- Modified by Hoài Phong on 20/01/2021: Lấy thêm cột OOT90.ApprovingLevel
-- <Example>
---- 
/*
   exec OOP2032 @DivisionID=N'MK',@UserID=N'000021',@APKMaster=N'B9225D27-CD60-4E5F-85A6-008D2828A1FE',@tranMonth=8,@TranYear=2016
*/
CREATE PROCEDURE OOP2032
( 
...
SET @OrderBy = ''
SET @Level=ISNULL((SELECT TOP 1 LEVEL FROM OOT0010 WHERE DivisionID=@DivisionID AND AbsentType ='DXLTG' AND TranMonth=@TranMonth AND TranYear=@TranYear),0) 
...
SET @sSQL ='SELECT OOT90.APK, OOT90.DivisionID, A051.UserName CreateUserName, OOT90.TranMonth, OOT90.TranYear, OOT90.ID, OOT90.[Description],
		...
		A11.DepartmentName, A12.TeamName SectionName, A13.AnaName SubsectionName, A14.AnaName ProcessName,OOT90.Status AS ApprovingLevel,OOT90.Status,
		OOT90.WorkType, '+@sSQLLanguage2+' WorkTypeName,
		...
		'+@sSQLSL+'
			...
			LEFT JOIN OOT0099 O991 WITH (NOLOCK) ON O991.ID1=ISNULL(OOT90.WorkType,0) AND O991.CodeMaster=''Day'' 
