IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP21501]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP21501]
GO
...
-- <Summary>
---- Tạo các chuỗi xử lý lấy dữ liệu Thiết lập công việc theo bảng Đánh giá dự án
-- <Param>
---- 
-- <Return>
---- 
...
CREATE PROCEDURE [dbo].[OOP21501]
(
	@DivisionID NVARCHAR(250),
	@APK NVARCHAR(250) = NULL,
	@TableName NVARCHAR(250),
	@UserID NVARCHAR(250),
	@ObjectID NVARCHAR(250) = 'DA', -- DA hoặc CV
	@Mode INT = 0,
...
BEGIN
	...
	IF @Mode = 0
	BEGIN
		INSERT INTO @OOP21501Temp
		SELECT REPLACE(O1.TargetsGroupID, '.', '') AS TargetsGroupID
		FROM OOT2150 O1 WHERE APKMaster = @APK
	END
	-- Lấy dữ liệu Thiết lập đánh giá công việc theo DivisionID
	ELSE
	BEGIN
		INSERT INTO @OOP21501Temp
		SELECT REPLACE(O2.TargetsGroupID, '.', '') AS TargetsGroupID
