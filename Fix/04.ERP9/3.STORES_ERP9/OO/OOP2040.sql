IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP2040]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP2040]
GO
...
-- <Summary>
---- Load Grid Form OOF2040: Đơn xin bổ sung/hủy quẹt thẻ
-- <Param>
---- 
-- <Return>
---- 
...
/*-- <Example>
	OOP2040 @DivisionID='MK',@UserID='000218',@PageNumber=1,@PageSize=25,@IsSearch=1, @TranMonth=8, @TranYear=2016,
	@ID=NULL,@DepartmentID=NULL,@SectionID=NULL, @SubsectionID=NULL,@ProcessID=NULL,
	@CreateUserID=NULL,@Status=NULL,@CreateFromDate='2016-07-13',@CreateToDate='2016-07-14',@FromDate =NULL,@ToDate=NULL
----*/

CREATE PROCEDURE OOP2040
( 
	...
	@ConditionTimekeepingID VARCHAR(MAX)
)
AS 
...
--SET @Level=ISNULL((SELECT TOP 1 LEVEL FROM OOT0010 WHERE DivisionID=@DivisionID AND AbsentType ='DXBSQT'AND TranMonth=@TranMonth AND TranYear=@TranYear),0)              
...
BEGIN
	...
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2040.Date,120), 126) >= '''+CONVERT(VARCHAR(10),@FromDate,126)+''' '
	IF @ToDate IS NOT NULL SET @sWhere = @sWhere + '
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2040.Date,120), 126) <= '''+CONVERT(VARCHAR(10),@ToDate,126)+''' '	
END

IF Isnull(@ConditionTimekeepingID, '') != ''
		SET @sWhere = @sWhere + ' AND ISNULL(OOT90.CreateUserID,'''') in (N'''+@ConditionTimekeepingID+''' )'
...
SET @sSQL =' 
	...
	(
		SELECT DISTINCT OOT90.APK, OOT90.DivisionID,OOT90.TranMonth, OOT90.TranYear, OOT90.ID,
		       ...
		       OOT90.SubsectionID, OOT90.ProcessID, MIN(OOT2040.Status) AS Status,
		...
		LEFT JOIN OOT2040 WITH (NOLOCK) ON OOT2040.DivisionID = OOT90.DivisionID AND OOT90.APK = OOT2040.APKMaster
...
IF @IsSearch = 1 AND @Status IS NOT NULL
	SET @sSQL1=@sSQL1+'
	HAVING MIN(OOT2040.Status) LIKE ''%'+@Status+'%'' '
