IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP2105_DTI]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP2105_DTI]
GO
...
CREATE PROCEDURE [dbo].[OOP2105_DTI]
(
...
BEGIN
	...
	-- Lấy thông tin phòng ban của người xem danh sách công việc.
	SET @DepartmentID = (SELECT TOP 1 DepartmentID FROM HT1400 WITH (NOLOCK) WHERE DivisionID = @DivisionID AND EmployeeID = @UserID)
	...
	BEGIN
		...
		SET @sSQL2 = @sSQL2 +
					-- Phòng SALES có thể xem chi tiết được các công việc
					CASE WHEN @DepartmentID = 'KDTT' 
						THEN
							'
							, CAST(1 AS TINYINT) AS ViewTask
							'
						ELSE
							'
