...
(
    SELECT TOP 1
           1
    FROM dbo.sysobjects
    WHERE id = OBJECT_ID(N'[DBO].[OOP2051_NQH]')
          AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
    DROP PROCEDURE [dbo].[OOP2051_NQH];
GO
...
---- 
-- <History>
--- Created by 12/01/2021 by Hoài Phong: Tách SP Cho NQH
--- Update by 19/01/2021 by Hoài Phong: Lấy trạng thái của detail để biết dưới lưới ông nào duyệt hay không duyệt
/*-- <Example>
exec OOP2051_NQH @DivisionID=N'MK',@UserID=N'000144',@APK=N'1617FA53-C90A-4EAE-AD6D-0FF4E8D80D18',@Type=N'DXP',@APKMaster=N'0449BE38-F007-410A-AC6F-CC2C6994BCC6'
----*/

CREATE PROCEDURE OOP2051_NQH
(
...
FROM OOT9000 OOT90 WITH (NOLOCK)
    ...
    (
        ...
        ON OOT9.DivisionID = OOT90.DivisionID
    ...
    INNER JOIN OOT9001 OOT91 WITH (NOLOCK)
        ON OOT91.DivisionID = OOT9.DivisionID
           ...
           AND ISNULL(OOT91.APKDetail, '00000000-0000-0000-0000-000000000000') = ISNULL(
                                                                                           OOT9.APKDetail,
    ...
    LEFT JOIN OOT9001 OOT912 WITH (NOLOCK)
        ON OOT912.DivisionID = OOT91.DivisionID
           ...
           AND ISNULL(OOT912.APKDetail, '00000000-0000-0000-0000-000000000000') = ISNULL(
                                                                                            OOT91.APKDetail,
                                                                                            '00000000-0000-0000-0000-000000000000'
                                                                                        )
...
FROM #TAM T1
    ...
    (
        ...
        ON T1.DivisionID = T2.DivisionID
           AND T1.APKMaster = T2.APKMaster
           AND ISNULL(T1.APKDetail, '00000000-0000-0000-0000-000000000000') = ISNULL(
                                                                                        T2.APKDetail,
                                                                                        '00000000-0000-0000-0000-000000000000'
                                                                                    )
           AND T1.ApprovePersonID = T2.ApprovePersonID
WHERE T1.Status = 1;
...
(
    SELECT TOP 1
           1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[KQ]')
          AND type IN ( N'U' )
)
    EXEC ('DROP TABLE KQ');

SELECT @RowTotal = COUNT(APK)
FROM #TAM;
...
BEGIN
    ...
    BEGIN
        SET @TableID = 'OOT2000';
        SET @sSelect
            = 'OOT20.DivisionID, OOT20.EmployeeID, OOT20.D01, OOT20.D02, OOT20.D03, OOT20.D04, OOT20.D05,
        ...
        BEGIN
            SET @sSQL0
				...
				SELECT TOP 0 OOT20.DivisionID, OOT20.EmployeeID, OOT20.D01, OOT20.D02, OOT20.D03, OOT20.D04, OOT20.D05,
				   ...
				   0 As IsLock, CAST(N'''' AS NVARCHAR(250)) AS WorkTypeName, CAST(N'''' AS NVARCHAR(250)) AS DepartmentName, 
				   	CAST(N'''' AS NVARCHAR(250)) AS SectionName, CAST(N'''' AS NVARCHAR(250)) AS SubsectionName, ''00000000-0000-0000-0000-000000000000'' AS APKOOT9001, NULL AS Level
				INTO KQ
				FROM (SELECT TOP 0 * FROM OOT2000 WITH (NOLOCK)) OOT20
				LEFT JOIN (SELECT TOP 0 WorkType, ID, DepartmentID, Description,SectionID,SubsectionID FROM OOT9000 WITH (NOLOCK)) OOT90 ON 1 = 1';
        END;
    END;
    ELSE IF @Type = 'DXP'
    BEGIN
        SET @TableID = 'OOT2010';
        SET @Join
            = 'LEFT JOIN OOT1000 WITH (NOLOCK) ON OOT1000.DivisionID=OOT20.DivisionID AND OOT1000.AbsentTypeID=OOT20.AbsentTypeID';
        SET @sSelect
            = 'OOT20.DivisionID, OOT20.APK,OOT20.APKMaster,OOT20.EmployeeID,OOT20.Reason, OOT20.AbsentTypeID,OOT20.TimeAllowance, OOT20.OffsetTime,
    ...
    BEGIN
        ...
        SET @sSelect
            = 'OOT20.DivisionID, OOT20.EmployeeID,OOT20.Reason, OOT20.Place, OOT20.GoFromDate, OOT20.GoStraight, OOT20.GoToDate, OOT20.ComeStraight,
    ...
    BEGIN
        SET @TableID = 'OOT2030';
        SET @Join
            = 'LEFT JOIN HT1020 WITH (NOLOCK) ON HT1020.DivisionID = OOT20.DivisionID AND HT1020.ShiftID=OOT20.ShiftID
					LEFT JOIN #Shift ON #Shift.EmployeeID = OOT20.EmployeeID AND #Shift.[Date] = CONVERT(Date,OOT20.WorkFromDate,120)';
        SET @sSelect
            = 'OOT20.DivisionID, OOT20.Reason, OOT20.EmployeeID, OOT20.WorkFromDate,OOT20.WorkToDate,OOT20.WorkDate, OOT20.Note,OOT20.ShiftID, OOT20.OvertTime,
		 OOT20.OvertTimeNN, OOT20.OvertTimeCompany,OOT20.DeleteFlag,HT1020.ShiftName,OOT20.APK,OOT20.APKMaster,
		 OOT20.TotalOT, #Shift.ShiftID ShiftNow, OOT20.FromWorkingDate, OOT20.ToWorkingDate';
        SET @sSQL1
            = 'DECLARE @Cur CURSOR, @EmployeeID VARCHAR(50), @Date DATETIME 
						...
						SET @Cur = CURSOR SCROLL KEYSET FOR
						SELECT EmployeeID,CONVERT(Date,WorkFromDate,113) FROM OOT2030 WITH (NOLOCK) WHERE APKMaster='''
              + @APKMaster
              + '''
						...
						BEGIN	
							...
							INSERT #Shift
							EXEC OOP2034 ''' + @DivisionID + ''', ''' + @UserID
              + ''', @Date, @EmployeeID
        ...
        BEGIN
            SET @sSQL0
                = '
				SELECT TOP 0 OOT20.DivisionID, OOT20.Reason, OOT20.EmployeeID, OOT20.WorkFromDate,OOT20.WorkToDate,OOT20.WorkDate, OOT20.Note,OOT20.ShiftID, OOT20.OvertTime,
						OOT20.OvertTimeNN, OOT20.OvertTimeCompany,OOT20.DeleteFlag,CAST(N'''' AS NVARCHAR(250)) AS ShiftName,OOT20.APK,OOT20.APKMaster,
						OOT20.TotalOT, CAST(N'''' AS NVARCHAR(250)) AS ShiftNow, OOT20.FromWorkingDate, OOT20.ToWorkingDate,
						0 AS Status, CAST(N'''' AS NVARCHAR(250)) AS EmployeeName, OOT90.*, CAST(N'''' AS NVARCHAR(250)) AS NoteOOT9001, 0 AS StatusOOT9001,
						0 As IsLock, CAST(N'''' AS NVARCHAR(250)) AS WorkTypeName, CAST(N'''' AS NVARCHAR(250)) AS DepartmentName, 
							CAST(N'''' AS NVARCHAR(250)) AS SectionName, CAST(N'''' AS NVARCHAR(250)) AS SubsectionName, ''00000000-0000-0000-0000-000000000000'' AS APKOOT9001, NULL AS Level
				INTO KQ
    ...
    BEGIN
        SET @TableID = 'OOT2040';
        SET @sSelect
            = 'OOT20.DivisionID, OOT20.EmployeeID,OOT20.Reason, OOT20.Date, OOT20.InOut, OOT0099.[Description] AS InOutName,OOT20.EditType, O991.Description EditTypeName,
					  OOT20.Note,OOT20.APK,OOT20.APKMaster, OOT20.WorkingDate';
        SET @Join
            = 'LEFT JOIN OOT0099 WITH (NOLOCK) ON OOT0099.ID1=OOT20.InOut AND OOT0099.CodeMaster=''InOut''
    ...
    BEGIN
        SET @TableID = 'OOT2070';
        SET @sSelect
            = 'OOT20.DivisionID, OOT20.Note,OOT20.APK,OOT20.APKMaster, OOT20.EmployeeID, OOT20.ShiftID,OOT20.ChangeFromDate, OOT20.ChangeToDate';
        SET @Join = '';
        ...
        BEGIN
            SET @sSQL0
				...
				SELECT TOP 0 OOT20.DivisionID, OOT20.Note,OOT20.APK,OOT20.APKMaster, OOT20.EmployeeID, OOT20.ShiftID,OOT20.ChangeFromDate, OOT20.ChangeToDate,
						0 AS Status, CAST(N'''' AS NVARCHAR(250)) AS EmployeeName, OOT90.*, CAST(N'''' AS NVARCHAR(250)) AS NoteOOT9001, 0 AS StatusOOT9001,
						0 As IsLock, CAST(N'''' AS NVARCHAR(250)) AS WorkTypeName, CAST(N'''' AS NVARCHAR(250)) AS DepartmentName, 
						CAST(N'''' AS NVARCHAR(250)) AS SectionName, CAST(N'''' AS NVARCHAR(250)) AS SubsectionName, 
						''00000000-0000-0000-0000-000000000000'' AS APKOOT9001, NULL AS Level
				INTO KQ
	...
	SELECT DISTINCT ' + @sSelect
          + ',OOT20.Status,HV14.FullName EmployeeName,
			OOT90.WorkType, OOT90.ID, OOT90.DepartmentID, OOT90.Description,OOT90.SectionID,OOT90.SubsectionID, #TAM.Note AS NoteOOT9001, #TAM.Status AS StatusOOT9001,0 As IsLock, OOT991.[Description] AS WorkTypeName,A11.DepartmentName,
			AT12.AnaName AS SectionName,AT13.AnaName AS SubsectionName, 
			 #TAM.APK AS APKOOT9001, #TAM.Level
	FROM #TAM
	LEFT JOIN ' + @TableID + ' OOT20 WITH (NOLOCK) ON #TAM.APKMaster = OOT20.APKMaster'
          + CASE
                WHEN @Type <> 'BPC' THEN
                    ' AND #TAM.APKDetail = OOT20.APK'
                ELSE
                    ''
            END
          + '
	...
	LEFT JOIN AT1011 AT12 WITH (NOLOCK) ON OOT90.DivisionID IN ( OOT90.DivisionID, N''@@@'' ) AND AT12.AnaID = OOT90.SectionID
    LEFT JOIN AT1011 AT13 WITH (NOLOCK) ON OOT90.DivisionID IN ( OOT90.DivisionID, N''@@@'' ) AND AT13.AnaID = OOT90.SubsectionID
	INNER JOIN HV1400 HV14 ON HV14.DivisionID=OOT20.DivisionID AND HV14.EmployeeID=OOT20.EmployeeID
	--LEFT JOIN OOT2001 OOT21 ON OOT21.ReAPK=OOT20.APK AND OOT21.APKMaster=OOT20.APKMaster AND OOT21.[Level]='
          + STR(@Level) + '
	' + @Join + '
	WHERE #TAM.APK = ''' + @APK + '''
...
(
    ...
    WHERE APKMaster = @APKMaster
          AND APKDetail IS NULL
)
    EXEC OOP2051_OLD @DivisionID, @UserID, @APKMaster, @APK, @Type;
ELSE IF EXISTS
(
    SELECT TOP 1
           1
