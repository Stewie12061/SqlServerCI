IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP3018]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP3018]
GO
...
GO
 ...
 /*-- <Example>
 	OOP3018 @DivisionID='MK',@FromDate = '2016-01-01 10:08:03.077', @ToDate= '2016-03-29 19:08:03.077',@DepartmentID ='%', @SectionID = '%',
 	@SubsectionID = '%',@ProcessID = '%', @TitleID='%'
 ----*/
CREATE PROCEDURE OOP3018
(
...
INSERT INTO #Title
SELECT TitleID, TitleName, TitleNameE FROM HT1106 WHERE DivisionID = @DivisionID AND [Disabled] = 0

CREATE TABLE #OOP3018 ( EmployeeID VARCHAR(50),  [Month] VARCHAR(10),
  TitleID VARCHAR(50),TitleName NVARCHAR(250),TitleNameE Nvarchar(250),LastModifyDate DATETIME)
  
BEGIN WITH TEMP AS
(
----Dữ liệu thay đồi chức danh trong khoảng thời gian lọc	
select HT1403_CT.DivisionID, HT1403_CT.EmployeeID,TitleID, ISNULL(HT1403_CT.LeaveDate,@ToDate) LeaveDate,MAX(ISNULL(HT1403_CT.LastModifyDate,@ToDate)) LastModifyDate
FROM HT1403_CT
...
WHERE HT1403_CT.DivisionID = @DivisionID
	 ...
	 AND ISNULL(TitleID,'') LIKE ISNULL(@TitleID,'%')
GROUP BY  HT1403_CT.DivisionID,HT1403_CT.EmployeeID, TitleID,LeaveDate

UNION ALL
----Dữ liệu thay đồi chức danh trước khoảng thời gian lọc	
SELECT HT1403_CT.DivisionID, HT1403_CT.EmployeeID,TitleID, ISNULL(HT1403_CT.LeaveDate,@ToDate) LeaveDate,ISNULL(HT1403_CT.LastModifyDate,@ToDate) LastModifyDate
FROM HT1403_CT
LEFT JOIN HT1400_CT HT40 ON HT1403_CT.ReAPK = HT40.ReAPK AND HT1403_CT.EmployeeID = HT40.EmployeeID
LEFT JOIN HT1414 HT14 ON HT14.DivisionID = HT1403_CT.DivisionID AND HT1403_CT.EmployeeID = HT14.EmployeeID
...
SELECT  distinct T.EmployeeID, 
 ...
 T.TitleID, TitleName, TitleNameE, T.LastModifyDate
FROM TEMP T
	LEFT JOIN HT1106 HT16 ON T.DivisionID = HT16.DivisionID AND  HT16.TitleID = T.TitleID 
...
CREATE TABLE #OOP3018a (EmployeeID VARCHAR(50), [Month] VARCHAR(50),TitleID VARCHAR(50), TitleName NVARCHAR(250), TitleNameE NVARCHAR(250),
LastModifyDate DATETIME, Fromdate DATETIME,Todate DATETIME)

WHILE MONTH(@Day) <= MONTH(@ToDate)
BEGIN
	INSERT INTO #OOP3018a
	SELECT DISTINCT EmployeeID,LEFT(DATENAME(mm,@Day),3)+'-'+RIGHT(CONVERT(VARCHAR(10),YEAR(@Day),112),2) [Month], NULL,NULL,NULL, LastModifyDate, 
...
UPDATE t1
	SET t1.TitleID = t2.TitleID, t1.TitleName = t2.TitleName,t1.TitleNameE = t2.TitleNameE
	FROM #OOP3018a t1
	inner JOIN 	#OOP3018 t2 ON t1.EmployeeID = t2.EmployeeID
	and CONVERT(VARCHAR(10),t2.LastModifyDate,112) <=  CONVERT(VARCHAR(10),t1.ToDate,112)
----insert những chức danh không có nhân viên nghỉ việc
DECLARE @Cur CURSOR,
		@ID VARCHAR(50),
		@Name NVARCHAR(250),
		@NameE NVARCHAR(250)
...
BEGIN	
	IF @ID NOT IN (SELECT DISTINCT TitleID FROM #OOP3018a)
	BEGIN
		INSERT INTO #OOP3018a
		VALUES( NULL, LEFT(DATENAME(mm,@ToDate),3)+'-'+RIGHT(CONVERT(VARCHAR(10),YEAR(@ToDate),112),2), @ID, @Name,@NameE, NULL, @FromDate, @ToDate)
	END
FETCH NEXT FROM @Cur INTO @ID, @Name, @NameE
END 
Close @Cur	

SELECT * FROM #OOP3018a ORDER BY MONTH(FromDate), EmployeeID, TitleID
