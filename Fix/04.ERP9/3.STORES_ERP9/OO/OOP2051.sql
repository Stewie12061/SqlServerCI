...
(
    SELECT TOP 1
           1
    FROM dbo.sysobjects
    WHERE id = OBJECT_ID(N'[DBO].[OOP2051]')
          AND OBJECTPROPERTY(id, N'IsProcedure') = 1
)
    DROP PROCEDURE [dbo].[OOP2051];
GO
...
--- Modified on 21/12/2020 by Hoài Phong: Lấy Tổng OT từ Table chính
--- Modified on 04/10/2021 by Văn Tài	: Hỗ trợ sắp xếp theo mã nhân viên cho: DXP, DXLTG.
--- Modified on 25/07/2023 by Kiều Nga	: Fix lỗi load lưới DXRN.
/*-- <Example>
exec OOP2051 @DivisionID=N'MK',@UserID=N'000144',@APK=N'1617FA53-C90A-4EAE-AD6D-0FF4E8D80D18',@Type=N'DXP',@APKMaster=N'0449BE38-F007-410A-AC6F-CC2C6994BCC6'
----*/

CREATE PROCEDURE OOP2051
(
...
BEGIN
	...
	SELECT DISTINCT ' + @sSelect
              + ',#TAM.Status,HV14.FullName EmployeeName,
    ...
    (
     ...
     BEGIN
		IF (@Type = 'DXLTG')
        EXEC ('SELECT * FROM KQ ORDER BY EmployeeID, WorkFromDate');
		ELSE
		IF (@Type = 'DXP')
		EXEC ('SELECT * FROM KQ ORDER BY EmployeeID, LeaveFromDate');
		ELSE
		EXEC ('SELECT * FROM KQ');
	END
