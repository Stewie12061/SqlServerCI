IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[OOP2030]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[OOP2030]
GO
...
-- <Summary>
---- Load Grid Form OOF2030: Đơn xin làm thêm giờ
-- <Param>
---- 
-- <Return>
---- 
...
---- Modified by Bảo Anh on 30/08/2018: Sửa lỗi dữ liệu không lên do phân quyền xem dữ liệu của người khác
...
---- Modified on 20/01/2021 by Hoài Phong: Tách SP Cho NQH
/*-- <Example>
	OOP2030 @DivisionID='MK',@UserID='000454',@PageNumber=1,@PageSize=25,@IsSearch=1, @TranMonth=8, @TranYear=2016,
	@ID=NULL,@DepartmentID=NULL,@SectionID=NULL, @SubsectionID=NULL,@ProcessID=NULL,
	@CreateUserID = NULL,@Status=NULL,@CreateFromDate=NULL,@CreateToDate=NULL,@WorkFromDate ='2016-06-07',@WorkToDate='2016-06-09'
----*/

CREATE PROCEDURE OOP2030
( 
...
BEGIN
    EXEC OOP2030_NQH @DivisionID, @UserID, @PageNumber,@PageSize,@IsSearch,@TranMonth,
	@TranYear,@ID,@DepartmentID,@SectionID,@SubsectionID,@ProcessID,@Status,@CreateUserID,
	@CreateFromDate,@CreateToDate,@WorkFromDate,@WorkToDate,@LanguageID,@ConditionOTFormID
	END
ELSE
BEGIN
...
--SET @Level=ISNULL((SELECT TOP 1 LEVEL FROM OOT0010 WITH (NOLOCK) WHERE DivisionID=@DivisionID AND AbsentType ='DXLTG'AND TranMonth=@TranMonth AND TranYear=@TranYear),0)              
...
BEGIN
	...
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2030.WorkFromDate,120), 126) BETWEEN '''+CONVERT(VARCHAR(10),@WorkFromDate,126)+''' AND '''+CONVERT(VARCHAR(10),@WorkToDate,126)+'''
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2030.WorkToDate,120), 126) BETWEEN '''+CONVERT(VARCHAR(10),@WorkFromDate,126)+''' AND '''+CONVERT(VARCHAR(10),@WorkToDate,126)+''' '
	IF (@WorkFromDate IS NOT NULL AND @WorkToDate IS NULL) SET @sWhere = @sWhere + '
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2030.WorkFromDate,120), 126) >= '''+CONVERT(VARCHAR(10),@WorkFromDate,126)+''' '
	IF (@WorkFromDate IS NULL AND @WorkToDate IS NOT NULL) SET @sWhere = @sWhere + '
	AND CONVERT(VARCHAR(10), CONVERT(DATE,OOT2030.WorkFromDate,120), 126) =< '''+CONVERT(VARCHAR(10),@WorkToDate,126)+''' '
END
IF Isnull(@ConditionOTFormID, '') != ''
		SET @sWhere = @sWhere + ' AND ISNULL(OOT90.CreateUserID,'''') in (N'''+@ConditionOTFormID+''' )'
...
SET @sSQL =' 
	...
	(
		SELECT OOT90.APK, OOT90.DivisionID,OOT90.TranMonth, OOT90.TranYear, OOT90.ID,
		       ...
		       OOT90.SubsectionID, OOT90.ProcessID, MIN(OOT2030.Status) AS Status,
		...
		LEFT JOIN OOT2030 WITH (NOLOCK) ON OOT2030.DivisionID = OOT90.DivisionID AND OOT90.APK = OOT2030.APKMaster
		LEFT JOIN AT1102 A11 WITH (NOLOCK) ON A11.DivisionID = OOT90.DivisionID AND A11.DepartmentID=OOT90.DepartmentID 
		LEFT JOIN HT1101 A12 WITH (NOLOCK) ON A12.DivisionID = OOT90.DivisionID AND A12.TeamID=OOT90.SectionID
		LEFT JOIN AT1011 A13 WITH (NOLOCK) ON A13.DivisionID = OOT90.DivisionID AND A13.AnaID=OOT90.SubsectionID AND A13.AnaTypeID=''A04''
		LEFT JOIN AT1011 A14 WITH (NOLOCK) ON A14.DivisionID = OOT90.DivisionID AND A14.AnaID=OOT90.ProcessID AND A14.AnaTypeID=''A05''
		...
		--LEFT JOIN AT0010 WITH (NOLOCK) ON AT0010.DivisionID = OOT90.DivisionID AND (AT0010.AdminUserID = '''+@UserID+''' OR AT0010.UserID = OOT90.CreateUserID)
		LEFT JOIN AT0010 WITH (NOLOCK) ON AT0010.DivisionID = OOT90.DivisionID 
											AND AT0010.AdminUserID = ''' + @UserID + '''
											AND AT0010.UserID = OOT90.CreateUserID'
SET @sSQL1='
		'+@sSQLJon+'
		WHERE OOT90.Type=''DXLTG''
		AND (OOT90.CreateUserID = AT0010.UserID OR OOT90.CreateUserID = ''' + @UserID + ''')
		'+@sWhere+'
...
IF @IsSearch = 1 AND @Status IS NOT NULL
	SET @sSQL1=@sSQL1+'
	HAVING MIN(OOT2030.Status) LIKE ''%'+@Status+'%'' '
