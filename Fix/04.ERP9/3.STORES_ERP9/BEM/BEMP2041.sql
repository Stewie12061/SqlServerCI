IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[BEMP2041]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[BEMP2041]
GO
...
BEGIN
	...
	BEGIN
		SET @sSQL = '
				...
				DECLARE @BEMT2040Temp TABLE (
						Status TINYINT, 
						MessageID VARCHAR(100), 
						Params NVARCHAR(4000))
				SET @Status = 0
				SET @Message = ''''
				INSERT INTO @BEMT2040Temp (	Status, MessageID, Params) 
											SELECT 2 AS Status, ''00ML000117'' AS MessageID, NULL AS Params
				SET @Cur = CURSOR SCROLL KEYSET FOR
				SELECT B1.APK, B1.DivisionID, B1.VoucherNo, B1.APKMaster_9000, O1.APK FROM BEMT2040 B1 WITH (NOLOCK) 
				...
				BEGIN
					...
					IF @Status = 1
						UPDATE @BEMT2040Temp SET Params = ISNULL(Params, '''') + @DelVoucherNo + '', '' WHERE MessageID = ''00ML000117''
					ELSE 
					BEGIN
						UPDATE BEMT2040 SET DeleteFlg = 1 WHERE CAST(APK AS VARCHAR(50)) = @DelAPK
				...
				END
				CLOSE @Cur
				SELECT Status, MessageID, LEFT(Params, LEN(Params) - 1) AS Params FROM @BEMT2040Temp WHERE Params IS NOT NULL'
			EXEC (@sSQL)
			PRINT (@sSQL)
	END
	...
	BEGIN
		SET @sSQL = '
				...
				INSERT INTO @BEMT2040Temp (	Status, MessageID, Params) 
											SELECT 2 AS Status, ''00ML000117'' AS MessageID, NULL AS Params
				SET @Cur = CURSOR SCROLL KEYSET FOR
				SELECT B1.APK, B1.DivisionID, B1.VoucherNo FROM BEMT2040 B1 WITH (NOLOCK) 
				WHERE CAST(B1.APK AS VARCHAR(50)) IN (''' + @APKList + ''')
				OPEN @Cur
