IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[POSP2051]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[POSP2051]
GO
...
---- <Example>
---- EXEC POSP2051 'NN', 'B34AEDB6-5BCD-49D9-82A0-16C8A7C021C4','1','ADMIN'

CREATE PROCEDURE POSP2051 ( 
...
SET @sSQL = '
	...
	SELECT @DelVoucherNo = VoucherNo FROM POST2050(NOLOCK)	WHERE APK IN ('''+@APKList+''')
	IF EXISTS (SELECT 1 FROM POST2050(NOLOCK) WHERE APK IN ('''+@APKList+''') and StatusID IN (6,7))
	BEGIN
	update @POST00801temp set Params = ISNULL(Params,'''') + @DelVoucherNo+'',''  where MessageID = ''POSFML000119''
	END
	ELSE
	BEGIN
		UPDATE POST2050 SET DeleteFlg = 1 WHERE APK IN ('''+@APKList+''')
	END
