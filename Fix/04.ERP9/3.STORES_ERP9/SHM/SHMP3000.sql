IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[SHMP3000]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[SHMP3000]
GO
...
-- <Summary>
---- Báo cáo danh sách chia cổ tức – SHMR3000
---- 
-- <Param>
---- 
-- <Return>
...
-- <History>
-- <Example> EXEC SHMP3000 'HCM', 'HCM', 1, '2017-01-01', '2018-12-31', '12/2017'',''01/2018'',''02/2018','','', 'ASOFTADMIN'

CREATE PROCEDURE SHMP3000 
(
...
BEGIN
		...
		SET @sSQL=N'	
			...
			FROM SHMT2040 M WITH (NOLOCK) INNER JOIN SHMT2041 D WITH (NOLOCK) ON M.APK = D.APKMaster and M.DeleteFlg = D.DeleteFlg
										  ...
										  LEFT JOIN AT9000 A90 WITH (NOLOCK) ON D.DivisionID = A90.DivisionID and CONVERT(VARCHAR(50),D.APK) = CONVERT(VARCHAR(50),A90.InheritTransactionID) and D.APKMaster = CONVERT(VARCHAR(50),A90.InheritVoucherID)
