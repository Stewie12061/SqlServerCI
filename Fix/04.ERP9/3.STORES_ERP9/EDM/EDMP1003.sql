IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[EDMP1003]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[EDMP1003]
GO
...
---- 
-- <History>
---- Created by Minh Hòa on 08/09/2018
---- Modified by on Minh Hòa
-- <Example>
/* 
 EXEC EDMP1003 @DivisionID, @UserID, @ImportTransTypeID, @XML
 */
 
CREATE PROCEDURE EDMP1003
( 
...
SELECT  X.Data.query('OrderNo').value('.', 'INT') AS [Row],
		...
		X.Data.query('GradeName').value('.', 'NVARCHAR(250)') AS GradeName, 
...
INSERT INTO #Data ([Row], Orders, DivisionID, LevelID, GradeID, GradeName, Notes, IsCommon)
SELECT [Row], Orders, DivisionID,  LevelID, GradeID, GradeName, Notes, IsCommon
FROM #EDMT1000
...
EXEC CMNP8100 @UserID = @UserID, @ImportTemplateID = @ImportTransTypeID, @CheckCode = 'CheckDuplicateData', @ColID = 'GradeID', @Param1 = 'Orders', @Param2 = 'EDMT1000', 
@Param3 = 'GradeID'
...
INSERT INTO EDMT1000 (APK, DivisionID,  LevelID, GradeID, GradeName, Notes, IsCommon, CreateUserID, CreateDate, LastModifyUserID, LastModifyDate)
SELECT NEWID(), CASE WHEN IsCommon = 1 THEN '@@@' ELSE DivisionID END AS DivisionID,  LevelID, GradeID, GradeName, Notes, IsCommon, @UserID AS CreateUserID, 
