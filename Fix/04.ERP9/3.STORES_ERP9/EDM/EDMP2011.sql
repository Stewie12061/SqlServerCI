IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[EDMP2011]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[EDMP2011]
GO
...
-- <Summary>
---- Load điều tra tâm lý của Hồ sơ học sinh
-- <Param>
---- 
-- <Return>
---- 
...
-- <Example>
---- 
--	EDMP2011 @DivisionID='MK', @UserID='ASOFTADMIN', @LanguageID = 'vi-VN', @APK=N'EE9DFD4C-430D-4B1E-9599-9F223283FE4A'

CREATE PROCEDURE [dbo].[EDMP2011]
( 
...
SELECT TOP 0 A.PsychologizeType, CAST('' AS NVARCHAR(250)) AS PsychologizeTypenName, A.PsychologizeID, A.PsychologizeName, ISNULL(A.PsychologizeGroup, '') AS PsychologizeGroup, A.Orders
	INTO #EDMT1070
	FROM EDMT1070 A WITH(NOLOCK)

SET @sSQL = '
INSERT INTO #EDMT1070
SELECT A.PsychologizeType, B.Description AS PsychologizeTypenName, A.PsychologizeID, A.PsychologizeName, ISNULL(A.PsychologizeGroup, '''') AS PsychologizeGroup, A.Orders
...
SELECT A.PsychologizeType, A.PsychologizeTypenName, A.PsychologizeID AS PsychologizeIDQuestion, A.PsychologizeGroup, A.Orders
...
LEFT JOIN (SELECT APK, APKMaster, PsychologizeID, Answer, CreateUserID, CreateDate, LastModifyUserID, LastModifyDate FROM EDMT2011 WITH(NOLOCK) WHERE APKMaster = @APK) AS C ON C.PsychologizeID = A.PsychologizeID
