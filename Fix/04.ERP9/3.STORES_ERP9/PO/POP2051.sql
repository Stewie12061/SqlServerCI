IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[POP2051]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[POP2051]
GO
...
-- <Summary>
---- Thông báo đơn hàng nhận trễ theo qui cách sản phẩm
-- <Param>
---- 
-- <Return>
---- 
...
BEGIN TRY
	...
	--Gọi store POP0019 kiểm tra có đơn hàng nhận trễ theo qui cách sản phẩm hay không?
	...
	--Nếu có đơn hàng nhận trễ thì thông báo chuông
	IF @isNotification = 1
	BEGIN
		--OOT9002 : Master chuông
		--OOT9003 : Detail chuông	
		DECLARE @APK_OOT9002 UNIQUEIDENTIFIER = NEWID(),
				@Description NVARCHAR(MAX)= concat(N'Có ',@Count,N' đơn hàng nhận trễ!'), 
				@ModuleID VARCHAR(50) = 'PO',
				@ScreenID VARCHAR(50) = 'POF2054',
				@UrlCustom NVARCHAR(500) = '/PopupSelectData/Index/PO/POF2054',
