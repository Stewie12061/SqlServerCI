...
BEGIN
	SET @sJoin = 'LEFT JOIN AT1303 A33 WITH (NOLOCK) ON A33.WareHouseID = W95.WareHouseID
				  ...
				  LEFT JOIN WQ1309 W09 WITH (NOLOCK) ON W09.DivisionID IN (W96.DivisionID,''@@@'') AND W09.InventoryID = W96.InventoryID AND W09.ConvertedUnitID = W96.ConvertedUnitID
				--AND (W09.S01ID = O99.S01ID OR (ISNULL(W09.S01ID, '''') =  ISNULL(O99.S01ID, '''')))
				--AND (W09.S02ID = O99.S02ID OR (ISNULL(W09.S02ID, '''') =  ISNULL(O99.S02ID, '''')))
				--AND (W09.S03ID = O99.S03ID OR (ISNULL(W09.S03ID, '''') =  ISNULL(O99.S03ID, '''')))
				--AND (W09.S04ID = O99.S04ID OR (ISNULL(W09.S04ID, '''') =  ISNULL(O99.S04ID, '''')))
				--AND (W09.S05ID = O99.S05ID OR (ISNULL(W09.S05ID, '''') =  ISNULL(O99.S05ID, '''')))
				--AND (W09.S06ID = O99.S06ID OR (ISNULL(W09.S06ID, '''') =  ISNULL(O99.S06ID, '''')))
				--AND (W09.S07ID = O99.S07ID OR (ISNULL(W09.S07ID, '''') =  ISNULL(O99.S07ID, '''')))
				--AND (W09.S08ID = O99.S08ID OR (ISNULL(W09.S08ID, '''') =  ISNULL(O99.S08ID, '''')))
				--AND (W09.S09ID = O99.S09ID OR (ISNULL(W09.S09ID, '''') =  ISNULL(O99.S09ID, '''')))
				--AND (W09.S10ID = O99.S10ID OR (ISNULL(W09.S10ID, '''') =  ISNULL(O99.S10ID, '''')))
				--AND (W09.S11ID = O99.S11ID OR (ISNULL(W09.S11ID, '''') =  ISNULL(O99.S11ID, '''')))
				--AND (W09.S12ID = O99.S12ID OR (ISNULL(W09.S12ID, '''') =  ISNULL(O99.S12ID, '''')))
				--AND (W09.S13ID = O99.S13ID OR (ISNULL(W09.S13ID, '''') =  ISNULL(O99.S13ID, '''')))
				--AND (W09.S14ID = O99.S14ID OR (ISNULL(W09.S14ID, '''') =  ISNULL(O99.S14ID, '''')))
				--AND (W09.S15ID = O99.S15ID OR (ISNULL(W09.S15ID, '''') =  ISNULL(O99.S15ID, '''')))
				--AND (W09.S16ID = O99.S16ID OR (ISNULL(W09.S16ID, '''') =  ISNULL(O99.S16ID, '''')))
				--AND (W09.S17ID = O99.S17ID OR (ISNULL(W09.S17ID, '''') =  ISNULL(O99.S17ID, '''')))
				--AND (W09.S18ID = O99.S18ID OR (ISNULL(W09.S18ID, '''') =  ISNULL(O99.S18ID, '''')))
				--AND (W09.S19ID = O99.S19ID OR (ISNULL(W09.S19ID, '''') =  ISNULL(O99.S19ID, '''')))
				--AND (W09.S20ID = O99.S20ID OR (ISNULL(W09.S20ID, '''') =  ISNULL(O99.S20ID, '''')))
				  '
	...
	SET @sSelect1 = ', (CASE WHEN W95.KindVoucherID IN (1,3,5,7,9) THEN A33.WareHouseName ELSE '''' END) ImWareHouseName,
		(CASE WHEN W95.KindVoucherID in (2,4,6,8,10) THEN A33.WareHouseName
		  ...
		  W09.ConversionFactor, W09.Operator, W09.FormulaID, W09.FormulaDes, W09.DataType'
END

IF @CustomerIndex = 166 ----NKC
BEGIN
	SET @sSelect = @sSelect + ', A131.FullName AS DriverName, C00.AssetID AS CarID, W96.InheritTableID	'
	SET @sJoin = @sJoin + '
							LEFT JOIN AT1103 A131 WITH (NOLOCK) ON A131.EmployeeID = W96.DriverID
							LEFT JOIN CIT1300 C00 WITH (NOLOCK) ON C00.AssetID = W96.CarID AND C00.DivisionID IN (W96.DivisionID, ''@@@'')
							'


	SET @sSQL7 = ''
	SET @sJoin1 = ' LEFT JOIN WQ1309 W09 WITH (NOLOCK) ON W09.DivisionID IN (W96.DivisionID,''@@@'') AND W09.InventoryID = W96.InventoryID AND W09.ConvertedUnitID = W96.ConvertedUnitID'
END
...
SELECT Distinct W95.ReDeTypeID, W95.VoucherTypeID, W95.VoucherNo, W95.VoucherDate, W95.IsGoodsFirstVoucher, W95.IsGoodsRecycled,
	V06.VoucherDate, W95.RefNo01, W95.RefNo02, W95.KindVoucherID, W95.RDAddress, W95.ContactPerson, '''' AS CheckPurchase,
	...
	(CASE WHEN W95.KindVoucherID IN (2,4,6,8,10) THEN W95.WareHouseID
		  ELSE CASE WHEN W95.KindVoucherID = 3 THEN W95.WareHouseID2 ELSE '''' END END) ExWareHouseID,W96.OriginalAmount AS ActualAmount,
	W95.EmployeeID, W96.TransactionID, W95.VoucherID, W96.InventoryID, A32.InventoryName, W96.UnitID, A04.UnitName,
    W96.APK, W96.ActualQuantity, W96.UnitPrice, W96.OriginalAmount, W96.ConvertedAmount, W95.[Description], W95.TranMonth,
	...
	W96.IsInheritWarranty, W95.EVoucherID, W96.WVoucherID, W95.IsVoucher, 
	W96.InheritTransactionID, W96.InheritVoucherID, W96.InheritTableID,W96.InheritApportionID,'
	
SET @sSQL0 = '
	W96.Orders, W96.LimitDate, W96.Notes , W96.ReVoucherID, W96.ReTransactionID, W96.IsProInventoryID,
	...
	W96.PeriodID, A39.UnitID ConversionUnitID, A39.ConversionFactor ConversionFactor2,
	------------------------
...
---- Cac thong tin lien quan den DVT qui doi  cho 
	...
	ISNULL(T09.ConversionFactor, 1) T09ConversionFactor,
