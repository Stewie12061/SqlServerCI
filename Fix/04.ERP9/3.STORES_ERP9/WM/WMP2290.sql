IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[WMP2290]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[WMP2290]
GO
...
-- <Summary>
---- Load Grid Form WMF2290 Xuất kho seri
-- <Param>
---- 
-- <Return>
---- 
-- <History>
----Created by: Hoàng Long, Date: 17/10/2023
-- <Example>
/*
EXEC [WMP2290]
...
IF ISNULL(@VoucherNo,'') <> '' 
	SET @sWhere = @sWhere + ' AND M.VoucherNo LIKE ''%'+@VoucherNo+'%'' '
IF ISNULL(@ObjectID,'') <> '' 
	SET @sWhere = @sWhere + ' AND (M.ObjectID LIKE N''%'+@ObjectID+'%'' OR AT1202.ObjectName LIKE N''%'+@ObjectID+'%'') '
...
SET @sSQL = N'
		...
		WHERE ' + @sWhere + ' AND M.KindVoucherID=2
