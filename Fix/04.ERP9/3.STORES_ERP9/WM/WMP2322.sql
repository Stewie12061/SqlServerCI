IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[WMP2322]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[WMP2322]
GO
...
SET @sSQL = N'
		SELECT DISTINCT T1.APK, T1.DivisionID, T1.TranMonth, T1.TranYear, T1.VoucherNo, T1.VoucherDate, T1.VoucherTypeID, T1.Description, T2.ImWareHouseID, T3.WareHouseName AS ImWareHouseName, 
        T6.WareHouseName AS ExWareHouseName, T1.ObjectID, T4.ObjectName, T1.EmployeeID, T5.FullName AS EmployeeName,
        T1.CreateUserID, T1.CreateDate, T1.LastModifyUserID, T7.FullName AS CreateUserName, T8.FullName AS LastModifyUserName, T1.LastModifyDate, T1.RefNo01 AS Ref01 , T1.RefNo02 AS Ref02, T9.VoucherTypeName, T2.InventoryID, T10.InventoryName, T2.UnitID , T2.ActualQuantity, T2.UnitPrice, T2.OriginalAmount, T2.DebitAccountID, T2.ImStoreManID, T2.SourceNo, T2.LimitDate
        ...
        WHERE T1.DivisionID = ''' + @DivisionID + ''' AND T1.APK = ''' + @APK + ''' AND T2.KindVoucherID = 1
	 '
