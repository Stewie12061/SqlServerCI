IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[CRMP30141]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[CRMP30141]
GO
SET QUOTED_IDENTIFIER ON
GO

-- <Summary>
----Báo cáo Thống kê hoạt động khách hàng không phát sinh cơ hội
-- <Param>
---- 
-- <Return>
---- 
...
---- 
-- <History>
----Created by: Kiều Nga on 26/07/2017
--- Modify by Trọng Kiên, Date 06/10/2020: Fix lỗi liên quan đến Email vì thay đổi cấu trúc (Invalid column RelatedToID)
-- <Example>
----    EXEC CRMP30141 'DTI','DTI',1,'2017-01-01 00:00:00','2017-12-30 00:00:00','07/2017', 'ASOFTADMIN'
CREATE PROCEDURE [dbo].[CRMP30141] ( 
...
where --P011.APK NOT IN(select RelatedToID from CMNT90051_REL with (nolock) where RelatedToTypeID_REL = 3)
P011.APK NOT IN(select RelatedToID from CRMT90051_REL with (nolock) where RelatedToTypeID_REL = 3)
and P011.APK NOT IN(select RelatedToID from CRMT90031_REL with (nolock) where RelatedToTypeID_REL = 3)
and P011.APK NOT IN(select AccountID from CRMT20501_CRMT10101_REL with (nolock))
and P011.MemberID NOT IN(select AccountID from CRMT20501 with (nolock) where AccountID is not null)
' +@sWhere2 + @sWhere
