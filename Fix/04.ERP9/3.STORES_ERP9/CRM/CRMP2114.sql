...
SET @sSQL = @sSQL + N'
	SELECT DISTINCT T1.APK, T1.DivisionID, T1.VoucherNo, T1.VoucherDate, T1.ObjectID, T4.ObjectName, T2.PaperTypeID, T6.Description AS PaperTypeName
		, T2.InventoryID, T3.InventoryName, T1.EmployeeID, T5.FullName AS EmployeeName, T1.DeliveryAddress, T2.ActualQuantity, T2.PrintTypeID, T2.PrintNumber
		, T2.ColorPrint01 AS SideColor1, T2.ColorPrint02 AS SideColor2, T2.SideColor1 AS IsSideColor1, T2.SideColor2 AS IsSideColor2, T2.APKMInherited
		, T2.APKDInherited, T2.Length, T2.Width, T2.Height, T2.FileName, T2.FilmDate, T2.DeliveryTime, T2.ProductQuality, T2.SampleContent, T2.ColorSample
	INTO #CRMT2100
	FROM CRMT2100 T1 WITH (NOLOCK)
		...
		LEFT JOIN CRMT0099 T6 WITH(NOLOCK) ON T2.PaperTypeID = T6.ID AND T6.CodeMaster = ''CRMT00000022''
	WHERE T1.DeleteFlg <> 1 AND T1.DivisionID = '''+@DivisionID+''' '
	 +@sWhere +'
'
