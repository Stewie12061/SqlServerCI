IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[NMP1002]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[NMP1002]
GO
...
-- <Summary>
---- Import Excel danh mục nhóm thực phẩm
-- <Param>
---- 
-- <Return>
---- 
...
SELECT  X.Data.query('OrderNo').value('.', 'INT') AS [Row],
	...
	X.Data.query('MaterialsTypeName').value('.', 'NVARCHAR(250)') AS MaterialsTypeName, 
...
INSERT INTO #Data ([Row],  DivisionID,  MaterialsTypeID, MaterialsTypeName, [Description],  IsCommon)
SELECT [Row],  DivisionID,  MaterialsTypeID, MaterialsTypeName, [Description],  IsCommon
FROM #NMP1002
-- Kiểm tra check code mặc định 
EXEC CMNP8105 @ImportTemplateID = @ImportTransTypeID, @UserID = @UserID

 --Kiểm tra trùng ID 
EXEC CMNP8100 @UserID = @UserID, @ImportTemplateID = @ImportTransTypeID, @CheckCode = 'CheckDuplicateData', @ColID = 'MaterialsTypeID', @Param1 = '', @Param2 = 'NMT1000', 
@Param3 = 'MaterialsTypeID'
...
INSERT INTO NMT1000 ( DivisionID, MaterialsTypeID, MaterialsTypeName, [Description], IsCommon, CreateUserID, CreateDate, LastModifyUserID, LastModifyDate)
SELECT DISTINCT DivisionID, MaterialsTypeID, MaterialsTypeName, [Description], IsCommon, @UserID AS CreateUserID, GETDATE() AS CreateDate, @UserID AS LastModifyUserID, GETDATE() AS LastModifyDate		
FROM #Data
