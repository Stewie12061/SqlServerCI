IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[NMP1042]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[NMP1042]
GO
...
-- <Summary>
---- Import Excel danh mục loại món ăn
---- 
-- <Return>
---- 
-- <Reference>
...
-- <Example>
/* 
 EXEC NMP1042 @DivisionID, @UserID, @ImportTransTypeID, @XML
 */
 
CREATE PROCEDURE NMP1042
( 
...
SELECT  X.Data.query('OrderNo').value('.', 'INT') AS [Row],
	...
	X.Data.query('DishTypeName').value('.', 'VARCHAR(50)') AS DishTypeName, 
...
INSERT INTO #Data ([Row],  DivisionID,  DishTypeID, DishTypeName,Description, IsCommon,Disabled)
SELECT [Row],  DivisionID,  DishTypeID, DishTypeName,Description, IsCommon,Disabled
FROM #NMP1042
...
EXEC CMNP8100 @UserID = @UserID, @ImportTemplateID = @ImportTransTypeID, @CheckCode = 'CheckDuplicateData', @ColID = 'DishTypeID', @Param1 = 'Orders', @Param2 = 'NMP1042', 
@Param3 = 'DishTypeID'
...
INSERT INTO EDMT1070 (APK, DivisionID,  DishTypeID, DishTypeName ,Description, IsCommon,Disabled, CreateUserID, CreateDate, LastModifyUserID, LastModifyDate)
SELECT NEWID(),  DivisionID, DishTypeID, DishTypeName,Description, IsCommon,Disabled, @UserID AS CreateUserID, 
