IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[MP2180]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[MP2180]
GO
...
----
-- <History>
---- Created by: Minh Phúc : 25/05/2021
...
SET @SQLPermission = @SQLPermission + ' INNER JOIN #PermissionMT2180 T1 ON MT01.CreateUserID = T1.Value  '

SET @sSQL0 ='
	SELECT Value
	INTO #PermissionMT2180
...
SET @sSQL =N'SELECT MT01.APK
				  ...
				  , MT01.Notes
				  , MT01.CreateUserID INTO #TemMT2180
			FROM MT2180 MT01 WITH (NOLOCK)
			'+@SQLPermission+'
...
SET @sSQL1 =N'SELECT ROW_NUMBER() OVER (Order BY '+@OrderBy+') AS RowNum
			...
			FROM #TemMT2180 MT01'+ISNULL(@SearchWhere,'')+'
			Order BY '+@OrderBy+'
