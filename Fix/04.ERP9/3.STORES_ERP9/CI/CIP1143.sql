IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[CIP1143]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[CIP1143]
GO
...
---- Modify by: Minh Hiếu, Date 04/04/2022 : bỏ dòng delete CIT1141
-- <Example>
---- 
/*-- <Example>
	CIP1143 @DivisionID = 'CH', @APK = 'FA0EBCE7-3241-46BF-AD5D-D6A76C89E6E7', @FormID = 'CIF1140', @Mode = 1, @IsDisable = '', @UserID = ''
	
	CIP1143 @DivisionID, @APK, @FormID, @Mode, @IsDisable, @UserID
----*/

CREATE PROCEDURE CIP1143
( 
...
BEGIN
	...
	BEGIN 
		...
		BEGIN
			...
			END 
			--ELSE IF EXISTS (SELECT TOP 1 1 FROM CIT1141 WITH (NOLOCK) WHERE APKMaster = @DelAPK)
			--BEGIN 
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			ELSE
			BEGIN
				IF '+ CAST(ISNULL(@IsDisable, '''') AS VARCHAR(50)) +'=1 -- Nếu chọn là Disable
				BEGIN
