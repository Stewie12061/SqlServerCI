IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[CIP12506]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[CIP12506]
GO
...
-- <Summary>
---- Load Popup Master Màn hình bảng giá (sell in)(màn hình cập nhật).
-- <Param>
---- 
-- <Return>
---- 
...
-- <Example> EXEC CIP12506 @DivisionID = 'GREE', @UserID = '', @APK = 'C69E5AE4-6FE1-4829-B99A-78D1AB7D89D3'
----
CREATE PROCEDURE CIP12506
( 
...
END
    ...
    BEGIN
        ...
        SET @i = @i + 1	
    END
		SET @sSQL =N'SELECT	T1.APK, T1.DivisionID, T1.ID, T1.[Description],
						T1.FromDate, T1.ToDate, T1.OID, T1.TypeID,
                        T1.TypeID AS IsPurchasePrice, T1.InventoryTypeID, T1.[Disabled], T1.CurrencyID, T1.InheritID,
						T1.IsConvertedPrice, T1.IsTaxIncluded, 
						T1.IsSetBonus, T1.CreateUserID, T1.CreateDate, T1.LastModifyUserID, T1.LastModifyDate,
						T1.StatusSS, T1.Levels, T1.ApproveLevel, 
						T1.ApprovingLevel, T1.APKMaster_9000, ApprovalNotes
						'+@sSQLSL+'
					FROM OT1301 T1  WITH (NOLOCK)
