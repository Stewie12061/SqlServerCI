IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[SOP2063]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[SOP2063]
GO
...
---- 
-- <History>
----Created by: Kiều Nga, Date: 04/11/2019
----Modified by: Văn Tài, Date: 27/07/2021 - Tách customize cho DTI.
----Modified by: Nhựt Trường on 09/08/2021 - Bổ sung trường InheritCurrencyID và InheritExchangeRate. 
-- <Example> EXEC SOP2063 'AS' , 'VoucherNo' , 'NV01' ,1 ,20

Create PROCEDURE SOP2063
(
...
AS
	...
	SET @sSQL = 'Select ROW_NUMBER() OVER (ORDER BY '+@OrderBy+') AS RowNum, '+@TotalRow+' AS TotalRow
						...
						, D.QuoQuantity
						, D0.UnitPrice * D0.Coefficient AS UnitPrice
						, D0.QuoQuantity * D0.UnitPrice * ISNULL(D0.Coefficient, 1) AS OriginalAmount
						, D0.QuoQuantity * D0.UnitPrice * ISNULL(D2.ExchangeRate, 1) * ISNULL(D0.Coefficient, 1) AS ConvertedAmount
						, D.Notes
						, D.VATPercent
						...
						, D2.CurrencyID
						, ISNULL(D2.ExchangeRate, 1) ExchangeRate
						...
						FROM OT2101 M WITH (NOLOCK) 
								...
								LEFT JOIN AT1302 D1  WITH (NOLOCK) ON D1.InventoryID IN (''@'', D.DivisionID) AND D.InventoryID = D1.InventoryID
