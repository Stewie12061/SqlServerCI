IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[SOP20001_AG]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[SOP20001_AG]
GO
...
---- 
-- <History>
----Created by Tiểu Mai on 22/04/2016: Viết store load danh sách cho khách hàng ANGEL
----Modified by Tiểu Mai on 28/07/2016: Fix bug xem quyền dữ liệu của người khác
----Modified by Tiểu Mai on 04/10/2017: Bổ sung ShipAmount, TotalAfterAmount, PayAmount
--- Modified by Văn Tài  on 19/11/2021: Check box kiểm tra đã xuất hóa đơn từ T.
--- Modified by Nhựt Trường  on 26/07/2022: Bổ sung trường EmployeeID, SalesManID.
-- <Example>
----    EXEC SOP20001_AG 'ANG','AS'',''GS','','','','','','','', '' , 1, 5, '2015-01-01', '2016-12-30', '01/2015'',''02/2016'',''03/2016' ,'NV01',1,20
----
CREATE PROCEDURE SOP20001_AG ( 
...
--- Đặc thù ANGEL: ko kiểm tra thiết lập hệ thống check phân quyền
...
, OT2001.ShipAmount, OT2002.TotalAfterAmount, ISNULL(OT2002.TotalAfterAmount,0) - ISNULL(OT2001.ShipAmount,0) AS PayAmount
, ISNULL((
				SELECT TOP 1 1 
				FROM  AT9000 AT90 WITH (NOLOCK)
				INNER JOIN OT2002 OT02  With (NOLOCK) ON OT02.DivisionID = AT90.DivisionID 
