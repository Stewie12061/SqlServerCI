IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[SOP10802]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[SOP10802]
GO
...
-- <Example>
--EXEC SOP10802 @DivisionID=N'DTI',@DivisionIDList=N'',@FromDate=NULL,@ToDate=NULL,@IsPeriod=0,@PeriodList=N'',@ObjectID=N'',@PageNumber=1,@PageSize=25

CREATE PROCEDURE [dbo].[SOP10802] (
...
END
    ...
    BEGIN
        ...
        SET @i = @i + 1	
    END
		SET @sSQL =N'			SELECT T1.APK, T1.DivisionID, T1.TargetsID, T1.ObjectID, T1.SalesYear, T1.FromDate, T1.ToDate, T1.[Description], T1.CreateDate,
								A1.FullName AS CreateUserID, T1.LastModifyDate, A2.FullName AS LastModifyUserID, T1.Levels,T1.ApproveLevel,T1.ApprovingLevel, T1.APKMaster_9000, T1.ApprovalNotes
								'+@sSQLSL+'
								FROM AT0161 T1 WITH (NOLOCK)    
								LEFT JOIN AT1103 A1 WITH (NOLOCK) ON A1.EmployeeID = T1.CreateUserID
								LEFT JOIN AT1103 A2 WITH (NOLOCK) ON A2.EmployeeID = T1.LastModifyUserID 
