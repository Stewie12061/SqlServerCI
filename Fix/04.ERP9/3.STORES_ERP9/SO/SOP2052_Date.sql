IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[SOP2052_DATE]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[SOP2052_DATE]
GO
...
-- <Summary>
---- Danh sách đơn hàng sắp đến ngày giao theo qui cách sản phẩm
-- <Param>
---- 
-- <Return>
---- 
...
-- <History>
-- <Example>
-- SOP2052_Date 'DTI', 'HUULOI'
-- </Example>
----Created by Trọng Kiên on 11/12/2020

CREATE PROCEDURE [dbo].[SOP2052_Date] 
...
Set @sSQL1 =N' 
	...
	INSERT INTO #temp
	EXEC OP0019_Date ''' + @DivisionID + ''','''','''+ @DeliveryDate+ ''','''+ @UserID+ '''

	Select DISTINCT OT2001.APK, OT2001.SOrderID,OT2001.VoucherTypeID, 
...
SET @sSQL3 = @sSQL3 + '
	...
	SELECT ROW_NUMBER() OVER (ORDER BY VoucherNo) AS RowNum, COUNT(*) OVER () AS TotalRow, * FROM #TempSOP0019 '+ @sWhere +'
