IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[SOP2051_DATE]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[SOP2051_DATE]
GO
...
-- <Summary>
---- Thông báo đơn hàng sắp đến ngày giao theo qui cách sản phẩm
-- <Param>
---- 
-- <Return>
---- 
...
BEGIN TRY
	...
	BEGIN
		...
		DECLARE @APK_OOT9002 UNIQUEIDENTIFIER = NEWID(),
				@Description NVARCHAR(MAX)= concat(N'Có ',@Count,N' đơn hàng sắp đến ngày giao hàng!'), 
				@ModuleID VARCHAR(50) = 'SO',
				@ScreenID VARCHAR(50) = 'SOF2055',
				@UrlCustom NVARCHAR(500) = '/PopupSelectData/Index/SO/SOF2055',
