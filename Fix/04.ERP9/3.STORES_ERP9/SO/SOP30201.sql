IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[SOP30201]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[SOP30201]
GO
...
--- Created by Nhựt Trường on 26/02/2022: Báo cáo tổng hợp doanh số sale in (CustomizeIndex = 57 ---- ANGEL)
...
--- Modified on 29/05/2023 by Anh Đô: Select thêm các trường dữ liệu cho báo cáo; Fix lỗi điều kiện lọc
/*
 * exec SOP30201 'ANG', 201601, 201601, '01/01/2016', '01/31/2016', '', 'KPP.HAG.0001', 'KPP.HYE.0001', 'HMP', 'MPB' , 0
 */
 

CREATE PROCEDURE [dbo].[SOP30201] 
(
...
---- Customer
IF CHARINDEX('%',@ListCustomerID) > 0 OR ISNULL(@ListCustomerID,'') = ''
BEGIN
	SET @StrCustomerID = N' LIKE N''%'''
END
ELSE
BEGIN
	SET @StrCustomerID = N'IN ('''+@ListCustomerID+''')'
END
...
BEGIN
	...
	WHERE OT2002.DivisionID = '''+@DivisionID+'''
		  ...
		  AND ISNULL(OT2001.ObjectID, '''') '+@StrCustomerID+'
	...
	OT2001.TranMonth, OT2001.TranYear, OT2001.ObjectID, AT1302.I08ID,  --AT1302.I04ID,
			 OT2001.Ana01ID, OT2001.Ana02ID, OT2001.Ana03ID, OT2001.Ana04ID, OT2001.Ana05ID
			 , OT2001.Ana03ID, OT2001.Ana04ID, OT2001.Ana05ID
	'
	...
	WHERE ISNULL(AT1202.O01ID,'''') '+@StrArea+'
		  AND ISNULL(AT0161.InventoryTypeID, '''') '+@StrInventoryTypeID+'
		  AND ISNULL(AT0161.ObjectID, '''') '+@StrCustomerID+'
		  AND ISNULL(AT0161.SOAna05ID, '''') '+@StrQLADMIN+'
		  AND ISNULL(AT0161.SOAna04ID, '''') '+@StrADMIN+'
		  AND ISNULL(AT0161.SOAna03ID, '''') '+@StrASMID+'
		  AND ISNULL(AT0161.SOAna02ID, '''') '+@StrSUPID+'
		  AND AT0161.DivisionID = '''+ @DivisionID +'''
	'+@sWhere3+'
	...
	WHERE ISNULL(AT1202.O01ID,'''') '+@StrArea+'
		  AND ISNULL(#TEMP22.InventoryTypeID, ISNULL(#TEMP1.I08ID, '''')) '+@StrInventoryTypeID+'
		  AND ISNULL(#TEMP22.ObjectID, ISNULL(#TEMP1.ObjectID, '''')) '+@StrCustomerID+'
		  AND ISNULL(#TEMP22.SOAna05ID, ISNULL(#TEMP1.Ana05ID, '''')) '+@StrQLADMIN+'
...
SET @sSQL2 = '
	SELECT #TEMP2.ObjectID
	, #TEMP2.ObjectName
	, SUM(ISNULL(#TEMP2.SalesMonth02,0)) AS SalesMonth02
	, (ISNULL(#TEMP2.SalesQuarter,0)) AS SalesQuarter
	, SUM(#TEMP2.ConvertedAmountAfterVAT) AS ConvertedAmountAfterVAT
	, SUM(#TEMP2.ConvertedAmountAfterVAT2) AS ConvertedAmountAfterVAT2
	, #TEMP2.InventoryTypeID
	, AT1015.AnaName as InventoryTypeName
	, ISNULL(#TEMP2.Ana01ID, '''')  AS Ana01ID
	, ISNULL(O01.AnaName, '''') AS AnaName01
	, ISNULL(#TEMP2.Ana02ID, '''') AS Ana02ID
	, ISNULL(O02.AnaName, '''') AS AnaName02
	, ISNULL(#TEMP2.Ana03ID, '''') AS Ana03ID
	, ISNULL(O03.AnaName, '''') AS AnaName03
	, ISNULL(#TEMP2.Ana04ID, '''') AS Ana04ID
	, ISNULL(O04.AnaName, '''') AS AnaName04
	, ISNULL(#TEMP2.Ana05ID, '''') AS Ana05ID
	, ISNULL(O05.AnaName, '''') AS AnaName05
	, O08.UserName as UserName01
	, O07.UserName as UserName02
	, ISNULL(A01.O01ID, '''') AS AreaID
	, ISNULL(A02.AnaName, '''') AS AreaName
	'+@Period+'
	FROM #TEMP2
	...
	LEFT JOIN OT1002 O01 ON #TEMP2.DivisionID IN (''@@@'',O01.DivisionID) AND O01.AnaID = #TEMP2.Ana01ID AND O01.AnaTypeID = ''S01''
	...
	LEFT JOIN OT1005 O08 ON O08.DivisionID in (''@@@'',#TEMP2.DivisionID) AND O08.AnaTypeID LIKE ''S01''
	LEFT JOIN AT1202 A01 ON A01.DivisionID IN (''@@@'', #TEMP2.DivisionID) AND A01.ObjectID = #TEMP2.ObjectID
	LEFT JOIN AT1015 A02 ON A02.DivisionID IN (''@@@'', #TEMP2.DivisionID) AND A02.AnaID = A01.O01ID AND A02.AnaTypeID = ''O01'' 
	...
	, #TEMP2.Ana03ID, O03.AnaName, #TEMP2.Ana01ID, O01.AnaName, O08.UserName, #TEMP2.Ana04ID, O04.AnaName, #TEMP2.Ana05ID, O05.AnaName
	, A01.O01ID, A02.AnaName
