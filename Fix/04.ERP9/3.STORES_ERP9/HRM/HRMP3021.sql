IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HRMP3021]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[HRMP3021]
GO
...
---- Modified by Kiều Nga on 30/06/2023: Bổ sung WITH (NOLOCK) và tách store cho Meiko.
...
IF(@CustomerName = 50)
BEGIN 
	EXEC HRMP3021_MK @DivisionID,@UserID,@TranMonth,@TranYear,@Mode,@ImportTransTypeID,@TransactionKey,@XML
END
ELSE
BEGIN
...
IF @ID=''		
	BEGIN		
		...
		SET @ID=(SELECT TOP 1 ID FROM HRMT2132 WHERE TransactionKey=@TransactionKey AND DepartmentID=@DepartmentID AND SectionID=@SectionID AND SubsectionID=@SubsectionID AND ProcessID=@ProcessID AND  ISNULL(ID,'') <>'')
		IF ISNULL(@ID,'')=''
		BEGIN
		...
		END
		UPDATE HRMT2132 SET ID = @ID WHERE Row=@Row AND  TransactionKey=@TransactionKey AND DepartmentID=@DepartmentID AND SectionID=@SectionID AND SubsectionID=@SubsectionID AND ProcessID=@ProcessID
	END
...
--- Kiểm tra ChangeFromDate và ChangeToDate
	...
	--ELSE
	--BEGIN
	--	IF EXISTS (SELECT 1 FROM OOT2070 WITH (NOLOCK) WHERE EmployeeID = @EmployeeID AND ChangeFromDate = @ChangeFromDate)
	--	BEGIN
	--		SET @ErrorMessage = @ErrorMessage + (SELECT TOP 1 DataCol FROM A00065 WITH (NOLOCK)
	--										WHERE ImportTransTypeID = @ImportTransTypeID AND ColID = 'ChangeFromDate')
...
BEGIN
	...
	BEGIN
		...
		WHERE APKMaster IN (SELECT TOP 1 OOT9.APK 
							FROM HRMT2132 WITH (NOLOCK) 
							INNER JOIN OOT9000 OOT9 WITH (NOLOCK) ON OOT9.DivisionID = HRMT2132.DivisionID AND OOT9.TranMonth = HRMT2132.TranMonth AND OOT9.TranYear = HRMT2132.TranYear AND OOT9.ID = HRMT2132.ID
							WHERE TransactionKey = @TransactionKey)
		...
		WHILE @j <= @TotalRow
		BEGIN
			SELECT @APK_AT2070 = APK, @i = 1 FROM #TEMP WHERE RowNum = @j
