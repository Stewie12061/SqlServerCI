...
BEGIN 
	...
	WHERE ISNULL(HRMT1020.Disabled,0) = 0 AND HRMT1020.DivisionID = '''+@DivisionID+'''  AND HRMT1020.DepartmentID = '''+@DepartmentID+''' AND '''+CONVERT(VARCHAR(10), CONVERT(DATE, @FromDate,120), 126)+''' BETWEEN HRMT1020.FromDate AND HRMT1020.ToDate 
...
BEGIN
	...
	FROM HRMT2000 WITH (NOLOCK) 
	LEFT JOIN HRMT2001 WITH (NOLOCK) ON HRMT2001.DivisionID = HRMT2000.DivisionID AND HRMT2001.RecruitPlanID = CONVERT(NVARCHAR(50),HRMT2000.APK)
	LEFT JOIN HRMT1020 WITH (NOLOCK) ON HRMT2000.DivisionID = HRMT1020.DivisionID AND HRMT2000.DepartmentID = HRMT1020.DepartmentID
	...
	LEFT JOIN (
		...
		'+CASE WHEN @Mode = 1 THEN 'AND HRMT2000.APK <> '''+@RecruitPlanID+'''' WHEN @Mode = 3 THEN '' ELSE '' END+' 
	...
	WHERE HRMT2000.DivisionID = '''+@DivisioNID+'''  AND (CONVERT(NVARCHAR(50),HRMT2000.RecruitPlanID) = '''+@RecruitPlanID+''' OR CONVERT(NVARCHAR(50), HRMT2000.APK) = '''+@RecruitPlanID+''' )'
END 
...
BEGIN
	...
	FROM HRMT2000 WITH (NOLOCK) 
	LEFT JOIN HRMT2001 WITH (NOLOCK) ON HRMT2000.DivisionID = HRMT2001.DivisionID AND CONVERT(NVARCHAR(50),HRMT2000.APK) = HRMT2001.RecruitPlanID
	LEFT JOIN HRMT1020 WITH (NOLOCK) ON HRMT2000.DivisionID = HRMT1020.DivisionID AND HRMT2000.DepartmentID = HRMT1020.DepartmentID
		AND (HRMT2000.FromDate BETWEEN HRMT1020.FromDate AND HRMT1020.ToDate OR HRMT2000.ToDate BETWEEN HRMT1020.FromDate AND HRMT1020.ToDate 
		OR (HRMT2000.FromDate <= HRMT1020.FromDate AND HRMT2000.ToDate >= HRMT1020.ToDate))
	INNER JOIN HRMT1021 WITH (NOLOCK) ON HRMT2000.DivisionID = HRMT1021.DivisionID AND CONVERT(NVARCHAR(50),HRMT1020.APK) = HRMT1021.BoundaryID
		AND HRMT2001.DutyID = HRMT1021.DutyID
	LEFT JOIN(
...
SET @sSQL = @sSQL + N'
	...
	LEFT JOIN HT1102 WITH (NOLOCK) ON HRMT2001.DivisionID = HT1102.DivisionID AND HRMT2001.DutyID = HT1102.DutyID
	LEFT JOIN HT0099 HT090 WITH (NOLOCK) ON HRMT2001.WorkType = HT090.ID AND HT090.CodeMaster = ''WorkType''
	WHERE HRMT2000.DivisionID = '''+@DivisionID+''' AND CONVERT(NVARCHAR(50),HRMT2000.APK) = '''+@RecruitPlanID+'''
	ORDER BY '+@OrderBy+' 
