IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[TP8002]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[TP8002]
GO
...
-- <Summary>
---- Kiểm tra cảnh báo công nợ phải thu đến hạn, quá hạn
-- <Param>
---- 
-- <Return>
---- 
...
---- 
/*-- <Example>
	EXEC TP8002 @DivisionID, @APK, @APKList, @FormID, @Mode, @UserID
----*/

CREATE PROCEDURE TP8002
( 
...
BEGIN TRY
	...
	BEGIN
		...
		IF(@CountDue > 0)
		BEGIN
			SET @Description = CONCAT(@Description,N' Có ',@CountDue,N' khách hàng có công nợ đến hạn!')
		END

		IF(@CountOver > 0)
		BEGIN
			SET @Description = CONCAT(@Description,N' Có ',@CountOver,N' khách hàng có công nợ quá hạn!')
		END
