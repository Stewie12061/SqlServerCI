IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[BP3900]') AND  OBJECTPROPERTY(ID, N'IsProcedure') = 1)			
DROP PROCEDURE [DBO].[BP3900]
GO
...
----------------- Purpose: In bao cao bang ket qua kinh doanh. theo ma phan tich ERP 9
---- Modified by on 06/02/2020 by Văn Tài : Copy từ store AP7620 xử lý cho ERP 8
-- <Example>
---- 
-- <Summary>
CREATE PROCEDURE [dbo].[BP3900]   
...
--------------------------------------------
	...
	BEGIN
		...
		SELECT @StrDivisionID_New = CASE WHEN @StrDivisionID = '%' 
		THEN ' LIKE ''' + @StrDivisionID + '''' 
		ELSE ' IN (''' + REPLACE(@StrDivisionID, ',',''',''') + ''')' 
		END  
