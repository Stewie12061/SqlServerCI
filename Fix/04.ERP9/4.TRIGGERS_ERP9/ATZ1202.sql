IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[ATZ1202]') AND OBJECTPROPERTY(ID, N'IsTrigger') = 1)
DROP TRIGGER [DBO].ATZ1202
GO
...
CREATE TRIGGER [dbo].ATZ1202 ON [dbo].[AT1202]  FOR UPDATE
AS
...
FROM INSERTED
SELECT @DivisionID = DivisionID, @ObjectID = ObjectID FROM DELETED
