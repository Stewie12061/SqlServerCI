...
WHILE @@FETCH_STATUS = 0
  BEGIN	
	IF EXISTS (SELECT TOP 1 1 FROM sysObjects WHERE XType = 'U' and Name = 'MTT0000')
	BEGIN
		IF NOT EXISTS (SELECT  TOP 1 1 FROM MTT0000 WHERE DivisionID = @DivisionID)
		INSERT  MTT0000  (DivisionID, TranMonth, TranYear)
		SELECT DivisionID, BeginMonth, BeginYear
		FROM AT1101 WHERE DivisionID = @DivisionID
	END
	IF EXISTS(SELECT TOP 1 1 FROM sysObjects WHERE XType = 'U' and Name = 'MTT0000')
	BEGIN
		IF NOT EXISTS (SELECT 1 FROM MTT9999 WHERE DivisionID = @DivisionID)
		INSERT  MTT9999  (DivisionID, TranMonth, TranYear, [Disabled],  BeginDate, EndDate )
