...
IF NOT EXISTS (SELECT TOP 1 1 FROM AT1404STD WHERE ModuleID = @ModuleID AND ScreenID = @ScreenID)
...
IF NOT EXISTS (SELECT TOP 1 1 FROM AT1404 WHERE ModuleID = @ModuleID AND ScreenID = @ScreenID)
INSERT INTO AT1404 (DivisionID, ModuleID, ScreenID,ScreenName,ScreenNameE, ScreenType, CreateUserID, CreateDate, LastModifyUserID, LastModifyDate,SourceID, CommonID, CustomerIndex) 
			(SELECT DivisionID, @ModuleID, @ScreenID, @ScreenName, @ScreenNameE, @ScreenType, 'ASOFTADMIN', GETDATE() , 'ASOFTADMIN', GETDATE(), @SourceID, @CommonID, @CustomerIndex
			FROM AT1101)
ELSE
BEGIN
	IF(@CommonID IS NULL)
	BEGIN
		...
		WHERE ModuleID = @ModuleID 
		AND ScreenID = @ScreenID
		AND EXISTS (SELECT TOP 1 1 FROM AT1101 WHERE AT1101.DivisionID=AT1404.DivisionID)

		UPDATE AT1403
		SET SourceID = @SourceID
		WHERE ModuleID = @ModuleID
		AND ScreenID = @ScreenID
		AND EXISTS (SELECT TOP 1 1 FROM AT1101 WHERE AT1101.DivisionID=AT1403.DivisionID)
	END
	ELSE
	BEGIN
		UPDATE AT1404  SET 
