...
--- Modify by Phương Thảo on 10/03/2016 : Bổ sung thông tin kế thừa từ bút toán chi phí (AT9000.IsFACost = 1)
IF EXISTS (SELECT TOP 1 1 FROM sysobjects WHERE [name] = 'AT1590' AND xtype = 'U')
    BEGIN
        IF NOT EXISTS (SELECT TOP 1 1 FROM syscolumns col INNER JOIN sysobjects tab
        ON col.id = tab.id WHERE tab.name = 'AT1590' AND col.name = 'ReVoucherID')
        ALTER TABLE AT1590 ADD ReVoucherID NVARCHAR(50) NULL
    END

IF EXISTS (SELECT TOP 1 1 FROM sysobjects WHERE [name] = 'AT1590' AND xtype = 'U')
    BEGIN
        IF NOT EXISTS (SELECT TOP 1 1 FROM syscolumns col INNER JOIN sysobjects tab
        ON col.id = tab.id WHERE tab.name = 'AT1590' AND col.name = 'ReTransactionID')
        ALTER TABLE AT1590 ADD ReTransactionID NVARCHAR(50) NULL
    END
