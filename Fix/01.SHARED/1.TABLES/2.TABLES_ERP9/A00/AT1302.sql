...
---- Modified by Kiều Nga on 30/08/2021: Bổ sung trường IsArea
...
BEGIN 
	IF NOT EXISTS (SELECT TOP 1 1 FROM syscolumns col INNER JOIN sysobjects tab 
	ON col.id=tab.id WHERE tab.name='AT1302' AND col.name='IsArea')
	ALTER TABLE AT1302 ADD IsArea TINYINT NULL
END
...
BEGIN 
   IF NOT EXISTS (SELECT TOP 1 1 FROM syscolumns col INNER JOIN sysobjects tab 
   ON col.id = tab.id WHERE tab.name = 'AT1302' AND col.name = 'IsDiscountWallet') 
   ALTER TABLE AT1302 ADD IsDiscountWallet TINYINT NULL 
END